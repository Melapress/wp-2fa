 ...
 * /       /
 */
namespace WP2FA_Vendor\Twilio\Rest\Chat\V2\Service;

use WP2FA_Vendor\Twilio\Exceptions\TwilioException;
use WP2FA_Vendor\Twilio\InstanceContext;
use WP2FA_Vendor\Twilio\ListResource;
use WP2FA_Vendor\Twilio\Options;
use WP2FA_Vendor\Twilio\Rest\Chat\V2\Service\Channel\InviteList;
use WP2FA_Vendor\Twilio\Rest\Chat\V2\Service\Channel\MemberList;
use WP2FA_Vendor\Twilio\Rest\Chat\V2\Service\Channel\MessageList;
use WP2FA_Vendor\Twilio\Rest\Chat\V2\Service\Channel\WebhookList;
...
class ChannelContext extends InstanceContext
{
    ...
    {
        $options = new Values($options);
        $data = Values::of(['FriendlyName' => $options['friendlyName'], 'UniqueName' => $options['uniqueName'], 'Attributes' => $options['attributes'], 'DateCreated' => Serialize::iso8601DateTime($options['dateCreated']), 'DateUpdated' => Serialize::iso8601DateTime($options['dateUpdated']), 'CreatedBy' => $options['createdBy']]);
        $headers = Values::of(['X-Twilio-Webhook-Enabled' => $options['xTwilioWebhookEnabled']]);
    ...
    {
        if (!$this->_webhooks) {
            $this->_webhooks = new WebhookList($this->version, $this->solution['serviceSid'], $this->solution['sid']);
        }
