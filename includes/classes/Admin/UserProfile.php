...
class UserProfile {
	...
	public function user_2fa_options( $user, $additional_args = array() ) {
		...
		if ( empty( SettingsUtils::get_option( WP_2FA_SETTINGS_NAME ) ) ) {
			return;
		}
		...
		if ( UserUtils::in_array_all( array( 'viewing_own_profile' ), $user_type ) ) {
			if (
				UserUtils::in_array_all( ['has_enabled_methods'], $user_type ) ||
				UserUtils::in_array_all( ['no_required_has_enabled'], $user_type)
			) {
				...
				if ( SettingsPage::are_backup_codes_enabled() ) {
			...
			if (
				...
				UserUtils::roleIsNot( $showIfUserIsNotIn, $user_type )
			) {
				$first_time_setup_url = Settings::getSetupPageLink();
		...
		if ( $show_form_table && ! empty( $form_content ) ) {
		  ...
		  <table class="form-table wp-2fa-user-profile-form" role="presentation">
			...
			if ( (isset($_GET['show']) && 'wp-2fa-setup' === $_GET['show']) || ( new User($user) )->getEnforcedInstantly() ) {
				$form_output .= '
					<script>
	...
	public function generate_inline_modals( $user_type = array() ) {
					...
					<div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
						...
						<?php
							...
							if ( UserUtils::in_array_all( array( 'user_needs_to_setup_2fa', 'viewing_own_profile' ), $user_type ) || UserUtils::in_array_all( array( 'no_required_not_enabled', 'viewing_own_profile' ), $user_type ) ) :
								...
								$available_methods = UserUtils::get_2fa_methods_available_to_user( $user );
						...
						if ( SettingsPage::are_backup_codes_enabled() ) { ?>
							...
							<?php WizardSteps::backupCodesConfigure() ?>
							<?php WizardSteps::generatedBackupCodes() ?>
							</div>
						<?php } else { ?>
							<div class="wizard-step" id="2fa-wizard-config-backup-codes">
							<?php WizardSteps::congratulationsStep();?>
							</div>
						<?php } ?>
						</main>
					</div>
		...
		<?php } ?>
		<?php if ( SettingsPage::are_backup_codes_enabled() ) { ?>
		<div>
			<div class="wp2fa-modal micromodal-slide" id="configure-2fa-backup-codes" aria-hidden="true">
				<div class="modal__overlay" tabindex="-1" data-micromodal-close>
					...
					<main class="modal__content" id="modal-1-content">
						<?php WizardSteps::generatedBackupCodes( true ) ?>
					</main>
					</div>
				</div>
			</div>
	...
	public static function can_user_remove_2fa( $user_id ) {
		...
		if ( 'no-grace-period' === $grace_policy ) {
			...
			if ( 'do-not-enforce' !== $enforcement_policy ) {
				...
				return WP2FA::isUserEnforced( $user_id );
			}
		}
