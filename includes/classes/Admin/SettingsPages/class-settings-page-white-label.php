...
if ( ! class_exists( '\WP2FA\Admin\SettingsPages\Settings_Page_White_Label' ) ) {
	...
	class Settings_Page_White_Label {
			...
			<table class="form-table">
				<tbody>
					<tr>
						...
						<td>
							<?php
								...
								echo self::create_standard_editor( 'default-text-code-page' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					<tr>
						...
						<td>
							<?php
								...
								echo self::create_standard_editor( 'default-backup-code-page' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					<tr>
						...
						<td>
							<?php
								echo self::create_standard_editor( 'login-to-view-area' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							?>
						</td>
					</tr>
		 ...
		 *
		 * @param string $requested_slide - The slide.
		 *
		 * @return void
		 *
		 * @since 2.8.1
		...
		public static function create_standard_editor( $requested_slide = 'welcome' ) {
			...
			$content = WP2FA::get_wp2fa_white_label_setting( sanitize_key( $requested_slide ), true );

			if ( isset( $content ) ) {
				\wp_editor( ( $content ), sanitize_key( $requested_slide ), $settings );
			}
		}
	}
}
