...
// phpcs:ignore
namespace WP2FA_Vendor\WP2FA\Cron;
...
class CronTasks
{
    ...
    public function wp_2fa_check_users_grace_period_status()
    {
        ...
        if (empty(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enable_grace_cron'))) {
            return;
        }
        //  grab all users
        ...
        foreach ($users as $index => $user_id) {
            ...
            $wp2faUser = new \WP2FA_Vendor\WP2FA\Admin\User($user_id);
    ...
    function register_check_users_grace_period_status_event()
    {
        ...
        if (!wp_next_scheduled('wp_2fa_check_grace_period_status') && !empty(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enable_grace_cron'))) {
            // Schedule the event
            wp_schedule_event(\time(), 'hourly', 'wp_2fa_check_grace_period_status');
        }
    }
    /**
    ...
    public static function send_expired_grace_email($user_id)
    {
        ...
        if ('enable_account_locked_email' !== \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_email_templates('send_account_locked_email')) {
            return \false;
        }
        // Grab user data
        ...
        $subject = wp_strip_all_tags(\WP2FA_Vendor\WP2FA\WP2FA::replace_email_strings(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_email_templates('user_account_locked_email_subject'), $user_id));
        $message = wpautop(\WP2FA_Vendor\WP2FA\WP2FA::replace_email_strings(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_email_templates('user_account_locked_email_body'), $user_id));
        \WP2FA_Vendor\WP2FA\Admin\SettingsPage::send_email($email, $subject, $message);
    }
}
