 ...
 * Migration class
 */
if (!\class_exists('WP2FA_Vendor\\WP2FA\\Utils\\Migration')) {
    /**
     ...
     * @since 1.6
     */
    class Migration extends \WP2FA_Vendor\WP2FA\Utils\AbstractMigration
    {
        /**
        ...
        protected static function migrateUpTo_160()
        {
            ...
            foreach ($settings_to_convert as $setting_name) {
                if (\array_key_exists($setting_name, $settings) && !\is_array($settings[$setting_name])) {
                    $settings[$setting_name] = \array_filter(\explode(',', $settings[$setting_name]));
                    $needsUpdate = \true;
                }
            }
        ...
        protected static function migrateUpTo_162()
        {
            ...
            foreach ($settings_to_convert as $setting_name) {
                if (\array_key_exists($setting_name, $settings) && !\is_array($settings[$setting_name])) {
                    $originalSettingsSplit = \array_filter(\explode(',', $settings[$setting_name]));
        ...
        protected static function migrateUpTo_170()
        {
            ...
            if (\is_array($email_settings) && \WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all($items_to_remove, $email_settings)) {
        ...
        {
            return \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option($setting_name);
        }
        /**
        ...
        {
            \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::update_option($setting_name, $settings);
        }
    }
}
