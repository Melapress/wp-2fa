...
namespace WP2FA_Vendor\WP2FA\Utils;
...
class UserUtils
{
    ...
    public static function determine_user_2fa_status($user)
    {
        ...
        $grace_period_expired = get_user_meta($user->ID, WP_2FA_PREFIX . 'user_grace_period_expired', \true);
        $is_user_excluded = \WP2FA_Vendor\WP2FA\WP2FA::is_user_excluded($user->ID);
        $isUserEnforced = \WP2FA_Vendor\WP2FA\WP2FA::isUserEnforced($user->ID);
        $isUserLocked = \WP2FA_Vendor\WP2FA\Admin\User::isUserLocked($user->ID);
        // First lets see if the user already has a token.
        $enabled_methods = get_user_meta($user->ID, WP_2FA_PREFIX . 'enabled_methods', \true);
        $noEnforcedMethods = \false;
        if ('do-not-enforce' === \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enforcement-policy')) {
            $noEnforcedMethods = \true;
        }
        $user_type = array();
        if (empty($roles)) {
            $user_type[] = 'orphan_user';
            // User has no role.
        }
    ...
    {
        $user_data = \WP2FA_Vendor\WP2FA\Utils\UserUtils::get_all_users_data($method, $users_args);
        $users = \array_map(function ($user) {
            return (int) $user->ID;
        }, $user_data);
    ...
    public static function getHumanReadableUserStatuses()
    {
        if (null === self::$statuses) {
            self::$statuses = ['has_enabled_methods' => __('Configured', 'wp-2fa'), 'user_needs_to_setup_2fa' => __('Required but not configured', 'wp-2fa'), 'no_required_has_enabled' => __('Configured (but not required)', 'wp-2fa'), 'no_required_not_enabled' => __('Not required & not configured', 'wp-2fa'), 'user_is_excluded' => __('Not allowed', 'wp-2fa'), 'user_is_locked' => __('Locked', 'wp-2fa')];
        }
