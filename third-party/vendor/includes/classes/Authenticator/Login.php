...
// phpcs:ignore
namespace WP2FA_Vendor\WP2FA\Authenticator;
...
use WP2FA_Vendor\WP2FA\Admin\SetupWizard as SetupWizard;
use WP2FA_Vendor\WP2FA\Authenticator\BackupCodes as BackupCodes;
...
class Login
{
    ...
    {
        $providers = array('totp', 'email', 'backup_codes');
        /**
    ...
    {
        if (empty($user) || !\is_a($user, 'WP2FA_Vendor\\WP_User')) {
            $user = wp_get_current_user();
        }
        $providers = self::get_providers();
        $enabled_providers = get_user_meta($user->ID, self::ENABLED_PROVIDERS_USER_META_KEY, \true);
    ...
    {
        $provider = get_user_meta($user_id, self::ENABLED_PROVIDERS_USER_META_KEY, \true);
    ...
    {
        $userStatus = \WP2FA_Vendor\WP2FA\Admin\User::getUserStatus($user);
        /**
        ...
        if (\WP2FA_Vendor\WP2FA\WP2FA::is_user_excluded($user->ID)) {
            return;
        }
        ...
        if (\true == $is_user_instantly_enforced) {
    ...
    private static function get_2fa_setup_url()
    {
        ...
        if (\WP2FA_Vendor\WP2FA\WP2FA::is_this_multisite()) {
            return \WP2FA_Vendor\WP2FA\Admin\Controllers\Settings::getSetupPageLink();
        }
    ...
    {
        $session_manager = \WP2FA_Vendor\WP_Session_Tokens::get_instance($user->ID);
    ...
    {
        if ($user instanceof \WP2FA_Vendor\WP2FA\Authenticator\WP_User && self::is_api_request() && self::is_user_using_two_factor($user->ID) && !self::is_user_api_login_enabled($user->ID)) {
            return new \WP_Error('invalid_application_credentials', esc_html__('Error: API login for user disabled.', 'wp-2fa'));
        }
        return $user;
    }
    /**
    ...
    public static function run_authentication_check($user, $password)
    {
        ...
        if (\is_a($user, 'WP2FA_Vendor\\WP_Error')) {
            return $user;
        }
        if (\WP2FA_Vendor\WP2FA\Admin\User::isUserLocked($user->ID)) {
    ...
    {
        return new \WP_Error('account_locked', esc_html__('Your user account has been locked because you have not configured 2FA within the grace period. Please contact the website administrator to unlock your user and you can configure 2FA.', 'wp-2fa'));
    }
    /**
    ...
    {
        if (\defined('WP2FA_Vendor\\XMLRPC_REQUEST') && XMLRPC_REQUEST) {
            return \true;
        }
        if (\defined('WP2FA_Vendor\\REST_REQUEST') && REST_REQUEST) {
            return \true;
        }
        return \false;
    }
    ...
    public static function backup_2fa()
    {
        ...
        if (\true !== self::verify_login_nonce($user->ID, $nonce)) {
    ...
    public static function login_html($user, $login_nonce, $redirect_to, $error_msg = '', $provider = null)
    {
        ...
        if (!\function_exists('WP2FA_Vendor\\login_header')) {
        ...
        echo esc_url(self::login_url(array('action' => 'validate_2fa'), 'login_post'));
        ...
        if (isset($codes_remaining) && $codes_remaining > 0) {
            $login_url = self::login_url(array('action' => 'backup_2fa', 'provider' => 'backup_codes', 'wp-auth-id' => $user->ID, 'wp-auth-nonce' => $login_nonce, 'redirect_to' => $redirect_to, 'rememberme' => $rememberme));
            ?>
            ...
            ?>">
						<?php 
            esc_html_e('Or, use a backup code.', 'wp-2fa');
            ?>
					</a>
				</p>
			</div>
	<?php 
        ...
        ?>" title="<?php 
        esc_attr_e('Are you lost?', 'wp-2fa');
        ?>">
				<?php 
    ...
    public static function create_login_nonce($user_id)
    {
        ...
        } catch (\WP2FA_Vendor\WP2FA\Authenticator\Exception $ex) {
            $login_nonce['key'] = wp_hash($user_id . \mt_rand() . \microtime(), 'nonce');
        }
        $login_nonce['expiration'] = \time() + HOUR_IN_SECONDS;
    ...
    {
        $login_nonce = get_user_meta($user_id, self::USER_META_NONCE_KEY, \true);
        if (!$login_nonce) {
            return \false;
        }
        if ($nonce !== $login_nonce['key'] || \time() > $login_nonce['expiration']) {
    ...
    public static function login_form_validate_2fa()
    {
        ...
        if ('email' === $provider && \true !== self::pre_process_email_authentication($user)) {
            $login_nonce = self::create_login_nonce($user->ID);
            if (!$login_nonce) {
                wp_die(esc_html__('Failed to create a login nonce.', 'wp-2fa'));
            }
        ...
        // Validate TOTP.
        if ('totp' === $provider && \true !== self::validate_totp_authentication($user)) {
        ...
        // Backup Codes.
        if ('backup_codes' === $provider && \true !== self::validate_backup_codes($user)) {
            do_action('wp_login_failed', $user->user_login);
            $login_nonce = self::create_login_nonce($user->ID);
            if (!$login_nonce) {
                wp_die(esc_html__('Failed to create a login nonce.', 'wp-2fa'));
        ...
        // Validate Email.
        if ('email' === $provider && \true !== self::validate_email_authentication($user)) {
            do_action('wp_login_failed', $user->user_login);
            $login_nonce = self::create_login_nonce($user->ID);
            if (!$login_nonce) {
                wp_die(esc_html__('Failed to create a login nonce.', 'wp-2fa'));
    ...
    public static function validate_totp_authentication($user)
    {
        if (!empty($_REQUEST['authcode'])) {
            // WPCS: CSRF ok, nonce verified by login_form_validate_2fa().
            return \WP2FA_Vendor\WP2FA\Authenticator\Authentication::is_valid_authcode(\WP2FA_Vendor\WP2FA\Authenticator\Authentication::get_user_totp_key($user->ID), sanitize_text_field($_REQUEST['authcode']));
        }
        return \false;
    }
    /**
    ...
    public static function email_authentication_page($user)
    {
        ...
        $has_token = \WP2FA_Vendor\WP2FA\Authenticator\Authentication::user_has_token($user->ID);
        if (empty($has_token) || !$has_token) {
            \WP2FA_Vendor\WP2FA\Admin\SetupWizard::send_authentication_setup_email($user->ID);
        }
        require_once ABSPATH . '/wp-admin/includes/template.php';
        ?>
	<p><?php 
	...
	<p>
		...
		<label for="authcode"><?php 
        esc_html_e('Verification Code:', 'wp-2fa');
        ?></label>
	...
	<p class="2fa-email-resend">
        ...
        echo esc_attr(self::INPUT_NAME_RESEND_CODE);
        ?>" value="<?php 
        esc_attr_e('Resend Code', 'wp-2fa');
        ?>" />
	</p>
	<script type="text/javascript">
		setTimeout( function(){
    ...
    public static function validate_email_authentication($user)
    {
        ...
        return \WP2FA_Vendor\WP2FA\Authenticator\Authentication::validate_token($user->ID, $_REQUEST['wp-2fa-email-code']);
    }
    /**
    ...
    public static function backup_codes_authentication_page($user)
    {
        ...
        ?>
		<p><?php 
        esc_html_e('Enter a backup verification code.', 'wp-2fa');
        ?></p><br/>
		<p>
			<label for="authcode"><?php 
        esc_html_e('Verification Code:', 'wp-2fa');
    ...
    public static function validate_backup_codes($user)
    {
        ...
        return \WP2FA_Vendor\WP2FA\Authenticator\BackupCodes::validate_code($user, sanitize_text_field($_POST['wp-2fa-backup-code']));
    }
}
