...
// phpcs:ignore
namespace WP2FA_Vendor\WP2FA;

use WP2FA_Vendor\WP2FA\Admin\User;
use WP2FA_Vendor\WP2FA\Admin\UserListing;
use WP2FA_Vendor\WP2FA\Admin\SettingsPage;
use WP2FA_Vendor\WP2FA\Utils\DateTimeUtils;
...
class WP2FA
{
    ...
    {
        self::$wp_2fa_options = \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option(WP_2FA_SETTINGS_NAME);
        self::$wp_2fa_email_templates = \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option(WP_2FA_EMAIL_SETTINGS_NAME);
        ...
        if (\WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option('wizard_not_finished') && !isset($_GET['is_initial_setup']) && !wp_doing_ajax() && !\defined('WP2FA_Vendor\\DOING_CRON')) {
            if (!\WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option(WP_2FA_SETTINGS_NAME)) {
                \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::update_option(WP_2FA_SETTINGS_NAME, self::getDefaultSettings());
            }
            // Set a flag so we know we have default values present, not custom.
            \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::update_option('default_settings_applied', \true);
            \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::delete_option('wizard_not_finished');
        }
        // Activation/Deactivation.
        register_activation_hook(WP_2FA_FILE, 'WP2FA_Vendor\\WP2FA\\Core\\activate');
        register_deactivation_hook(WP_2FA_FILE, 'WP2FA_Vendor\\WP2FA\\Core\\deactivate');
        // Register our uninstallation hook.
        register_uninstall_hook(WP_2FA_FILE, 'WP2FA_Vendor\\WP2FA\\Core\\uninstall');
    }
    public static function getDefaultSettings()
    {
        $default_settings = array('enable_totp' => 'enable_totp', 'enable_email' => 'enable_email', 'backup_codes_enabled' => 'yes', 'enforcement-policy' => 'do-not-enforce', 'excluded_users' => [], 'excluded_roles' => [], 'enforced_users' => [], 'enforced_roles' => [], 'grace-period' => 3, 'grace-period-denominator' => 'days', 'enable_grace_cron' => '', 'enable_destroy_session' => '', 'limit_access' => '', '2fa_settings_last_updated_by' => '', '2fa_main_user' => '', 'grace-period-expiry-time' => '', 'plugin_version' => WP_2FA_VERSION, 'delete_data_upon_uninstall' => '', 'excluded_sites' => '', 'included_sites' => [], 'create-custom-user-page' => 'no', 'redirect-user-custom-page' => '', 'redirect-user-custom-page-global' => '', 'custom-user-page-url' => '', 'custom-user-page-id' => '', 'hide_remove_button' => '', 'grace-policy' => 'use-grace-period', 'superadmins-role-add' => 'no', 'superadmins-role-exclude' => 'no', 'default-text-code-page' => __('Please enter the two-factor authentication (2FA) verification code below to login. Depending on your 2FA setup, you can get the code from the 2FA app or it was sent to you by email.', 'wp-2fa'));
    ...
    {
        // Bootstrap.
        \WP2FA_Vendor\WP2FA\Core\setup();
        $this->settings = new \WP2FA_Vendor\WP2FA\Admin\SettingsPage();
        $this->wizard = new \WP2FA_Vendor\WP2FA\Admin\SetupWizard();
        $this->authentication = new \WP2FA_Vendor\WP2FA\Authenticator\Authentication();
        $this->backupcodes = new \WP2FA_Vendor\WP2FA\Authenticator\BackupCodes();
        $this->login = new \WP2FA_Vendor\WP2FA\Authenticator\Login();
        $this->user_notices = new \WP2FA_Vendor\WP2FA\Admin\UserNotices();
        $this->crontasks = new \WP2FA_Vendor\WP2FA\Cron\CronTasks();
        $this->user_registered = new \WP2FA_Vendor\WP2FA\Admin\UserRegistered();
        $this->shortcodes = new \WP2FA_Vendor\WP2FA\Shortcodes\Shortcodes();
        global $pagenow;
        if ('profile.php' !== $pagenow || 'user-edit.php' !== $pagenow) {
            $this->user_profiles = new \WP2FA_Vendor\WP2FA\Admin\UserProfile();
        }
        if (is_admin()) {
            \WP2FA_Vendor\WP2FA\Admin\UserListing::init();
        }
        $this->add_actions();
    }
    /**
    ...
    {
        if (\function_exists('WP2FA_Vendor\\wp_get_current_user') && current_user_can('read')) {
    ...
    public function setup_redirect()
    {
        ...
        if (\WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option('redirect_on_activate', \false)) {
            // Delete redirect option.
            \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::delete_option('redirect_on_activate');
            \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::update_option('wizard_not_finished', \true);
            $page = self::is_this_multisite() && is_super_admin() ? network_admin_url('index.php') : admin_url('options-general.php');
            $redirect = add_query_arg(array('page' => 'wp-2fa-setup', 'is_initial_setup' => 'true'), admin_url('user-edit.php'));
    ...
    {
        return \function_exists('WP2FA_Vendor\\is_multisite') && is_multisite();
    }
    /**
     * Return user roles.
     *
    ...
    public static function is_user_excluded($user_id, $excluded_users = '', $excluded_roles = '', $excluded_sites = '', $included_sites = [])
    {
        ...
        if (\is_a($user_id, 'WP2FA_Vendor\\WP_User')) {
            $user = $user_id;
        ...
        if (isset($excluded_users) && !empty($excluded_users)) {
            ...
            $excluded_users = \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('excluded_users');
        }
        if (!empty($excluded_users)) {
            // Turn it into an array.
            $excluded_users_array = $excluded_users;
            // Compare our roles with the users and see if we get a match.
            $result = \in_array($user->user_login, $excluded_users_array, \true);
            if ($result) {
                $user_excluded = \true;
                return \true;
            }
        ...
        if (isset($excluded_roles) && !empty($excluded_roles)) {
            ...
            $excluded_roles = \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('excluded_roles');
        }
        ...
        if (self::is_this_multisite()) {
            if (isset($excluded_sites) && !empty($excluded_sites)) {
                ...
                $excluded_sites = \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('excluded_sites');
            }
            ...
            if (!isset($included_sites) || empty($included_sites)) {
                $included_sites = \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('included_sites');
            }
    ...
    public static function isUserEnforced($user_id, $current_policy = '', $excluded_users = '', $excluded_roles = '', $enforced_users = '', $enforced_roles = '')
    {
        ...
        $enabled_method = get_user_meta($user->ID, WP_2FA_PREFIX . 'enabled_methods', \true);
        $user_eligable = \false;
        ...
        if (empty($enabled_method) && self::is_this_multisite() && 'superadmins-only' === $current_policy) {
            ...
            return is_super_admin($user->ID) || \WP2FA_Vendor\WP2FA\Admin\User::isAdminUser($user->ID);
        } else {
            if ('all-users' === $current_policy && empty($enabled_method)) {
                if ('yes' === \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('superadmins-role-exclude') && is_super_admin($user->ID)) {
                    return \false;
                }
                ...
                if (!empty($excluded_roles)) {
                    ...
                    if (self::is_this_multisite()) {
                        ...
                        foreach ($subsites as $subsite) {
                            ...
                            $users_caps[] = get_user_meta($user->ID, 'wp_' . $subsite_id . '_capabilities', \true);
                        }
                        ...
                        foreach ($users_caps as $key => $value) {
                            if (!empty($value)) {
                                foreach ($value as $key => $value) {
                                    $result = \in_array($key, $excluded_roles_array, \true);
                                }
                            }
                        }
                        if (!empty($result)) {
                ...
                if (\true === $user_eligable || empty($enabled_method)) {
                    return \true;
                }
                ...
                if (!empty($enforced_users)) {
                    ...
                    // Compare our roles with the users and see if we get a match.
                    $result = \in_array($user->user_login, $enforced_users_array, \true);
                ...
                if (!empty($enforced_roles)) {
                    // Turn it into an array.
                    $enforced_roles_array = \WP2FA_Vendor\WP2FA\Admin\SettingsPage::extract_roles_from_input($enforced_roles);
                    // Compare our roles with the users and see if we get a match.
                    ...
                    if (self::is_this_multisite()) {
                        ...
                        foreach ($users_caps as $key => $value) {
                            if (!empty($value)) {
                                foreach ($value as $key => $value) {
                                    $result = \in_array($key, $enforced_roles_array, \true);
                                }
                            }
                        }
                        if (!empty($result)) {
                ...
                if (\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('superadmins-role-add')) {
                    return is_super_admin($user->ID);
                }
    ...
    public static function get_wp2fa_email_templates($setting_name = '', $grab_default = '')
    {
        ...
        $login_code_body = '<p>' . \sprintf(esc_html__('Enter %1$s to log in.', 'wp-2fa'), '<strong>{login_code}</strong>');
        $login_code_body .= '</p>';
        ...
        $user_locked_body .= '<p>' . \sprintf(esc_html__('Since you have not enabled two-factor authentication for the user %1$s on the website %2$s within the grace period, your account has been locked.', 'wp-2fa'), '{user_login_name}', '{site_name}');
        $user_locked_body .= '</p>';
        ...
        $default_settings = array('email_from_setting' => 'use-defaults', 'custom_from_email_address' => '', 'custom_from_display_name' => '', 'login_code_email_subject' => $login_code_subject, 'login_code_email_body' => $login_code_body, 'user_account_locked_email_subject' => $user_locked_subject, 'user_account_locked_email_body' => $user_locked_body, 'user_account_unlocked_email_subject' => $user_unlocked_subject, 'user_account_unlocked_email_body' => $user_unlocked_body, 'send_account_locked_email' => 'enable_account_locked_email', 'send_account_unlocked_email' => 'enable_account_unlocked_email');
    ...
    public static function replace_email_strings($input = '', $user_id = '', $token = '', $override_grace_period = '')
    {
        ...
        if (isset($override_grace_period) && !empty($override_grace_period)) {
            ...
            $grace_period_string = \WP2FA_Vendor\WP2FA\Utils\DateTimeUtils::format_grace_period_expiration_string($grace_policy);
        }
        // Setup user data.
        ...
        $replacements = array('{site_url}' => esc_url(get_bloginfo('url')), '{site_name}' => sanitize_text_field(get_bloginfo('name')), '{grace_period}' => sanitize_text_field($grace_period_string), '{user_login_name}' => sanitize_text_field($user->user_login), '{user_first_name}' => sanitize_text_field($user->firstname), '{user_last_name}' => sanitize_text_field($user->lastname), '{user_display_name}' => sanitize_text_field($user->display_name), '{login_code}' => sanitize_text_field($login_code), '{2fa_settings_page_url}' => esc_url($new_page_permalink));
        $replacements = apply_filters('wp_2fa_replacement_email_strings', $replacements);
    ...
    {
        global $pagenow;
        if ('use-grace-period' !== \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('grace-policy')) {
            $user = wp_get_current_user();
            $is_user_instantly_enforced = get_user_meta($user->ID, WP_2FA_PREFIX . 'user_enforced_instantly', \true);
            $grace_period_expiry_time = get_user_meta($user->ID, WP_2FA_PREFIX . 'grace_period_expiry', \true);
            $time_now = \time();
            if ($is_user_instantly_enforced && !empty($grace_period_expiry_time) && $grace_period_expiry_time < $time_now && !\WP2FA_Vendor\WP2FA\WP2FA::is_user_excluded($user->ID)) {
                /*
                ...
                if (!empty($custom_user_page_id)) {
                    wp_redirect(\WP2FA_Vendor\WP2FA\Admin\Controllers\Settings::getCustomPageLink());
                    exit;
                }
                //  custom 2FA page is not set-up, force redirect to the wizard in administration
                wp_redirect(\WP2FA_Vendor\WP2FA\Admin\Controllers\Settings::getSetupPageLink());
                exit;
            }
        }
    }
    ...
    public function update_usermeta_if_required()
    {
        ...
        $user = wp_get_current_user();
        $users_settings_hash = get_user_meta($user->ID, WP_2FA_PREFIX . 'global_settings_hash', \true);
        $current_settings_hash = \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option('settings_hash');
        if ($users_settings_hash !== $current_settings_hash) {
            ...
            new \WP2FA_Vendor\WP2FA\Admin\User();
        }
    }
    /**
    ...
    public function handle_send_test_email_ajax()
    {
        ...
        if ('config_test' === $email_id) {
            $email_sent = \WP2FA_Vendor\WP2FA\Admin\SettingsPage::send_email($email, esc_html__('Test email from WP 2FA', 'wp-2fa'), esc_html__('This email was sent by the WP 2FA plugin to test the email delivery.', 'wp-2fa'));
            if ($email_sent) {
        ...
        foreach ($email_templates as $email_template) {
            if ($email_id === $email_template->getEmailContentId()) {
                ...
                $subject = wp_strip_all_tags(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_email_templates($email_id . '_email_subject'));
                $message = wpautop(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_email_templates($email_id . '_email_body'), $user_id);
                $email_sent = \WP2FA_Vendor\WP2FA\Admin\SettingsPage::send_email($email, $subject, $message);
                if ($email_sent) {
    ...
    public static function getMultiSites()
    {
        if (self::is_this_multisite()) {
            if (empty(self::$sites)) {
                self::$sites = \WP2FA_Vendor\get_sites();
            }
            return self::$sites;
        }
        return [];
    ...
    public static function getNumberOfPluginsText()
    {
        ...
        return \sprintf(\WP2FA_Vendor\_n('There is %s method available from which you can choose for 2FA:', 'There are %s methods available from which you can choose for 2FA:', self::$methodsCount, 'wp-2fa'), $methodsCount);
    }
    /**
