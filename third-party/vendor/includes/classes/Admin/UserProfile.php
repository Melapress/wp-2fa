...
use WP2FA_Vendor\WP2FA\Core as Core;
use WP2FA_Vendor\WP2FA\WP2FA as WP2FA;
use WP2FA_Vendor\WP2FA\Utils\GenerateModal;
use WP2FA_Vendor\WP2FA\Utils\DateTimeUtils;
use WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps;
use WP2FA_Vendor\WP2FA\Authenticator\BackupCodes;
use WP2FA_Vendor\WP2FA\Admin\Controllers\Settings;
use WP2FA_Vendor\WP2FA\Authenticator\Authentication;
use WP2FA_Vendor\WP2FA\Utils\UserUtils as UserUtils;
...
class UserProfile
{
    ...
    public function user_2fa_options($user, $additional_args = array())
    {
        ...
        if (empty(\WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option(WP_2FA_SETTINGS_NAME))) {
            return;
        }
        $show_preamble = \true;
        ...
        $user_type = \WP2FA_Vendor\WP2FA\Utils\UserUtils::determine_user_2fa_status($user);
        $form_output = '';
        $form_content = '';
        $description = __('Add two-factor authentication to strengthen the security of your WordPress user account.', 'wp-2fa');
        $show_form_table = \true;
        $page_url = \WP2FA_Vendor\WP2FA\WP2FA::is_this_multisite() ? 'index.php' : 'options-general.php';
        // Orpan user (a user with no role or capabitlies).
        if (\in_array('orphan_user', $user_type, \true)) {
            // We want to use the same form/buttons used in the shortcode.
            $additional_args['is_shortcode'] = \true;
            // Create useful message for admin.
            if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('user_needs_to_setup_2fa', 'can_manage_options'), $user_type)) {
                $description = __('This user is required to setup 2FA but has not yet done so.', 'wp-2fa');
            }
            if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('user_is_excluded', 'can_manage_options'), $user_type)) {
                $description = __('This user is excluded from configuring 2FA.', 'wp-2fa');
            }
        }
        // Excluded user.
        if (\in_array('user_is_excluded', $user_type, \true)) {
            return;
        }
        // A user viewing their own profile AND has a 2FA method configured.
        if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('viewing_own_profile'), $user_type)) {
            if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(['has_enabled_methods'], $user_type) || \WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(['no_required_has_enabled'], $user_type)) {
                // Create wizard link based on which 2fa methods are allowed by admin.
                if (!empty(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enable_totp')) && !empty(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enable_email'))) {
                    $setup_2fa_url = add_query_arg(array('page' => 'wp-2fa-setup', 'current-step' => 'user_choose_2fa_method', 'wizard_type' => 'user_2fa_config'), admin_url($page_url));
                } else {
                    $setup_2fa_url = add_query_arg(array('page' => 'wp-2fa-setup', 'current-step' => 'reconfigure_method', 'wizard_type' => 'user_reconfigure_config'), admin_url($page_url));
                }
                // Create backup codes URL;
                $backup_codes_url = add_query_arg(array('page' => 'wp-2fa-setup', 'current-step' => 'backup_codes', 'wizard_type' => 'backup_codes_config'), admin_url($page_url));
                ...
                $form_content .= '<br /><br />';
                if (\WP2FA_Vendor\WP2FA\Admin\SettingsPage::are_backup_codes_enabled()) {
                    $form_content .= '<a href="' . esc_url($backup_codes_url) . '" class="button button-primary">' . __('Generate backup codes', 'wp-2fa') . '</a>';
                    $codes_remaining = \WP2FA_Vendor\WP2FA\Authenticator\BackupCodes::codes_remaining_for_user($user);
                ...
                if (isset($additional_args['is_shortcode']) && $additional_args['is_shortcode']) {
                    ...
                    if (\WP2FA_Vendor\WP2FA\Admin\SettingsPage::are_backup_codes_enabled()) {
                        ...
                        $form_content .= \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::getGenerateCodesLink() . $backup_codes_desc;
                    }
                }
            }
            $showIfUserIsNotIn = ['user_is_excluded', 'has_enabled_methods', 'no_required_has_enabled'];
            // User viewing own profile and needs to enable 2FA.
            if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('user_needs_to_setup_2fa'), $user_type) || \WP2FA_Vendor\WP2FA\Utils\UserUtils::roleIsNot($showIfUserIsNotIn, $user_type)) {
                $first_time_setup_url = \WP2FA_Vendor\WP2FA\Admin\Controllers\Settings::getSetupPageLink();
                if (isset($additional_args['is_shortcode']) && $additional_args['is_shortcode']) {
        ...
        if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('can_manage_options', 'has_enabled_methods'), $user_type) && !\in_array('viewing_own_profile', $user_type, \true)) {
            ...
            $remove_users_2fa_url = add_query_arg(array('action' => 'remove_user_2fa', 'user_id' => $user->ID, 'wp_2fa_nonce' => wp_create_nonce('wp-2fa-remove-user-2fa-nonce'), 'admin_reset' => 'yes'), admin_url('user-edit.php'));
        ...
        if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('can_manage_options', 'grace_has_expired'), $user_type)) {
            $unlock_user_url = add_query_arg(array('action' => 'unlock_account', 'user_id' => $user->ID, 'wp_2fa_nonce' => wp_create_nonce('wp-2fa-unlock-account-nonce')), admin_url('user-edit.php'));
        ...
        if ($show_form_table && !empty($form_content)) {
		  ...
		  <table class="form-table wp-2fa-user-profile-form" role="presentation">
            ...
            if (isset($_GET['show']) && 'wp-2fa-setup' === $_GET['show'] || (new \WP2FA_Vendor\WP2FA\Admin\User($user))->getEnforcedInstantly()) {
                $form_output .= '
					<script>
    ...
    public function generate_inline_modals($user_type = array())
    {
        \ob_start();
        $user = wp_get_current_user();
        if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('user_needs_to_setup_2fa', 'viewing_own_profile'), $user_type) || \WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('has_enabled_methods', 'viewing_own_profile'), $user_type) || \WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('no_required_not_enabled', 'viewing_own_profile'), $user_type)) {
            ?>
		<div>
			<div class="wp2fa-modal micromodal-slide" id="configure-2fa" aria-hidden="true">
				<div class="modal__overlay" tabindex="-1" data-micromodal-close>
					<div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
						<?php 
            echo \WP2FA_Vendor\WP2FA\Utils\GenerateModal::generate_modal('notify-users', __('Are you sure?', 'wp-2fa'), __('Any unsaved changes will be lost!', 'wp-2fa'), ['<button class="modal__btn button-confirm" data-micromodal-close aria-label="Close this dialog window and the wizard">' . __('Yes', 'wp-2fa') . '</button>', '<button class="modal__btn button-decline" data-micromodal-close aria-label="Close this dialog window">' . __('No', 'wp-2fa') . '</button>'], '', '430px');
            ?>
						<button class="modal__close" aria-label="Close modal"></button>
						<main class="modal__content" id="modal-1-content">
						<?php 
            $logoSection = '<p style="text-align: center; padding:0; margin: 0;"><img style="filter: invert(76.4%); width: 50px; margin: 0 auto;" src="' . WP_2FA_URL . 'dist/images/wp-2fa-white-icon20x28.svg' . '" /></p>';
            $logoSection = apply_filters('wp_fa_plugin_logo_wizard', $logoSection);
            echo $logoSection;
            if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('user_needs_to_setup_2fa', 'viewing_own_profile'), $user_type) || \WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('no_required_not_enabled', 'viewing_own_profile'), $user_type)) {
                $available_methods = \WP2FA_Vendor\WP2FA\Utils\UserUtils::get_2fa_methods_available_to_user($user);
                $intro_text = esc_html__('Choose the 2FA authentication method', 'wp-2fa');
                if (\count($available_methods) > 1) {
                    $sub_text = \WP2FA_Vendor\WP2FA\WP2FA::getNumberOfPluginsText();
                } else {
                ...
                ?>
								...
								<div class="wizard-step active">
									<h3><?php 
                echo sanitize_text_field($intro_text);
                ?></h3>
									<p><?php 
                echo sanitize_text_field($sub_text);
                ?></p>
									<fieldset>
									    <?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::totpOption();
                ?>
										<?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::emailOption();
                ?>
									</fieldset>
									<br>
									<a href="#" class="modal__btn button button-primary 2fa-choose-method" data-name="next_step_setting_modal_wizard" data-next-step><?php 
                esc_html_e('Next Step', 'wp-2fa');
                ?></a>
            ...
            ?>
							...
							<?php 
            if (\WP2FA_Vendor\WP2FA\Utils\UserUtils::in_array_all(array('has_enabled_methods', 'viewing_own_profile'), $user_type)) {
                ?>
								<div class="wizard-step active">
									<fieldset>
										<?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::totpReConfigure();
                ?>
										<?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::emailReConfigure();
                ?>
									</fieldset>
								</div>
							<?php 
            ...
            ?>
							...
							<?php 
            \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::showModalMethods();
            ?>
						<?php 
            ...
            if (\WP2FA_Vendor\WP2FA\Admin\SettingsPage::are_backup_codes_enabled()) {
                ?>
							...
							<?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::backupCodesConfigure();
                ?>
							<?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::generatedBackupCodes();
                ?>
							</div>
						<?php 
            } else {
                ?>
							<div class="wizard-step" id="2fa-wizard-config-backup-codes">
							<?php 
                \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::congratulationsStep();
                ?>
							</div>
						<?php 
            }
		...
		<div>
			<div class="wp2fa-modal micromodal-slide" id="configure-2fa-backup-codes" aria-hidden="true">
				<div class="modal__overlay" tabindex="-1" data-micromodal-close>
					...
					<main class="modal__content" id="modal-1-content">
						<?php 
            \WP2FA_Vendor\WP2FA\Admin\Views\WizardSteps::generatedBackupCodes(\true);
            ?>
					</main>
					</div>
				</div>
	...
	<?php 
		...
		<?php 
        if (self::can_user_remove_2fa($user->ID)) {
            echo \WP2FA_Vendor\WP2FA\Utils\GenerateModal::generate_modal('confirm-remove-2fa', __('Remove 2FA?', 'wp-2fa'), __('Are you sure you want to remove two-factor authentication and lower the security of your user account?', 'wp-2fa'), array('<a href="#" class="modal__btn modal__btn-primary button button-primary" data-trigger-remove-2fa data-user-id="' . esc_attr($user->ID) . '" data-nonce="' . wp_create_nonce('wp-2fa-remove-user-2fa-nonce') . '">' . __('Yes', 'wp-2fa') . '</a>', '<button class="modal__btn button" data-close-2fa-modal aria-label="Close this dialog window">' . __('No', 'wp-2fa') . '</button>'));
        }
     ...
     */
    public function inline_2fa_profile_form($is_shortcode = '', $show_preamble = \true)
    {
        if (isset($_GET['user_id'])) {
            $user_id = (int) $_GET['user_id'];
            $user = get_user_by('id', $user_id);
        ...
        if (!\is_a($user, 'WP2FA_Vendor\\WP_User') || !\is_a($current_user, 'WP2FA_Vendor\\WP_User')) {
            return;
        }
        $additional_args = array('is_shortcode' => $is_shortcode, 'show_preamble' => $show_preamble);
    ...
    public function user_2fa_row_actions($actions, $user_object)
    {
        ...
        $grace_period_expired = get_user_meta($user_object->ID, WP_2FA_PREFIX . 'user_grace_period_expired', \true);
        $url = add_query_arg(array('action' => 'unlock_account', 'user_id' => $user_object->ID, 'wp_2fa_nonce' => $nonce), admin_url('users.php'));
    ...
    public function save_user_2fa_options($input)
    {
        ...
        if (isset($_POST['wp_2fa_enabled_methods']) && 'totp' === $_POST['wp_2fa_enabled_methods'] || isset($_POST['wp_2fa_enabled_methods']) && 'email' === $_POST['wp_2fa_enabled_methods']) {
            ...
            \WP2FA_Vendor\WP2FA\Admin\User::setUserStatus($user);
        }
    ...
    {
        global $wpdb;
        $wpdb->query($wpdb->prepare("\n\t\t\t\tDELETE FROM {$wpdb->usermeta}\n\t\t\t\tWHERE user_id = %d\n\t\t\t\tAND meta_key IN ( %s, %s )\n\t\t\t\t", [$user_id, WP_2FA_PREFIX . 'grace_period_expiry', WP_2FA_PREFIX . 'user_enforced_instantly']));
    }
    /**
    ...
    public function validate_authcode_via_ajax()
    {
        ...
        if (!empty($input['wp-2fa-totp-authcode']) && !empty($current_key)) {
            if (\WP2FA_Vendor\WP2FA\Authenticator\Authentication::is_valid_key($current_key) || !\is_numeric($input['wp-2fa-totp-authcode'])) {
                if (!\WP2FA_Vendor\WP2FA\Authenticator\Authentication::is_valid_authcode($current_key, sanitize_text_field(wp_unslash($input['wp-2fa-totp-authcode'])))) {
            ...
            if (!\WP2FA_Vendor\WP2FA\Authenticator\Authentication::validate_token($user->ID, sanitize_text_field(wp_unslash($input['wp-2fa-email-authcode'])))) {
        ...
        if (!empty($our_errors)) {
            // Send the response.
            wp_send_json_error(array('error' => $our_errors));
        } else {
    ...
    public static function can_user_remove_2fa($user_id)
    {
        ...
        if (\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('hide_remove_button')) {
            return \false;
        }
        ...
        if ('no-grace-period' === $grace_policy) {
            ...
            // is no grace period
            $enforcement_policy = \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enforcement-policy');
            ...
            if ('do-not-enforce' !== $enforcement_policy) {
                ...
                return \WP2FA_Vendor\WP2FA\WP2FA::isUserEnforced($user_id);
            }
        }
        return \true;
    }
