...
use WP2FA_Vendor\WP2FA\Cron\CronTasks;
 ...
 */
if (!\class_exists('WP2FA_Vendor\\WP2FA\\Admin\\User')) {
    /**
    ...
    class User
    {
        ...
        {
            $this->setUser($user);
            $this->globalSettingsHash = \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::create_settings_hash(\WP2FA_Vendor\WP2FA\WP2FA::getAllSettings());
        ...
        public function lock_user_account_if_needed()
        {
            ...
            $settings = \WP2FA_Vendor\WP2FA\Utils\SettingsUtils::get_option(WP_2FA_SETTINGS_NAME);
            ...
            $is_user_instantly_enforced = get_user_meta($user_id, WP_2FA_PREFIX . 'user_enforced_instantly', \true);
            if ($is_user_instantly_enforced) {
                //  no need to lock the account if the user is enforced to set 2FA up instantly
                return \false;
            }
            if (\WP2FA_Vendor\WP2FA\WP2FA::is_user_excluded($user_id)) {
                return \false;
            }
            $grace_period_expiry_time = get_user_meta($user_id, WP_2FA_PREFIX . 'grace_period_expiry', \true);
            if (!empty($grace_period_expiry_time) && $grace_period_expiry_time < \time()) {
                //  set "grace period expired" flag
                $this->setUserMeta(WP_2FA_PREFIX . 'user_grace_period_expired', \true);
                // Send the email to alert the user, only if we have not done so before.
                $account_notification = get_user_meta($user_id, WP_2FA_PREFIX . 'locked_account_notification', \true);
                if (!$account_notification) {
                    \WP2FA_Vendor\WP2FA\Cron\CronTasks::send_expired_grace_email($user_id);
                    $this->setUserMeta(WP_2FA_PREFIX . 'locked_account_notification', \true);
                }
                // Grab user session and kill it, preferably with fire.
                $manager = \WP2FA_Vendor\WP_Session_Tokens::get_instance($user_id);
                $manager->destroy_all();
                return \true;
            }
            return \false;
        ...
        public function setUser($user = '')
        {
            ...
            if (\is_int($user)) {
                $this->user = new \WP2FA_Vendor\WP_User($user);
                return;
            }
            if (empty($user)) {
                $this->user = \WP2FA_Vendor\wp_get_current_user();
            }
        }
        /**
        ...
        {
            if ($this->isUserSet()) {
                $this->setUserMeta(WP_2FA_PREFIX . 'user_needs_to_reconfigure_2fa', \true);
            }
        }
        /**
        ...
        {
            if ($this->isUserSet()) {
                \WP2FA_Vendor\delete_user_meta($this->user->ID, $metaName);
            }
        }
        /**
        ...
        {
            if ($this->isUserSet()) {
                return $this->setUserMeta(WP_2FA_PREFIX . 'user_enforced_instantly', \true);
            }
        }
        /**
        ...
        {
            if ('' === \trim($this->totpKey)) {
                $this->totpKey = \WP2FA_Vendor\WP2FA\Authenticator\Authentication::get_user_totp_key($this->user->ID);
                if (empty($this->totpKey)) {
                    $this->totpKey = \WP2FA_Vendor\WP2FA\Authenticator\Authentication::generate_key();
        ...
        {
            if ($this->isUserSet()) {
                $this->setUserMeta(WP_2FA_PREFIX . 'update_nag_dismissed', \true);
            }
        }
        /**
        ...
        public function checkMethodsAndSetUser()
        {
            if ($this->isUserSet() && !$this->needsToReconfigure2FA()) {
                ...
                $globalMethods = [];
                if (!empty(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enable_totp'))) {
                    $globalMethods[] = 'totp';
                }
                if (!empty(\WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enable_email'))) {
                    $globalMethods[] = 'email';
                }
                if (empty(\array_intersect([$enabledMethodsForTheUser], $globalMethods))) {
                    $this->deleteUserMeta(WP_2FA_PREFIX . 'enabled_methods');
                    if (\WP2FA_Vendor\WP2FA\WP2FA::isUserEnforced($this->user->ID)) {
        ...
        {
            if (\is_a($this->user, 'WP2FA_Vendor\\WP_User')) {
                return \true;
            }
            return \false;
        }
        ...
        private function setUserPoliciesAndGrace()
        {
            ...
            $is_needed = \WP2FA_Vendor\WP2FA\WP2FA::isUserEnforced($this->user->ID);
            $excluded = \WP2FA_Vendor\WP2FA\WP2FA::is_user_excluded($this->user->ID);
            if ($is_needed && !$excluded) {
                ...
                if ('use-grace-period' === $grace_policy) {
                    $custom_grace_period_duration = \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('grace-period') . ' ' . \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('grace-period-denominator');
        ...
        {
            \WP2FA_Vendor\update_user_meta($this->user->ID, $metaKey, $value);
        }
        /**
         ...
         * @since 1.7.0
         */
        public static function setUserStatus(\WP2FA_Vendor\WP_User $user)
        {
            $status = \WP2FA_Vendor\WP2FA\Utils\UserUtils::determine_user_2fa_status($user);
            $status_data = \WP2FA_Vendor\WP2FA\Utils\UserUtils::extractStatuses($status);
        ...
        public static function getUserStatus(\WP2FA_Vendor\WP_User $user)
        {
            return get_user_meta($user->ID, WP_2FA_PREFIX . '2fa_status', \true);
        }
        /**
        ...
        {
            return (bool) get_user_meta($user_id, WP_2FA_PREFIX . 'user_grace_period_expired', \true);
        }
    }
}
