 ...
 */
if (!\class_exists('WP2FA_Vendor\\WP2FA\\Admin\\UserListing')) {
    /**
    ...
    class UserListing
    {
        ...
        private static function getUser2faStatus($userId)
        {
            ...
            $status_meta_value = get_user_meta($userId, WP_2FA_PREFIX . '2fa_status', \true);
            if (!empty($status_meta_value)) {
                //  the status id is available, grab the label to display
                $status_data = \WP2FA_Vendor\WP2FA\Utils\UserUtils::extractStatuses([$status_meta_value]);
            ...
            return \WP2FA_Vendor\WP2FA\Admin\User::setUserStatus(new \WP2FA_Vendor\WP_User($userId));
        }
        /**
