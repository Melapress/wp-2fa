...
class UserNotices
{
    ...
    public function __construct()
    {
        ...
        if (!empty($enforcement_policy)) {
            ...
            } elseif ('do-not-enforce' === \WP2FA_Vendor\WP2FA\WP2FA::get_wp2fa_setting('enforcement-policy')) {
    ...
    public function user_setup_2fa_nag($is_shortcode = '', $configure_2fa_url = '')
    {
        ...
        if (!$current_profile_user_id || isset($_GET['user_id']) && $_GET['user_id'] !== $this->wp2faUser->getUser()->ID || $this->wp2faUser->getEnforcedInstantly()) {
            return;
        }
        ...
        $is_nag_needed = \WP2FA_Vendor\WP2FA\WP2FA::isUserEnforced($this->wp2faUser->getUser()->ID);
        $is_user_excluded = \WP2FA_Vendor\WP2FA\WP2FA::is_user_excluded($this->wp2faUser->getUser()->ID);
        ...
        $setup_url = \WP2FA_Vendor\WP2FA\Admin\Controllers\Settings::getSetupPageLink();
        ...
        if (\WP2FA_Vendor\WP2FA\WP2FA::is_this_multisite() && !is_user_member_of_blog($this->wp2faUser->getUser()->ID)) {
            $new_page_id = \false;
        }
        ...
        if (!$is_nag_dismissed && $is_nag_needed && empty($enabled_methods) && !$is_user_excluded && !empty($grace_expiry)) {
            ...
            echo ' <span class="grace-period-countdown">' . esc_attr(\WP2FA_Vendor\WP2FA\Utils\DateTimeUtils::format_grace_period_expiration_string(null, $grace_expiry)) . '</span>';
    ...
    public function user_reconfigure_2fa_nag()
    {
        ...
        if ($this->wp2faUser->needsToReconfigureMethod()) {
            ...
            echo ' <a href="' . esc_url(\WP2FA_Vendor\WP2FA\Admin\Controllers\Settings::getSetupPageLink()) . '" class="button button-primary">' . esc_html__('Configure 2FA now', 'wp-2fa') . '</a>';
    ...
    {
        $this->wp2faUser = new \WP2FA_Vendor\WP2FA\Admin\User($user_id);
    ...
    public function ensureUser()
    {
        if (!isset($this->wp2faUser)) {
            $this->wp2faUser = new \WP2FA_Vendor\WP2FA\Admin\User();
        }
    }
}
