...
class FS_Admin_Notices
{
    ...
    static function instance($id, $title = '', $module_unique_affix = '', $is_network_and_blog_admins = \false)
    {
        if (!isset(self::$_instances[$id])) {
            self::$_instances[$id] = new \WP2FA_Vendor\FS_Admin_Notices($id, $title, $module_unique_affix, $is_network_and_blog_admins);
        }
    ...
    protected function __construct($id, $title = '', $module_unique_affix = '', $is_network_and_blog_admins = \false)
    {
        ...
        if ($this->_is_multisite) {
            ...
            $this->_network_notices = \WP2FA_Vendor\FS_Admin_Notice_Manager::instance($id, $title, $module_unique_affix, $is_network_and_blog_admins, \true);
        }
        $this->_notices = \WP2FA_Vendor\FS_Admin_Notice_Manager::instance($id, $title, $module_unique_affix, \false, $this->_blog_id);
    }
    /**
     ...
     * @uses   add_action()
     */
    function add($message, $title = '', $type = 'success', $is_sticky = \false, $id = '', $store_if_sticky = \true, $network_level_or_blog_id = null)
    {
        ...
        $notices->add($message, $title, $type, $is_sticky, $id, $store_if_sticky);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.7
     ...
     *                                                blog admin pages.
     */
    function add_sticky($message, $id, $title = '', $type = 'success', $network_level_or_blog_id = null, $wp_user_id = null, $plugin_title = null, $is_network_and_blog_admins = \false)
    {
        if ($this->should_use_network_notices($id, $network_level_or_blog_id)) {
            $notices = $this->_network_notices;
        } else {
    ...
    {
        $this->add($message, $title, $type, $is_sticky, \true, $id);
    }
    #--------------------------------------------------------------------------------
    ...
    private function get_site_notices($blog_id = 0)
    {
        ...
        return \WP2FA_Vendor\FS_Admin_Notice_Manager::instance($this->_id, $this->_title, $this->_module_unique_affix, \false, $blog_id);
    }
    /**
    ...
    private function should_use_network_notices($id = '', $network_level_or_blog_id = null)
    {
        ...
        return \WP2FA_Vendor\fs_is_network_admin();
    }
    #endregion
}
