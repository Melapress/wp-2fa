...
// "final class"
class Freemius extends \WP2FA_Vendor\Freemius_Abstract
{
    /**
     * SDK Version
     *
     * @var string
     */
    public $version = \WP2FA_Vendor\WP_FS__SDK_VERSION;
    #region Plugin Info
    /**
     * @since 1.0.1
     *
    ...
    private function __construct($module_id, $slug = \false, $is_init = \false)
    {
        ...
        $this->_blog_id = \is_multisite() ? \WP2FA_Vendor\get_current_blog_id() : null;
        $this->_storage = \WP2FA_Vendor\FS_Storage::instance($this->_module_type, $this->_slug);
        $this->_cache = \WP2FA_Vendor\FS_Cache_Manager::get_manager(\WP2FA_Vendor\WP_FS___OPTION_PREFIX . "cache_{$module_id}");
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_' . $this->get_unique_affix(), \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
        $this->_plugin_main_file_path = $this->_find_caller_plugin_file($is_init);
        $this->_plugin_dir_path = \WP2FA_Vendor\plugin_dir_path($this->_plugin_main_file_path);
        $this->_plugin_basename = $this->get_plugin_basename();
        $this->_free_plugin_basename = \str_replace('-premium/', '/', $this->_plugin_basename);
        $this->_is_multisite_integrated = \defined("WP_FS__PRODUCT_{$module_id}_MULTISITE") && \true === \constant("WP_FS__PRODUCT_{$module_id}_MULTISITE");
        $this->_is_network_active = \is_multisite() && $this->_is_multisite_integrated && $this->is_plugin() && (\WP2FA_Vendor\is_plugin_active_for_network($this->_plugin_basename) || \WP2FA_Vendor\fs_is_network_admin() && \WP2FA_Vendor\is_plugin_inactive($this->_plugin_basename));
        $this->_storage->set_network_active($this->_is_network_active, $this->is_delegated_connection());
        ...
        if (\is_multisite()) {
             ...
             * @since  2.0.0
             */
            if (\false === $this->_storage->get('install_timestamp', \false, \true) && \false !== $this->_storage->get('install_timestamp', \false, \false)) {
        ...
        if (!isset($this->_storage->install_timestamp)) {
            $this->_storage->install_timestamp = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
        }
        if (!\is_object($this->_plugin)) {
            $this->_plugin = \WP2FA_Vendor\FS_Plugin_Manager::instance($this->_module_id)->get();
        }
        $this->_admin_notices = \WP2FA_Vendor\FS_Admin_Notices::instance(
        ...
        if ('true' === \WP2FA_Vendor\fs_request_get('fs_clear_api_cache') || \WP2FA_Vendor\fs_request_is_action('restart_freemius')) {
            \WP2FA_Vendor\FS_Api::clear_cache();
            $this->_cache->clear();
        }
    ...
    private function maybe_adjust_storage()
    {
        ...
        $is_network_admin = \WP2FA_Vendor\fs_is_network_admin();
        $network_install_timestamp = $this->_storage->get('install_timestamp', null, \true);
        if (!$is_network_admin) {
            ...
            $prev_is_premium = $this->_storage->get('prev_is_premium', null, \true);
        } else {
            ...
            foreach ($sites as $site) {
                ...
                $is_earlier_install = !\is_null($install_timestamp) && $blog_install_timestamp < $install_timestamp;
                ...
                if (!\is_object($install)) {
                    ...
                    if (empty($network_user_info)) {
                        ...
                        $update_network_user_info = \true;
                    }
                    if (!$update_network_user_info && \is_object($current_fs_user) && $network_user_info['user_id'] != $current_fs_user->id && $install->user_id == $current_fs_user->id) {
                        // If an install that is owned by the current WP user is found, use its user information instead.
                        $update_network_user_info = \true;
                    }
                    if (!$update_network_user_info && $is_earlier_install && (!\is_object($current_fs_user) || $current_fs_user->id == $install->user_id)) {
                ...
                if ($update_network_user_info) {
                    $network_user_info = array('user_id' => $install->user_id, 'blog_id' => $blog_id);
                }
            ...
            if ($sites_count === $installs_count + $skips_count) {
                if (!empty($network_user_info)) {
                    ...
                    foreach ($blog_id_2_install_map as $blog_id => $install) {
                        ...
                        $this->_storage->store('is_delegated_connection', \true, $blog_id);
                    }
                }
                if ($sites_count === $skips_count) {
                     ...
                     */
                    $options_to_update['is_anonymous_ms'] = \true;
                } else {
                    if ($sites_count === $installs_count) {
                        /**
                         ...
                         */
                        $options_to_update['is_network_connected'] = \true;
                    }
                }
            }
        }
    ...
    private function adjust_storage($options, $is_network_admin)
    {
        foreach ($options as $name => $value) {
            $this->_storage->store($name, $value, $is_network_admin ? \true : null);
        }
    }
    /**
    ...
    {
        return $this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin() ? $this->_menu->has_network_menu() : $this->_menu->has_menu();
    }
    /**
    ...
    {
        return $this->is_theme() && $this->is_org_repo_compliant() && !$this->is_premium();
    }
    /**
    ...
    function is_submenu_item_visible($slug, $is_tabs_visibility_check = \false)
    {
        ...
        if (!$is_tabs_visibility_check && $this->is_org_repo_compliant() && $this->show_settings_with_tabs()) {
            /**
    ...
    function is_page_visible($slug)
    {
        ...
        return $this->_menu->is_submenu_item_visible($slug, \true, \true);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.9
    ...
    private function _version_updates_handler()
    {
        if (!isset($this->_storage->sdk_version) || $this->_storage->sdk_version != $this->version) {
            ...
            if (empty($this->_storage->sdk_last_version) || \version_compare($this->_storage->sdk_last_version, $this->version, '<')) {
                $this->_storage->sdk_upgrade_mode = \true;
                $this->_storage->sdk_downgrade_mode = \false;
            } else {
                $this->_storage->sdk_downgrade_mode = \true;
        ...
        if (!isset($this->_storage->plugin_version) || $this->_storage->plugin_version != $plugin_version) {
            ...
            if (empty($this->_storage->plugin_last_version) || \version_compare($this->_storage->plugin_last_version, $plugin_version, '<')) {
                $this->_storage->plugin_upgrade_mode = \true;
                $this->_storage->plugin_downgrade_mode = \false;
            } else {
                $this->_storage->plugin_downgrade_mode = \true;
    ...
    function _sdk_version_update($sdk_prev_version, $sdk_version)
    {
        ...
        if (\version_compare($sdk_prev_version, '2.1.0', '<') && \version_compare($sdk_version, '2.1.0', '>=')) {
            $this->_storage->handle_gdpr_admin_notice = \true;
        }
        if (\version_compare($sdk_prev_version, '2.0.0', '<') && \version_compare($sdk_version, '2.0.0', '>=')) {
            ...
            $this->_storage->remove('trial_plan', \true, \false);
        }
        if (\version_compare($sdk_prev_version, '1.2.3', '<') && \version_compare($sdk_version, '1.2.3', '>=')) {
            /**
        ...
        if (\version_compare($sdk_prev_version, '1.1.5', '<') && \version_compare($sdk_version, '1.1.5', '>=')) {
        ...
        if (\version_compare($sdk_prev_version, '2.2.1', '<') && \version_compare($sdk_version, '2.2.1', '>=')) {
            /**
     ...
     */
    private static function migrate_install_plan_to_plan_id(\WP2FA_Vendor\FS_Storage $storage, $blog_id = null)
    {
        ...
        if (isset($install->plan) && \is_object($install->plan)) {
            ...
            self::set_account_option_by_module($module_type, 'sites', $installs, \true, $blog_id);
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    private function migrate_to_subscriptions_collection()
    {
        ...
        if (isset($this->_storage->subscription) && \is_object($this->_storage->subscription)) {
            $this->_storage->subscriptions = array(\WP2FA_Vendor\fs_get_entity($this->_storage->subscription, \WP2FA_Vendor\FS_Subscription::get_class_name()));
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    {
        $plugin_licenses = self::get_account_option('licenses', \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN);
        if (isset($plugin_licenses[$this->_slug])) {
            $plugin_licenses = $plugin_licenses[$this->_slug];
        } else {
            $plugin_licenses = array();
        }
        $theme_licenses = self::get_account_option('licenses', \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME);
        ...
        self::store_user_id_license_ids_map($user_id_license_ids_map, $this->_module_id);
        $this->_store_licenses(\true, $this->_module_id, $all_licenses);
    }
    /**
    ...
    private function remove_invalid_paths()
    {
        ...
        foreach ($file_slug_map as $plugin_basename => $slug) {
            if ($slug === $this->_slug && $plugin_basename !== $this->_plugin_basename && !\file_exists($this->get_absolute_path($plugin_basename))) {
                unset($file_slug_map[$plugin_basename]);
                self::$_accounts->set_option('file_slug_map', $file_slug_map, \true);
                break;
            }
        }
    }
    ...
    private static function migrate_accounts_to_network()
    {
        ...
        foreach ($sites as $site) {
            ...
            foreach ($id_slug_type_path_map as $module_id => $data) {
                if (\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN === $data['type']) {
                    $plugin_slugs[$data['slug']] = \true;
                }
            }
            $installs = self::get_account_option('sites', \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $blog_id);
            if (\is_array($installs)) {
                foreach ($installs as $slug => $install) {
                    if (!isset($connection_status[$slug])) {
                        $connection_status[$slug] = array();
                    }
                    if (\is_object($install) && \WP2FA_Vendor\FS_Site::is_valid_id($install->id) && \WP2FA_Vendor\FS_User::is_valid_id($install->user_id)) {
        ...
        foreach ($plugin_slugs as $slug => $true) {
            ...
            foreach ($sites as $site) {
                ...
                $storage = \WP2FA_Vendor\FS_Storage::instance(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $slug);
                ...
                if (!\is_null($is_anonymous)) {
                    ...
                    if (\is_bool($is_anonymous) && \true === $is_anonymous) {
        ...
        foreach ($connection_status as $slug => $blogs_status) {
            ...
            foreach ($blogs_status as $blog_id => $status_or_user_id) {
                if ('skipped' === $status_or_user_id) {
                    ...
                    if ('ignored' === $status_or_user_id) {
                        $ignores++;
                    } else {
                        if (\WP2FA_Vendor\FS_User::is_valid_id($status_or_user_id)) {
                            $connections++;
                            if (!isset($opted_in_users[$status_or_user_id])) {
                                $opted_in_users[$status_or_user_id] = array();
                            }
                            $opted_in_users[$status_or_user_id][] = $blog_id;
                            if (isset($super_admins[$status_or_user_id]) || self::is_super_admin($status_or_user_id)) {
                                // Cache super-admin data.
                                $super_admins[$status_or_user_id] = \true;
                                // Remember opted-in super-admins for the plugin.
                                $opted_in_super_admins[$status_or_user_id] = \true;
                            }
                        }
                    }
                }
            ...
            if ($sites_count == $skips) {
                ...
                if ($sites_count == $ignores) {
                    // Don't do anything, still in activation mode.
                    $all_migrated = \true;
                } else {
                    if (0 < \count($opted_in_super_admins)) {
                        ...
                        if ($sites_count == $connections && 1 == \count($opted_in_super_admins)) {
                            ...
                            $storage->is_network_connected = \true;
                            $all_migrated = \true;
                        }
                        // Store network user.
                        $storage->network_user_id = $main_super_admin_user_id;
                        $storage->network_install_blog_id = $sites_count == $connections ? \WP2FA_Vendor\get_current_blog_id() : $opted_in_users[$main_super_admin_user_id][0];
                        /**
                        ...
                        if (0 == $ignores) {
                            ...
                            $storage->store('is_delegated_connection', \true, \true);
                            $all_migrated = \true;
                        }
                    }
                }
            ...
            if (!$all_migrated) {
                ...
                foreach ($blogs_status as $blog_id => $status_or_user_id) {
                    ...
                    if (\WP2FA_Vendor\FS_User::is_valid_id($status_or_user_id) || 'skipped' === $status_or_user_id && \is_null($main_super_admin_user_id)) {
                        $storage->store('is_delegated_connection', \true, $blog_id);
                    }
                }
            }
     ...
     * @return bool
     */
    private static function set_network_upgrade_mode(\WP2FA_Vendor\FS_Storage $storage)
    {
        return $storage->is_network_activation = \true;
    }
    /**
    ...
    {
        if (\WP2FA_Vendor\fs_is_network_admin() && $this->is_network_upgrade_mode()) {
    ...
    function _plugins_loaded()
    {
        ...
        $this->_storage->was_plugin_loaded = \true;
        /**
         ...
         * @since  1.2.2.2
         */
        if ($this->is_plugin() && !isset($this->_storage->is_plugin_new_install)) {
            $this->_storage->is_plugin_new_install = !\WP2FA_Vendor\is_plugin_active($this->_plugin_basename) && empty($this->_storage->plugin_last_version);
        }
    }
    /**
    ...
    {
        $admin_param = \WP2FA_Vendor\is_network_admin() ? '_fs_network_admin' : '_fs_blog_admin';
        ?>
    ...
    private function register_constructor_hooks()
    {
        ...
        if (\WP2FA_Vendor\is_admin()) {
            \add_action('admin_init', array(&$this, '_hook_action_links_and_register_account_hooks'));
            if ($this->is_plugin()) {
                if (self::is_plugin_install_page() && \true !== \WP2FA_Vendor\fs_request_get_bool('fs_allow_updater_and_dialog')) {
                    /**
                    ...
                    if (self::is_plugins_page() || self::is_updates_page()) {
                        /**
                 ...
                 */
                \WP2FA_Vendor\register_activation_hook($plugin_dir . $this->_free_plugin_basename, array(&$this, '_activate_plugin_event_hook'));
                \WP2FA_Vendor\register_activation_hook($plugin_dir . $this->premium_plugin_basename(), array(&$this, '_activate_plugin_event_hook'));
            } else {
            ...
            if (empty($this->_storage->was_plugin_loaded)) {
                 ...
                 * @author Vova Feldman (@svovaf)
                 */
                if ($this->is_plugin() && $this->is_activation_mode(\false) && 0 == \WP2FA_Vendor\did_action('plugins_loaded')) {
            ...
            if (!self::is_ajax()) {
                if (!$this->is_addon()) {
                    \add_action('init', array(&$this, '_add_default_submenu_items'), \WP2FA_Vendor\WP_FS__LOWEST_PRIORITY);
                }
            }
        ...
        if ($this->is_plugin()) {
            ...
            \WP2FA_Vendor\register_deactivation_hook($this->_plugin_main_file_path, array(&$this, '_deactivate_plugin_hook'));
        }
        if (\is_multisite()) {
        ...
        if ($this->is_theme() && self::is_customizer() && $this->apply_filters('show_customizer_upsell', \true)) {
            // Register customizer upsell.
            \add_action('customize_register', array(&$this, '_customizer_register'));
        }
        \add_action('admin_init', array(&$this, '_redirect_on_clicked_menu_link'), \WP2FA_Vendor\WP_FS__LOWEST_PRIORITY);
        ...
        if ($this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin()) {
            $this->add_ajax_action('network_activate', array(&$this, '_network_activate_ajax_action'));
        }
        $this->add_ajax_action('install_premium_version', array(&$this, '_install_premium_version_ajax_action'));
        $this->add_ajax_action('submit_affiliate_application', array(&$this, '_submit_affiliate_application'));
        $this->add_action('after_plans_sync', array(&$this, '_check_for_trial_plans'));
        $this->add_action('sdk_version_update', array(&$this, '_sdk_version_update'), \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, 2);
        $this->add_action('plugin_version_update', array(&$this, '_after_version_update'), \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, 2);
         ...
         * @since  1.2.1.5
         */
        if (\WP2FA_Vendor\fs_request_is_action('reset_anonymous_mode') && $this->get_unique_affix() === \WP2FA_Vendor\fs_request_get('fs_unique_affix')) {
    ...
    {
        if (\WP2FA_Vendor\is_admin() && $this->is_theme() && $this->is_premium() && !$this->has_active_valid_license()) {
            $this->add_ajax_action('delete_theme_update_data', array(&$this, '_delete_theme_update_data_action'));
        }
        ...
        if (!self::is_ajax()) {
            if (!$this->is_addon() || $this->is_only_premium()) {
                \add_action(($this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin() ? 'network_' : '') . 'admin_menu', array(&$this, '_prepare_admin_menu'), \WP2FA_Vendor\WP_FS__LOWEST_PRIORITY);
            }
        }
    }
    /**
    ...
    static function _prepend_fs_allow_updater_and_dialog_flag_url_param()
    {
        ...
        ?>
            <script type="text/javascript">
            (function( $ ) {
                var slugBasenameMap = <?php 
        echo \json_encode($slug_basename_map);
        ?>;
    ...
    static function _maybe_add_beta_label_styles()
    {
        ...
        foreach (self::$_instances as $instance) {
            if ($instance->is_beta()) {
                $has_any_beta_version = \true;
                break;
            }
        }
        if ($has_any_beta_version) {
            \WP2FA_Vendor\fs_enqueue_local_style('fs_plugins', '/admin/plugins.css');
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    static function _maybe_add_beta_label_to_plugins_and_handle_confirmation()
    {
        ...
        foreach (self::$_instances as $instance) {
            ...
            $is_beta = $instance->is_plugin() && $instance->is_beta();
            ...
            $beta_data[$instance->get_plugin_basename()]['beta_version_update_confirmation_message'] = \sprintf('%s %s', \sprintf(\WP2FA_Vendor\fs_esc_attr_inline('An update to a Beta version will replace your installed version of %s with the latest Beta release - use with caution, and not on production sites. You have been warned.', 'beta-version-update-caution', $instance->get_slug()), $instance->get_plugin_title()), \WP2FA_Vendor\fs_esc_attr_inline('Would you like to proceed with the update?', 'update-confirmation', $instance->get_slug()));
        }
        if (empty($beta_data)) {
            return;
        }
        ...
        ?>;
                    ...
                    for ( var pluginBasename in betaData ) {
                        ...
                        $parentContainer.find( '.plugin-title > strong:first-child').append(
                            '<span class="fs-tag fs-info"><?php 
        \WP2FA_Vendor\fs_esc_js_echo_inline('Beta', 'beta');
        ?></span>'
                        );
                    }
    ...
    {
        $uninstallable_plugins = (array) \WP2FA_Vendor\get_option('uninstall_plugins');
        unset($uninstallable_plugins[$this->_free_plugin_basename]);
        unset($uninstallable_plugins[$this->premium_plugin_basename()]);
        \WP2FA_Vendor\update_option('uninstall_plugins', $uninstallable_plugins);
    }
    /**
    ...
    {
        if (!isset($this->_storage->plugin_main_file) || empty($this->_storage->plugin_main_file->path)) {
            return;
        }
        if (!$store_prev_path) {
            /**
        ...
        self::$_accounts->set_option('id_slug_type_path_map', $id_slug_type_path_map, \true);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.0.0
    ...
    private function _register_account_hooks()
    {
         ...
         * @since 1.2.1.6
         */
        $this->add_ajax_action('submit_uninstall_reason', array(&$this, '_submit_uninstall_reason_action'));
        $this->add_ajax_action('cancel_subscription_or_trial', array(&$this, 'cancel_subscription_or_trial_ajax_action'));
        if (!$this->is_addon() || $this->is_parent_plugin_installed()) {
            if ($this->is_plugin() && self::is_plugins_page() || $this->is_theme() && self::is_themes_page()) {
    ...
    private function _find_caller_plugin_file($is_init = \false)
    {
        ...
        if (!$is_init) {
            ...
            if (isset($this->_storage->plugin_main_file) && !empty($this->_storage->plugin_main_file->prev_path)) {
                $absolute_path = $this->get_absolute_path($this->_storage->plugin_main_file->prev_path);
                if (\file_exists($absolute_path)) {
                    return $absolute_path;
                }
            }
            \wp_die($this->get_text_inline('Freemius SDK couldn\'t find the plugin\'s main file. Please contact sdk@freemius.com with the current error.', 'failed-finding-main-path') . " Module: {$this->_slug}; SDK: " . \WP2FA_Vendor\WP_FS__SDK_VERSION . ";", $this->get_text_inline('Error', 'error'), array('back_link' => \true));
        }
        /**
         * @since 1.2.1
         *
        ...
        $this->_storage->plugin_main_file = (object) array('path' => $id_slug_type_path_map[$this->_module_id]['path']);
    ...
    private function get_absolute_path($path, $module_type = \false)
    {
        ...
        if (0 !== \strpos($path, $module_root_dir)) {
            $path = \WP2FA_Vendor\fs_normalize_path($module_root_dir . $path);
        }
        return $path;
    }
    /**
    ...
    {
        $is_plugin = empty($module_type) ? $this->is_plugin() : \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN === $module_type;
        return \WP2FA_Vendor\fs_normalize_path(\trailingslashit($is_plugin ? \WP2FA_Vendor\WP_PLUGIN_DIR : \WP2FA_Vendor\get_theme_root(\get_stylesheet())));
    }
    /**
     * @author Leo Fajardo (@leorw)
     *
    ...
    private function store_id_slug_type_path_map($module_id, $slug)
    {
        ...
        if (!isset($id_slug_type_path_map[$module_id])) {
            $id_slug_type_path_map[$module_id] = array('slug' => $slug);
            $store_option = \true;
        } else {
            if (isset($id_slug_type_path_map[$module_id]['slug']) && $slug !== $id_slug_type_path_map[$module_id]['slug']) {
                $id_slug_type_path_map[$module_id]['slug'] = $slug;
                $store_option = \true;
            }
        }
        if (empty($id_slug_type_path_map[$module_id]['path']) || !\file_exists($this->get_absolute_path($id_slug_type_path_map[$module_id]['path'], $id_slug_type_path_map[$module_id]['type']))) {
    ...
    private function get_caller_main_file_and_type()
    {
        ...
        foreach ($all_plugins as $relative_path => $data) {
            if (\false === \strpos(\WP2FA_Vendor\fs_normalize_path($relative_path), '/')) {
                /**
            ...
            $all_plugins_paths[] = \WP2FA_Vendor\fs_normalize_path(\realpath(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $relative_path));
        }
        $caller_file_candidate = \false;
        $caller_map = array();
        $module_type = \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN;
        $themes_dir = \WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\get_theme_root(\get_stylesheet()));
        ...
        for ($i = 1, $bt = \debug_backtrace(), $len = \count($bt); $i < $len; $i++) {
            ...
            if (!empty($bt[$i]['function']) && \in_array($bt[$i]['function'], array(
                ...
                if ('activate_plugin' === $bt[$i]['function']) {
                     ...
                     * @since 2.3.0
                     */
                    $caller_file_path = \WP2FA_Vendor\fs_normalize_path($bt[$i]['file']);
                    foreach ($all_plugins_paths as $plugin_path) {
                        $plugin_dir = \WP2FA_Vendor\fs_normalize_path(\dirname($plugin_path) . '/');
            ...
            if ('functions.php' === \basename($caller_file_path)) {
                 ...
                 * @since  1.2.2.5
                 */
                if ($caller_file_path == \WP2FA_Vendor\fs_normalize_path(\realpath(\trailingslashit($themes_dir) . \basename(\dirname($caller_file_path)) . '/' . \basename($caller_file_path)))) {
                    $module_type = \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME;
                    /**
                     ...
                     * @author Leo Fajardo (@leorw)
                     */
                    $caller_file_candidate = \basename(\dirname($caller_file_path)) . '/' . \basename($caller_file_path);
                    continue;
                }
            }
            ...
            if (!isset($caller_map[$caller_file_hash])) {
                foreach ($all_plugins_paths as $plugin_path) {
                    ...
                    if (\false !== \strpos($caller_file_path, \WP2FA_Vendor\fs_normalize_path(\dirname($plugin_path) . '/'))) {
                        $caller_map[$caller_file_hash] = \WP2FA_Vendor\fs_normalize_path($plugin_path);
                        break;
                    }
                }
            }
            if (isset($caller_map[$caller_file_hash])) {
                $module_type = \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN;
                $caller_file_candidate = \WP2FA_Vendor\plugin_basename($caller_map[$caller_file_hash]);
            }
        }
        return (object) array('module_type' => $module_type, 'path' => $caller_file_candidate);
    }
    #----------------------------------------------------------------------------------
    ...
    {
        $subscription_cancellation_dialog_box_template_params = $this->apply_filters('show_deactivation_subscription_cancellation', \true) ? $this->_get_subscription_cancellation_dialog_box_template_params() : array();
        /**
         ...
         */
        $show_deactivation_feedback_form = \true;
        if ($this->has_filter('show_deactivation_feedback_form')) {
            $show_deactivation_feedback_form = $this->apply_filters('show_deactivation_feedback_form', \true);
        } else {
            if ($this->is_addon()) {
                /**
                 * If the add-on's 'show_deactivation_feedback_form' is not set, try to inherit the value from the parent.
                 */
                $show_deactivation_feedback_form = $this->get_parent_instance()->apply_filters('show_deactivation_feedback_form', \true);
            }
        }
        $uninstall_confirmation_message = $this->apply_filters('uninstall_confirmation_message', '');
        if (empty($subscription_cancellation_dialog_box_template_params) && !$show_deactivation_feedback_form && empty($uninstall_confirmation_message)) {
            return;
        }
        ...
        if ($show_deactivation_feedback_form) {
             ...
             */
            $is_long_term_user = \true;
            // Check if the site is at least 2 days old.
            $time_installed = $this->_storage->install_timestamp;
            // Difference in seconds.
            $date_diff = \time() - $time_installed;
         ...
         */
        \WP2FA_Vendor\fs_require_template('forms/deactivation/form.php', $vars);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  1.1.2
    ...
    function _get_uninstall_reasons($user_type = 'long-term')
    {
        ...
        $internal_message_template_var = array('id' => $this->_module_id);
        $plan = $this->get_plan();
        if ($this->is_registered() && \is_object($plan) && $plan->has_technical_support()) {
            $contact_support_template = \WP2FA_Vendor\fs_get_template('forms/deactivation/contact.php', $internal_message_template_var);
        } else {
            $contact_support_template = '';
        }
        $reason_found_better_plugin = array('id' => self::REASON_FOUND_A_BETTER_PLUGIN, 'text' => \sprintf($this->get_text_inline('I found a better %s', 'reason-found-a-better-plugin'), $module_type), 'input_type' => 'textfield', 'input_placeholder' => \sprintf($this->get_text_inline("What's the %s's name?", 'placeholder-plugin-name'), $module_type));
        $reason_temporary_deactivation = array('id' => self::REASON_TEMPORARY_DEACTIVATION, 'text' => \sprintf($this->get_text_inline("It's a temporary %s. I'm just debugging an issue.", 'reason-temporary-x'), \strtolower($this->is_plugin() ? $this->get_text_inline('Deactivation', 'deactivation') : $this->get_text_inline('Theme Switch', 'theme-switch'))), 'input_type' => '', 'input_placeholder' => '');
        $reason_other = array('id' => self::REASON_OTHER, 'text' => $this->get_text_inline('Other', 'reason-other'), 'input_type' => 'textfield', 'input_placeholder' => '');
        ...
        if ($this->is_paying()) {
            $long_term_user_reasons[] = array('id' => self::REASON_CANT_PAY_ANYMORE, 'text' => $this->get_text_inline("I can't pay for it anymore", 'reason-cant-pay-anymore'), 'input_type' => 'textfield', 'input_placeholder' => $this->get_text_inline('What price would you feel comfortable paying?', 'placeholder-comfortable-price'));
        }
        $reason_dont_share_info = array('id' => self::REASON_DONT_LIKE_TO_SHARE_MY_INFORMATION, 'text' => $this->get_text_inline("I don't like to share my information with you", 'reason-dont-like-to-share-my-information'), 'input_type' => '', 'input_placeholder' => '');
        /**
    ...
    function _submit_uninstall_reason_action()
    {
        ...
        $reason_id = \WP2FA_Vendor\fs_request_get('reason_id');
        // Check if the given reason ID is an unsigned integer.
        if (!\ctype_digit($reason_id)) {
            exit;
        }
        $reason_info = \trim(\WP2FA_Vendor\fs_request_get('reason_info', ''));
        if (!empty($reason_info)) {
            $reason_info = \substr($reason_info, 0, 128);
        }
        $reason = (object) array('id' => $reason_id, 'info' => $reason_info, 'is_anonymous' => \WP2FA_Vendor\fs_request_get_bool('is_anonymous'));
        ...
        if ($this->is_theme()) {
            if ($this->is_premium() && !$this->has_active_valid_license()) {
                \WP2FA_Vendor\FS_Plugin_Updater::instance($this)->delete_update_data();
            }
    ...
    function cancel_subscription_or_trial_ajax_action()
    {
        ...
        $result = $this->cancel_subscription_or_trial(\WP2FA_Vendor\fs_request_get('plugin_id', $this->get_id()), \false);
    ...
    private function cancel_subscription_or_trial($plugin_id)
    {
        ...
        $result = null;
        if (!\is_null($fs)) {
            $result = $fs->is_paid_trial() ? $fs->_cancel_trial() : $fs->_downgrade_site();
        }
        return $result;
    }
    /**
    ...
    static function instance($module_id, $slug = \false, $is_init = \false)
    {
        ...
        if (!\is_numeric($module_id)) {
            ...
            $slug = $module_id;
            $module = \WP2FA_Vendor\FS_Plugin_Manager::instance($slug)->get();
            if (\is_object($module)) {
                $module_id = $module->id;
            }
        }
        ...
        if (!isset(self::$_instances[$key])) {
            self::$_instances[$key] = new \WP2FA_Vendor\Freemius($module_id, $slug, $is_init);
        }
     ...
     * @return number|false
     */
    private static function get_module_id($id_or_slug, $module_type = \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN)
    {
    ...
    {
        return isset(self::$_instances['m_' . $id]) ? self::$_instances['m_' . $id] : \false;
    }
    /**
     *
     * @author Vova Feldman (@svovaf)
     ...
     * @return false|Freemius
     */
    static function get_instance_by_file($plugin_file, $module_type = \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN)
    {
        $slug = self::find_slug_by_basename($plugin_file);
        return \false !== $slug ? self::instance(self::get_module_id($slug, $module_type)) : \false;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.6
    ...
    function is_parent_plugin_installed()
    {
        ...
        if (\is_object($fs_active_plugins) && \is_array($fs_active_plugins->plugins)) {
            $active_theme = \WP2FA_Vendor\wp_get_theme();
            foreach ($fs_active_plugins->plugins as $sdk => $module) {
                if (\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME === $module->type) {
    ...
    {
        return \WP2FA_Vendor\fs_is_network_admin() ? $this->is_network_activation_mode($and_on) : $this->is_site_activation_mode($and_on);
    }
    /**
     * Is plugin in activation mode.
     *
    ...
    {
        return ($this->is_on() || !$and_on) && ($this->is_premium() && \true === $this->_storage->require_license_activation || (!$this->is_registered() || $this->is_only_premium() && !$this->has_features_enabled_license()) && (!$this->is_enable_anonymous() || !$this->is_anonymous() && !$this->is_pending_activation()));
    }
    /**
    ...
    private function is_network_activation_mode($and_on = \true)
    {
        ...
        if ($this->is_network_anonymous() && \true !== $this->_storage->require_license_activation) {
    ...
    private static function get_active_plugins_basenames($blog_id = 0)
    {
        if (\is_multisite() && $blog_id > 0) {
            $active_basenames = \WP2FA_Vendor\get_blog_option($blog_id, 'active_plugins');
        } else {
            $active_basenames = \WP2FA_Vendor\get_option('active_plugins');
        }
        if (!\is_array($active_basenames)) {
            $active_basenames = array();
        }
        if (\is_multisite()) {
            $network_active_basenames = \WP2FA_Vendor\get_site_option('active_sitewide_plugins');
    ...
    static function get_active_plugins_directories_map($blog_id = 0)
    {
        ...
        foreach ($active_basenames as $active_basename) {
            $active_basename = \WP2FA_Vendor\fs_normalize_path($active_basename);
            if (\false === \strpos($active_basename, '/')) {
                continue;
            }
            $map[\dirname($active_basename)] = \true;
        }
        return $map;
    }
    /**
    ...
    private static function get_active_plugins($blog_id = 0)
    {
        self::require_plugin_essentials();
        $active_plugin = array();
        $all_plugins = \WP2FA_Vendor\fs_get_plugins();
    ...
    {
        $active_basenames = \is_multisite() && $blog_id > 0 ? \WP2FA_Vendor\get_blog_option($blog_id, 'active_plugins') : \WP2FA_Vendor\get_option('active_plugins');
        $active = array();
        if (!\is_array($active_basenames)) {
            return $active;
        }
    ...
    private static function get_all_plugins($blog_id = 0)
    {
        ...
        foreach ($active_plugins_basenames as $basename) {
            if (isset($all_plugins[$basename])) {
                $all_plugins[$basename]['is_active'] = \true;
            }
        }
        return $all_plugins;
    }
    ...
    private static function get_network_plugins()
    {
        self::require_plugin_essentials();
        $all_plugins = \WP2FA_Vendor\fs_get_plugins();
        $network_active_basenames = \is_multisite() ? \WP2FA_Vendor\get_site_option('active_sitewide_plugins') : array();
        foreach ($all_plugins as $basename => &$data) {
            // By default set to inactive (next foreach update the active plugins).
            $data['is_active'] = \false;
            // Enrich with plugin slug.
    ...
    private static function _load_required_static()
    {
        ...
        self::$_static_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG, \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
        self::$_static_logger->entrance();
        self::$_accounts = \WP2FA_Vendor\FS_Options::instance(\WP2FA_Vendor\WP_FS__ACCOUNTS_OPTION_NAME, \true);
        if (\is_multisite()) {
            $has_skipped_migration = null === self::$_accounts->get_option('id_slug_type_path_map', null, \false) && null !== self::$_accounts->get_option('file_slug_map', null, \false);
            /**
             ...
             * @since  2.0.0
             */
            if ($has_skipped_migration && \true !== self::$_accounts->get_option('ms_migration_complete', \false, \true) || null === self::$_accounts->get_option('file_slug_map', null, \true) && null !== self::$_accounts->get_option('file_slug_map', null, \false)) {
                self::migrate_options_to_network();
            }
        }
        self::$_global_admin_notices = \WP2FA_Vendor\FS_Admin_Notices::instance('global');
        if (!\WP2FA_Vendor\WP_FS__DEMO_MODE) {
            \add_action((\WP2FA_Vendor\fs_is_network_admin() ? 'network_' : '') . 'admin_menu', array('Freemius', '_add_debug_section'));
        }
        ...
        if (0 == \WP2FA_Vendor\did_action('plugins_loaded')) {
    ...
    private static function migrate_options_to_network()
    {
        ...
        $api_network_options = \WP2FA_Vendor\FS_Option_Manager::get_manager(\WP2FA_Vendor\WP_FS__OPTIONS_OPTION_NAME, \true, \true);
        $api_network_options->migrate_to_network();
        // Migrate API cache to network level storage.
        \WP2FA_Vendor\FS_Cache_Manager::get_manager(\WP2FA_Vendor\WP_FS__API_CACHE_OPTION_NAME)->migrate_to_network();
        self::$_accounts->set_option('ms_migration_complete', \true, \true);
    }
    #----------------------------------------------------------------------------------
    #region Localization
    #----------------------------------------------------------------------------------
    ...
    static function _load_textdomain()
    {
        ...
        \WP2FA_Vendor\load_plugin_textdomain('freemius', \false, $fs_active_plugins->newest->sdk_path . '/languages/');
    }
    #endregion
    #----------------------------------------------------------------------------------
    #region Debugging
    ...
    {
        if (!\WP2FA_Vendor\is_super_admin()) {
            // Add debug page only for super-admins.
            return;
        }
        self::$_static_logger->entrance();
        $title = \sprintf('%s [v.%s]', \WP2FA_Vendor\fs_text_inline('Freemius Debug'), \WP2FA_Vendor\WP_FS__SDK_VERSION);
        if (\WP2FA_Vendor\WP_FS__DEV_MODE) {
            // Add top-level debug menu item.
            $hook = \WP2FA_Vendor\FS_Admin_Menu_Manager::add_page($title, $title, 'manage_options', 'freemius', array('Freemius', '_debug_page_render'));
        } else {
            // Add hidden debug page.
            $hook = \WP2FA_Vendor\FS_Admin_Menu_Manager::add_subpage(null, $title, $title, 'manage_options', 'freemius', array('Freemius', '_debug_page_render'));
        }
        if (!empty($hook)) {
    ...
    static function _toggle_debug_mode()
    {
        ...
        $is_on = \WP2FA_Vendor\fs_request_get('is_on', \false, 'post');
        if (\WP2FA_Vendor\fs_request_is_post() && \in_array($is_on, array(0, 1))) {
            \WP2FA_Vendor\update_option('fs_debug_mode', $is_on);
            // Turn on/off storage logging.
            \WP2FA_Vendor\FS_Logger::_set_storage_logging(1 == $is_on);
        }
        exit;
    }
    /**
    ...
    {
        $logs = \WP2FA_Vendor\FS_Logger::load_db_logs(\WP2FA_Vendor\fs_request_get('filters', \false, 'post'), !empty($_POST['limit']) && \is_numeric($_POST['limit']) ? $_POST['limit'] : 200, !empty($_POST['offset']) && \is_numeric($_POST['offset']) ? $_POST['offset'] : 0);
    ...
    {
        \WP2FA_Vendor\check_admin_referer('fs_get_db_option');
        $option_name = \WP2FA_Vendor\fs_request_get('option_name');
        if (!\WP2FA_Vendor\is_super_admin() || !\WP2FA_Vendor\fs_starts_with($option_name, 'fs_')) {
            self::shoot_ajax_failure();
        }
        $value = \WP2FA_Vendor\get_option($option_name);
        $result = array('name' => $option_name);
        if (\false !== $value) {
            if (!\is_string($value)) {
    ...
    {
        \WP2FA_Vendor\check_admin_referer('fs_set_db_option');
        $option_name = \WP2FA_Vendor\fs_request_get('option_name');
        if (!\WP2FA_Vendor\is_super_admin() || !\WP2FA_Vendor\fs_starts_with($option_name, 'fs_')) {
            self::shoot_ajax_failure();
        }
        $option_value = \WP2FA_Vendor\fs_request_get('option_value');
        if (!empty($option_value)) {
            \WP2FA_Vendor\update_option($option_name, $option_value);
        }
    ...
    static function _debug_page_actions()
    {
        ...
        if (\WP2FA_Vendor\fs_request_is_action('restart_freemius')) {
            \WP2FA_Vendor\check_admin_referer('restart_freemius');
            if (!\is_multisite()) {
                // Clear accounts data.
                self::$_accounts->clear(null, \true);
            } else {
                $sites = self::get_sites();
                foreach ($sites as $site) {
                    $blog_id = self::get_site_blog_id($site);
                    self::$_accounts->clear($blog_id, \true);
                }
                // Clear network level storage.
                self::$_accounts->clear(\true, \true);
            }
            // Clear SDK reference cache.
            \WP2FA_Vendor\delete_option('fs_active_plugins');
        } else {
            if (\WP2FA_Vendor\fs_request_is_action('clear_updates_data')) {
                \WP2FA_Vendor\check_admin_referer('clear_updates_data');
                if (!\is_multisite()) {
                    \set_site_transient('update_plugins', null);
                    \set_site_transient('update_themes', null);
                } else {
                    $current_blog_id = \WP2FA_Vendor\get_current_blog_id();
                    $sites = self::get_sites();
                    foreach ($sites as $site) {
                        \WP2FA_Vendor\switch_to_blog(self::get_site_blog_id($site));
                        \set_site_transient('update_plugins', null);
                        \set_site_transient('update_themes', null);
                    }
                    \WP2FA_Vendor\switch_to_blog($current_blog_id);
                }
            } else {
                if (\WP2FA_Vendor\fs_request_is_action('simulate_trial')) {
                    \WP2FA_Vendor\check_admin_referer('simulate_trial');
                    $fs = \WP2FA_Vendor\freemius(\WP2FA_Vendor\fs_request_get('module_id'));
                    // Update SDK install to at least 24 hours before.
                    $fs->_storage->install_timestamp = \time() - \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC;
                    // Unset the trial shown timestamp.
                    unset($fs->_storage->trial_promotion_shown);
                } else {
                    if (\WP2FA_Vendor\fs_request_is_action('simulate_network_upgrade')) {
                        \WP2FA_Vendor\check_admin_referer('simulate_network_upgrade');
                        $fs = \WP2FA_Vendor\freemius(\WP2FA_Vendor\fs_request_get('module_id'));
                        self::set_network_upgrade_mode($fs->_storage);
                    } else {
                        if (\WP2FA_Vendor\fs_request_is_action('delete_install')) {
                            \WP2FA_Vendor\check_admin_referer('delete_install');
                            self::_delete_site_by_slug(\WP2FA_Vendor\fs_request_get('slug'), \WP2FA_Vendor\fs_request_get('module_type'), \true, \WP2FA_Vendor\fs_request_get('blog_id', null));
                        } else {
                            if (\WP2FA_Vendor\fs_request_is_action('delete_user')) {
                                \WP2FA_Vendor\check_admin_referer('delete_user');
                                self::delete_user(\WP2FA_Vendor\fs_request_get('user_id'));
                            } else {
                                if (\WP2FA_Vendor\fs_request_is_action('download_logs')) {
                                    \WP2FA_Vendor\check_admin_referer('download_logs');
                                    $download_url = \WP2FA_Vendor\FS_Logger::download_db_logs(\WP2FA_Vendor\fs_request_get('filters', \false, 'post'));
                                    ...
                                    \WP2FA_Vendor\fs_redirect($download_url);
                                } else {
                                    if (\WP2FA_Vendor\fs_request_is_action('migrate_options_to_network')) {
                                        \WP2FA_Vendor\check_admin_referer('migrate_options_to_network');
                                        self::migrate_options_to_network();
                                    }
                                }
                            }
    ...
    static function _debug_page_render()
    {
        self::$_static_logger->entrance();
        if (!\is_multisite()) {
            $all_plugins_installs = self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN);
            $all_themes_installs = self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME);
        } else {
            $sites = self::get_sites();
            ...
            foreach ($sites as $site) {
                $blog_id = self::get_site_blog_id($site);
                $plugins_installs = self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $blog_id);
                ...
                $themes_installs = self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME, $blog_id);
        ...
        $vars = array('plugin_sites' => $all_plugins_installs, 'theme_sites' => $all_themes_installs, 'users' => self::get_all_users(), 'addons' => self::get_all_addons(), 'account_addons' => self::get_all_account_addons(), 'plugin_licenses' => $licenses_by_module_type[\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN], 'theme_licenses' => $licenses_by_module_type[\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME]);
        \WP2FA_Vendor\fs_enqueue_local_style('fs_debug', '/admin/debug.css');
        \WP2FA_Vendor\fs_require_once_template('debug.php', $vars);
    }
    #endregion
    #----------------------------------------------------------------------------------
    ...
    private function should_run_connectivity_test($flush_if_no_connectivity = \false)
    {
        ...
        if (\WP2FA_Vendor\WP_FS__PING_API_ON_IP_OR_HOST_CHANGES) {
            if (\WP2FA_Vendor\WP_FS__IS_HTTP_REQUEST) {
                if ($_SERVER['HTTP_HOST'] != $this->_storage->connectivity_test['host']) {
                    // Domain changed.
                    return \true;
                }
                if (\WP2FA_Vendor\WP_FS__REMOTE_ADDR != $this->_storage->connectivity_test['server_ip']) {
                    // Server IP changed.
                    return \true;
                }
            }
        }
        if ($this->_storage->connectivity_test['is_connected'] && $this->_storage->connectivity_test['is_active']) {
        ...
        if ($flush_if_no_connectivity) {
             ...
             */
            return isset($this->_storage->connectivity_test['timestamp']) && \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME - $this->_storage->connectivity_test['timestamp'] > 10;
        }
        /**
    ...
    {
        if (\WP2FA_Vendor\WP_FS__SIMULATE_NO_API_CONNECTIVITY) {
            return \false;
        }
        $version = $this->get_plugin_version();
        $is_update = $this->apply_filters('is_plugin_update', $this->is_plugin_update());
        return $this->get_api_plugin_scope()->ping($this->get_anonymous_id($blog_id), array('is_update' => \json_encode($is_update), 'version' => $version, 'sdk' => $this->version, 'is_admin' => \json_encode(\WP2FA_Vendor\is_admin()), 'is_ajax' => \json_encode(self::is_ajax()), 'is_cron' => \json_encode(self::is_cron()), 'is_gdpr_test' => $is_gdpr_test, 'is_http' => \json_encode(\WP2FA_Vendor\WP_FS__IS_HTTP_REQUEST)));
    }
    /**
    ...
    function has_api_connectivity($flush_if_no_connectivity = \false)
    {
        ...
        if (\WP2FA_Vendor\WP_FS__SIMULATE_NO_API_CONNECTIVITY && isset($this->_storage->connectivity_test) && \true === $this->_storage->connectivity_test['is_connected']) {
            unset($this->_storage->connectivity_test);
        }
        if (!$this->should_run_connectivity_test($flush_if_no_connectivity)) {
             ...
             */
            $this->_is_on = $this->_storage->connectivity_test['is_active'] || $this->is_premium() || \WP2FA_Vendor\WP_FS__DEV_MODE && $this->_has_api_connection && !\WP2FA_Vendor\WP_FS__SIMULATE_FREEMIUS_OFF;
            return $this->_has_api_connection;
        }
        $pong = $this->ping();
        ...
        if ($is_connected) {
            \WP2FA_Vendor\FS_GDPR_Manager::instance()->store_is_required($pong->is_gdpr_required);
        }
    ...
    private function store_connectivity_info($pong, $is_connected)
    {
        $this->_logger->entrance();
        $version = $this->get_plugin_version();
        if (!$is_connected || \WP2FA_Vendor\WP_FS__SIMULATE_FREEMIUS_OFF) {
            $is_active = \false;
        } else {
            $is_active = isset($pong->is_active) && \true == $pong->is_active;
        }
        $is_active = $this->apply_filters('is_on', $is_active, $this->is_plugin_update(), $version);
        $this->_storage->connectivity_test = array(
            'is_connected' => $is_connected,
            'host' => $_SERVER['HTTP_HOST'],
            'server_ip' => \WP2FA_Vendor\WP_FS__REMOTE_ADDR,
            'is_active' => $is_active,
            'timestamp' => \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME,
            // Last version with connectivity attempt.
            'version' => $version,
        );
        $this->_has_api_connection = $is_connected;
        $this->_is_on = $is_active || \WP2FA_Vendor\WP_FS__DEV_MODE && $is_connected && !\WP2FA_Vendor\WP_FS__SIMULATE_FREEMIUS_OFF;
    }
    /**
    ...
    private function turn_on()
    {
        ...
        $updated_connectivity['is_active'] = \true;
        $updated_connectivity['timestamp'] = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
    ...
    function get_anonymous_id($blog_id = null)
    {
        ...
        if (empty($unique_id) || !\is_string($unique_id)) {
            $key = \WP2FA_Vendor\fs_strip_url_protocol(\WP2FA_Vendor\get_site_url($blog_id));
            $secure_auth = \WP2FA_Vendor\SECURE_AUTH_KEY;
            if (empty($secure_auth) || \false !== \strpos($secure_auth, ' ') || 'put your unique phrase here' === $secure_auth) {
            ...
            self::$_accounts->set_option('unique_id', $unique_id, \true, $blog_id);
        }
    ...
    static function _get_current_wp_user()
    {
        ...
        return \WP2FA_Vendor\wp_get_current_user();
    }
    /**
    ...
    {
        if (\defined('WP2FA_Vendor\\LOGGED_IN_COOKIE') && (\defined('WP2FA_Vendor\\AUTH_COOKIE') || \defined('WP2FA_Vendor\\SECURE_AUTH_COOKIE'))) {
            return;
        }
        /**
         ...
         * @since 1.5.0
         */
        if (!\defined('WP2FA_Vendor\\COOKIEHASH')) {
            $siteurl = \WP2FA_Vendor\get_site_option('siteurl');
            if ($siteurl) {
                \define('WP2FA_Vendor\\COOKIEHASH', \md5($siteurl));
            } else {
                \define('WP2FA_Vendor\\COOKIEHASH', '');
            }
        }
        if (!\defined('WP2FA_Vendor\\LOGGED_IN_COOKIE')) {
            \define('WP2FA_Vendor\\LOGGED_IN_COOKIE', 'wordpress_logged_in_' . \WP2FA_Vendor\COOKIEHASH);
        }
        /**
         * @since 2.5.0
         */
        if (!\defined('WP2FA_Vendor\\AUTH_COOKIE')) {
            \define('WP2FA_Vendor\\AUTH_COOKIE', 'wordpress_' . \WP2FA_Vendor\COOKIEHASH);
        }
        /**
         * @since 2.6.0
         */
        if (!\defined('WP2FA_Vendor\\SECURE_AUTH_COOKIE')) {
            \define('WP2FA_Vendor\\SECURE_AUTH_COOKIE', 'wordpress_sec_' . \WP2FA_Vendor\COOKIEHASH);
        }
    }
    /**
     * @author Vova Feldman (@svovaf)
    ...
    static function is_valid_email($email)
    {
        ...
        $blacklist = array('admin.', 'webmaster.', 'localhost.', 'dev.', 'development.', 'test.', 'stage.', 'staging.');
    ...
    function _add_connectivity_issue_message($api_result, $is_first_failure = \true)
    {
        ...
        if (!\function_exists('WP2FA_Vendor\\wp_nonce_url')) {
        ...
        $message = \false;
        if (\is_object($api_result) && isset($api_result->error) && isset($api_result->error->code)) {
            switch ($api_result->error->code) {
                case 'curl_missing':
                    $missing_methods = '';
                    if (\is_array($api_result->missing_methods) && !empty($api_result->missing_methods)) {
                ...
                case 'squid_cache_block':
                    $message = \sprintf($x_requires_access_to_api . ' ' . $this->esc_html_inline('It looks like your server is using Squid ACL (access control lists), which blocks the connection.', 'squid-blocks-connection-message') . ' %s', '<b>' . $this->get_plugin_name() . '</b>', \sprintf('<ol id="fs_firewall_issue_options"><li>%s</li><li>%s</li><li>%s</li></ol>', \sprintf('<a class="fs-resolve" data-type="squid" href="#"><b>%s</b></a> - %s', $this->esc_html_inline('I don\'t know what is Squid or ACL, help me!', 'squid-no-clue-title'), \sprintf($this->esc_html_inline('We\'ll make sure to contact your hosting company and resolve the issue. You will get a follow-up email to %s once we have an update.', 'squid-no-clue-desc'), '<a href="mailto:' . $admin_email . '">' . $admin_email . '</a>')), \sprintf('<b>%s</b> - %s', $sysadmin_title, \sprintf(
                        ...
                        $this->_module_type
                    )), \sprintf('<a href="%s"><b>%s</b></a> - %s', \WP2FA_Vendor\wp_nonce_url('plugins.php?action=deactivate&amp;plugin=' . $this->_plugin_basename . '&amp;plugin_status=all&amp;paged=1&amp;s=', 'deactivate-plugin_' . $this->_plugin_basename), $deactivate_plugin_title, $deactivate_plugin_desc)));
                    break;
            }
        }
        ...
        $this->_admin_notices->add_sticky($message, $message_id, $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...', $type);
    }
    /**
    ...
    function _email_about_firewall_issue()
    {
        ...
        $current_user = self::_get_current_wp_user();
        $admin_email = $current_user->user_email;
        $error_type = \WP2FA_Vendor\fs_request_get('error_type', 'general');
        switch ($error_type) {
            case 'squid':
        ...
        $custom_email_sections['api_error'] = array('title' => 'API Error', 'rows' => array('ping' => array('API Error', \is_string($pong) ? \htmlentities($pong) : \json_encode($pong))));
    ...
    {
        $params = array();
        \WP2FA_Vendor\fs_require_once_template('firewall-issues-js.php', $params);
    }
    #endregion
    #----------------------------------------------------------------------------------
    #region Email
    ...
    private function send_email($to_address, $subject, $sections = array(), $headers = array())
    {
        ...
        $message = \WP2FA_Vendor\fs_get_template('email.php', $vars);
        // Set the type of email to HTML.
        $headers[] = 'Content-type: text/html; charset=UTF-8';
        $header_string = \implode("\r\n", $headers);
        return \WP2FA_Vendor\wp_mail($to_address, $subject, $message, $header_string);
    }
    /**
    ...
    private function get_email_sections()
    {
        ...
        $api_domain = \substr(\WP2FA_Vendor\FS_API__ADDRESS, \strpos(\WP2FA_Vendor\FS_API__ADDRESS, ':') + 3);
        // Generate the default email sections.
        $sections = array('sdk' => array('title' => 'SDK', 'rows' => array('fs_version' => array('FS Version', $this->version), 'curl_version' => array('cURL Version', $curl_version_information['version']))), 'plugin' => array('title' => \ucfirst($this->get_module_type()), 'rows' => array('name' => array('Name', $this->get_plugin_name()), 'version' => array('Version', $this->get_plugin_version()))), 'api' => array('title' => 'API Subdomain', 'rows' => array('dns' => array('DNS_CNAME', \function_exists('dns_get_record') ? \var_export(\dns_get_record($api_domain, \DNS_CNAME), \true) : 'dns_get_record() disabled/blocked'), 'ip' => array('IP', \function_exists('gethostbyname') ? \gethostbyname($api_domain) : 'gethostbyname() disabled/blocked'))), 'site' => array('title' => 'Site', 'rows' => array('unique_id' => array('Unique ID', $this->get_anonymous_id()), 'address' => array('Address', \site_url()), 'host' => array('HTTP_HOST', !empty($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ''), 'hosting' => array('Hosting Company' => \WP2FA_Vendor\fs_request_has('hosting_company') ? \WP2FA_Vendor\fs_request_get('hosting_company') : 'Unknown'), 'server_addr' => array('SERVER_ADDR', '<a href="http://www.projecthoneypot.org/ip_' . $server_ip . '">' . $server_ip . '</a>'))), 'user' => array('title' => 'User', 'rows' => array('email' => array('Email', $current_user->user_email), 'first' => array('First', $current_user->user_firstname), 'last' => array('Last', $current_user->user_lastname))), 'plugins' => array('title' => 'Plugins', 'rows' => array('active_plugins' => array('Active Plugins', $active_plugin_string))), 'php_info' => array('title' => 'PHP Info', 'rows' => array('info' => array($php_info))));
    ...
    {
        $this->_logger->entrance();
        $this->dynamic_init(array('id' => $id, 'public_key' => $public_key, 'is_live' => $is_live, 'is_premium' => $is_premium));
    }
    /**
    ...
    function dynamic_init(array $plugin_info)
    {
        ...
        if (!$this->is_registered()) {
            if ($this->is_anonymous()) {
                ...
                $this->_has_api_connection = \true;
                $this->_is_on = \true;
            } else {
                if (!$this->has_api_connectivity()) {
                    if ($this->_admin_notices->has_sticky('failed_connect_api_first') || $this->_admin_notices->has_sticky('failed_connect_api')) {
                        if (!$this->_enable_anonymous || $this->is_premium()) {
                            // If anonymous mode is disabled, add firewall admin-notice message.
                            \add_action('admin_footer', array('Freemius', '_add_firewall_issues_javascript'));
                            $ajax_action_suffix = $this->_slug . ($this->is_theme() ? ':theme' : '');
                            \add_action("wp_ajax_fs_resolve_firewall_issues_{$ajax_action_suffix}", array(&$this, '_email_about_firewall_issue'));
                            \add_action("wp_ajax_fs_retry_connectivity_test_{$ajax_action_suffix}", array(&$this, '_retry_connectivity_test'));
                            /**
                    ...
                    $this->_admin_notices->remove_sticky(array('failed_connect_api_first', 'failed_connect_api'));
                    if ($this->_anonymous_mode) {
                        // Simulate anonymous mode.
                        $this->_is_anonymous = \true;
                    }
                }
            }
        }
         ...
         * @author Vova Feldman (@svovaf)
         */
        if ($this->is_user_in_admin() && 'plugin-information' === \WP2FA_Vendor\fs_request_get('tab', \false) && $this->should_use_freemius_updater_and_dialog() && ($this->is_addon() && $this->get_slug() == \WP2FA_Vendor\fs_request_get('plugin', \false) || $this->has_addons() && $this->get_id() == \WP2FA_Vendor\fs_request_get('parent_plugin_id', \false))) {
            require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/fs-plugin-info-dialog.php';
            new \WP2FA_Vendor\FS_Plugin_Info_Dialog($this->is_addon() ? $this->get_parent_instance() : $this);
        }
        ...
        if ($this->has_api_connectivity()) {
            if (self::is_cron()) {
                ...
                if ($this->is_user_in_admin()) {
                     ...
                     */
                    if (\WP2FA_Vendor\fs_request_has('background_sync')) {
            ...
            if ($this->is_addon()) {
                if (!$this->is_parent_plugin_installed()) {
                    ...
                    if (!$this->_parent->is_registered() && $this->is_registered()) {
                        ...
                        if ($this->_parent->is_registered() && !$this->is_registered() && ($this->is_network_active() || !$is_network_admin)) {
                            $premium_license = null;
                            if (!$this->has_free_plan() && $this->is_bundle_license_auto_activation_enabled() && $this->_parent->is_activated_with_bundle_license()) {
                                /**
                                ...
                                if (\is_object($bundle_license) && !empty($bundle_license->products) && \in_array($this->get_id(), $bundle_license->products)) {
                                    $premium_license = $bundle_license;
                                }
                            }
                            if ($this->has_free_plan() || \is_object($premium_license)) {
                                // If parent plugin activated, automatically install add-on for the user.
                                $this->_activate_addon_account($this->_parent, $this->is_network_active() && $is_network_admin ? \true : \WP2FA_Vendor\get_current_blog_id(), $premium_license);
                            }
                        }
                    }
                    ...
                    if ($this->is_premium()) {
                        ...
                        $this->_parent->_admin_notices->remove_sticky(array('addon_plan_upgraded_' . $this->_slug, 'no_addon_license_' . $this->_slug));
                    }
         ...
         * @since  1.2.1.6
         */
        if ($this->should_use_freemius_updater_and_dialog() && ($this->is_premium() || \file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $this->premium_plugin_basename()))) && $this->has_release_on_freemius()) {
            \WP2FA_Vendor\FS_Plugin_Updater::instance($this);
        }
        ...
        if (!$this->is_addon()) {
            if ($this->is_registered()) {
                ...
                if (!isset($this->_storage->activation_timestamp)) {
                    $this->_storage->activation_timestamp = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
                }
    ...
    {
        return \true === \WP2FA_Vendor\fs_request_get_bool('fs_allow_updater_and_dialog') || !self::is_plugin_install_page() && 'install-plugin' !== \WP2FA_Vendor\fs_request_get('action');
    }
    /**
     * @author Leo Fajardo (@leorw)
     *
    ...
    function _stop_tracking_callback()
    {
        ...
        $result = $this->stop_tracking(\WP2FA_Vendor\fs_is_network_admin());
        if (\true === $result) {
            self::shoot_ajax_success();
        }
        $this->_logger->api_error($result);
        self::shoot_ajax_failure(\sprintf($this->get_text_inline('Unexpected API error. Please contact the %s\'s author with the following error.', 'unexpected-api-error'), $this->_module_type) . ($this->is_api_error($result) && isset($result->error) ? $result->error->message : \var_export($result, \true)));
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  1.2.1.5
    ...
    function _allow_tracking_callback()
    {
        ...
        $result = $this->allow_tracking(\WP2FA_Vendor\fs_is_network_admin());
        if (\true === $result) {
            self::shoot_ajax_success();
        }
        $this->_logger->api_error($result);
    ...
    function stop_site_tracking()
    {
        ...
        // Send update to FS.
        $result = $this->get_api_site_scope()->call('/?fields=is_disconnected', 'put', array('is_disconnected' => \true));
        if (!$this->is_api_result_entity($result) || !isset($result->is_disconnected) || !$result->is_disconnected) {
            $this->_logger->api_error($result);
            return $result;
        }
    ...
    function stop_network_tracking()
    {
        ...
        $params[] = array('is_disconnected' => \true);
        // Send update to FS.
        ...
        foreach ($result->installs as $r_install) {
            ...
            $this->_store_site(\true, $blog_id, $install);
        }
    ...
    {
        $this->_logger->entrance();
        return $is_network_action ? $this->stop_network_tracking() : $this->stop_site_tracking();
    }
    /**
    ...
    function allow_site_tracking()
    {
        ...
        $result = $this->get_api_site_scope()->call('/?is_disconnected', 'put', array('is_disconnected' => \false));
        if (!$this->is_api_result_entity($result) || !isset($result->is_disconnected) || $result->is_disconnected) {
            $this->_logger->api_error($result);
            return $result;
        }
    ...
    {
        $this->_logger->entrance();
        return $is_network_action ? $this->allow_network_tracking() : $this->allow_site_tracking();
    }
    /**
    ...
    private function reconnect_locally($is_context_single_site = \false)
    {
        ...
        if (!\WP2FA_Vendor\fs_is_network_admin() || $is_context_single_site) {
            if ($this->is_tracking_prohibited()) {
    ...
    {
        return \true === $this->apply_filters('is_extensions_tracking_allowed', $this->_storage->get('is_extensions_tracking_allowed', null));
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.3.2
    ...
    function _update_tracking_permission_callback()
    {
        ...
        $is_enabled = \WP2FA_Vendor\fs_request_get_bool('is_enabled', null);
        if (!\is_bool($is_enabled)) {
            self::shoot_ajax_failure();
        }
        $permission = \WP2FA_Vendor\fs_request_get('permission');
        switch ($permission) {
            case 'extensions':
        ...
        self::shoot_ajax_success(array('permissions' => array($permission => $is_enabled)));
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.3.2
    ...
    private function parse_settings(&$plugin_info)
    {
        ...
        if (\false === $id) {
            throw new \WP2FA_Vendor\Freemius_Exception(array('error' => array('type' => 'ParameterNotSet', 'message' => 'Plugin id parameter is not set.', 'code' => 'plugin_id_not_set', 'http' => 500)));
        }
        if (\false === $public_key) {
            throw new \WP2FA_Vendor\Freemius_Exception(array('error' => array('type' => 'ParameterNotSet', 'message' => 'Plugin public_key parameter is not set.', 'code' => 'plugin_public_key_not_set', 'http' => 500)));
        }
        $plugin = $this->_plugin instanceof \WP2FA_Vendor\FS_Plugin ? $this->_plugin : new \WP2FA_Vendor\FS_Plugin();
        $premium_suffix = $this->get_option($plugin_info, 'premium_suffix', '(Premium)');
        $plugin->update(array('id' => $id, 'type' => $this->get_option($plugin_info, 'type', $this->_module_type), 'public_key' => $public_key, 'slug' => $this->_slug, 'premium_slug' => $this->get_option($plugin_info, 'premium_slug', "{$this->_slug}-premium"), 'parent_plugin_id' => $parent_id, 'version' => $this->get_plugin_version(), 'title' => $this->get_plugin_name($premium_suffix), 'file' => $this->_plugin_basename, 'is_premium' => $this->get_bool_option($plugin_info, 'is_premium', \true), 'premium_suffix' => $premium_suffix, 'is_live' => $this->get_bool_option($plugin_info, 'is_live', \true), 'affiliate_moderation' => $this->get_option($plugin_info, 'has_affiliation'), 'bundle_id' => $this->get_option($plugin_info, 'bundle_id', null), 'bundle_public_key' => $this->get_option($plugin_info, 'bundle_public_key', null)));
        if ($plugin->is_updated()) {
            // Update plugin details.
            $this->_plugin = \WP2FA_Vendor\FS_Plugin_Manager::instance($this->_module_id)->store($plugin);
        }
         ...
         * @since  2.4.5
         */
        if ($this->is_network_active() && \WP2FA_Vendor\fs_is_network_admin()) {
            if (isset($plugin_info['menu_network']) && \is_array($plugin_info['menu_network']) && !empty($plugin_info['menu_network'])) {
                $plugin_info['menu'] = $plugin_info['menu_network'];
            }
        }
        if (!isset($plugin_info['menu'])) {
            $plugin_info['menu'] = array();
            if (!empty($this->_storage->sdk_last_version) && \version_compare($this->_storage->sdk_last_version, '1.1.2', '<=')) {
                // Backward compatibility to 1.1.2
                $plugin_info['menu']['slug'] = isset($plugin_info['menu_slug']) ? $plugin_info['menu_slug'] : $this->_slug;
            }
        }
        $this->_menu = \WP2FA_Vendor\FS_Admin_Menu_Manager::instance($this->_module_id, $this->_module_type, $this->get_unique_affix());
        $this->_menu->init($plugin_info['menu'], $this->is_addon());
        $this->_has_addons = $this->get_bool_option($plugin_info, 'has_addons', \false);
        $this->_has_paid_plans = $this->get_bool_option($plugin_info, 'has_paid_plans', \true);
        ...
        $this->_is_org_compliant = $this->get_bool_option($plugin_info, 'is_org_compliant', \true);
        ...
        if ($this->_is_premium_only) {
            ...
            $this->_enable_anonymous = $this->get_bool_option($plugin_info, 'enable_anonymous', \true);
        ...
        $this->_navigation = $this->get_option($plugin_info, 'navigation', $this->is_free_wp_org_theme() ? self::NAVIGATION_TABS : self::NAVIGATION_MENU);
    }
    /**
    ...
    private function should_stop_execution()
    {
        ...
        if ($this->is_activation_mode()) {
            ...
            if (self::is_ajax() && !$this->_admin_notices->has_sticky('failed_connect_api_first') && !$this->_admin_notices->has_sticky('failed_connect_api')) {
                /**
    ...
    function _after_code_type_change()
    {
        ...
        if ($this->is_registered()) {
            if (!$this->is_addon()) {
                \add_action(\WP2FA_Vendor\is_admin() ? 'admin_init' : 'init', array(&$this, '_plugin_code_type_changed'));
            }
            if ($this->is_premium()) {
    ...
    function _plugin_code_type_changed()
    {
        ...
        if ($this->is_premium()) {
            ...
            $this->_admin_notices->remove_sticky(array('trial_started', 'plan_upgraded', 'plan_changed', 'license_activated'));
            $notice = '';
            ...
            if (!empty($notice)) {
                $this->_admin_notices->add_sticky(\trim($notice), 'premium_activated', $this->get_text_x_inline('W00t', 'Used to express elation, enthusiasm, or triumph (especially in electronic communication).', 'woot') . '!');
            }
        } else {
            ...
            if ($this->is_paying() && !$this->is_premium()) {
                $this->_admin_notices->add_sticky(\sprintf(
                    ...
                    $this->get_plan_title()
                ) . $this->get_complete_upgrade_instructions(), 'plan_upgraded', $this->get_text_x_inline('Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw') . '!');
            }
        }
    ...
    {
        $this->_logger->entrance();
        $sites = self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN);
        $addon_id = self::get_module_id($id_or_slug);
        ...
        $site = $sites[$slug];
        $plugin = \WP2FA_Vendor\FS_Plugin_Manager::instance($addon_id)->get();
        if ($plugin->parent_plugin_id != $this->_plugin->id) {
            // The given slug do NOT belong to any of the plugin's add-ons.
            return \false;
        }
        return \is_object($site) && \is_numeric($site->id) && \is_numeric($site->user_id) && \WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($site->plan_id);
    }
    /**
    ...
    function is_addon_installed($id_or_slug)
    {
        $this->_logger->entrance();
        $addon_id = self::get_module_id($id_or_slug);
        return \file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $this->get_addon_basename($addon_id)));
    }
    /**
     * Get add-on basename.
     *
    ...
    function get_addon_basename($id_or_slug)
    {
        ...
        if (\file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $premium_basename))) {
            return $premium_basename;
        }
        $all_plugins = $this->get_all_plugins();
        foreach ($all_plugins as $basename => $data) {
            if ($addon->slug === $data['slug'] || $addon->premium_slug === $data['slug']) {
                return $basename;
            }
        }
    ...
    {
        return $this->is_addon() && $parent_product_id == $this->_plugin->parent_plugin_id;
    }
    /**
    ...
    function init_sandbox($secret_key)
    {
        $this->_plugin->secret_key = $secret_key;
        // Update plugin details.
        \WP2FA_Vendor\FS_Plugin_Manager::instance($this->_module_id)->update($this->_plugin, \true);
    }
    /**
    ...
    function is_anonymous()
    {
        if (!isset($this->_is_anonymous)) {
            if ($this->is_network_anonymous()) {
                ...
                if (!\WP2FA_Vendor\fs_is_network_admin()) {
                    if (!isset($this->_storage->is_anonymous)) {
                        // Not skipped.
                        $this->_is_anonymous = \false;
                    } else {
                        if (\is_bool($this->_storage->is_anonymous)) {
    ...
    {
        $this->_logger->entrance($name);
        $this->_storage->store("{$name}_cron", (object) array('version' => $this->get_plugin_version(), 'blog_id' => $cron_blog_id, 'sdk_version' => $this->version, 'timestamp' => \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME, 'on' => \true));
    }
    /**
    ...
    private function get_cron_blog_id($name)
    {
        ...
        return \is_object($cron_data) && \is_numeric($cron_data->blog_id) ? $cron_data->blog_id : 0;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.0.0
    ...
    private function is_cron_on($name)
    {
         ...
         */
        $cron_data = $this->get_cron_data($name);
        return !\is_null($cron_data) && \true === $cron_data->on;
    }
    /**
    ...
    {
        $this->_logger->entrance($name);
        $this->_storage->store("{$name}_timestamp", \time());
    }
    /**
    ...
    {
        $this->_logger->entrance();
        $this->_storage->store('keepalive_timestamp', \time(), $use_network_level_storage);
    }
    /**
     ...
     * @return bool
     */
    private function is_cron_executed($name, $period = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC)
    {
        $this->_logger->entrance($name);
        ...
        return $last_execution > \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME - $period;
    }
    /**
    ...
    private function get_cron_target_blog_id($except_blog_id = 0)
    {
        ...
        if ($this->_is_network_active && \is_numeric($this->_storage->network_install_blog_id) && $except_blog_id != $this->_storage->network_install_blog_id && self::is_site_active($this->_storage->network_install_blog_id)) {
            // Try to run cron from the main network blog.
            $install = $this->get_install_by_blog_id($this->_storage->network_install_blog_id);
            if (\is_object($install) && ($this->is_premium() || $install->is_tracking_allowed())) {
        ...
        foreach ($installs as $blog_id => $install) {
            if ($except_blog_id != $blog_id && self::is_site_active($blog_id) && ($this->is_premium() || $install->is_tracking_allowed())) {
                return $blog_id;
            }
        }
        return 0;
    ...
    private function clear_cron($name, $action_tag = '', $is_network_clear = \false)
    {
         ...
         */
        $cron_data = $this->get_cron_data($name);
        $cron_blog_id = \is_object($cron_data) && isset($cron_data->blog_id) ? $cron_data->blog_id : 0;
        $this->clear_cron_data($name);
        if (0 < $cron_blog_id) {
            \WP2FA_Vendor\switch_to_blog($cron_blog_id);
        }
        if (empty($action_tag)) {
            $action_tag = $name;
        }
        ...
        if (0 < $cron_blog_id) {
            \WP2FA_Vendor\restore_current_blog();
        }
    }
    /**
     ...
     * @param int    $except_blog_id  Target any except the excluded blog ID.
     */
    private function schedule_cron($name, $action_tag = '', $recurrence = 'single', $start_at = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME, $randomize_start = \true, $except_blog_id = 0)
    {
        $this->_logger->entrance($name);
        $this->clear_cron($name, $action_tag, \true);
        ...
        if ('daily' === $recurrence) {
            if ($randomize_start) {
                ...
                $start_at += \rand(0, \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC / 2);
            }
            // Schedule daily WP cron.
            \wp_schedule_event($start_at, 'daily', $this->get_action_tag($action_tag));
        } else {
            if ('single' === $recurrence) {
                // Schedule single cron.
                \WP2FA_Vendor\wp_schedule_single_event($start_at, $this->get_action_tag($action_tag));
            }
        }
    ...
    private function execute_cron($name, $callable)
    {
        ...
        if (\WP2FA_Vendor\FS_Api::is_temporary_down()) {
            return;
        }
        ...
        if (\is_multisite()) {
            $this->switch_to_blog($current_blog_id, \WP2FA_Vendor\fs_is_network_admin() ? $this->get_network_install() : null);
    ...
    private function run_manual_sync()
    {
        ...
        $this->schedule_sync_cron(\time() + \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, \false);
    }
    /**
    ...
    function _sync_cron_method(array $blog_ids, $current_blog_id = null)
    {
        if ($this->is_registered()) {
            if ($this->has_paid_plan()) {
                ...
                $this->_sync_license(\true, \false, $current_blog_id);
                if ($this->is_paying()) {
                    // Check for premium plugin updates.
                    $this->check_updates(\true);
                }
            } else {
     ...
     * @return bool
     */
    private function is_sync_executed($period = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC)
    {
     ...
     */
    private function schedule_sync_cron($start_at = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME, $randomize_start = \true, $except_blog_id = 0)
    {
        $this->schedule_cron('sync', 'data_sync', 'daily', $start_at, $randomize_start, $except_blog_id);
    }
    /**
    ...
    {
        $this->schedule_cron('install_sync', 'install_sync', 'single', \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME, \false, $except_blog_id);
    }
    /**
    ...
    function _sync_install_cron_method(array $blog_ids, $current_blog_id = null)
    {
        if ($this->is_registered()) {
            if (1 < \count($blog_ids)) {
                $this->sync_installs(array(), \true);
            } else {
                $this->sync_install(array(), \true);
            }
            $this->maybe_sync_install_user();
        }
    }
    ...
    function _admin_init_action()
    {
        ...
        if ($this->is_plugin_activation()) {
            \WP2FA_Vendor\delete_transient("fs_{$this->_module_type}_{$this->_slug}_activated");
        ...
        if (\WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_skip_activation')) {
            \WP2FA_Vendor\check_admin_referer($this->get_unique_affix() . '_skip_activation');
            $this->skip_connection(null, \WP2FA_Vendor\fs_is_network_admin());
            \WP2FA_Vendor\fs_redirect($this->get_after_activation_url('after_skip_url'));
        }
        if ($this->is_network_activation_mode() && \WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_delegate_activation')) {
            \WP2FA_Vendor\check_admin_referer($this->get_unique_affix() . '_delegate_activation');
            $this->delegate_connection();
            \WP2FA_Vendor\fs_redirect($this->get_after_activation_url('after_delegation_url'));
        }
        $this->_add_upgrade_action_link();
        if (!(!$this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin()) && (\true === $this->_storage->require_license_activation || !$this->is_registered() && !$this->is_anonymous() || \WP2FA_Vendor\fs_is_network_admin() && $this->_is_network_active && $this->is_network_upgrade_mode())) {
            if (!$this->is_pending_activation()) {
                if (!$this->is_activation_page()) {
                    ...
                    if ($this->is_theme() && !$this->has_settings_menu() && !isset($_REQUEST['fs_action']) && $this->can_activate_previous_theme()) {
                        ...
                        if (\true === $this->_storage->require_license_activation) {
                            $this->_storage->require_license_activation = \false;
                        }
                    }
                    if (!\WP2FA_Vendor\fs_is_network_admin() && $this->is_network_activation_mode() && !$this->is_delegated_connection()) {
                        return;
                    }
                    if ($this->is_plugin_new_install() || $this->is_only_premium()) {
                        if (!$this->_anonymous_mode && (!$this->is_addon() || !$this->_parent->is_anonymous())) {
                        ...
                        if ($this->has_filter('optin_pointer_element')) {
                            ...
                            \WP2FA_Vendor\wp_enqueue_script('wp-pointer');
                            \WP2FA_Vendor\wp_enqueue_style('wp-pointer');
                            $this->_enqueue_connect_essentials();
                            \add_action('admin_print_footer_scripts', array($this, '_add_connect_pointer_script'));
                        }
                    }
                }
            }
            if ($this->show_opt_in_on_themes_page() && $this->is_activation_page()) {
    ...
    {
        if (!$this->_is_network_active || !\WP2FA_Vendor\fs_is_network_admin()) {
            $this->_storage->sticky_optin_added = \true;
        } else {
            $this->_storage->sticky_optin_added_ms = \true;
        }
        // Show notice for new plugin installations.
        $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('We made a few tweaks to the %s, %s', 'few-plugin-tweaks'), $this->_module_type, \sprintf('<b><a href="%s">%s</a></b>', $this->get_activation_url(), \sprintf($this->get_text_inline('Opt in to make "%s" better!', 'optin-x-now'), $this->get_plugin_name()))), 'connect_account', '', 'update-nag');
    }
    /**
    ...
    {
        \WP2FA_Vendor\wp_enqueue_script('jquery');
        \WP2FA_Vendor\wp_enqueue_script('json2');
        \WP2FA_Vendor\fs_enqueue_local_script('postmessage', 'nojquery.ba-postmessage.min.js');
        \WP2FA_Vendor\fs_enqueue_local_script('fs-postmessage', 'postmessage.js');
        \WP2FA_Vendor\fs_enqueue_local_style('fs_connect', '/admin/connect.css');
    }
    /**
    ...
    {
        $vars = array('id' => $this->_module_id);
        $pointer_content = \WP2FA_Vendor\fs_get_template('connect.php', $vars);
        ?>
    ...
    {
        return \WP2FA_Vendor\fs_is_plugin_page($this->_menu->get_raw_slug()) || \WP2FA_Vendor\fs_is_plugin_page($this->_slug);
    }
    /* Events
    		------------------------------------------------------------------------------------------------------------------*/
    /**
     ...
     */
    function _delete_site($store = \true, $blog_id = null)
    {
     ...
     * @return false|int The install ID if deleted. Otherwise, FALSE (when install not exist).
     */
    static function _delete_site_by_slug($slug, $module_type, $store = \true, $blog_id = null)
    {
    ...
    {
        return isset($this->_storage->is_plugin_new_install) && $this->_storage->is_plugin_new_install;
    }
    /**
    ...
    {
        return isset($this->_storage->previous_theme) ? $this->_storage->previous_theme : \false;
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  1.2.2
    ...
    private function can_activate_previous_theme()
    {
        ...
        if (\false !== $slug && \WP2FA_Vendor\current_user_can('switch_themes')) {
            $theme_instance = \WP2FA_Vendor\wp_get_theme($slug);
    ...
    {
        \WP2FA_Vendor\switch_theme($this->get_previous_theme_slug());
        ...
        if ('themes.php' === $pagenow) {
             ...
             * @since  1.2.2
             */
            \WP2FA_Vendor\fs_redirect($this->admin_url($pagenow));
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    function get_previous_theme_activation_url()
    {
         ...
         * @since  1.2.2
         */
        return \WP2FA_Vendor\wp_nonce_url($this->admin_url('themes.php?action=activate&stylesheet=' . \urlencode($this->get_previous_theme_slug())), 'switch-theme_' . $this->get_previous_theme_slug());
    }
    /**
    ...
    {
        $this->_storage->previous_theme = \false !== $old_theme ? $old_theme->get_stylesheet() : $slug_or_name;
    ...
    function _activate_plugin_event_hook()
    {
        ...
        \WP2FA_Vendor\FS_Api::clear_cache();
        $is_premium_version_activation = $this->is_plugin() ? \WP2FA_Vendor\current_filter() !== 'activate_' . $this->_free_plugin_basename : $this->is_premium();
        ...
        if ($this->is_plugin()) {
            ...
            $other_version_basename = $is_premium_version_activation ? $this->_free_plugin_basename : $this->premium_plugin_basename();
            if (!$this->_is_network_active) {
                /**
                 ...
                 * @author Vova Feldman
                 */
                $this->_is_network_active = $this->_is_multisite_integrated && \WP2FA_Vendor\fs_is_network_admin();
            }
            /**
             ...
             * @since  1.2.2
             */
            if (\WP2FA_Vendor\is_plugin_active($other_version_basename) && $this->apply_filters('deactivate_on_activation', \true)) {
                \WP2FA_Vendor\deactivate_plugins($other_version_basename);
            }
        }
        if ($this->is_registered()) {
            ...
            if ($is_premium_version_activation) {
                $this->_admin_notices->add(\sprintf($this->get_text_inline('The upgrade of %s was successfully completed.', 'successful-version-upgrade-message'), \sprintf('<b>%s</b>', $this->_plugin->title)), $this->get_text_x_inline('W00t', 'Used to express elation, enthusiasm, or triumph (especially in electronic communication).', 'woot') . '!');
            }
        } else {
            if ($this->is_anonymous()) {
                 ...
                 */
                if (\WP2FA_Vendor\WP_FS__DEV_MODE || ($this->is_plugin() || \defined('WP2FA_Vendor\\WP_DEBUG') && \WP2FA_Vendor\WP_DEBUG) && $this->get_plugin_version() == $plugin_version) {
        ...
        if ($this->is_addon() && !$is_trial_or_has_features_enabled_license) {
             ...
             */
            $is_trial_or_has_features_enabled_license = $this->is_trial() || $this->has_features_enabled_license();
            if ($is_trial_or_has_features_enabled_license && \true === $this->_storage->require_license_activation) {
                $this->_storage->require_license_activation = \false;
            }
        }
        if ($is_premium_version_activation && (!$this->is_registered() && $this->is_anonymous() || $this->is_registered() && !$is_trial_or_has_features_enabled_license)) {
            $this->_storage->require_license_activation = \true;
        }
         ...
         * @since 2.2.3.1
         */
        $has_api_connectivity = $this->has_api_connectivity(\WP2FA_Vendor\WP_FS__DEV_MODE || $is_premium_version_activation);
        if (!$this->_anonymous_mode && $has_api_connectivity && !$this->_isAutoInstall) {
            // Store hint that the plugin was just activated to enable auto-redirection to settings.
            \set_transient("fs_{$this->_module_type}_{$this->_slug}_activated", \true, 60);
        }
        /**
    ...
    private function maybe_activate_addon_license()
    {
        ...
        if ($this->is_bundle_license_auto_activation_enabled() && !empty($license->products)) {
            $this->activate_bundle_license($license);
            return;
        }
        if (!$this->is_registered()) {
            // Opt in with a license key.
            $this->opt_in($parent_fs->get_current_or_network_user()->email, \false, \false, $license->secret_key);
        } else {
            // Activate the license.
            $install = $this->get_api_site_scope()->call('/', 'put', array('license_key' => $this->apply_filters('license_key', $license->secret_key)));
            if (!\WP2FA_Vendor\FS_Api::is_api_error($install)) {
                $this->_sync_addon_license($this->get_id(), \true);
            }
        }
    }
    /**
    ...
    private function maybe_network_activate_addon_license($license = null)
    {
        ...
        $license = !\is_null($license) ? $license : $this->get_active_parent_license();
        if (!\is_object($license)) {
            return;
        }
        if ($this->is_bundle_license_auto_activation_enabled() && !empty($license->products)) {
        ...
        if (!$this->is_network_registered()) {
            ...
            // Opt in with a license key.
            $this->opt_in($parent_fs->get_user()->email, \false, \false, $license->secret_key, \false, \false, \false, null, $sites);
        } else {
            $blog_2_install_map = array();
            $site_ids = array();
            $all_sites = \WP2FA_Vendor\Freemius::get_sites();
            foreach ($all_sites as $site) {
                $blog_id = \WP2FA_Vendor\Freemius::get_site_blog_id($site);
                $install = $this->get_install_by_blog_id($blog_id);
                if (\is_object($install) && \WP2FA_Vendor\FS_Plugin_License::is_valid_id($install->license_id)) {
     ...
     * @param int               $blog_id
     */
    private function maybe_activate_bundle_license(\WP2FA_Vendor\FS_Plugin_License $license = null, $sites = array(), $blog_id = 0)
    {
        ...
        $parent_license = !empty($license->products) ? $license : $this->get_active_parent_license($license->secret_key);
    ...
    private function activate_bundle_license($license, $sites = array(), $current_blog_id = 0)
    {
        ...
        foreach ($license->products as $product_id) {
            ...
            if (!$fs->is_addon() && !\WP2FA_Vendor\FS_Plan_Manager::instance()->has_paid_plan($fs->_plans)) {
                /**
            ...
            if (!$is_network_admin || $current_blog_id > 0) {
                ...
                foreach ($sites as $site) {
                    ...
                    if (isset($installs[$fs->get_slug()])) {
                        ...
                        if (\is_object($install) && (!\WP2FA_Vendor\FS_Site::is_valid_id($install->id) || !\WP2FA_Vendor\FS_User::is_valid_id($install->user_id) || !\WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($install->plan_id))) {
                            $install = null;
                        }
                    }
                    if (\is_object($install) && \WP2FA_Vendor\FS_Plugin_License::is_valid_id($install->license_id)) {
                        $has_install_with_license = \true;
                        break;
                    }
            ...
            $fs->activate_migrated_license($license->secret_key, null, null, $sites, $current_blog_id > 0 ? $current_blog_id : null);
        }
    }
    /**
     ...
     */
    function get_active_parent_license($license_key = null, $flush = \true)
    {
        ...
        $foreign_licenses = $fs->get_foreign_licenses_info(self::get_all_licenses($this->get_parent_id()));
        ...
        if (!$this->is_api_result_object($result, 'licenses') || !\is_array($result->licenses) || empty($result->licenses)) {
            return null;
        }
        $parent_license = null;
        ...
        if (!\is_null($parent_license)) {
            $parent_license = new \WP2FA_Vendor\FS_Plugin_License($parent_license);
        }
    ...
    function get_sites_for_network_level_optin()
    {
        ...
        foreach ($all_sites as $site) {
            $blog_id = self::get_site_blog_id($site);
            if (!$this->is_site_delegated_connection($blog_id) && !$this->is_installed_on_site($blog_id)) {
    ...
    function delete_account_event($check_user = \true)
    {
        ...
        $delete_network_common_data = \true;
        if ($this->_is_network_active) {
            $installs = $this->get_blog_install_map();
        ...
        $this->_storage->clear_all(\true, array('is_delegated_connection', 'connectivity_test', 'is_on'), \false);
        // Send delete event.
    ...
    function delete_network_account_event($check_user = \true)
    {
        ...
        $this->_delete_account_addons(\false);
        // @todo Delete plans and licenses of add-ons.
        self::$_accounts->store(\true);
        /**
         * IMPORTANT:
         *  Clear crons must be executed before clearing all storage.
         *  Otherwise, the cron will not be cleared.
         */
        $this->clear_sync_cron(\true);
        $this->clear_install_sync_cron(\true);
        $sites = self::get_sites();
        $install_ids = array();
        foreach ($sites as $site) {
            $blog_id = self::get_site_blog_id($site);
            ...
            $install_id = $this->_delete_site(\true, $blog_id);
            // Clear all storage data.
            $this->_storage->clear_all(\true, array('connectivity_test'), $blog_id);
            if (\WP2FA_Vendor\FS_Site::is_valid_id($install_id)) {
                $install_ids[] = $install_id;
            }
            \WP2FA_Vendor\switch_to_blog($blog_id);
            $this->do_action('after_account_delete');
            \WP2FA_Vendor\restore_current_blog();
        }
        $this->_storage->clear_all(\true, array('connectivity_test', 'is_on'), \true);
        // Send delete event.
    ...
    function _deactivate_plugin_hook()
    {
        ...
        $is_network_deactivation = \WP2FA_Vendor\fs_is_network_admin();
        ...
        \WP2FA_Vendor\register_uninstall_hook($this->_plugin_main_file_path, array('Freemius', '_uninstall_plugin_hook'));
        $this->clear_module_main_file_cache();
        ...
        if ($this->is_registered()) {
            ...
            if ($is_network_deactivation) {
                ...
                $this->sync_installs(array('is_active' => \false));
            } else {
                // Send deactivation event.
    ...
    private function remove_sdk_reference()
    {
        ...
        \WP2FA_Vendor\fs_fallback_to_newest_active_sdk();
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.3
    ...
    private function set_anonymous_mode($is_anonymous = \true, $network_or_blog_id = 0)
    {
        ...
        // again in the future.
        $skip_info = array('is' => $is_anonymous, 'timestamp' => \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME, 'version' => $this->get_plugin_version());
        if (\true === $network_or_blog_id) {
    ...
    {
        $this->_logger->entrance();
        if ($this->is_premium() && $this->is_network_connected() && \is_object($this->_license) && $this->_license->can_activate(\WP2FA_Vendor\FS_Site::is_localhost_by_address($domain)) && $this->is_license_network_active($blog_id)) {
            /**
            ...
            $this->install_with_user($this->get_network_user(), $license->secret_key, \false, \false, \false);
            if (\is_object($this->_site)) {
        ...
        if ($this->is_network_anonymous()) {
            ...
            if ($this->is_network_delegated_connection()) {
                ...
                if ($this->is_network_connected()) {
                    ...
                    $this->switch_to_blog($blog_id);
                    // Opt-in with network user.
                    $this->install_with_user($this->get_network_user(), \false, \false, \false, \false);
                    $this->switch_to_blog($current_blog_id);
                } else {
                    /**
    ...
    private function reset_anonymous_mode($network_or_blog_id = 0)
    {
        if (\true === $network_or_blog_id) {
            ...
            $this->_storage->remove('is_anonymous', \true, $network_or_blog_id);
        }
        /**
         ...
         * @since  1.2.2
         */
        if (!$this->_is_network_active || 0 === $network_or_blog_id || \WP2FA_Vendor\get_current_blog_id() == $network_or_blog_id || \true === $network_or_blog_id && \WP2FA_Vendor\fs_is_network_admin()) {
    ...
    private function maybe_set_slug_and_network_menu_exists_flag()
    {
        if (!empty($this->_dynamically_added_top_level_page_hook_name)) {
            $this->_menu->set_slug_and_network_menu_exists_flag($this->_menu->has_menu() ? $this->_menu->get_slug() : $this->_slug);
        }
    }
    /**
    ...
    function connect_again()
    {
        ...
        $this->reset_anonymous_mode(\WP2FA_Vendor\fs_is_network_admin());
        $this->maybe_set_slug_and_network_menu_exists_flag();
        \WP2FA_Vendor\fs_redirect($this->get_activation_url());
    }
    /**
    ...
    function skip_connection($sites = null, $skip_all_network = \false)
    {
        ...
        if ($skip_all_network) {
            $this->set_anonymous_mode(\true, \true);
        }
        if (!$skip_all_network && empty($sites)) {
            ...
            $this->get_api_plugin_scope()->call('skip.json', 'put', array('uids' => $uids));
        }
        $this->network_upgrade_mode_completed();
    }
    /**
     ...
     */
    private function skip_site_connection($blog_id = null, $send_skip = \true)
    {
        $this->_logger->entrance();
        $this->_admin_notices->remove_sticky('connect_account', $blog_id);
        $this->set_anonymous_mode(\true, $blog_id);
        if ($send_skip) {
            $this->get_api_plugin_scope()->call('skip.json', 'put', array('uids' => array($this->get_anonymous_id($blog_id))));
        }
    }
    /**
    ...
    private function get_plugins_thumbprint($plugins)
    {
        ...
        foreach ($plugins as $basename => $data) {
            $thumbprint .= $data['slug'] . ',' . $data['Version'] . ',' . ($data['is_active'] ? '1' : '0') . ';';
        }
    ...
    private function get_plugins_data_for_api()
    {
        ...
        if (!\is_object($site_active_plugins_cache)) {
            $site_active_plugins_cache = (object) array('timestamp' => '', 'md5' => '', 'plugins' => array());
        }
        $time = \time();
        if (!empty($site_active_plugins_cache->timestamp) && $time - $site_active_plugins_cache->timestamp < \WP2FA_Vendor\WP_FS__TIME_5_MIN_IN_SEC) {
        ...
        self::$_accounts->set_option($site_active_plugins_option_name, $site_active_plugins_cache, \true);
        // Reload options from DB.
        self::$_accounts->load(\true);
        $site_active_plugins_cache = self::$_accounts->get_option($site_active_plugins_option_name);
        ...
        if (!\is_object($network_plugins_cache)) {
            $network_plugins_cache = (object) array('timestamp' => '', 'md5' => '', 'plugins' => array());
        }
        ...
        if (!$network_plugins_changed && !$site_active_plugins_changed) {
            // No changes.
            return array();
        }
        ...
        foreach ($network_plugins_cache->plugins as $basename => $data) {
            if (!isset($network_plugins[$basename])) {
                ...
                $uninstalled_plugin_data['is_uninstalled'] = \true;
            ...
            $was_active = $data['is_active'] || isset($site_active_plugins_cache->plugins[$basename]) && \true === $site_active_plugins_cache->plugins[$basename]['is_active'];
            $is_active = $network_plugins[$basename]['is_active'] || isset($site_active_plugins[$basename]) && $site_active_plugins[$basename]['is_active'];
            if (!isset($site_active_plugins_cache->plugins[$basename]) && isset($site_active_plugins[$basename])) {
                // Plugin was site level activated.
                $site_active_plugins_cache->plugins[$basename] = $network_plugins[$basename];
                $site_active_plugins_cache->plugins[$basename]['is_active'] = \true;
            } else {
                if (isset($site_active_plugins_cache->plugins[$basename]) && !isset($site_active_plugins[$basename])) {
        ...
        foreach ($network_plugins as $basename => $data) {
            if (!isset($network_plugins_cache->plugins[$basename])) {
                // New plugin.
                $new_plugin = array('slug' => $data['slug'], 'version' => $data['Version'], 'title' => $data['Name'], 'is_active' => $data['is_active'], 'is_uninstalled' => \false);
                $network_plugins_cache->plugins[$basename] = $new_plugin;
                $is_site_level_active = isset($site_active_plugins[$basename]) && $site_active_plugins[$basename]['is_active'];
                /**
        ...
        self::$_accounts->set_option($network_plugins_option_name, $network_plugins_cache, \true);
    ...
    private function get_themes_data_for_api()
    {
        ...
        $time = \time();
        if (!empty($all_cached_themes->timestamp) && $time - $all_cached_themes->timestamp < \WP2FA_Vendor\WP_FS__TIME_5_MIN_IN_SEC) {
            // Don't send theme updates if last update was in the past 5 min.
            return \false;
        }
        // Write timestamp to lock the logic.
        $all_cached_themes->timestamp = $time;
        self::$_accounts->set_option($option_name, $all_cached_themes, \true);
        // Reload options from DB.
        self::$_accounts->load(\true);
        $all_cached_themes = self::$_accounts->get_option($option_name);
        ...
        $all_themes = \WP2FA_Vendor\wp_get_themes();
        ...
        if ($themes_changed) {
            ...
            foreach ($all_cached_themes->themes as $slug => $data) {
                ...
                if (!isset($all_themes[$slug])) {
                    ...
                    $uninstalled_theme_data['is_uninstalled'] = \true;
            ...
            $all_cached_themes->timestamp = \time();
            self::$_accounts->set_option($option_name, $all_cached_themes, \true);
        }
    ...
    private function get_install_data_for_api(array $override, $include_plugins = \true, $include_themes = \true, $include_blog_data = \true)
    {
        if ($this->is_extensions_tracking_allowed()) {
            if (!\defined('WP2FA_Vendor\\WP_FS__TRACK_PLUGINS') || \false !== \WP2FA_Vendor\WP_FS__TRACK_PLUGINS) {
                /**
            ...
            if (!\defined('WP2FA_Vendor\\WP_FS__TRACK_THEMES') || \false !== \WP2FA_Vendor\WP_FS__TRACK_THEMES) {
                /**
        ...
        $blog_data = $include_blog_data ? array('language' => \get_bloginfo('language'), 'charset' => \get_bloginfo('charset'), 'title' => \get_bloginfo('name'), 'url' => \WP2FA_Vendor\get_site_url()) : array();
    ...
    private function get_installs_data_for_api(array $override, $only_diff = \false, $include_plugins = \true, $include_themes = \true)
    {
        ...
        // Common properties.
        $versions = $this->get_versions();
        $common = \array_merge($versions, array('version' => $this->get_plugin_version(), 'is_premium' => $this->is_premium()), $override);
    ...
    private function get_install_diff_for_api($site, $install, $override = array())
    {
        ...
        foreach ($site as $p => $v) {
            if (\property_exists($install, $p)) {
                if ((\is_bool($install->{$p}) || !empty($install->{$p})) && $install->{$p} != $v) {
                    $install->{$p} = $v;
                    $diff[$p] = $v;
                }
            } else {
                $special[$p] = $v;
                if (isset($override[$p]) || 'plugins' === $p || 'themes' === $p) {
                    $special_override = \true;
                }
            }
        }
    ...
    private function send_installs_update($override = array(), $flush = \false)
    {
        ...
        if (empty($installs_data)) {
             ...
             * @since 2.2.3
             */
            $keepalive_only_update = $this->should_send_keepalive_update(\true);
            if (!$keepalive_only_update) {
                /**
                 * There are no updates to send including keepalive.
                 *
         ...
         * @since 2.2.3
         */
        $this->set_keepalive_timestamp(\true);
        // Send updated values to FS.
    ...
    private function should_send_keepalive_update($use_network_level_storage = null)
    {
        ...
        if ($keepalive_timestamp < \time() - \WP2FA_Vendor\WP_FS__TIME_WEEK_IN_SEC) {
    ...
    private function sync_install($override = array(), $flush = \false)
    {
        ...
        $this->_site = new \WP2FA_Vendor\FS_Site($site);
        $this->_store_site(\true);
    }
    /**
     * Update install only if changed.
     *
    ...
    private function sync_installs($override = array(), $flush = \false)
    {
        ...
        foreach ($result->installs as $install) {
            $this->_site = new \WP2FA_Vendor\FS_Site($install);
            $address = \trailingslashit(\WP2FA_Vendor\fs_strip_url_protocol($install->url));
            $blog_id = $address_to_blog_map[$address];
            $this->_store_site(\true, $blog_id);
        }
    }
    /**
    ...
    public function track_event($name, $properties = array(), $process_at = \false, $once = \false)
    {
        ...
        if (\is_numeric($process_at) && $process_at > \time()) {
        ...
        if (!empty($properties)) {
            ...
            foreach ($properties as $k => $v) {
                if (!\is_scalar($v)) {
                    throw new \WP2FA_Vendor\Freemius_InvalidArgumentException('The $properties argument must be an associative key/value array with primitive values only.');
                }
            }
            $event['properties'] = $properties;
        }
        $result = $this->get_api_site_scope()->call('events.json', 'post', $event);
        return $this->is_api_error($result) ? \false : $result;
    }
    /**
    ...
    {
        return $this->track_event($name, $properties, $process_at, \true);
    }
    /**
    ...
    {
        $this->_logger->entrance('slug = ' . $this->_slug);
        if ($check_user && !\WP2FA_Vendor\current_user_can('activate_plugins')) {
            return;
        }
        $params = array();
        ...
        if (!$this->is_registered()) {
            ...
            if (isset($uninstall_reason)) {
                if (isset($uninstall_reason->is_anonymous) && !$uninstall_reason->is_anonymous) {
                    $this->opt_in(\false, \false, \false, \false, \true);
                } else {
                    $params['uid'] = $this->get_anonymous_id();
            ...
            $params = \array_merge($params, array('is_active' => \false, 'is_uninstalled' => \true));
            if ($this->_is_network_active) {
    ...
    {
        $basename = \WP2FA_Vendor\plugin_basename($caller);
        $current_basename = $is_premium ? $this->_premium_plugin_basename : $this->_free_plugin_basename;
        ...
        $plugin_dir = \dirname($this->_plugin_dir_path) . '/';
        \WP2FA_Vendor\register_activation_hook($plugin_dir . $basename, array(&$this, '_activate_plugin_event_hook'));
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.1
    ...
    function premium_plugin_basename()
    {
        if (!isset($this->_premium_plugin_basename)) {
            $this->_premium_plugin_basename = $this->is_premium() ? $this->_plugin_basename : $this->get_premium_slug() . '/' . \basename($this->_free_plugin_basename);
        }
    ...
    public static function _uninstall_plugin_hook()
    {
        self::_load_required_static();
        self::$_static_logger->entrance();
        if (!\WP2FA_Vendor\current_user_can('activate_plugins')) {
            return;
        }
        $plugin_file = \substr(\WP2FA_Vendor\current_filter(), \strlen('uninstall_'));
        self::$_static_logger->info('plugin = ' . $plugin_file);
        \define('WP2FA_Vendor\\WP_FS__UNINSTALL_MODE', \true);
        $fs = self::get_instance_by_file($plugin_file);
        if (\is_object($fs)) {
            $fs->remove_sdk_reference();
            self::require_plugin_essentials();
            if (\WP2FA_Vendor\is_plugin_active($fs->_free_plugin_basename) || \WP2FA_Vendor\is_plugin_active($fs->premium_plugin_basename())) {
    ...
    {
        if (!\function_exists('WP2FA_Vendor\\wp_get_current_user')) {
    ...
    function get_plugin_data($reparse_plugin_metadata = \false)
    {
        if (!isset($this->_plugin_data) || $reparse_plugin_metadata) {
            ...
            if ($this->is_plugin()) {
                 ...
                 */
                $plugin_data = \WP2FA_Vendor\get_plugin_data($this->_plugin_main_file_path, \false, \false);
            } else {
                $theme_data = \WP2FA_Vendor\wp_get_theme();
                if ($this->_plugin_basename !== $theme_data->get_stylesheet() && \WP2FA_Vendor\is_child_theme()) {
                    $parent_theme = $theme_data->parent();
                    if ($parent_theme instanceof \WP2FA_Vendor\WP_Theme && $this->_plugin_basename === $parent_theme->get_stylesheet()) {
                        $theme_data = $parent_theme;
                    }
                }
                $plugin_data = array('Name' => $theme_data->get('Name'), 'Version' => $theme_data->get('Version'), 'Author' => $theme_data->get('Author'), 'Description' => $theme_data->get('Description'), 'PluginURI' => $theme_data->get('ThemeURI'));
            }
    ...
    {
        return \is_object($this->_plugin) ? $this->_plugin->premium_slug : "{$this->_slug}-premium";
    }
    /**
    ...
    {
        return $this->can_use_premium_code() ? $this->_plugin->premium_slug : $this->_slug;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.1
    ...
    {
        return isset($this->_plugin->bundle_id) && \WP2FA_Vendor\FS_Plugin::is_valid_id($this->_plugin->bundle_id) ? $this->_plugin->bundle_id : null;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.3.1
    ...
    {
        return isset($this->_plugin->bundle_public_key) ? $this->_plugin->bundle_public_key : null;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.2.1.5
    ...
    {
        return $this->is_addon() ? $this->get_parent_instance()->get_id() : $this->_plugin->id;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.3.1
    ...
    {
        return $this->apply_filters('usage_tracking_terms_url', "https://freemius.com/wordpress/usage-tracking/{$this->_plugin->id}/{$this->_slug}/");
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.3.1
    ...
    {
        return $this->apply_filters('eula_url', "https://freemius.com/terms/{$this->_plugin->id}/{$this->_slug}/");
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.1
    ...
    function get_plugin_name($premium_suffix = \false)
    {
        ...
        if (!isset($this->_plugin_name)) {
            ...
            if (!empty($premium_suffix) && (!\is_object($this->_plugin) || $this->_plugin->premium_suffix !== $premium_suffix)) {
    ...
    private function set_name($premium_suffix = '')
    {
        ...
        if (\is_string($premium_suffix)) {
            $premium_suffix = \trim($premium_suffix);
            if (!empty($premium_suffix)) {
                ...
                if (\strlen($plugin_data['Name']) > $suffix_len && $suffix === \substr(\strtolower($plugin_data['Name']), -$suffix_len)) {
    ...
    {
        $label = $this->is_addon() ? $this->get_text_inline('Add-On', 'addon') : ($this->is_plugin() ? $this->get_text_inline('Plugin', 'plugin') : $this->get_text_inline('Theme', 'theme'));
        if ($lowercase) {
    ...
    function get_plugin_basename()
    {
        if (!isset($this->_plugin_basename)) {
            if ($this->is_plugin()) {
                $this->_plugin_basename = \WP2FA_Vendor\plugin_basename($this->_plugin_main_file_path);
            } else {
    ...
    private function store_file_slug_map()
    {
        ...
        if (!isset($file_slug_map[$this->_plugin_basename]) || $file_slug_map[$this->_plugin_basename] !== $this->_slug) {
     ...
     */
    private static function get_all_sites($module_type = \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $blog_id = null)
    {
    ...
    {
        if (!\is_null($module_type) && \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN !== $module_type) {
    ...
    {
        self::set_account_option_by_module($this->_module_type, $option_name, $option_value, $store, $network_level_or_blog_id);
    }
    /**
     * @author Vova Feldman (@svovaf)
     *
     ...
     * @param null|int $network_level_or_blog_id Since 2.0.0
     */
    private static function set_account_option_by_module($module_type, $option_name, $option_value, $store, $network_level_or_blog_id = null)
    {
        if (\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN != $module_type) {
            $option_name = $module_type . '_' . $option_name;
        }
    ...
    private static function maybe_get_entities_account_option($option_name, $default = null, $network_level_or_blog_id = null)
    {
        ...
        $class_name = '';
        if (\WP2FA_Vendor\fs_starts_with($option_name, \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME . '_')) {
            $option_name = \str_replace(\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME . '_', '', $option_name);
        }
        switch ($option_name) {
            case 'plugins':
            case 'themes':
            case 'addons':
                $class_name = \WP2FA_Vendor\FS_Plugin::get_class_name();
                break;
            case 'users':
                $class_name = \WP2FA_Vendor\FS_User::get_class_name();
                break;
            case 'sites':
                $class_name = \WP2FA_Vendor\FS_Site::get_class_name();
                break;
            case 'licenses':
            case 'all_licenses':
                $class_name = \WP2FA_Vendor\FS_Plugin_License::get_class_name();
                break;
            case 'plans':
                $class_name = \WP2FA_Vendor\FS_Plugin_Plan::get_class_name();
                break;
            case 'updates':
                $class_name = \WP2FA_Vendor\FS_Plugin_Tag::get_class_name();
                break;
        }
        if (empty($class_name)) {
            return $option;
        }
        return \WP2FA_Vendor\fs_get_entities($option, $class_name);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.6
    ...
    private static function get_all_licenses($module_id = null)
    {
        ...
        $licenses = isset($licenses[$module_id]) ? $licenses[$module_id] : array();
        return $licenses;
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    {
        $licenses = self::get_account_option('all_licenses');
        $licenses_by_module_type = array(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN => array(), \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME => array());
        if (!\is_array($licenses)) {
    ...
    private static function get_user_id_license_ids_map($module_id, $user_id = null)
    {
        ...
        $user_id_license_ids_map = isset($all_modules_user_id_license_ids_map[$module_id]) ? $all_modules_user_id_license_ids_map[$module_id] : array();
        if (\WP2FA_Vendor\FS_User::is_valid_id($user_id)) {
            $user_id_license_ids_map = isset($user_id_license_ids_map[$user_id]) ? $user_id_license_ids_map[$user_id] : array();
        }
    ...
    private static function store_user_id_license_ids_map($new_user_id_license_ids_map, $module_id, $user_id = null)
    {
        ...
        self::$_accounts->set_option('user_id_license_ids_map', $all_modules_user_id_license_ids_map, \true);
    }
    /**
    ...
    {
        return $this->is_addon() ? \is_object($this->_parent) && $this->_parent->is_bundle_license_auto_activation_enabled() : $this->_is_bundle_license_auto_activation_enabled;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.4
    ...
    function get_addons($flush = \false)
    {
        ...
        return !\is_array($addons) || empty($addons) ? \false : $addons;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.6
    ...
    function get_account_addons()
    {
        ...
        if (!\is_array($addons) || !isset($addons[$this->_plugin->id]) || !\is_array($addons[$this->_plugin->id]) || 0 === \count($addons[$this->_plugin->id])) {
            return \false;
        }
    ...
    function _get_addon_info($addon_id, $is_installed)
    {
        ...
        $slug = $addon->slug;
        $addon_storage = \WP2FA_Vendor\FS_Storage::instance(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $slug);
        if (!\WP2FA_Vendor\fs_is_network_admin()) {
            ...
            if ($this->is_addon_activated($addon_id) && $this->get_addon_instance($addon_id)->is_network_active()) {
                if (\WP2FA_Vendor\FS_Site::is_valid_id($addon_storage->network_install_blog_id)) {
                    // Get network-level activated installations.
                    $sites = self::maybe_get_entities_account_option('sites', array(), $addon_storage->network_install_blog_id);
                }
            }
        }
        $addon_info = array('is_connected' => \false, 'slug' => $slug, 'title' => $addon->title, 'is_whitelabeled' => $addon_storage->is_whitelabeled);
        if (!$is_installed) {
            ...
            if (isset($plans_and_pricing_by_addon_id[$addon_id])) {
                ...
                if (\is_array($plans) && \count($plans) > 0) {
                    foreach ($plans as $plan) {
                        if (isset($plan->pricing) && \is_array($plan->pricing) && \count($plan->pricing) > 0) {
                            $has_paid_plan = \true;
                            break;
                        }
                    }
        ...
        $site = $sites[$slug];
        $addon_info['is_connected'] = $addon->parent_plugin_id == $this->get_id() && \is_object($site) && \WP2FA_Vendor\FS_Site::is_valid_id($site->id) && \WP2FA_Vendor\FS_User::is_valid_id($site->user_id) && \WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($site->plan_id);
        if ($addon_info['is_connected'] && $is_installed) {
            return $addon_info;
        }
        $addon_info['site'] = $site;
        $plugins_data = self::maybe_get_entities_account_option(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN . 's', array());
        ...
        if (isset($all_plans[$slug])) {
            ...
            foreach ($plans as $plan) {
                if ($site->plan_id == \WP2FA_Vendor\Freemius::_decrypt($plan->id)) {
                    $addon_info['plan_name'] = \WP2FA_Vendor\Freemius::_decrypt($plan->name);
                    $addon_info['plan_title'] = \WP2FA_Vendor\Freemius::_decrypt($plan->title);
                    break;
                }
            }
        }
        ...
        if (isset($addon_info['license'])) {
            if (isset($addon_storage->subscriptions) && !empty($addon_storage->subscriptions)) {
                $addon_subscriptions = \WP2FA_Vendor\fs_get_entities($addon_storage->subscriptions, \WP2FA_Vendor\FS_Subscription::get_class_name());
    ...
    static function _get_user_by_id($user_id)
    {
        ...
        $users = self::get_all_users();
        if (\is_array($users)) {
            if (isset($users[$user_id]) && $users[$user_id] instanceof \WP2FA_Vendor\FS_User && $user_id == $users[$user_id]->id) {
    ...
    private static function is_super_admin($user_id)
    {
        ...
        if ($user instanceof \WP2FA_Vendor\FS_User && !empty($user->email)) {
            self::require_pluggable_essentials();
            $wp_user = \WP2FA_Vendor\get_user_by('email', $user->email);
            if ($wp_user instanceof \WP2FA_Vendor\WP_User) {
                $super_admins = \WP2FA_Vendor\get_super_admins();
    ...
    function is_premium()
    {
         ...
         * @since 2.2.3
         */
        return \is_object($this->_plugin) ? $this->_plugin->is_premium : \false;
    }
    /**
     * Get site's plan ID.
     *
    ...
    function get_plan()
    {
        ...
        return \WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($this->_site->plan_id) ? $this->_get_plan_by_id($this->_site->plan_id) : \false;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.3
    ...
    function get_trial_plan()
    {
        ...
        if (!\is_object($trial_plan)) {
             ...
             */
            if ($trial_plan instanceof \WP2FA_Vendor\FS_Plugin_Plan) {
         ...
         */
        $trial_plan = new \WP2FA_Vendor\FS_Plugin_Plan();
    ...
    function is_paying()
    {
        ...
        return !$this->is_trial() && 'free' !== $this->get_plan_name() && $this->has_active_valid_license();
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.4
    ...
    function is_free_plan()
    {
        ...
        return 'free' === $this->get_plan_name() || !$this->has_features_enabled_license();
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.5
    ...
    private function sync_plan_if_not_exist($plan_id)
    {
        ...
        if ($plan instanceof \WP2FA_Vendor\FS_Plugin_Plan) {
            $this->_plans[] = $plan;
            $this->_store_plans();
            return $plan;
        }
    ...
    private function sync_license_if_not_exist($license_id, $license_key)
    {
        ...
        if ($license instanceof \WP2FA_Vendor\FS_Plugin_License) {
    ...
    {
        if (!\is_multisite()) {
            if (!\is_object($this->_site) || !\WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($this->_site->plan_id)) {
                return array();
            }
        ...
        foreach ($sites as $site) {
            $blog_id = self::get_site_blog_id($site);
            $install = $this->get_install_by_blog_id($blog_id);
            if (!\is_object($install) || !\WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($install->plan_id)) {
                continue;
            }
            $plan_ids[$install->plan_id] = \true;
        }
    ...
    {
        if (!$this->_is_network_active) {
            if (!\is_object($this->_site) || !\WP2FA_Vendor\FS_Plugin_License::is_valid_id($this->_site->license_id)) {
                return array();
            }
        ...
        foreach ($sites as $site) {
            ...
            $license_ids[$install->license_id] = \true;
        }
    ...
    function _sync_licenses($site_license_id = \false, $blog_id = null)
    {
        ...
        foreach ($all_licenses as $license) {
            $all_licenses_map[$license->id] = \true;
        }
        ...
        if ($this->is_array_instanceof($licenses, 'FS_Plugin_License')) {
            ...
            $this->_store_licenses(\true, $this->_module_id, $licenses);
        }
    ...
    {
        $this->_logger->entrance();
        if (!\WP2FA_Vendor\FS_Plugin_License::is_valid_id($id)) {
            return \false;
        }
        /**
         ...
         * @since  2.0.0
         */
        if (\WP2FA_Vendor\fs_is_network_admin() && $this->is_network_registered() && (!\is_object($this->_user) || $this->_storage->network_user_id != $this->_user->id)) {
    ...
    private function sync_site_license()
    {
        ...
        $result = $api->get("/licenses/{$this->_license->id}.json?license_key=" . \urlencode($this->_license->secret_key), \true);
        if (!$this->is_api_result_entity($result)) {
            return $result;
        }
        $license = $this->_update_site_license(new \WP2FA_Vendor\FS_Plugin_License($result));
        $this->_store_licenses();
        return $license;
    }
    /**
     ...
     * @return bool
     */
    private function try_activate_license_on_network(\WP2FA_Vendor\FS_User $user, \WP2FA_Vendor\FS_Plugin_License $license)
    {
        $this->_logger->entrance();
     ...
     * }
     */
    private function can_activate_license_on_network(\WP2FA_Vendor\FS_Plugin_License $license)
    {
        $sites = self::get_sites();
        $production_count = 0;
        $localhost_count = 0;
        ...
        foreach ($sites as $site) {
            ...
            $install = $this->get_install_by_blog_id($blog_id);
            if (\is_object($install)) {
                if (\WP2FA_Vendor\FS_Plugin_License::is_valid_id($install->license_id)) {
                ...
                $url = \is_object($site) ? $site->siteurl : \WP2FA_Vendor\get_site_url($blog_id);
                $disconnected_site_ids[] = $blog_id;
            }
            if (\WP2FA_Vendor\FS_Site::is_localhost_by_address($url)) {
                $localhost_count++;
            } else {
                $production_count++;
            }
        ...
        return array('installs' => $installs_without_license, 'sites' => $disconnected_site_ids, 'production_count' => $production_count, 'localhost_count' => $localhost_count);
    }
    /**
     ...
     * @return mixed|true
     */
    private function activate_license_on_many_installs(\WP2FA_Vendor\FS_User $user, $license_key, array $blog_2_install_map)
    {
        $params = array(array('license_key' => $this->apply_filters('license_key', $license_key)));
        $install_2_blog_map = array();
        foreach ($blog_2_install_map as $blog_id => $install) {
            $params[] = array('id' => $install->id);
            $install_2_blog_map[$install->id] = $blog_id;
        }
        $result = $this->get_api_user_scope_by_user($user)->call("plugins/{$this->_plugin->id}/installs.json", 'PUT', $params);
        if (!$this->is_api_result_object($result, 'installs')) {
            return $result;
        }
        foreach ($result->installs as $r_install) {
            $install = new \WP2FA_Vendor\FS_Site($r_install);
            $install->is_disconnected = \false;
            // Update install.
            $this->_store_site(\true, $install_2_blog_map[$r_install->id], $install);
        }
        return \true;
    }
    /**
     ...
     */
    private function activate_license_on_site(\WP2FA_Vendor\FS_User $user, $license_key)
    {
     ...
     * @return true|mixed True if successful, otherwise, the API result.
     */
    private function activate_license_on_many_sites(\WP2FA_Vendor\FS_User $user, $license_key, array $site_ids = array())
    {
        $sites = array();
        ...
        // Install the plugin.
        $result = $this->create_installs_with_user($user, $license_key, \false, $sites, \false, \true);
        if (!$this->is_api_result_entity($result) && !$this->is_api_result_object($result, 'installs')) {
            return $result;
        }
        $installs = array();
        if ($this->is_api_result_entity($result)) {
            $install = new \WP2FA_Vendor\FS_Site($result);
            $this->_user = $user;
            $this->_store_site(\true, null, $install);
            $this->_site = $install;
            $this->reset_anonymous_mode();
        } else {
            foreach ($result->installs as $install) {
                $installs[] = new \WP2FA_Vendor\FS_Site($install);
            }
            ...
            if (!\WP2FA_Vendor\FS_Site::is_valid_id($this->_storage->network_install_blog_id)) {
     ...
     * @param \FS_Plugin_License $license
     */
    private function set_license(\WP2FA_Vendor\FS_Plugin_License $license = null)
    {
        $this->_license = $license;
    ...
    {
        $is_whitelabeled = isset($this->_storage->is_whitelabeled) ? $this->_storage->is_whitelabeled : \false;
        if (\is_object($license)) {
            ...
            if ($license->is_whitelabeled) {
                ...
                if ($this->is_registered() && $this->_user->id == $license->user_id) {
     ...
     */
    private function store_last_activated_license_data(\WP2FA_Vendor\FS_Plugin_License $license, $license_user = null)
    {
        if (!\is_object($license_user)) {
    ...
    {
        if (\true !== $this->_storage->is_whitelabeled) {
            return \false;
        } else {
            if ($ignore_data_debug_mode) {
                return \true;
            }
        }
    ...
    {
        return \WP2FA_Vendor\FS_User::is_valid_id($this->_storage->last_license_user_id) ? $this->_storage->last_license_user_id : null;
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.3.1
    ...
    function is_whitelabeled($ignore_data_debug_mode = \false, $blog_id = null)
    {
        ...
        if (!\is_null($this->is_whitelabeled)) {
            ...
            $is_whitelabeled = \false;
            $is_whitelabeled_flag = $this->is_whitelabeled_by_flag(\true);
            if (!$this->has_addons()) {
                $is_whitelabeled = $is_whitelabeled_flag;
            } else {
                if ($is_whitelabeled_flag) {
                    $is_whitelabeled = \true;
                } else {
                    ...
                    if (!empty($addon_ids)) {
                        ...
                        $is_network_level = \WP2FA_Vendor\fs_is_network_admin() && $this->is_network_active();
                        foreach ($addon_ids as $addon_id) {
                            $addon = $this->get_addon($addon_id);
                            if (!\is_object($addon)) {
                                continue;
                            }
                            $addon_storage = \WP2FA_Vendor\FS_Storage::instance(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $addon->slug);
                            $fs_addon = $this->is_addon_activated($addon_id) ? self::get_addon_instance($addon_id) : null;
                            $was_addon_network_activated = \false;
                            if (\is_object($fs_addon)) {
                                $was_addon_network_activated = $fs_addon->is_network_active();
                            } else {
                                if ($is_network_level) {
                                    $was_addon_network_activated = $addon_storage->get('was_plugin_loaded', \false, \true);
                                }
                            }
                            $network_delegated_connection = $was_addon_network_activated && $addon_storage->get('is_delegated_connection', \false, \true);
                            if ($is_network_level && (!$was_addon_network_activated || $network_delegated_connection)) {
                                $sites = self::get_sites();
                                /**
                                 ...
                                 */
                                if (\true === $addon_storage->is_whitelabeled) {
                                    $is_whitelabeled = \true;
                                    break;
                                }
                            }
        ...
        return $is_whitelabeled && ($ignore_data_debug_mode || !$this->is_data_debug_mode());
    }
    /**
    ...
    private function _sync_site_subscription($license)
    {
        ...
        $subscription = $license->is_lifetime() ? \false : $this->_fetch_site_license_subscription();
    ...
    {
        if (!\WP2FA_Vendor\fs_is_network_admin() || \is_object($this->_license)) {
            return $this->_license;
        }
    ...
    {
        if (!isset($this->_storage->subscriptions) || empty($this->_storage->subscriptions)) {
            return null;
        }
        foreach (\WP2FA_Vendor\fs_get_entities($this->_storage->subscriptions, \WP2FA_Vendor\FS_Subscription::get_class_name()) as $subscription) {
     ...
     */
    function store_subscription(\WP2FA_Vendor\FS_Subscription $subscription)
    {
        ...
        $subscriptions = \WP2FA_Vendor\fs_get_entities($this->_storage->subscriptions, \WP2FA_Vendor\FS_Subscription::get_class_name());
    ...
    {
        if (!isset($this->_storage->subscriptions) || empty($this->_storage->subscriptions) || \count($this->_storage->subscriptions) < 3) {
            return;
        }
        if (!\is_multisite()) {
        ...
        foreach ($sites as $site) {
            ...
            $subscriptions_to_keep_by_license_id_map[$install->license_id] = \true;
        }
    ...
    function is_plan($plan, $exact = \false)
    {
        ...
        for ($i = 0, $len = \count($this->_plans); $i < $len; $i++) {
            if ($plan === $this->_plans[$i]->name) {
                ...
                if ($current_plan_name === $this->_plans[$i]->name) {
    ...
    {
        $this->_logger->entrance();
        if (!$this->is_registered() || !\is_array($this->_plans) || 0 === \count($this->_plans)) {
            return \true;
        }
    ...
    function is_trial_plan($plan, $exact = \false)
    {
        ...
        for ($i = 0, $len = \count($this->_plans); $i < $len; $i++) {
            if ($plan === $this->_plans[$i]->name) {
                ...
                if ($trial_plan->name === $this->_plans[$i]->name) {
                    $current_plan_order = $i;
                }
            }
        }
    ...
    {
        return $this->_has_paid_plans || \WP2FA_Vendor\FS_Plan_Manager::instance()->has_paid_plan($this->_plans);
    }
    /**
    ...
    {
        $vars = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_template('forms/license-activation.php', $vars);
        \WP2FA_Vendor\fs_require_template('forms/resend-key.php', $vars);
    }
    /**
    ...
    {
        $installs = array();
        if (\is_object($this->_license) && $this->_site->user_id != $this->_license->user_id) {
        ...
        foreach ($installs_by_slug_map as $slug => $install_info) {
            ...
            if (\is_object($license) && $install->user_id != $license->user_id) {
                $installs[] = $install->id;
            }
        }
        return $installs;
    ...
    {
        $vars = array('id' => $this->_module_id, 'license_owners' => $this->fetch_installs_licenses_owners_data($install_ids));
        \WP2FA_Vendor\fs_require_template('forms/user-change.php', $vars);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.3.1
    ...
    {
        $vars = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_template('forms/data-debug-mode.php', $vars);
    }
    /**
    ...
    function _get_subscription_cancellation_dialog_box_template_params($is_license_deactivation = \false)
    {
         ...
         * @since 2.2.1
         */
        if (!\is_object($license) || $license->is_lifetime() || !$license->is_single_site() && $license->activated > 1) {
            return array();
        }
        /**
         * @var FS_Subscription $subscription
        ...
        return array('id' => $this->_module_id, 'license' => $license, 'has_trial' => $this->is_paid_trial(), 'is_license_deactivation' => $is_license_deactivation);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.0.2
    ...
    function _add_premium_version_upgrade_selection_dialog_box()
    {
        ...
        $vars = array('id' => $this->_module_id, 'new_version' => \is_object($modules_update->response[$this->_plugin_basename]) ? $modules_update->response[$this->_plugin_basename]->new_version : $modules_update->response[$this->_plugin_basename]['new_version']);
        \WP2FA_Vendor\fs_require_template('forms/premium-versions-upgrade-metadata.php', $vars);
        \WP2FA_Vendor\fs_require_once_template('forms/premium-versions-upgrade-handler.php', $vars);
    }
    /**
    ...
    function _add_optout_dialog()
    {
        if ($this->is_theme()) {
            $vars = null;
            \WP2FA_Vendor\fs_require_once_template('/js/jquery.content-change.php', $vars);
        }
        $vars = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_template('forms/optout.php', $vars);
    }
    /**
    ...
    function _add_license_activation()
    {
        ...
        if ($this->has_premium_version() && !$this->is_premium() && !\is_object($this->_get_license())) {
        ...
        if (self::is_plugins_page()) {
            $is_network_admin = \WP2FA_Vendor\fs_is_network_admin();
            if ($is_network_admin && $this->is_network_active() && !$this->is_network_delegated_connection() || !$is_network_admin && (!$this->is_network_active() || $this->is_delegated_connection())) {
                /**
    ...
    function should_handle_user_change()
    {
        ...
        if ($this->is_network_active() && (\WP2FA_Vendor\fs_is_network_admin() || !$this->is_site_delegated_connection())) {
    ...
    function _toggle_whitelabel_mode_ajax_handler()
    {
        ...
        $license = $this->get_api_user_scope()->call("/licenses/{$this->_site->license_id}.json", 'put', array('is_whitelabeled' => !$this->_license->is_whitelabeled));
        if (!$this->is_api_result_entity($license)) {
            self::shoot_ajax_failure(\WP2FA_Vendor\FS_Api::is_api_error_object($license) ? $license->error->message : \WP2FA_Vendor\fs_text_inline("An unknown error has occurred while trying to toggle the license's white-label mode.", 'unknown-error-occurred', $this->get_slug()));
        }
    ...
    function _set_beta_mode_ajax_handler()
    {
        ...
        $is_beta = \trim(\WP2FA_Vendor\fs_request_get('is_beta', '', 'post'));
        if (empty($is_beta) || !\in_array($is_beta, array('true', 'false'))) {
            self::shoot_ajax_failure();
        }
        $site = $this->get_api_site_scope()->call('', 'put', array('is_beta' => 'true' == $is_beta, 'fields' => 'is_beta'));
        if (!$this->is_api_result_entity($site)) {
            self::shoot_ajax_failure(\WP2FA_Vendor\FS_Api::is_api_error_object($site) ? $site->error->message : \WP2FA_Vendor\fs_text_inline("An unknown error has occurred while trying to set the user's beta mode.", 'unknown-error-occurred', $this->get_slug()));
        }
    ...
    function _activate_license_ajax_action()
    {
        ...
        $license_key = \trim(\WP2FA_Vendor\fs_request_get('license_key'));
        if (empty($license_key)) {
            exit;
        }
        $sites = \WP2FA_Vendor\fs_is_network_admin() ? \WP2FA_Vendor\fs_request_get('sites', array(), 'post') : array();
        $result = $this->activate_license($license_key, $sites, \WP2FA_Vendor\fs_request_get_bool('is_marketing_allowed', null), \WP2FA_Vendor\fs_request_get('blog_id', null), \WP2FA_Vendor\fs_request_get('module_id', null, 'post'), \WP2FA_Vendor\fs_request_get('user_id', null), \WP2FA_Vendor\fs_request_get_bool('is_extensions_tracking_allowed', null));
        if ($result['success'] && $this->is_bundle_license_auto_activation_enabled()) {
            $license = new \WP2FA_Vendor\FS_Plugin_License();
    ...
    function _user_change_ajax_action()
    {
        ...
        $new_email_address = \trim(\WP2FA_Vendor\fs_request_get('email_address', ''));
        $new_user_id = \WP2FA_Vendor\fs_request_get('user_id');
        if (empty($new_email_address) && !\WP2FA_Vendor\FS_User::is_valid_id($new_user_id)) {
            self::shoot_ajax_failure(\WP2FA_Vendor\fs_text_inline('Invalid new user ID or email address.', 'invalid-new-user-id-or-email', $this->get_slug()));
        }
        $params = array();
        ...
        if (\WP2FA_Vendor\FS_Api::is_api_error($install)) {
            ...
            if (empty($error)) {
                $error = \WP2FA_Vendor\FS_Api::is_api_error_object($install) ? $install->error->message : \var_export($install->error, \true);
            }
            self::shoot_ajax_failure($error);
        } else {
            if ($this->get_user()->id != $install->user_id || !empty($new_email_address)) {
    ...
    function starting_migration()
    {
        ...
        $this->_storage->license_migration = array('is_migrating' => \true, 'start_timestamp' => \time());
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.3.2.14
    ...
    function is_migration()
    {
        ...
        return \time() - $this->_storage->license_migration['start_timestamp'] <= \WP2FA_Vendor\WP_FS__TIME_5_MIN_IN_SEC;
    }
    /**
     *
     ...
     * @uses Freemius::activate_license()
     */
    function activate_migrated_license($license_key, $is_marketing_allowed = null, $plugin_id = null, $sites = array(), $blog_id = null)
    {
        $this->_logger->entrance();
        $result = $this->activate_license($license_key, empty($sites) && \is_null($blog_id) && $this->is_network_active() ? $this->get_sites_for_network_level_optin() : $sites, $is_marketing_allowed, $blog_id, $plugin_id);
            ...
            if (empty($pricing_js_path)) {
                ...
                foreach ($fs_active_plugins->plugins as $sdk_path => $data) {
                    if ($data->plugin_path == $this->get_plugin_basename()) {
                        $plugin_or_theme_root_dir = $this->is_plugin() ? \WP2FA_Vendor\WP_PLUGIN_DIR : \WP2FA_Vendor\get_theme_root(\get_stylesheet());
                        $pricing_js_path = $plugin_or_theme_root_dir . '/' . \str_replace('../' . \basename($plugin_or_theme_root_dir) . '/', '', $sdk_path) . '/includes/freemius-pricing/freemius-pricing.js';
                        break;
                    }
                }
            }
     ...
     * }
     */
    private function activate_license($license_key, $sites = array(), $is_marketing_allowed = null, $blog_id = null, $plugin_id = null, $license_owner_id = null, $is_extensions_tracking_allowed = null)
    {
        $this->_logger->entrance();
        $license_key = \trim($license_key);
        $is_network_activation_or_migration = \WP2FA_Vendor\fs_is_network_admin() || !empty($sites) && $this->is_migration();
        ...
        $fs = empty($plugin_id) || $plugin_id == $this->_module_id ? $this : $this->get_addon_instance($plugin_id);
        ...
        if (\is_object($user)) {
            if ($is_network_activation_or_migration && !$has_valid_blog_id) {
                ...
                if (!empty($blog_2_install_map)) {
                    ...
                    if (\true !== $result) {
                        $error = \WP2FA_Vendor\FS_Api::is_api_error_object($result) ? $result->error->message : \var_export($result, \true);
                    }
                }
                ...
                if ($fs->is_registered()) {
                    $params = array('license_key' => $fs->apply_filters('license_key', $license_key));
                    $install_ids = array();
                    $change_owner = \WP2FA_Vendor\FS_User::is_valid_id($license_owner_id);
                    if ($change_owner) {
                    ...
                    if (\WP2FA_Vendor\FS_Api::is_api_error($install)) {
                        ...
                        if ($change_owner && $fs->get_user()->id != $install->user_id) {
                            $fs->complete_ownership_change_by_license($install->user_id, $install_ids);
                        }
                    }
                } else {
                    /* ( $fs->is_addon() && $fs->get_parent_instance()->is_registered() ) */
            ...
            if (empty($error)) {
                ...
                $fs->_user = $user;
                if (\WP2FA_Vendor\fs_is_network_admin() && !$has_valid_blog_id) {
                    $fs->_site = $fs->get_network_install();
                }
                $fs->_sync_license(\true, $has_valid_blog_id);
                $this->maybe_sync_install_user();
                $next_page = $fs->is_addon() ? $fs->get_parent_instance()->get_account_url() : $fs->get_after_activation_url('after_connect_url');
            }
        } else {
            $next_page = $fs->opt_in(\false, \false, \false, $license_key, \false, \false, \false, $is_marketing_allowed, $sites);
            if (isset($next_page->error)) {
                ...
                if ($is_network_activation_or_migration) {
                    ...
                    foreach ($sites as $site) {
                        if (isset($site['blog_id']) && \is_numeric($site['blog_id'])) {
                            $connected_sites[$site['blog_id']] = \true;
                        }
                    }
                    $all_sites = self::get_sites();
                    $pending_sites = array();
        ...
        if (\false === $error && \true === $fs->_storage->require_license_activation) {
    ...
    private function get_parent_and_addons_installs_info()
    {
        ...
        $addons_ids = \array_unique(\array_merge($installed_addons_ids, $fs->get_updated_account_addons()));
        // Add parent product info.
        $installs_info_by_slug_map = array($fs->get_slug() => array('install' => $fs->get_site(), 'license' => $fs->_get_license()));
        foreach ($addons_ids as $addon_id) {
            ...
            $installs_info_by_slug_map[$addon_info['slug']] = array('install' => $addon_info['site'], 'license' => isset($addon_info['license']) ? $addon_info['license'] : null);
        }
    ...
    function _network_activate_ajax_action()
    {
        ...
        $plugin_id = \WP2FA_Vendor\fs_request_get('module_id', '', 'post');
        $fs = $plugin_id == $this->_module_id ? $this : $this->get_addon_instance($plugin_id);
        $error = \false;
        $sites = \WP2FA_Vendor\fs_request_get('sites', array(), 'post');
        if (\is_array($sites) && !empty($sites)) {
            $sites_by_action = array('allow' => array(), 'delegate' => array(), 'skip' => array());
            foreach ($sites as $site) {
            ...
            $next_page = '';
            $has_any_install = \WP2FA_Vendor\fs_request_get_bool('has_any_install');
            if ($total_sites === $total_sites_to_delegate && !$this->is_network_upgrade_mode() && !$has_any_install) {
                $this->delegate_connection();
            } else {
                ...
                if (empty($sites_by_action['allow'])) {
                    ...
                    if (!$fs->is_registered() || !$this->_is_network_active) {
                        $next_page = $fs->opt_in(\false, \false, \false, \false, \false, \false, \false, \WP2FA_Vendor\fs_request_get_bool('is_marketing_allowed', null), $sites_by_action['allow']);
                    } else {
                        $next_page = $fs->install_with_user($this->get_network_user(), \false, \false, \false, \true, $sites_by_action['allow']);
                    }
    ...
    function _update_billing_ajax_action()
    {
        ...
        $billing = \WP2FA_Vendor\fs_request_get('billing');
        $api = $this->get_api_user_scope();
        $result = $api->call('/billing.json', 'put', \array_merge($billing, array('plugin_id' => $this->get_parent_id())));
        if (!$this->is_api_result_entity($result)) {
            self::shoot_ajax_failure();
        }
    ...
    function _start_trial_ajax_action()
    {
        ...
        $trial_data = \WP2FA_Vendor\fs_request_get('trial');
        $next_page = $this->opt_in(\false, \false, \false, \false, \false, $trial_data['plan_id']);
        if (\is_object($next_page) && $this->is_api_error($next_page)) {
            self::shoot_ajax_failure(isset($next_page->error) ? $next_page->error->message : \var_export($next_page, \true));
        }
    ...
    function _resend_license_key_ajax_action()
    {
        ...
        $email_address = \WP2FA_Vendor\sanitize_email(\trim(\WP2FA_Vendor\fs_request_get('email', '', 'post')));
        if (empty($email_address)) {
            exit;
        }
        $error = \false;
        $api = $this->get_api_plugin_scope();
        $result = $api->call('/licenses/resend.json', 'post', array('email' => $email_address, 'url' => \home_url()));
        ...
        $licenses = array('success' => \false === $error);
        if (\false !== $error) {
    ...
    static function get_current_page()
    {
        if (!isset(self::$_pagenow)) {
            global $pagenow;
            if (empty($pagenow) && \WP2FA_Vendor\is_admin() && \is_multisite()) {
                /**
                 ...
                 * @since 2.2.3
                 */
                if (\WP2FA_Vendor\is_network_admin()) {
                    \preg_match('#/wp-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
                } else {
                    if (\WP2FA_Vendor\is_user_admin()) {
            ...
            if (self::is_ajax() && 'admin-ajax.php' === $pagenow) {
                $referer = \WP2FA_Vendor\fs_get_raw_referer();
    ...
    private function fetch_affiliate_terms()
    {
        if (!\is_object($this->plugin_affiliate_terms)) {
            ...
            $this->plugin_affiliate_terms = new \WP2FA_Vendor\FS_AffiliateTerms($affiliate_terms);
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
        ...
        if (!empty($this->_storage->affiliate_application_data)) {
            ...
            if ($this->is_api_result_object($result, 'affiliates')) {
                if (!empty($result->affiliates)) {
                    $affiliate = new \WP2FA_Vendor\FS_Affiliate($result->affiliates[0]);
                    ...
                    if ($affiliate->is_using_custom_terms) {
                        ...
                        if ($this->is_api_result_entity($affiliate_terms)) {
                            $this->custom_affiliate_terms = new \WP2FA_Vendor\FS_AffiliateTerms($affiliate_terms);
                        }
                    }
    ...
    {
        return \is_object($this->custom_affiliate_terms) ? $this->custom_affiliate_terms : $this->plugin_affiliate_terms;
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  1.2.3
    ...
    function _submit_affiliate_application()
    {
        ...
        $affiliate = \WP2FA_Vendor\fs_request_get('affiliate');
        ...
        if (!$this->is_registered()) {
            ...
            $next_page = $this->opt_in(\false, \false, \false, \false, \false, \false, \true);
            if (\is_object($next_page) && $this->is_api_error($next_page)) {
                self::shoot_ajax_failure(isset($next_page->error) ? $next_page->error->message : \var_export($next_page, \true));
            } else {
                if ($this->is_pending_activation()) {
        ...
        $this->fetch_affiliate_terms();
        $api = $this->get_api_user_scope();
        $result = $api->call("/plugins/{$this->_plugin->id}/aff/{$this->plugin_affiliate_terms->id}/affiliates.json", 'post', $affiliate);
        if ($this->is_api_error($result)) {
            self::shoot_ajax_failure(isset($result->error) ? $result->error->message : \var_export($result, \true));
        } else {
            if ($this->_admin_notices->has_sticky('affiliate_program')) {
                $this->_admin_notices->remove_sticky('affiliate_program');
            }
            $affiliate_application_data = array('status' => 'pending', 'stats_description' => $affiliate['stats_description'], 'promotion_method_description' => $affiliate['promotion_method_description']);
            if (!empty($affiliate['promotion_methods'])) {
     ...
     * @return string
     */
    function get_upgrade_url($period = \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY, $is_trial = \false)
    {
    ...
    {
        return $this->get_upgrade_url(\WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY, \true);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.1.4
    ...
    function version_upgrade_checkout_link($new_version)
    {
        if (!\is_object($this->_license)) {
            ...
            $subscription = $this->_get_subscription($this->_license->id);
            $url = $this->checkout_url(\is_object($subscription) ? 1 == $subscription->billing_cycle ? \WP2FA_Vendor\WP_FS__PERIOD_MONTHLY : \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY : \WP2FA_Vendor\WP_FS__PERIOD_LIFETIME, \false, array('licenses' => $this->_license->quota));
            $purchase_license_text = $this->get_text_inline('Renew your license now', 'renew-license-now');
        }
        return \sprintf($this->get_text_inline('%s to access version %s security & feature updates, and support.', 'x-for-updates-and-support'), \sprintf('<a href="%s">%s</a>', $url, $purchase_license_text), $new_version);
    }
    /**
     ...
     * @return string
     */
    function pricing_url($billing_cycle = \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY, $is_trial = \false)
    {
        $this->_logger->entrance();
        $params = array('billing_cycle' => $billing_cycle);
        if ($is_trial) {
            $params['trial'] = 'true';
        }
        $url = $this->is_addon() ? $this->_parent->addon_url($this->_slug) : $this->_get_admin_page_url('pricing', $params);
     ...
     * @return string
     */
    function checkout_url($billing_cycle = \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY, $is_trial = \false, $extra = array(), $network = null)
    {
        $this->_logger->entrance();
        $params = array('checkout' => 'true', 'billing_cycle' => $billing_cycle);
        if ($is_trial) {
            $params['trial'] = 'true';
        }
        /**
     ...
     * @return string
     */
    function addon_checkout_url($addon_id, $pricing_id, $billing_cycle = \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY, $is_trial = \false, $network = null)
    {
        return $this->checkout_url($billing_cycle, $is_trial, array('plugin_id' => $addon_id, 'pricing_id' => $pricing_id), $network);
    }
    #endregion
    #endregion ------------------------------------------------------------------
    /**
    ...
    function is_plugin()
    {
        return \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN === $this->_module_type;
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  1.2.2
    ...
    function is_ssl()
    {
        return \WP2FA_Vendor\WP_FS__IS_HTTPS;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.9
    ...
    {
        return \defined('WP2FA_Vendor\\DOING_AJAX') && \WP2FA_Vendor\DOING_AJAX;
    }
    /**
    ...
    function is_ajax_action($actions)
    {
        ...
        if ($this->_module_id != \WP2FA_Vendor\fs_request_get('module_id')) {
            return \false;
        }
        ...
        if (\is_array($actions) && 0 < \count($actions)) {
            $ajax_action = \WP2FA_Vendor\fs_request_get('action');
    ...
    static function is_ajax_action_static($actions, $module_id = null)
    {
        ...
        if (!empty($module_id)) {
            // Verify the call is relevant for the plugin.
            if ($module_id != \WP2FA_Vendor\fs_request_get('module_id')) {
                return \false;
            }
        }
        // Verify it's one of the specified actions.
    ...
    {
        return \defined('WP2FA_Vendor\\DOING_CRON') && \WP2FA_Vendor\DOING_CRON;
    }
    /**
    ...
    {
        return \WP2FA_Vendor\is_admin() && !self::is_ajax() && !self::is_cron() && 'admin-post.php' !== self::get_current_page();
    }
    /**
    ...
    {
        if (\is_null($network)) {
            $network = $this->_is_network_active && (\WP2FA_Vendor\fs_is_network_admin() || !$this->is_delegated_connection());
        }
        if (0 < \count($params)) {
        ...
        if (empty($page) && $this->show_opt_in_on_themes_page()) {
            $params[$this->get_unique_affix() . '_show_optin'] = 'true';
            return \add_query_arg($params, $this->admin_url('themes.php', 'admin', $network));
        }
        if (!$this->has_settings_menu()) {
            if (!empty($page)) {
                // Module doesn't have a setting page, but since the request is for
                // a specific Freemius page, use the admin.php path.
                return \add_query_arg(\array_merge($params, array('page' => $page_param)), $this->admin_url('admin.php', 'admin', $network));
            } else {
                if ($this->is_activation_mode()) {
                    /**
                     * @author Vova Feldman
                     ...
                     */
                    return \add_query_arg(\array_merge($params, array('page' => $this->_slug)), $this->admin_url('admin.php', 'admin', $network));
                } else {
                    // Plugin without a settings page.
                    return \add_query_arg($params, $this->admin_url('plugins.php', 'admin', $network));
                }
            }
        }
        ...
        if (!$this->_menu->is_top_level()) {
            ...
            return \add_query_arg(\array_merge($params, array('page' => $page_param)), $this->admin_url($menu_file, 'admin', $network));
        }
        ...
        if ($this->_menu->is_cpt()) {
            if (empty($page) && $this->is_activation_mode()) {
                ...
                return \add_query_arg($params, $this->admin_url($this->_menu->get_raw_slug(), 'admin', $network));
            }
        }
    ...
    private function delegate_connection($sites = null)
    {
        ...
        if (\is_null($sites)) {
            ...
            $this->_storage->store('is_delegated_connection', \true, \true, \true);
        } else {
    ...
    {
        $this->_storage->store('is_delegated_connection', \true, $blog_id, \true);
    }
    /**
    ...
    function is_network_delegated_connection()
    {
        ...
        return $this->_storage->get('is_delegated_connection', \false, \true);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.0.0
    ...
    function is_site_delegated_connection($blog_id = 0)
    {
        ...
        if (0 == $blog_id) {
            $blog_id = \WP2FA_Vendor\get_current_blog_id();
        }
    ...
    function is_delegated_connection($blog_id = 0)
    {
        ...
        if (\WP2FA_Vendor\fs_is_network_admin() && 0 == $blog_id) {
            return $this->is_network_delegated_connection();
        }
        return $this->is_network_delegated_connection() || $this->is_site_delegated_connection($blog_id);
    }
    /**
    ...
    function is_active_for_site($blog_id)
    {
        ...
        return \in_array($this->_plugin_basename, (array) \WP2FA_Vendor\get_blog_option($blog_id, 'active_plugins', array()));
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.0.0
    ...
    static function get_sites()
    {
        ...
        if (\function_exists('WP2FA_Vendor\\get_sites')) {
            // For WP 4.6 and above.
            return \WP2FA_Vendor\get_sites($args);
        } else {
            if (\function_exists('wp_' . 'get_sites')) {
                ...
                return \WP2FA_Vendor\get_blog_list(0, 'all');
            }
        }
    }
    /**
    ...
    private static function is_site_active($blog_id)
    {
        ...
        return \true == $blog_info->public && \false == $blog_info->archived && \false == $blog_info->mature && \false == $blog_info->spam && \false == $blog_info->deleted;
    }
    /**
    ...
    private function get_address_to_blog_map()
    {
        ...
        foreach ($sites as $site) {
            $blog_id = self::get_site_blog_id($site);
            $address = \trailingslashit(\WP2FA_Vendor\fs_strip_url_protocol(\WP2FA_Vendor\get_site_url($blog_id)));
    ...
    private static function get_user_opted_in_module_ids_map($fs_user_id)
    {
        self::$_static_logger->entrance();
        if (!\is_multisite()) {
            $installs = \array_merge(self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN), self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME));
        } else {
            $sites = self::get_sites();
            $installs = array();
            foreach ($sites as $site) {
                $blog_id = self::get_site_blog_id($site);
                $installs = \array_merge($installs, self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN, $blog_id), self::get_all_sites(\WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME, $blog_id));
            }
        }
        $module_ids_map = array();
        foreach ($installs as $install) {
            if (\is_object($install) && \WP2FA_Vendor\FS_Site::is_valid_id($install->id) && \WP2FA_Vendor\FS_User::is_valid_id($install->user_id) && $install->user_id == $fs_user_id) {
                $module_ids_map[$install->plugin_id] = \true;
            }
        }
    ...
    function find_first_install()
    {
        ...
        foreach ($sites as $site) {
            ...
            $install = $this->get_install_by_blog_id($blog_id);
            if (\is_object($install)) {
                return array('install' => $install, 'blog_id' => $blog_id);
            }
        }
        return null;
    }
     ...
     */
    function switch_to_blog($blog_id, \WP2FA_Vendor\FS_Site $install = null)
    {
        ...
        $this->_site = \is_object($install) ? $install : $this->get_install_by_blog_id($blog_id);
        $this->_user = \false;
        ...
        if (\is_object($this->_site)) {
            ...
            if (!\is_object($this->_user) && \WP2FA_Vendor\FS_User::is_valid_id($this->_storage->prev_user_id)) {
            ...
            if (!empty($all_plugin_licenses)) {
                if (!\WP2FA_Vendor\FS_Plugin_License::is_valid_id($this->_site->license_id)) {
                    $this->_license = null;
                } else {
                    $license_found = \false;
    ...
    {
        return $site instanceof \WP2FA_Vendor\WP_Site ? $site->blog_id : (\is_object($site) && isset($site->userblog_id) ? $site->userblog_id : $site['blog_id']);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.0.0
    ...
    function get_site_info($site = null)
    {
        ...
        $switched = \false;
        if (\is_null($site)) {
            $url = \WP2FA_Vendor\get_site_url();
            $name = \get_bloginfo('name');
            $blog_id = null;
        } else {
            $blog_id = self::get_site_blog_id($site);
            if (\WP2FA_Vendor\get_current_blog_id() != $blog_id) {
                \WP2FA_Vendor\switch_to_blog($blog_id);
                $switched = \true;
            }
            if ($site instanceof \WP2FA_Vendor\WP_Site) {
                $url = $site->siteurl;
                $name = $site->blogname;
            } else {
                $url = \WP2FA_Vendor\get_site_url($blog_id);
                $name = \get_bloginfo('name');
            }
        }
        $info = array('uid' => $this->get_anonymous_id($blog_id), 'url' => $url, 'title' => $name, 'language' => \get_bloginfo('language'), 'charset' => \get_bloginfo('charset'));
    ...
    function get_install_by_blog_id($blog_id = null)
    {
        ...
        if (\is_object($install) && \is_numeric($install->id) && \is_numeric($install->user_id) && \WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($install->plan_id)) {
            // Load site.
    ...
    function is_installed_on_site($blog_id = null)
    {
        $installs = self::get_all_sites($this->_module_type, $blog_id);
        $install = isset($installs[$this->_slug]) ? $installs[$this->_slug] : null;
        return \is_object($install) && \is_numeric($install->id) && \is_numeric($install->user_id) && \WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($install->plan_id);
    }
    /**
    ...
    function is_network_registered()
    {
        ...
        return \WP2FA_Vendor\FS_User::is_valid_id($this->_storage->network_user_id);
    }
    /**
    ...
    function get_network_user()
    {
        ...
        return \WP2FA_Vendor\FS_User::is_valid_id($this->_storage->network_user_id) ? self::_get_user_by_id($this->_storage->network_user_id) : null;
    }
    /**
    ...
    {
        return $this->_user instanceof \WP2FA_Vendor\FS_User ? $this->_user : $this->get_network_user();
    }
    /**
    ...
    function get_network_install()
    {
        ...
        return \WP2FA_Vendor\FS_Site::is_valid_id($this->_storage->network_install_blog_id) ? $this->get_install_by_blog_id($this->_storage->network_install_blog_id) : null;
    }
    /**
    ...
    function get_network_install_blog_id()
    {
        ...
        return \WP2FA_Vendor\FS_Site::is_valid_id($this->_storage->network_install_blog_id) ? $this->_storage->network_install_blog_id : null;
    }
    /**
    ...
    {
        return $this->_site instanceof \WP2FA_Vendor\FS_Site ? $this->_site : $this->get_network_install();
    }
    /**
    ...
    private function is_network_level_site_specific_action()
    {
        ...
        $blog_id = \WP2FA_Vendor\fs_request_get('blog_id', '');
    ...
    {
        return $this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin();
    }
    /**
    ...
    private function update_multisite_data_after_site_deactivation($context_blog_id = 0)
    {
        ...
        if ($this->is_sync_cron_scheduled() && $context_blog_id == $this->get_sync_cron_blog_id()) {
            $this->schedule_sync_cron(\WP2FA_Vendor\WP_FS__SCRIPT_START_TIME, \true, $context_blog_id);
        }
        if ($this->is_install_sync_scheduled() && $context_blog_id == $this->get_install_sync_cron_blog_id()) {
    ...
    public function _after_site_reactivated_callback($context_blog_id = 0)
    {
        ...
        $this->sync_install(array('is_active' => \true));
        $this->switch_to_blog($current_blog_id);
    }
    #endregion Multisite
    /**
    ...
    {
        return $this->_is_network_active && $network ? \WP2FA_Vendor\network_admin_url($path, $scheme) : \admin_url($path, $scheme);
    }
    /**
    ...
    {
        return $this->_menu->get_slug($page) === \WP2FA_Vendor\fs_request_get('page', '', 'get');
    }
    /**
    ...
    {
        return \WP2FA_Vendor\fs_starts_with(\WP2FA_Vendor\fs_request_get('page', '', 'get'), $this->_menu->get_slug());
    }
    /**
    ...
    function _get_sync_license_url($plugin_id = \false, $add_action_nonce = \true, $params = array())
    {
        ...
        return $this->get_account_url($this->get_unique_affix() . '_sync_license', $params, $add_action_nonce);
    }
    /**
    ...
    function get_account_url($action = \false, $params = array(), $add_action_nonce = \true)
    {
        ...
        self::require_pluggable_essentials();
        return $add_action_nonce && \is_string($action) ? \WP2FA_Vendor\fs_nonce_url($this->_get_admin_page_url('account', $params), $action) : $this->_get_admin_page_url('account', $params);
    }
    /**
     * @author  Vova Feldman (@svovaf)
     * @since   1.2.0
    ...
    {
        return $this->_get_admin_page_url('addons', array('slug' => $slug));
    }
    /**
     * Add-ons URL.
     *
    ...
    {
        return \WP2FA_Vendor\FS_Logger::get_logger(($prefix_slug ? $this->_slug : '') . (!$prefix_slug || empty($id) ? '' : '_') . $id);
    }
    /**
    ...
    {
        return \WP2FA_Vendor\FS_Option_Manager::get_manager(($prefix_slug ? $this->_slug : '') . (!$prefix_slug || empty($id) ? '' : '_') . $id, $load_options);
    }
    /* Security
    		------------------------------------------------------------------------------------------------------------------*/
     ...
     */
    private static function _encrypt_entity(\WP2FA_Vendor\FS_Entity $entity)
    {
        $clone = clone $entity;
     ...
     */
    private static function decrypt_entity(\WP2FA_Vendor\FS_Entity $entity)
    {
        $clone = clone $entity;
        $props = \get_object_vars($entity);
        foreach ($props as $key => $val) {
    ...
    function _activate_account()
    {
        ...
        self::_clean_admin_content_section();
        if (\WP2FA_Vendor\fs_request_is_action('activate') && \WP2FA_Vendor\fs_request_is_post()) {
            //				check_admin_referer( 'activate_' . $this->_plugin->public_key );
            // Verify matching plugin details.
            if ($this->_plugin->id != \WP2FA_Vendor\fs_request_get('plugin_id') || $this->_slug != \WP2FA_Vendor\fs_request_get('plugin_slug')) {
                return;
            }
            $user = new \WP2FA_Vendor\FS_User();
            $user->id = \WP2FA_Vendor\fs_request_get('user_id');
            $user->public_key = \WP2FA_Vendor\fs_request_get('user_public_key');
            $user->secret_key = \WP2FA_Vendor\fs_request_get('user_secret_key');
            $user->email = \WP2FA_Vendor\fs_request_get('user_email');
            $user->first = \WP2FA_Vendor\fs_request_get('user_first');
            $user->last = \WP2FA_Vendor\fs_request_get('user_last');
            $user->is_verified = \WP2FA_Vendor\fs_request_get_bool('user_is_verified');
            $site = new \WP2FA_Vendor\FS_Site();
            $site->id = \WP2FA_Vendor\fs_request_get('install_id');
            $site->public_key = \WP2FA_Vendor\fs_request_get('install_public_key');
            $site->secret_key = \WP2FA_Vendor\fs_request_get('install_secret_key');
            $site->plan_id = \WP2FA_Vendor\fs_request_get('plan_id');
            $plans = array();
            $plans_data = \json_decode(\urldecode(\WP2FA_Vendor\fs_request_get('plans')));
            foreach ($plans_data as $p) {
                $plan = new \WP2FA_Vendor\FS_Plugin_Plan($p);
                if ($site->plan_id == $plan->id) {
                    $plan->title = \WP2FA_Vendor\fs_request_get('plan_title');
                    $plan->name = \WP2FA_Vendor\fs_request_get('plan_name');
                }
                $plans[] = $plan;
            }
            $this->_set_account($user, $site, $plans);
            // Reload the page with the keys.
            \WP2FA_Vendor\fs_redirect($this->_get_admin_page_url());
        }
    }
    /**
     * @author Vova Feldman (@svovaf)
    ...
    static function _get_user_by_email($email)
    {
        ...
        if (\is_array($users)) {
            foreach ($users as $user) {
                if ($email === \trim(\strtolower($user->email))) {
                    return $user;
                }
            }
        }
    ...
    private function _load_account()
    {
        ...
        $users = self::get_all_users();
        $plans = self::get_all_plans($this->_module_type);
        if ($this->_logger->is_on() && \WP2FA_Vendor\is_admin()) {
            $this->_logger->log('users = ' . \var_export($users, \true));
            $this->_logger->log('plans = ' . \var_export($plans, \true));
        }
        $site = \WP2FA_Vendor\fs_is_network_admin() ? $this->get_network_install() : $this->get_install_by_blog_id();
        if (\WP2FA_Vendor\fs_is_network_admin() && $this->is_network_active() && !\is_object($site) && \WP2FA_Vendor\FS_Site::is_valid_id($this->_storage->network_install_blog_id)) {
        ...
        if (\is_object($site) && \is_numeric($site->id) && \is_numeric($site->user_id) && \WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($site->plan_id)) {
            // Load site.
            $this->_site = $site;
            // Load plans.
            ...
            if (!\is_array($this->_plans) || empty($this->_plans)) {
                ...
                for ($i = 0, $len = \count($this->_plans); $i < $len; $i++) {
                    if ($this->_plans[$i] instanceof \WP2FA_Vendor\FS_Plugin_Plan) {
        ...
        $user = null;
        if (\WP2FA_Vendor\fs_is_network_admin() && $this->_is_network_active) {
        ...
        if (\is_object($user)) {
            ...
            if ($this->_site) {
                ...
                if (!\is_object($user) && \WP2FA_Vendor\FS_User::is_valid_id($this->_storage->prev_user_id)) {
                    /**
                ...
                $this->_user = $user instanceof \WP2FA_Vendor\FS_User ? clone $user : null;
            }
        }
        if (\is_object($this->_user)) {
            // Load licenses.
        ...
        if (\true === $this->_storage->require_license_activation && !\WP2FA_Vendor\fs_request_get_bool('require_license', \true)) {
            $this->_storage->require_license_activation = \false;
        }
        if ($this->is_theme()) {
            $this->_register_account_hooks();
    ...
    {
        $site_user_id = \WP2FA_Vendor\FS_Site::is_valid_id($site_user_id) ? $site_user_id : $this->_site->user_id;
        $api = $this->get_api_site_scope();
        $uid = $this->get_anonymous_id();
        $request_path = "/users/{$site_user_id}.json?uid={$uid}";
        $result = $api->get($request_path, \false, \WP2FA_Vendor\WP_FS__TIME_10_MIN_IN_SEC);
        if ($this->is_api_result_entity($result)) {
            $user = new \WP2FA_Vendor\FS_User($result);
            $this->_user = $user;
            $this->_store_user();
            return $user;
        }
        $error_code = \WP2FA_Vendor\FS_Api::get_error_code($result);
        if (\in_array($error_code, array('invalid_unique_id', 'user_cannot_be_recovered'))) {
             ...
             */
            $api->update_cache_expiration($request_path, \WP2FA_Vendor\WP_FS__TIME_WEEK_IN_SEC);
        }
        return $result;
    }
    /**
     ...
     */
    private function _set_account(\WP2FA_Vendor\FS_User $user, \WP2FA_Vendor\FS_Site $site, $plans = \false)
    {
    ...
    {
        return !empty($this->_storage->beta_data) && \true === $this->_storage->beta_data['is_beta'] && \version_compare($this->_storage->beta_data['version'], $this->get_plugin_version(), '>');
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.3.0
    ...
    function is_beta()
    {
        return !empty($this->_storage->beta_data) && \true === $this->_storage->beta_data['is_beta'] && $this->get_plugin_version() === $this->_storage->beta_data['version'];
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.7.4
    ...
    function get_opt_in_params($override_with = array(), $network_level_or_blog_id = null)
    {
        ...
        $return_url = $this->is_anonymous() ? $this->get_account_url($activation_action, array(), \false) : $this->get_after_activation_url('after_connect_url', array('fs_action' => $activation_action));
        $versions = $this->get_versions();
        $params = \array_merge($versions, array('user_firstname' => $current_user->user_firstname, 'user_lastname' => $current_user->user_lastname, 'user_nickname' => $current_user->user_nicename, 'user_email' => $current_user->user_email, 'user_ip' => \WP2FA_Vendor\WP_FS__REMOTE_ADDR, 'plugin_slug' => $this->_slug, 'plugin_id' => $this->get_id(), 'plugin_public_key' => $this->get_public_key(), 'plugin_version' => $this->get_plugin_version(), 'return_url' => \WP2FA_Vendor\fs_nonce_url($return_url, $activation_action), 'account_url' => \WP2FA_Vendor\fs_nonce_url($this->_get_admin_page_url('account', array('fs_action' => 'sync_user')), 'sync_user'), 'is_premium' => $this->is_premium(), 'is_active' => \true, 'is_uninstalled' => \false));
        if ($this->is_addon()) {
            $parent_fs = $this->get_parent_instance();
            $params['parent_plugin_slug'] = $parent_fs->_slug;
            $params['parent_plugin_id'] = $parent_fs->get_id();
        }
        if (\true === $network_level_or_blog_id) {
            if (!isset($override_with['sites'])) {
                $params['sites'] = $this->get_sites_for_network_level_optin();
            }
        } else {
            $site = \is_numeric($network_level_or_blog_id) ? array('blog_id' => $network_level_or_blog_id) : null;
            $site = $this->get_site_info($site);
            $params = \array_merge($params, array('site_uid' => $site['uid'], 'site_url' => $site['url'], 'site_name' => $site['title'], 'language' => $site['language'], 'charset' => $site['charset']));
        }
        if ($this->is_pending_activation() && !empty($this->_storage->pending_license_key)) {
            $params['license_key'] = $this->_storage->pending_license_key;
        }
        if (\WP2FA_Vendor\WP_FS__SKIP_EMAIL_ACTIVATION && $this->has_secret_key()) {
            ...
            $params['ts'] = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
            $params['salt'] = \md5(\uniqid(\rand()));
            $params['secure'] = \md5($params['ts'] . $params['salt'] . $this->get_secret_key());
        }
     ...
     * @use    WP_Error
     */
    function opt_in($email = \false, $first = \false, $last = \false, $license_key = \false, $is_uninstall = \false, $trial_plan_id = \false, $is_disconnected = \false, $is_marketing_allowed = null, $sites = array())
    {
        $this->_logger->entrance();
        if (\false === $email) {
            $current_user = self::_get_current_wp_user();
        ...
        if (empty($license_key)) {
            ...
            if (!$is_uninstall) {
                $fs_user = \WP2FA_Vendor\Freemius::_get_user_by_email($email);
                if (\is_object($fs_user) && !$this->is_pending_activation()) {
                    return $this->install_with_user($fs_user, \false, $trial_plan_id, \true, \true, $sites);
                }
            }
        }
        $user_info = array();
        ...
        if (\is_string($license_key)) {
            ...
            if (\WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($trial_plan_id)) {
        ...
        if (isset($params['license_key'])) {
            ...
            if (\is_object($fs_user)) {
                 ...
                 * @since  1.2.3
                 */
                $params = \array_merge($params, \WP2FA_Vendor\FS_Security::instance()->get_context_params($fs_user, \false, 'install_with_existing_user'));
            }
        }
        ...
        $params['format'] = 'json';
        $request = array('method' => 'POST', 'body' => $params, 'timeout' => \WP2FA_Vendor\WP_FS__DEBUG_SDK ? 60 : 30);
        $url = $this->add_show_pending(\WP2FA_Vendor\WP_FS__ADDRESS . '/action/service/user/install/');
        ...
        if (\is_wp_error($response)) {
            ...
            $result->error = (object) array('type' => $error_type, 'message' => $response->get_error_message(), 'code' => $error_code, 'http' => 402);
         ...
         */
        $decoded = \is_string($response['body']) ? \json_decode($response['body']) : null;
        if (empty($decoded)) {
            return \false;
        }
        if (!$this->is_api_result_object($decoded)) {
            ...
            if (isset($decoded->pending_activation) && $decoded->pending_activation) {
                if ($is_network) {
                     ...
                     * @author Leo Fajardo (@leorw)
                     */
                    $this->_storage->pending_sites_info = array('blog_ids' => $site_ids, 'license_key' => $license_key, 'trial_plan_id' => $trial_plan_id);
                }
                // Pending activation, add message.
                return $this->set_pending_confirmation(isset($decoded->email) ? $decoded->email : \true, \false, $filtered_license_key, !empty($params['trial_plan_id']));
            } else {
                if (isset($decoded->install_secret_key)) {
                    return $this->install_with_new_user($decoded->user_id, $decoded->user_public_key, $decoded->user_secret_key, isset($decoded->is_marketing_allowed) && !\is_null($decoded->is_marketing_allowed) ? $decoded->is_marketing_allowed : null, isset($decoded->is_extensions_tracking_allowed) && !\is_null($decoded->is_extensions_tracking_allowed) ? $decoded->is_extensions_tracking_allowed : null, $decoded->install_id, $decoded->install_public_key, $decoded->install_secret_key, \false);
                } else {
                    if (\is_array($decoded->installs)) {
                        return $this->install_many_with_new_user($decoded->user_id, $decoded->user_public_key, $decoded->user_secret_key, isset($decoded->is_marketing_allowed) && !\is_null($decoded->is_marketing_allowed) ? $decoded->is_marketing_allowed : null, isset($decoded->is_extensions_tracking_allowed) && !\is_null($decoded->is_extensions_tracking_allowed) ? $decoded->is_extensions_tracking_allowed : null, $decoded->installs, \false);
                    }
                }
            }
        }
     ...
     */
    function setup_account(\WP2FA_Vendor\FS_User $user, \WP2FA_Vendor\FS_Site $site, $redirect = \true, $auto_install = \false)
    {
        return $this->setup_network_account($user, array($site), $redirect, $auto_install, \false);
    }
    /**
     * Set user and site identities.
     *
     ...
     * @return string If redirect is `false`, returns the next page the user should be redirected to.
     */
    function setup_network_account(\WP2FA_Vendor\FS_User $user, array $installs, $redirect = \true, $auto_install = \false, $is_network_level_opt_in = \true)
    {
        $first_install = $installs[0];
        $this->_user = $user;
        $this->_site = $first_install;
        $this->_sync_plans();
        if ($this->_storage->handle_gdpr_admin_notice && $this->should_handle_gdpr_admin_notice() && \WP2FA_Vendor\FS_GDPR_Manager::instance()->should_show_opt_in_notice()) {
            /**
             * Clear user lock after an opt-in.
             */
            require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/class-fs-user-lock.php';
            \WP2FA_Vendor\FS_User_Lock::instance()->unlock();
        }
        if (1 < \count($installs)) {
            // Only network level opt-in can have more than one install.
            $is_network_level_opt_in = \true;
        }
        //            $is_network_level_opt_in = self::is_ajax_action_static( 'network_activate', $this->_module_id );
        // If Freemius was OFF before, turn it on.
        $this->turn_on();
        $this->handle_account_connection($installs, !$this->_is_network_active || !$is_network_level_opt_in);
        ...
        if ($this->is_pending_activation() || !$this->has_settings_menu()) {
            ...
            if (!$this->is_paying_or_trial()) {
                $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('%s activation was successfully completed.', 'plugin-x-activation-message'), '<b>' . $this->get_plugin_name() . '</b>'), 'activation_complete');
            }
        }
        if ($this->is_paying_or_trial()) {
            if (!$this->is_premium() || !$this->has_premium_version() || !$this->has_settings_menu()) {
                if ($this->is_paying()) {
                    $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('Your account was successfully activated with the %s plan.', 'activation-with-plan-x-message'), $this->get_plan_title()) . $this->get_complete_upgrade_instructions(), 'plan_upgraded', $this->get_text_x_inline('Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw') . '!');
                } else {
                    $trial_plan = $this->get_trial_plan();
                    $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('Your trial has been successfully started.', 'trial-started-message'), '<i>' . $this->get_plugin_name() . '</i>') . $this->get_complete_upgrade_instructions($trial_plan->title), 'trial_started', $this->get_text_x_inline('Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw') . '!');
                }
            }
            $this->_admin_notices->remove_sticky(array('trial_promotion'));
        }
        $plugin_id = \WP2FA_Vendor\fs_request_get('plugin_id', \false);
        ...
        if (\is_numeric($plugin_id)) {
            ...
            if ($this->is_anonymous() || $plugin_id != $this->_plugin->id) {
                ...
                $next_page = $this->_get_sync_license_url($plugin_id, \true, $extra);
            }
        } else {
            /**
             * @author Vova Feldman (@svovaf)
            ...
            // Reload the page with the keys.
            $next_page = $this->is_anonymous() ? $this->get_account_url(\false, $extra) : $this->get_after_activation_url('after_connect_url', array(), $is_network_level_opt_in);
        }
        if (!empty($next_page) && $redirect) {
            \WP2FA_Vendor\fs_redirect($next_page);
        }
        return $next_page;
    }
    /**
    ...
    function _install_with_new_user()
    {
        ...
        if ($this->is_plugin() && \WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_activate_new') || $this->is_theme()) {
            //				check_admin_referer( $this->_slug . '_activate_new' );
            if (\WP2FA_Vendor\fs_request_has('user_secret_key')) {
                if (\WP2FA_Vendor\fs_is_network_admin() && isset($this->_storage->pending_sites_info)) {
                    $pending_sites_info = $this->_storage->pending_sites_info;
                    $this->install_many_pending_with_user(\WP2FA_Vendor\fs_request_get('user_id'), \WP2FA_Vendor\fs_request_get('user_public_key'), \WP2FA_Vendor\fs_request_get('user_secret_key'), \WP2FA_Vendor\fs_request_get_bool('is_marketing_allowed', null), \WP2FA_Vendor\fs_request_get_bool('is_extensions_tracking_allowed', null), $pending_sites_info['blog_ids'], $pending_sites_info['license_key'], $pending_sites_info['trial_plan_id']);
                } else {
                    $this->install_with_new_user(\WP2FA_Vendor\fs_request_get('user_id'), \WP2FA_Vendor\fs_request_get('user_public_key'), \WP2FA_Vendor\fs_request_get('user_secret_key'), \WP2FA_Vendor\fs_request_get_bool('is_marketing_allowed', null), \WP2FA_Vendor\fs_request_get_bool('is_extensions_tracking_allowed', null), \WP2FA_Vendor\fs_request_get('install_id'), \WP2FA_Vendor\fs_request_get('install_public_key'), \WP2FA_Vendor\fs_request_get('install_secret_key'), \true, \WP2FA_Vendor\fs_request_get_bool('auto_install'));
                }
            } else {
                if (\WP2FA_Vendor\fs_request_has('pending_activation')) {
                    $this->set_pending_confirmation(\WP2FA_Vendor\fs_request_get('user_email'), \true);
                }
            }
        }
    }
    ...
    private function setup_user($id, $public_key, $secret_key)
    {
        ...
        if (\is_object($user)) {
            ...
            $user = new \WP2FA_Vendor\FS_User($user_result);
            $this->_user = $user;
            $this->_store_user();
        }
        return $user;
    ...
    private function install_with_new_user($user_id, $user_public_key, $user_secret_key, $is_marketing_allowed, $is_extensions_tracking_allowed, $install_id, $install_public_key, $install_secret_key, $redirect = \true, $auto_install = \false)
    {
        ...
        $site = new \WP2FA_Vendor\FS_Site($site_result);
        $this->_site = $site;
        if (!\is_null($is_marketing_allowed)) {
            $this->disable_opt_in_notice_and_lock_user();
        }
        $this->update_extensions_tracking_flag($is_extensions_tracking_allowed);
        return $this->setup_account($this->_user, $this->_site, $redirect, $auto_install);
    }
    /**
    ...
    private function install_many_pending_with_user($user_id, $user_public_key, $user_secret_key, $is_marketing_allowed, $is_extensions_tracking_allowed, $site_ids, $license_key = \false, $trial_plan_id = \false, $redirect = \true)
    {
        ...
        $this->install_with_user($user, $license_key, $trial_plan_id, $redirect, \true, $sites);
    }
    /**
    ...
    private function install_many_with_new_user($user_id, $user_public_key, $user_secret_key, $is_marketing_allowed, $is_extensions_tracking_allowed, array $installs, $redirect = \true, $auto_install = \false)
    {
        ...
        foreach ($installs as &$install) {
            $install = new \WP2FA_Vendor\FS_Site($install);
        }
        return $this->setup_network_account($this->_user, $installs, $redirect, $auto_install);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.7.4
    ...
    private function set_pending_confirmation($email = \false, $redirect = \true, $license_key = \false, $is_pending_trial = \false)
    {
        if ($this->_ignore_pending_mode) {
            ...
            $this->_storage->is_pending_activation = \true;
        ...
        $this->_admin_notices->remove_sticky(array('connect_account', 'trial_promotion'));
    ...
    function _install_with_current_user()
    {
        ...
        if (\WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_activate_existing') && \WP2FA_Vendor\fs_request_is_post()) {
            //				check_admin_referer( 'activate_existing_' . $this->_plugin->public_key );
            /**
             * @author Vova Feldman (@svovaf)
             * @since  1.1.9 Add license key if given.
             */
            $license_key = \WP2FA_Vendor\fs_request_get('license_secret_key');
            $this->update_extensions_tracking_flag(\WP2FA_Vendor\fs_request_get_bool('is_extensions_tracking_allowed', null));
    ...
    private function install_with_current_user($license_key = \false, $trial_plan_id = \false, $sites = array(), $redirect = \true)
    {
        ...
        return $this->install_with_user($user, $license_key, $trial_plan_id, $redirect, \true, $sites);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.0.0
     ...
     */
    function install_with_user(\WP2FA_Vendor\FS_User $user, $license_key = \false, $trial_plan_id = \false, $redirect = \true, $setup_account = \true, $sites = array())
    {
        // We have to set the user before getting user scope API handler.
        $this->_user = $user;
        // Install the plugin.
        $result = $this->create_installs_with_user($user, $license_key, $trial_plan_id, $sites, $redirect);
        if (!$this->is_api_result_entity($result) && !$this->is_api_result_object($result, 'installs')) {
            // @todo Handler potential API error of the $result
        }
        if (empty($sites)) {
            $site = new \WP2FA_Vendor\FS_Site($result);
            $this->_site = $site;
            if (!$setup_account) {
                $this->_store_site();
                $this->sync_plan_if_not_exist($site->plan_id);
                if (!empty($license_key) && \WP2FA_Vendor\FS_Plugin_License::is_valid_id($site->license_id)) {
            ...
            return $this->setup_network_account($user, $installs, $redirect);
        }
    }
    /**
     ...
     * @return object|mixed
     */
    private function create_installs_with_user(\WP2FA_Vendor\FS_User $user, $license_key = \false, $trial_plan_id = \false, $sites = array(), $redirect = \false, $silent = \false)
    {
        $extra_install_params = array('uid' => $this->get_anonymous_id(), 'is_disconnected' => \false);
        if (!empty($license_key)) {
            $extra_install_params['license_key'] = $this->apply_filters('license_key', $license_key);
            if ($silent) {
                $extra_install_params['ignore_license_owner'] = \true;
            }
        } else {
            if (\WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($trial_plan_id)) {
        ...
        // Install the plugin.
        $result = $this->get_api_user_scope_by_user($user)->call("/plugins/{$this->get_id()}/installs.json", 'post', $args);
        if (!$this->is_api_result_entity($result) && !$this->is_api_result_object($result, 'installs')) {
            ...
            if (!$silent) {
                $this->_admin_notices->add(\sprintf($this->get_text_inline('Couldn\'t activate %s.', 'could-not-activate-x'), $this->get_plugin_name()) . ' ' . $this->get_text_inline('Please contact us with the following message:', 'contact-us-with-error-message') . ' ' . '<b>' . $result->error->message . '</b>', $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...', 'error');
            }
            if ($redirect) {
                /**
                 ...
                 */
                $this->_user = null;
                \WP2FA_Vendor\fs_redirect($this->get_activation_url(array('error' => $result->error->message)));
            }
        }
        return $result;
    }
     ...
     */
    private function _activate_addon_account(\WP2FA_Vendor\Freemius $parent_fs, $network_level_or_blog_id = null, \WP2FA_Vendor\FS_Plugin_License $bundle_license = null)
    {
        if ($this->is_registered()) {
            // Already activated.
            return;
         ...
         * @since 2.3.0
         */
        $uid_param_to_override = \true === $network_level_or_blog_id ? array() : array('uid' => $this->get_anonymous_id());
        ...
        $result = $parent_fs->get_api_site_scope()->call("/addons/{$this->_plugin->id}/installs.json", 'post', $params);
        if (!$this->is_api_result_object($result, 'installs')) {
            if (\is_object($bundle_license)) {
                /**
                ...
                $error_message = \WP2FA_Vendor\FS_Api::is_api_error_object($result) ? $result->error->message : $this->get_text_inline('An unknown error has occurred.', 'unknown-error');
                $this->_admin_notices->add(\sprintf($this->get_text_inline('Couldn\'t activate %s.', 'could-not-activate-x'), $this->get_plugin_name()) . ' ' . $this->get_text_inline('Please contact us with the following message:', 'contact-us-with-error-message') . ' ' . '<b>' . $error_message . '</b>', $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...', 'error');
            }
            return;
        }
        $addon_installs = $result->installs;
        foreach ($addon_installs as $key => $addon_install) {
            $addon_installs[$key] = new \WP2FA_Vendor\FS_Site($addon_install);
        }
        ...
        // Sync add-on plans.
        $this->_sync_plans();
        $this->handle_account_connection($addon_installs, !\WP2FA_Vendor\fs_is_network_admin());
        ...
        if (!\WP2FA_Vendor\fs_is_network_admin()) {
            // Try to activate premium license.
            $this->_activate_license(\true, $bundle_license);
            if (\is_object($bundle_license)) {
            ...
            if (\is_object($bundle_license)) {
                ...
                $license_id = \WP2FA_Vendor\fs_request_get('license_id');
                if (\is_object($this->_site) && \WP2FA_Vendor\FS_Plugin_License::is_valid_id($license_id) && $license_id == $this->_site->license_id) {
                    // License is already activated.
                    return;
                }
                $premium_license = \WP2FA_Vendor\FS_Plugin_License::is_valid_id($license_id) ? $this->_get_license_by_id($license_id) : $this->_get_available_premium_license();
            }
    ...
    private function handle_account_connection($installs, $is_site_level)
    {
        ...
        if ($is_site_level) {
            ...
            if (!\WP2FA_Vendor\FS_User::is_valid_id($this->_storage->network_user_id) || !\is_object(self::_get_user_by_id($this->_storage->network_user_id))) {
                // Store network user.
            ...
            if (\count($installs) === \count($address_to_blog_map)) {
                // Super admin opted in for all sites in the network.
                $this->_storage->is_network_connected = \true;
            }
            ...
            $this->_site = $first_install;
            $current_blog = \WP2FA_Vendor\get_current_blog_id();
            foreach ($blog_2_install_map as $blog_id => $install) {
                $this->switch_to_blog($blog_id);
     ...
     * @param Freemius $parent_fs
     */
    private function activate_parent_account(\WP2FA_Vendor\Freemius $parent_fs)
    {
        if (!$this->is_addon()) {
        ...
        $parent_install = $this->get_api_user_scope()->call("/plugins/{$parent_fs->_plugin->id}/installs.json", 'post', $parent_fs->get_install_data_for_api(array('uid' => $parent_fs->get_anonymous_id()), \false, \false));
        if (isset($parent_install->error)) {
            $this->_admin_notices->add(\sprintf($this->get_text_inline('Couldn\'t activate %s.', 'could-not-activate-x'), $this->get_plugin_name()) . ' ' . $this->get_text_inline('Please contact us with the following message:', 'contact-us-with-error-message') . ' ' . '<b>' . $parent_install->error->message . '</b>', $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...', 'error');
            return;
        }
        ...
        // be able to invoke API calls.
        $parent_fs->_site = new \WP2FA_Vendor\FS_Site($parent_install);
    ...
    function _prepare_admin_menu()
    {
         ...
         * @since  2.0.0
         */
        $should_hide_site_admin_settings = $this->_is_network_active && !\WP2FA_Vendor\fs_is_network_admin() && !$this->is_delegated_connection() && !$this->is_anonymous() && !$this->is_registered();
        ...
        if (!$this->has_api_connectivity() && !$this->is_enable_anonymous() || $should_hide_site_admin_settings) {
            $this->_menu->remove_menu_item($should_hide_site_admin_settings);
        } else {
            $this->do_action(\WP2FA_Vendor\fs_is_network_admin() ? 'before_network_admin_menu_init' : 'before_admin_menu_init');
    ...
    private function add_menu_action()
    {
        if ($this->is_activation_mode()) {
            if ($this->show_opt_in_on_setting_page()) {
                 ...
                 */
                if (\WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_activate_existing')) {
                    \add_action('load-themes.php', array(&$this, '_install_with_current_user'));
                } else {
                    if (\WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_activate_new') || \WP2FA_Vendor\fs_request_get_bool('pending_activation')) {
            ...
            if (!$this->is_registered()) {
                ...
                if (\WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_activate_new')) {
                    $this->_install_with_new_user();
                }
            } else {
                if (\WP2FA_Vendor\fs_request_is_action('sync_user') && (!$this->has_settings_menu() || $this->show_opt_in_on_themes_page())) {
    ...
    {
        $this->_logger->entrance();
        $page = \WP2FA_Vendor\fs_request_get('page');
            ...
            foreach ($items as $item) {
                if (isset($item['url'])) {
                    if ($page === $this->_menu->get_slug(\strtolower($item['menu_slug']))) {
                        ...
                        \WP2FA_Vendor\fs_redirect($item['url']);
                    }
                }
            }
        }
    ...
    private function override_plugin_menu_with_activation()
    {
        ...
        if (!$this->has_settings_menu()) {
            ...
            $hook = \WP2FA_Vendor\FS_Admin_Menu_Manager::add_subpage(null, $this->get_plugin_name(), $this->get_plugin_name(), 'manage_options', $this->_slug, array(&$this, '_connect_page_render'));
        } else {
            if ($this->_menu->is_top_level()) {
                ...
                if (\false === $hook) {
                    ...
                    $hook = \WP2FA_Vendor\FS_Admin_Menu_Manager::add_page($this->get_plugin_name(), $this->get_plugin_name(), 'manage_options', $this->_menu->get_slug(), array(&$this, '_connect_page_render'));
                }
            } else {
    ...
    private function add_network_menu_when_missing()
    {
        ...
        if (!\WP2FA_Vendor\WP_FS__SHOW_NETWORK_EVEN_WHEN_DELEGATED) {
            if ($this->is_network_delegated_connection()) {
        ...
        if (!$this->_menu->has_menu() || $this->_menu->is_top_level()) {
            if ($this->_menu->has_menu() || !$this->is_addon() || $this->is_activation_mode()) {
                $this->_dynamically_added_top_level_page_hook_name = $this->_menu->add_page_and_update($this->get_plugin_name(), $this->get_plugin_name(), 'manage_options', $this->_menu->has_menu() ? $this->_menu->get_slug() : $this->_slug);
            }
        } else {
            $this->_menu->add_subpage_and_update($this->_menu->get_parent_slug(), $this->get_plugin_name(), $this->get_plugin_name(), 'manage_options', $this->_menu->get_slug());
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    function get_pricing_cta_label()
    {
        ...
        if ($this->is_in_trial_promotion() && !$this->is_paying_or_trial()) {
    ...
    {
        return $this->has_paid_plan() && $this->is_page_visible('pricing') && (!$this->is_paying() || !$this->is_single_plan(\true));
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.3.0
    ...
    private function should_add_submenu_or_action_links($is_activation_mode)
    {
        ...
        if (\WP2FA_Vendor\fs_is_network_admin()) {
             ...
             */
            return $this->_is_network_active && (\WP2FA_Vendor\WP_FS__SHOW_NETWORK_EVEN_WHEN_DELEGATED || !$this->is_network_delegated_connection());
        }
    ...
    private function add_submenu_items()
    {
        ...
        if ($add_submenu_items) {
            if ($this->has_affiliate_program()) {
                ...
                $this->add_submenu_item($this->get_text_inline('Affiliation', 'affiliation'), array(&$this, '_affiliation_page_render'), $this->get_plugin_name() . ' &ndash; ' . $this->get_text_inline('Affiliation', 'affiliation'), 'manage_options', 'affiliation', 'Freemius::_clean_admin_content_section', \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $this->is_submenu_item_visible('affiliation'));
            }
        }
        if ($add_submenu_items || $is_activation_mode && $this->is_only_premium() && $this->is_admin_page('account') && \WP2FA_Vendor\fs_request_is_action($this->get_unique_affix() . '_sync_license')) {
            if (!\WP2FA_Vendor\WP_FS__DEMO_MODE && $this->is_registered()) {
                $show_account = $this->is_submenu_item_visible('account') && (!$this->is_free_wp_org_theme() || $this->has_paid_plan());
                // Add user account page.
                $this->add_submenu_item($this->get_text_inline('Account', 'account'), array(&$this, '_account_page_render'), $this->get_plugin_name() . ' &ndash; ' . $this->get_text_inline('Account', 'account'), 'manage_options', 'account', array(&$this, '_account_page_load'), \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $add_submenu_items && $show_account);
            }
        }
        if ($add_submenu_items) {
            if (!\WP2FA_Vendor\WP_FS__DEMO_MODE && !$this->is_whitelabeled()) {
                // Add contact page.
                $this->add_submenu_item($this->get_text_inline('Contact Us', 'contact-us'), array(&$this, '_contact_page_render'), $this->get_plugin_name() . ' &ndash; ' . $this->get_text_inline('Contact Us', 'contact-us'), 'manage_options', 'contact', 'Freemius::_clean_admin_content_section', \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $this->is_submenu_item_visible('contact'));
            }
            if ($this->has_addons()) {
                $this->add_submenu_item($this->get_text_inline('Add-Ons', 'add-ons'), array(&$this, '_addons_page_render'), $this->get_plugin_name() . ' &ndash; ' . $this->get_text_inline('Add-Ons', 'add-ons'), 'manage_options', 'addons', array(&$this, '_addons_page_load'), \WP2FA_Vendor\WP_FS__LOWEST_PRIORITY - 1, $this->is_submenu_item_visible('addons'));
            }
        }
        if ($add_submenu_items || $is_activation_mode && $this->is_only_premium() && $this->is_admin_page('pricing')) {
            if (!\WP2FA_Vendor\WP_FS__DEMO_MODE && !$this->is_whitelabeled()) {
                $show_pricing = $this->is_submenu_item_visible('pricing') && $this->is_pricing_page_visible();
                ...
                $this->add_submenu_item($pricing_cta_text . '&nbsp;&nbsp;' . (\WP2FA_Vendor\is_rtl() ? $this->get_text_x_inline('&#x2190;', 'ASCII arrow left icon', 'symbol_arrow-left') : $this->get_text_x_inline('&#x27a4;', 'ASCII arrow right icon', 'symbol_arrow-right')), array(&$this, '_pricing_page_render'), $this->get_plugin_name() . ' &ndash; ' . $this->get_text_x_inline('Pricing', 'noun', 'pricing'), 'manage_options', 'pricing', 'Freemius::_clean_admin_content_section', \WP2FA_Vendor\WP_FS__LOWEST_PRIORITY, $add_submenu_items && $show_pricing, $pricing_class);
            }
        }
        if (!$is_activation_mode || \true !== $this->_storage->require_license_activation) {
            /**
            ...
            if (0 < \count($this->_menu_items)) {
                if (!$this->_menu->is_top_level()) {
                    \WP2FA_Vendor\fs_enqueue_local_style('fs_common', '/admin/common.css');
    ...
    {
        $item_template = $this->_menu->is_top_level() ? '<span class="fs-submenu-item %s %s %s">%s</span>' : '<span class="fs-submenu-item fs-sub %s %s %s">%s</span>';
        $top_level_menu_capability = $this->get_top_level_menu_capability();
        \ksort($this->_menu_items);
        $is_first_submenu_item = \true;
        foreach ($this->_menu_items as $priority => $items) {
            foreach ($items as $item) {
                $capability = !empty($item['capability']) ? $item['capability'] : $top_level_menu_capability;
                $menu_item = \sprintf($item_template, $this->get_unique_affix(), $item['menu_slug'], !empty($item['class']) ? $item['class'] : '', $item['menu_title']);
                $top_level_menu_slug = $this->get_top_level_menu_slug();
                $menu_slug = $this->_menu->get_slug($item['menu_slug']);
                if (!isset($item['url'])) {
                    $hook = \WP2FA_Vendor\FS_Admin_Menu_Manager::add_subpage($item['show_submenu'] ? $top_level_menu_slug : null, $item['page_title'], $menu_item, $capability, $menu_slug, $item['render_function']);
                    if (\false !== $item['before_render_function']) {
                        \add_action("load-{$hook}", $item['before_render_function']);
                    }
                } else {
                    \WP2FA_Vendor\FS_Admin_Menu_Manager::add_subpage($item['show_submenu'] ? $top_level_menu_slug : null, $item['page_title'], $menu_item, $capability, $menu_slug, array($this, ''));
                }
                if ($item['show_submenu'] && $is_first_submenu_item) {
                    if ($this->_is_network_active && !empty($this->_dynamically_added_top_level_page_hook_name)) {
                         ...
                         * @author Leo Fajardo (@leorw)
                         */
                        \WP2FA_Vendor\remove_submenu_page($top_level_menu_slug, $top_level_menu_slug);
                    }
    ...
    function _add_default_submenu_items()
    {
        ...
        if (!$this->is_activation_mode() && ($this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin() || !$this->_is_network_active && \WP2FA_Vendor\is_admin())) {
            $this->add_submenu_link_item($this->apply_filters('support_forum_submenu', $this->get_text_inline('Support Forum', 'support-forum')), $this->get_support_forum_url(), 'wp-support-forum', null, 50, $this->is_submenu_item_visible('support'));
        }
    }
    /**
     * @author Vova Feldman (@svovaf)
     ...
     */
    function add_submenu_item($menu_title, $render_function, $page_title = \false, $capability = 'manage_options', $menu_slug = \false, $before_render_function = \false, $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $show_submenu = \true, $class = '')
    {
        $this->_logger->entrance('Title = ' . $menu_title);
        if ($this->is_addon()) {
            $parent_fs = $this->get_parent_instance();
            if (\is_object($parent_fs)) {
                $parent_fs->add_submenu_item($menu_title, $render_function, $page_title, $capability, $menu_slug, $before_render_function, $priority, $show_submenu, $class);
                return;
            }
        }
        ...
        $this->_menu_items[$priority][] = array('page_title' => \is_string($page_title) ? $page_title : $menu_title, 'menu_title' => $menu_title, 'capability' => $capability, 'menu_slug' => \is_string($menu_slug) ? $menu_slug : \strtolower($menu_title), 'render_function' => $render_function, 'before_render_function' => $before_render_function, 'show_submenu' => $show_submenu, 'class' => $class);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.1
     ...
     * @param bool   $show_submenu
     */
    function add_submenu_link_item($menu_title, $url, $menu_slug = \false, $capability = 'read', $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $show_submenu = \true)
    {
        $this->_logger->entrance('Title = ' . $menu_title . '; Url = ' . $url);
        if ($this->is_addon()) {
            $parent_fs = $this->get_parent_instance();
            if (\is_object($parent_fs)) {
                $parent_fs->add_submenu_link_item($menu_title, $url, $menu_slug, $capability, $priority, $show_submenu);
                return;
            }
        }
        if (!isset($this->_menu_items[$priority])) {
        ...
        $this->_menu_items[$priority][] = array('menu_title' => $menu_title, 'capability' => $capability, 'menu_slug' => \is_string($menu_slug) ? $menu_slug : \strtolower($menu_title), 'url' => $url, 'page_title' => $menu_title, 'render_function' => 'fs_dummy', 'before_render_function' => '', 'show_submenu' => $show_submenu);
    }
    #endregion ------------------------------------------------------------------
    #--------------------------------------------------------------------------------
    ...
    static function get_module_unique_affix($slug, $is_plugin = \true)
    {
        $affix = $slug;
        if (!$is_plugin) {
            $affix .= '-' . \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME;
        }
        return $affix;
    }
    /**
    ...
    function do_action($tag, $arg = '')
    {
        ...
        $args = \func_get_args();
        \call_user_func_array('do_action', \array_merge(array($this->get_action_tag($tag)), \array_slice($args, 1)));
    }
    /**
     ...
     * @uses   add_action()
     */
    function add_action($tag, $function_to_add, $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $accepted_args = 1)
    {
        $this->_logger->entrance($tag);
     ...
     */
    function add_ajax_action($tag, $function_to_add, $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY)
    {
        $this->_logger->entrance($tag);
        return self::add_ajax_action_static($tag, $function_to_add, $priority, $this->_module_id);
    }
    /**
     * Add AJAX action.
     *
     ...
     *
     */
    static function add_ajax_action_static($tag, $function_to_add, $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $module_id = null)
    {
        self::$_static_logger->entrance($tag);
        if (!self::is_ajax_action_static($tag, $module_id)) {
            return \false;
        }
        \add_action('wp_ajax_' . self::get_ajax_action_static($tag, $module_id), $function_to_add, $priority, 0);
    ...
    {
        \WP2FA_Vendor\wp_send_json($response);
    }
    /**
    ...
    {
        \WP2FA_Vendor\wp_send_json_success($data);
    }
    /**
    ...
    static function shoot_ajax_failure($error = '')
    {
        ...
        \WP2FA_Vendor\wp_send_json($result);
    }
    /**
     ...
     * @uses   add_filter()
     */
    function add_filter($tag, $function_to_add, $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $accepted_args = 1)
    {
        $this->_logger->entrance($tag);
    ...
    {
        $this->_logger->entrance($tag);
        return \WP2FA_Vendor\has_filter($this->get_action_tag($tag), $function_to_check);
    }
    #endregion
    /**
    ...
    {
        \WP2FA_Vendor\fs_override_i18n($key_value, $this->_slug);
    }
    /* Account Page
    		------------------------------------------------------------------------------------------------------------------*/
    /**
     ...
     */
    private function _store_site($store = \true, $network_level_or_blog_id = null, \WP2FA_Vendor\FS_Site $site = null)
    {
        $this->_logger->entrance();
        if (\is_null($site)) {
            $site = $this->_site;
        ...
        if (\is_object($this->_user) && $this->_user->id != $site->user_id) {
            ...
            if (empty($prev_stored_user_id) && \is_object($this->_user) && $this->_user->id != $site->user_id) {
                /**
    ...
    private function _store_licenses($store = \true, $module_id = \false, $licenses = array())
    {
        $this->_logger->entrance();
        $all_licenses = self::get_all_licenses();
        if (!\WP2FA_Vendor\FS_Plugin::is_valid_id($module_id)) {
            $module_id = $this->_module_id;
            $user_licenses = \is_array($this->_licenses) ? $this->_licenses : array();
    ...
    {
        $this->_logger->entrance();
        if ($update instanceof \WP2FA_Vendor\FS_Plugin_Tag) {
            $update->updated = \time();
        }
        if (!\is_numeric($plugin_id)) {
    ...
    {
        $foreign_licenses = array('ids' => array(), 'license_keys' => array());
        ...
        foreach ($all_licenses as $license) {
            ...
            if ($include_parent_licenses && \is_object($this->_license) && \WP2FA_Vendor\FS_Plugin_License::is_valid_id($this->_license->parent_license_id) && !isset($parent_license_ids_map[$this->_license->parent_license_id])) {
                /**
                ...
                $foreign_licenses['license_keys'][] = $license->secret_key;
                $parent_license_ids_map[$this->_license->parent_license_id] = \true;
            }
        }
    ...
    {
        $user_licenses_endpoint = '/licenses.json?type=active' . (\WP2FA_Vendor\FS_Plugin::is_valid_id($this->get_bundle_id()) ? '&is_enriched=true' : '');
        $foreign_licenses = $this->get_foreign_licenses_info(self::get_all_licenses($this->_module_id), null, \true);
        if (!empty($foreign_licenses)) {
            $foreign_licenses = array(
                // Prefix with `+` to tell the server to include foreign licenses in the licenses collection.
                'ids' => \urlencode('+') . \implode(',', $foreign_licenses['ids']),
    ...
    private function fetch_valid_user_licenses()
    {
        ...
        if (!$this->is_api_result_object($result, 'licenses') || !\is_array($result->licenses)) {
            return array();
        }
    ...
    function get_updated_account_addons()
    {
        ...
        $user_licenses = $this->is_registered() ? $this->fetch_valid_user_licenses() : array();
        if (empty($user_licenses)) {
    ...
    private function _fetch_site_license_subscription($license_id = \false)
    {
        ...
        if (!\is_numeric($license_id)) {
            $license_id = \WP2FA_Vendor\FS_Plugin_License::is_valid_id($this->_license->parent_license_id) ? $this->_license->parent_license_id : $this->_license->id;
        }
        $result = $api->get("/licenses/{$license_id}/subscriptions.json", \true);
        return !isset($result->error) ? \is_array($result->subscriptions) && 0 < \count($result->subscriptions) ? new \WP2FA_Vendor\FS_Subscription($result->subscriptions[0]) : \false : $result;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.4
    ...
    private function _fetch_site_plan($plan_id = \false)
    {
        ...
        $plan = $api->get("/plans/{$plan_id}.json", \true);
        return !isset($plan->error) ? new \WP2FA_Vendor\FS_Plugin_Plan($plan) : $plan;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.5
    ...
    private function _fetch_plugin_plans()
    {
         ...
         */
        $result = $api->get($this->add_show_pending("/plugins/{$this->_module_id}/plans.json"), \true);
        if ($this->is_api_result_object($result, 'plans') && \is_array($result->plans)) {
            for ($i = 0, $len = \count($result->plans); $i < $len; $i++) {
                $result->plans[$i] = new \WP2FA_Vendor\FS_Plugin_Plan($result->plans[$i]);
            }
            $result = $result->plans;
        }
        return $result;
    ...
    private function fetch_plan_by_id($plan_id)
    {
        $this->_logger->entrance();
        $api = $this->get_current_or_network_user_api_scope();
        $result = $api->get("/plugins/{$this->_module_id}/plans/{$plan_id}.json", \true);
        return $this->is_api_result_entity($result) ? new \WP2FA_Vendor\FS_Plugin_Plan($result) : $result;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.5
     ...
     * @return FS_Plugin_License[]|object
     */
    private function _fetch_licenses($plugin_id = \false, $site_license_id = \false, $foreign_licenses = array(), $blog_id = null)
    {
        $this->_logger->entrance();
        $api = $this->get_api_user_scope();
        if (!\is_numeric($plugin_id)) {
        ...
        $result = $api->get($user_licenses_endpoint, \true);
        $is_site_license_synced = \false;
        $api_errors = array();
        if ($this->is_api_result_object($result, 'licenses') && \is_array($result->licenses)) {
            for ($i = 0, $len = \count($result->licenses); $i < $len; $i++) {
                $result->licenses[$i] = new \WP2FA_Vendor\FS_Plugin_License($result->licenses[$i]);
        ...
        if (!$is_site_license_synced) {
            ...
            if (\is_numeric($site_license_id)) {
                ...
                if ($this->is_api_result_object($api_result, 'licenses') && \is_array($api_result->licenses)) {
                    $licenses = $api_result->licenses;
                    if (!empty($licenses)) {
                        $result[] = new \WP2FA_Vendor\FS_Plugin_License($licenses[0]);
                    }
                } else {
                    $api_errors[] = $api_result;
                }
            } else {
                if (\is_object($this->_license) && $this->_license->plugin_id == $plugin_id) {
                    $is_license_in_result = \false;
                    if (!empty($result)) {
                        foreach ($result as $license) {
                            if ($license->id == $this->_license->id) {
                                $is_license_in_result = \true;
                                break;
                            }
                        }
                    }
                    if (!$is_license_in_result) {
                        ...
                        $license = $api->get("/licenses/{$this->_license->id}.json?license_key=" . \urlencode($this->_license->secret_key) . '&is_enriched=true');
                        if ($this->is_api_result_entity($license)) {
                            $result[] = new \WP2FA_Vendor\FS_Plugin_License($license);
                        } else {
                            $api_errors[] = $license;
                        }
                    }
    ...
    private function fetch_license_by_key($license_id, $license_key)
    {
        ...
        return $this->is_api_result_entity($result) ? new \WP2FA_Vendor\FS_Plugin_License($result) : $result;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.2.0
    ...
    function _fetch_payments($plugin_id = \false, $flush = \false)
    {
        ...
        $include_bundles = \is_object($this->_plugin) && \WP2FA_Vendor\FS_Plugin::is_valid_id($this->_plugin->bundle_id);
        $result = $api->get("/plugins/{$plugin_id}/payments.json?include_addons=true" . ($include_bundles ? '&include_bundles=true' : ''), $flush);
        if (!isset($result->error)) {
            for ($i = 0, $len = \count($result->payments); $i < $len; $i++) {
                $result->payments[$i] = new \WP2FA_Vendor\FS_Payment($result->payments[$i]);
            }
    ...
    {
        require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/entities/class-fs-billing.php';
        $billing = $this->get_api_user_scope()->get('billing.json', $flush);
        if ($this->is_api_result_entity($billing)) {
            $billing = new \WP2FA_Vendor\FS_Billing($billing);
        }
        return $billing;
    }
    /**
    ...
    private function _update_licenses($licenses, $module_id)
    {
        ...
        if (\is_array($licenses)) {
            for ($i = 0, $len = \count($licenses); $i < $len; $i++) {
                $licenses[$i]->updated = \time();
            }
        }
        $this->_store_licenses(\true, $module_id, $licenses);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.4
     ...
     */
    private function _fetch_newer_version($plugin_id = \false, $flush = \true, $expiration = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, $newer_than = \false)
    {
        ...
        $has_new_version = $this->_is_addon_id($plugin_id) && !$this->is_addon_activated($plugin_id) || \version_compare($plugin_version, $latest_tag->version, '<');
        $this->_logger->departure($has_new_version ? 'Found newer plugin version ' . $latest_tag->version : 'No new version');
        $is_latest_version_beta = 'beta' === $latest_tag->release_mode;
        $this->_storage->beta_data = array('is_beta' => $is_latest_version_beta, 'version' => $latest_tag->version);
     ...
     */
    function get_update($plugin_id = \false, $flush = \true, $expiration = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, $newer_than = \false)
    {
        $this->_logger->entrance();
        if (!\is_numeric($plugin_id)) {
            $plugin_id = $this->_plugin->id;
        }
        $this->check_updates(\true, $plugin_id, $flush, $expiration, $newer_than);
    ...
    {
        return \is_object($this->_license) && \is_numeric($this->_license->id) && !$this->_license->is_expired();
    }
    /**
    ...
    function is_data_debug_mode()
    {
        ...
        $fs = $this->is_addon() ? $this->get_parent_instance() : $this;
        if ($fs->is_network_active() && \WP2FA_Vendor\fs_is_network_admin()) {
    ...
    {
        if (!$this->is_whitelabeled(\true)) {
            return;
        }
        $license_or_user_key = \WP2FA_Vendor\fs_request_get('license_or_user_key');
        $transient_value = !empty($license_or_user_key) ? 'true' : 'false';
        if ('true' === $transient_value) {
            $stored_key = $this->_storage->get(!\WP2FA_Vendor\FS_User::is_valid_id($this->_storage->last_license_user_id) ? 'last_license_key' : 'last_license_user_key');
            if (\md5($license_or_user_key) !== $stored_key) {
                $this->shoot_ajax_failure(\sprintf('%s... %s', $this->get_text_x_inline('Oops', 'exclamation', 'oops'), $this->get_text_inline('seems like the key you entered doesn\'t match our records.', 'developer-or-license-not-found')));
            }
        }
        if ($this->is_network_active() && \WP2FA_Vendor\fs_is_network_admin()) {
            \set_site_transient("fs_{$this->get_id()}_data_debug_mode", $transient_value, \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC / 24);
        } else {
            \set_transient("fs_{$this->get_id()}_data_debug_mode", $transient_value, \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC / 24);
        }
        if ('true' === $transient_value) {
            $this->_admin_notices->add_sticky($this->get_text_inline('Debug mode was successfully enabled and will be automatically disabled in 60 min. You can also disable it earlier by clicking the "Stop Debug" link.', 'data_debug_mode_enabled'), 'data_debug_mode_enabled');
        }
        $this->shoot_ajax_success();
    }
    /**
    ...
    {
        return \is_object($license) && \WP2FA_Vendor\FS_Plugin_License::is_valid_id($license->id) && $license->is_active() && (!$check_expiration || $license->is_valid());
    }
    /**
    ...
    function is_activated_with_bundle_license()
    {
        ...
        return \WP2FA_Vendor\FS_Plugin_License::is_valid_id($this->_license->parent_license_id);
    }
    /**
    ...
    function is_user_admin()
    {
         ...
         * @since  2.0.0
         */
        if ($this->_is_network_active && (\WP2FA_Vendor\fs_is_network_admin() || !$this->is_delegated_connection())) {
            return \WP2FA_Vendor\is_super_admin();
        }
        return $this->is_plugin() && \WP2FA_Vendor\current_user_can(\is_multisite() ? 'manage_options' : 'activate_plugins') || $this->is_theme() && \WP2FA_Vendor\current_user_can('switch_themes');
    }
    /**
     * Sync site's plan.
     *
    ...
    {
        $this->_logger->entrance();
        $plugin_id = \WP2FA_Vendor\fs_request_get('plugin_id', $this->get_id());
        $is_addon_sync = !$this->_plugin->is_addon() && $plugin_id != $this->get_id();
        if ($is_addon_sync) {
            $this->_sync_addon_license($plugin_id, $background);
        } else {
            $this->_sync_plugin_license($background, \true, $is_context_single_site, $current_blog_id);
        }
    ...
    private function _sync_addon_license($addon_id, $background)
    {
        ...
        if ($this->is_addon_activated($addon_id)) {
            ...
            if ($fs_addon->is_network_active() || self::is_cron() || !\WP2FA_Vendor\fs_is_network_admin()) {
        ...
        if ($this->is_array_instanceof($licenses, 'FS_Plugin_License')) {
            ...
            if (!$this->is_addon_installed($addon->id) && \WP2FA_Vendor\FS_License_Manager::has_premium_license($licenses)) {
     ...
     */
    private function _sync_plugin_license($background = \false, $send_installs_update = \true, $is_context_single_site = \false, $current_blog_id = null)
    {
        $this->_logger->entrance();
        $plan_change = 'none';
        $is_site_level_sync = $is_context_single_site || \WP2FA_Vendor\fs_is_blog_admin() || !$this->_is_network_active;
        if (!$send_installs_update) {
            ...
            if ($is_site_level_sync) {
                ...
                $result = $this->send_install_update(array(), \true);
                $is_valid = $this->is_api_result_entity($result);
            } else {
                $result = $this->send_installs_update(array(), \true);
            ...
            if ($is_site_level_sync) {
                ...
                foreach ($result->installs as $install) {
                    if ($install->id == $this->_site->id) {
                        $site = new \WP2FA_Vendor\FS_Site($install);
                    } else {
                        $address = \trailingslashit(\WP2FA_Vendor\fs_strip_url_protocol($install->url));
                        $blog_id = $address_to_blog_map[$address];
                        $this->_store_site(\true, $blog_id, new \WP2FA_Vendor\FS_Site($install));
                    }
                }
            }
            // Sync plans.
        ...
        if (!$this->has_paid_plan()) {
            $this->_site = $site;
            $this->_store_site(\true, $is_site_level_sync ? null : $this->get_network_install_blog_id());
        } else {
            $context_blog_id = 0;
            if ($is_context_single_site) {
                $context_blog_id = \WP2FA_Vendor\get_current_blog_id();
             ...
             */
            $this->_sync_licenses($site->license_id, $is_context_single_site ? $context_blog_id : null);
            if ($is_context_single_site) {
                $this->switch_to_blog($context_blog_id);
            }
            // Check if plan / license changed.
            if ($site->plan_id != $this->_site->plan_id || $site->trial_plan_id != $this->_site->trial_plan_id || $site->trial_ends != $this->_site->trial_ends || $site->license_id != $this->_site->license_id) {
                    ...
                    if ($this->_site->is_trial() && !$site->is_trial() && !\is_numeric($site->license_id)) {
                        ...
                        if ($is_free && \is_null($new_license) && $this->has_any_license() && $this->_license->is_cancelled) {
                            ...
                            if ($is_free && (!\is_object($new_license) || $new_license->is_expired())) {
                                 ...
                                 */
                                $this->_update_site_license($new_license);
                                if (!$is_context_single_site && \WP2FA_Vendor\fs_is_network_admin() && $this->_is_network_active && $new_license->quota > 1 && \WP2FA_Vendor\get_blog_count() > 1) {
                                    // See if license can activated on all sites.
                                    if (!$this->try_activate_license_on_network($this->_user, $new_license)) {
                                        if (!\WP2FA_Vendor\fs_request_get_bool('auto_install')) {
                                ...
                                $this->_store_licenses();
                                $plan_change = $is_free ? $this->is_only_premium() ? 'activated' : 'upgraded' : (\is_object($new_license) ? 'changed' : 'downgraded');
                            }
                        }
                    }
                }
                ...
                if (!\is_object($this->_license)) {
                    $this->maybe_update_whitelabel_flag(\WP2FA_Vendor\FS_Plugin_License::is_valid_id($site->license_id) ? $this->get_license_by_id($site->license_id) : null);
                } else {
                    ...
                    if ($this->_license->is_expired()) {
                        if (!$this->has_features_enabled_license()) {
                            ...
                            if (\time() - 14 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC >= $last_time_expired_license_notice_was_shown) {
                                /**
            ...
            if (!$this->is_addon() && $this->_site->is_beta() !== $site->is_beta) {
                // Beta flag updated.
                $this->_site = $site;
                $this->_store_site(\true, $is_site_level_sync ? null : $this->get_network_install_blog_id());
            }
        ...
        if ($this->apply_filters('has_paid_plan_account', $this->has_paid_plan())) {
            switch ($plan_change) {
                ...
                case 'activated':
                    ...
                    $this->_admin_notices->remove_sticky(array('trial_started', 'trial_promotion', 'trial_expired', 'activation_complete', 'license_expired'));
                    break;
                case 'changed':
                    $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('Your plan was successfully changed to %s.', 'plan-changed-to-x-message'), $this->get_plan_title()), 'plan_changed');
                    $this->_admin_notices->remove_sticky(array('trial_started', 'trial_promotion', 'trial_expired', 'activation_complete'));
                    break;
                case 'downgraded':
                    $this->_admin_notices->add_sticky($this->has_free_plan() ? \sprintf($this->get_text_inline('Your license has expired. You can still continue using the free %s forever.', 'license-expired-blocking-message'), $this->_module_type) : \sprintf($this->get_text_inline('Your license has expired. %1$sUpgrade now%2$s to continue using the %3$s without interruptions.', 'license-expired-blocking-message_premium-only'), \sprintf('<a href="%s">', $this->pricing_url()), '</a>', $this->get_module_label(\true)), 'license_expired', $hmm_text);
                    $this->_admin_notices->remove_sticky('plan_upgraded');
                    break;
                case 'cancelled':
                    $this->_admin_notices->add($this->get_text_inline('Your license has been cancelled. If you think it\'s a mistake, please contact support.', 'license-cancelled') . ' ' . \sprintf('<a href="%s">%s</a>', $this->contact_url('bug'), $this->get_text_inline('Please contact us here', 'contact-us-here')), $hmm_text, 'error');
                    $this->_admin_notices->remove_sticky('plan_upgraded');
                    break;
                case 'expired':
                    $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('Your license has expired. You can still continue using all the %s features, but you\'ll need to renew your license to continue getting updates and support.', 'license-expired-non-blocking-message'), $this->get_plan()->title), 'license_expired', $hmm_text);
                    $this->_storage->expired_license_notice_shown = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
                    $this->_admin_notices->remove_sticky('plan_upgraded');
                    break;
                case 'trial_started':
    ...
    public function _open_license_activation_dialog_box()
    {
        ...
        \WP2FA_Vendor\fs_require_once_template('js/open-license-activation.php', $vars);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.5
    ...
    protected function _activate_license($background = \false, $premium_license = null)
    {
        ...
        if (!\is_object($this->_site)) {
            ...
            if (\is_object($user)) {
                ...
                $this->opt_in(\false, \false, \false, $premium_license->secret_key);
                return;
            }
        }
        /**
        ...
        if (!$this->is_api_result_entity($license)) {
            if (!$background) {
                $this->_admin_notices->add(\sprintf('%s %s', $this->get_text_inline('It looks like the license could not be activated.', 'license-activation-failed-message'), \is_object($license) && isset($license->error) ? $license->error->message : \sprintf('%s<br><code>%s</code>', $this->get_text_inline('Error received from the server:', 'server-error-message'), \var_export($license, \true))), $this->get_text_x_inline('Hmm', 'something somebody says when they are thinking about what you have just said.', 'hmm') . '...', 'error');
            }
            return;
        }
        $premium_license = new \WP2FA_Vendor\FS_Plugin_License($license);
        // Updated site plan.
        $site = $this->get_api_site_scope()->get('/', \true);
        if ($this->is_api_result_entity($site)) {
            $this->_site = new \WP2FA_Vendor\FS_Site($site);
        }
        $this->_update_site_license($premium_license);
        ...
        if (!$background) {
            $this->_admin_notices->add_sticky($this->get_text_inline('Your license was successfully activated.', 'license-activated-message') . $this->get_complete_upgrade_instructions(), 'license_activated', $this->get_text_x_inline('Yee-haw', 'interjection expressing joy or exuberance', 'yee-haw') . '!');
        }
    ...
    protected function _deactivate_license($show_notice = \true)
    {
        ...
        $hmm_text = $this->get_text_x_inline('Hmm', 'something somebody says when they are thinking about what you have just said.', 'hmm') . '...';
        if (!\WP2FA_Vendor\FS_Plugin_License::is_valid_id($this->_site->license_id)) {
            $this->_admin_notices->add(\sprintf($this->get_text_inline('It looks like your site currently doesn\'t have an active license.', 'no-active-license-message'), $this->get_plan_title()), $hmm_text);
            return;
        }
        $api = $this->get_api_site_scope();
    ...
    private function handle_license_deactivation_result($license, $hmm_text = \false, $show_notice = \true)
    {
        if (isset($license->error)) {
            $this->_admin_notices->add($this->get_text_inline('It looks like the license deactivation failed.', 'license-deactivation-failed-message') . '<br> ' . $this->get_text_inline('Error received from the server:', 'server-error-message') . ' ' . \var_export($license->error, \true), $hmm_text, 'error');
            return;
        }
        ...
        if (\is_array($this->_licenses)) {
            for ($i = 0, $len = \count($this->_licenses); $i < $len; $i++) {
                if ($license->id == $this->_licenses[$i]->id) {
                    $this->_licenses[$i] = new \WP2FA_Vendor\FS_Plugin_License($license);
                }
            }
        }
        ...
        $this->_store_account();
        if ($show_notice) {
            $this->_admin_notices->add(\sprintf($this->is_only_premium() ? $this->get_text_inline('Your %s license was successfully deactivated.', 'license-deactivation-message_premium-only') : $this->get_text_inline('Your license was successfully deactivated, you are back to the %s plan.', 'license-deactivation-message'), $this->get_plan_title()), $this->get_text_inline('O.K', 'ok'));
        }
        $this->_admin_notices->remove_sticky(array('plan_upgraded', 'license_activated'));
    }
    /**
     * Site plan downgrade.
     *
    ...
    {
        $this->_logger->entrance();
        $deactivate_license = \WP2FA_Vendor\fs_request_get_bool('deactivate_license');
        $api = $this->get_api_site_scope();
        ...
        if ($this->is_api_result_entity($site)) {
            ...
            $plan_downgraded = $plan instanceof \WP2FA_Vendor\FS_Plugin_Plan && $prev_plan_id != $plan->id || \is_object($subscription) && !isset($subscription->error) && !$subscription->is_active();
        } else {
            // handle different error cases.
            $this->handle_license_deactivation_result($site, $this->get_text_x_inline('Hmm', 'something somebody says when they are thinking about what you have just said.', 'hmm') . '...');
        }
        if (!$plan_downgraded) {
            return (object) array('error' => (object) array('message' => $this->get_text_inline('Seems like we are having some temporary issue with your subscription cancellation. Please try again in few minutes.', 'subscription-cancellation-failure-message')));
        }
        // Remove previous sticky message about upgrade (if exist).
        $this->_admin_notices->remove_sticky('plan_upgraded');
        $this->_admin_notices->add(\sprintf($this->get_text_inline('Your subscription was successfully cancelled. Your %s plan license will expire in %s.', 'plan-x-downgraded-message'), $plan->title, \WP2FA_Vendor\human_time_diff(\time(), \strtotime($this->_license->expiration))));
        // Store site updates.
        $this->_store_site();
        if ($deactivate_license && !\WP2FA_Vendor\FS_Plugin_License::is_valid_id($site->license_id)) {
    ...
    function start_trial($plan_name = \false)
    {
        ...
        if ($this->is_trial()) {
            ...
            $this->_admin_notices->add(\sprintf($this->get_text_inline('You are already running the %s in a trial mode.', 'in-trial-mode'), $this->_module_type), $oops_text, 'error');
            return \false;
        }
        if ($this->_site->is_trial_utilized()) {
            // Trial was already utilized.
            $this->_admin_notices->add($this->get_text_inline('You already utilized a trial before.', 'trial-utilized'), $oops_text, 'error');
            return \false;
        }
        if (\false !== $plan_name) {
            ...
            if (\false === $plan) {
                // Plan doesn't exist.
                $this->_admin_notices->add(\sprintf($this->get_text_inline('Plan %s do not exist, therefore, can\'t start a trial.', 'trial-plan-x-not-exist'), $plan_name), $oops_text, 'error');
                return \false;
            }
            if (!$plan->has_trial()) {
                // Plan doesn't exist.
                $this->_admin_notices->add(\sprintf($this->get_text_inline('Plan %s does not support a trial period.', 'plan-x-no-trial'), $plan_name), $oops_text, 'error');
                return \false;
            }
        } else {
            if (!$this->has_trial_plan()) {
                // None of the plans have a trial.
                $this->_admin_notices->add(\sprintf($this->get_text_inline('None of the %s\'s plans supports a trial period.', 'no-trials'), $this->_module_type), $oops_text, 'error');
                return \false;
            }
            $plans_with_trial = \WP2FA_Vendor\FS_Plan_Manager::instance()->get_trial_plans($this->_plans);
        ...
        if (!$this->is_api_result_entity($plan)) {
            ...
            $this->_admin_notices->add(\sprintf($this->get_text_inline('Unexpected API error. Please contact the %s\'s author with the following error.', 'unexpected-api-error'), $this->_module_type) . ' ' . \var_export($plan, \true), $oops_text, 'error');
            return \false;
        }
        // Sync license.
        $this->_sync_license();
    ...
    private function _cancel_trial()
    {
        $this->_logger->entrance();
        if (!$this->is_trial()) {
            return (object) array('error' => (object) array('message' => $this->get_text_inline('It looks like you are not in trial mode anymore so there\'s nothing to cancel :)', 'trial-cancel-no-trial-message')));
        }
        $trial_plan = $this->get_trial_plan();
        $api = $this->get_api_site_scope();
        ...
        if ($this->is_api_result_entity($site)) {
            ...
            if ($this->is_paid_trial()) {
                ...
                $this->_license->is_cancelled = \true;
        ...
        if (!$trial_cancelled) {
            return (object) array('error' => (object) array('message' => $this->get_text_inline('Seems like we are having some temporary issue with your trial cancellation. Please try again in few minutes.', 'trial-cancel-failure-message')));
        }
        // Remove previous sticky messages about upgrade or trial (if exist).
        $this->_admin_notices->remove_sticky(array('trial_started', 'trial_promotion', 'plan_upgraded'));
        // Store site updates.
        $this->_store_site();
        if (!$this->is_addon() || !$this->deactivate_premium_only_addon_without_license(\true)) {
            $this->_admin_notices->add(\sprintf($this->get_text_inline('Your %s free trial was successfully cancelled.', 'trial-cancel-message'), $trial_plan->title));
        }
        return $site;
    }
    /**
    ...
    {
        return $this->has_any_active_valid_license() || $this->is_trial() && !$this->get_trial_plan()->is_free();
    }
    /**
     *
     * @author Vova Feldman (@svovaf)
    ...
    private function _get_latest_version_endpoint($addon_id = \false, $type = 'json')
    {
        ...
        $endpoint = ($is_addon ? "/addons/{$addon_id}" : '') . '/updates/latest.' . $type;
        ...
        if ($this->has_secret_key()) {
            ...
            if (\is_object($this->_site) && $this->_site->is_beta()) {
     ...
     */
    function _fetch_latest_version($addon_id = \false, $flush = \true, $expiration = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, $newer_than = \false, $fetch_readme = \true)
    {
        $this->_logger->entrance();
        ...
        $current_blog_id = \is_numeric($switch_to_blog_id) ? \WP2FA_Vendor\get_current_blog_id() : 0;
        ...
        $tag = $this->get_api_site_or_plugin_scope()->get($latest_version_endpoint, $flush, $expiration);
        if (\is_numeric($switch_to_blog_id)) {
            $this->switch_to_blog($current_blog_id);
        }
    ...
    {
        $this->_logger->entrance();
        \WP2FA_Vendor\wp_redirect($this->get_latest_download_api_url($plugin_id));
    }
    /**
    ...
    {
        $this->_logger->entrance();
        return $this->get_api_site_scope()->get_signed_url($this->_get_latest_version_endpoint($plugin_id, 'zip'));
    }
    /**
    ...
    {
        $this->_logger->entrance();
        $url = $this->get_api_user_scope()->get_signed_url("/payments/{$payment_id}/invoice.pdf");
        if (!\WP2FA_Vendor\fs_starts_with($url, 'https://')) {
    ...
    {
        return \sprintf('<a target="_blank" rel="noopener" href="%s">%s</a>', $this->_get_latest_download_local_url($plugin_id), $label);
    }
    /**
    ...
    function _get_latest_download_local_url($plugin_id = \false)
    {
        ...
        $params = array('ts' => \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME);
        if (!empty($plugin_id)) {
            $params['plugin_id'] = $plugin_id;
        } else {
            if ($this->is_addon()) {
     ...
     * @param bool|string $newer_than Since 2.2.1
     */
    private function check_updates($background = \false, $plugin_id = \false, $flush = \true, $expiration = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, $newer_than = \false)
    {
        $this->_logger->entrance();
        ...
        if (\is_object($new_version)) {
            $update = new \WP2FA_Vendor\FS_Plugin_Tag($new_version);
            if (!$background) {
                $this->_admin_notices->add(\sprintf(
                    ...
                    $update->version,
                    \sprintf('<a href="%s" target="_blank" rel="noopener">%s</a>', $this->get_account_url('download_latest'), \sprintf(
            ...
            if (\false === $new_version && !$background) {
                $this->_admin_notices->add($this->get_text_inline('Seems like you got the latest release.', 'you-have-latest'), $this->get_text_inline('You are all good!', 'you-are-good'));
            }
        }
        $this->_store_update($update, \true, $plugin_id);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.0.4
    ...
    private function sync_addons($flush = \false)
    {
        ...
        if (!$flush && $api->is_cached($path)) {
            $addons = self::get_all_addons();
            return isset($addons[$this->_plugin->id]) ? $addons[$this->_plugin->id] : array();
        }
        $result = $api->get($path, $flush);
        $addons = array();
        if ($this->is_api_result_object($result, 'plugins') && \is_array($result->plugins)) {
            for ($i = 0, $len = \count($result->plugins); $i < $len; $i++) {
                $addons[$i] = new \WP2FA_Vendor\FS_Plugin($result->plugins[$i]);
            }
            $this->_store_addons($addons, \true);
        }
        return $addons;
    }
    /**
    ...
    private function update_email($new_email)
    {
        $this->_logger->entrance();
        $api = $this->get_api_user_scope();
        $user = $api->call("?plugin_id={$this->_plugin->id}&fields=id,email,is_verified", 'put', array('email' => $new_email, 'after_email_confirm_url' => $this->_get_admin_page_url('account', array('fs_action' => 'sync_user'))));
        if (!isset($user->error)) {
            $this->_user->email = $user->email;
    ...
    {
        return \WP2FA_Vendor\FS_Api::is_api_error($result);
    }
    /**
    ...
    {
        return \WP2FA_Vendor\FS_Api::is_api_result_object($result, $required_property);
    }
    /**
    ...
    {
        return \WP2FA_Vendor\FS_Api::is_api_result_entity($result);
    }
    #endregion
    /**
    ...
    private function init_change_owner($candidate_email)
    {
        $this->_logger->entrance();
        $api = $this->get_api_site_scope();
        $result = $api->call("/users/{$this->_user->id}.json", 'put', array('email' => $candidate_email, 'after_confirm_url' => $this->_get_admin_page_url('account', array('fs_action' => 'change_owner'))));
    ...
    {
        $this->_logger->entrance();
        $site_result = $this->get_api_site_scope(\true)->get();
        $site = new \WP2FA_Vendor\FS_Site($site_result);
        $this->_site = $site;
        $user = new \WP2FA_Vendor\FS_User();
        $user->id = \WP2FA_Vendor\fs_request_get('user_id');
        ...
        $this->_user = $user;
        $user_result = $this->get_api_user_scope(\true)->get();
        $user = new \WP2FA_Vendor\FS_User($user_result);
        $this->_user = $user;
    ...
    private function complete_ownership_change_by_license($user_id, $install_ids_by_slug_map)
    {
        ...
        $result = $this->get_api_user_scope(\true)->get("/installs.json?install_ids=" . \implode(',', $install_ids_by_slug_map));
        if ($this->is_api_result_object($result, 'installs')) {
            ...
            $this->set_account_option('sites', $sites, \true);
        }
    }
    /**
    ...
    {
        $this->_logger->entrance();
        $name = \WP2FA_Vendor\fs_request_get('fs_user_name_' . $this->get_unique_affix(), '');
        $api = $this->get_api_user_scope();
        $user = $api->call("?plugin_id={$this->_plugin->id}&fields=id,first,last", 'put', array('name' => $name));
        if (!isset($user->error)) {
            $this->_user->first = $user->first;
            $this->_user->last = $user->last;
            $this->_store_user();
    ...
    private function verify_email()
    {
        ...
        $api = $this->get_api_site_scope();
        $result = $api->call("/users/{$this->_user->id}/verify.json", 'put', array('after_email_confirm_url' => $this->_get_admin_page_url('account', array('fs_action' => 'sync_user'))));
        if (!isset($result->error)) {
            $this->_admin_notices->add(\sprintf($this->get_text_inline('Verification mail was just sent to %s. If you can\'t find it after 5 min, please check your spam box.', 'verification-email-sent-message'), \sprintf('<a href="mailto:%1$s">%2$s</a>', \esc_url($this->_user->email), $this->_user->email)));
        } else {
            // handle different error cases.
        }
    }
    ...
    {
        if ($this->show_opt_in_on_themes_page() && (\WP2FA_Vendor\fs_request_has('pending_activation') || \WP2FA_Vendor\fs_request_get_bool($this->get_unique_affix() . '_show_optin'))) {
            $first_time_path = '';
        } else {
            $first_time_path = $this->_menu->get_first_time_path(\WP2FA_Vendor\fs_is_network_admin() && $this->_is_network_active);
        }
        if ($this->_is_network_active && \WP2FA_Vendor\fs_is_network_admin() && !$this->_menu->has_network_menu() && $this->is_network_registered()) {
            $target_url = $this->get_account_url();
        } else {
            // Default plugin's page.
            $target_url = $this->_get_admin_page_url('', array(), $network);
        }
        return \add_query_arg($params, $this->apply_filters($filter, empty($first_time_path) ? $target_url : $first_time_path));
    }
    /**
    ...
    private function _handle_account_edits()
    {
        ...
        $action = \WP2FA_Vendor\fs_get_action();
        if (empty($action)) {
            return;
        }
        $plugin_id = \WP2FA_Vendor\fs_request_get('plugin_id', $this->get_id());
        $install_id = \WP2FA_Vendor\fs_request_get('install_id', '');
        // Alias.
        $oops_text = $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...';
        ...
        switch ($action) {
            case 'opt_in':
                \WP2FA_Vendor\check_admin_referer(\trim("{$action}:{$blog_id}:{$install_id}", ':'));
                if ($is_parent_plugin_action) {
                    if ($is_network_action && !empty($blog_id)) {
                        if (!$this->is_registered()) {
                            $this->install_with_user($this->get_network_user(), \false, \false, \false, \false);
                            $this->_admin_notices->add($this->get_text_inline('Site successfully opted in.', 'successful-opt-in'), $this->get_text_inline('Awesome', 'awesome'));
                        }
                    }
                }
                break;
                        ...
                        if ($this->is_registered()) {
                            if ($this->is_tracking_prohibited()) {
                                if ($this->allow_site_tracking()) {
                                    $this->_admin_notices->add(\sprintf($this->get_text_inline('We appreciate your help in making the %s better by letting us track some usage data.', 'opt-out-message-appreciation'), $this->_module_type), $this->get_text_inline('Thank you!', 'thank-you'));
                                }
                            } else {
                                if ($this->stop_site_tracking()) {
                                    $this->_admin_notices->add(\sprintf($this->get_text_inline('We will no longer be sending any usage data of %s on %s to %s.', 'opted-out-successfully'), $this->get_plugin_title(), \WP2FA_Vendor\fs_strip_url_protocol(\WP2FA_Vendor\get_site_url($blog_id)), \sprintf('<a href="%s" target="_blank" rel="noopener">%s</a>', 'https://freemius.com', 'freemius.com')));
                                }
                            }
                        }
                    }
            ...
            case 'delete_account':
                ...
                if ($is_parent_plugin_action) {
                    ...
                    if ($this->is_addon_activated($plugin_id)) {
                        ...
                        \WP2FA_Vendor\fs_redirect($this->_get_admin_page_url('account'));
                    }
                }
                return;
            case 'downgrade_account':
                if (\is_numeric($blog_id)) {
                    ...
                    \WP2FA_Vendor\check_admin_referer($action);
                }
                $switch_to_network_install_blog_after_cancellation = \is_numeric($blog_id) && $plugin_id == $this->get_id() && !$this->is_trial();
                $result = $this->cancel_subscription_or_trial($plugin_id);
                if ($this->is_api_error($result)) {
                    $this->_admin_notices->add($result->error->message, $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...', 'error');
                }
            ...
            case 'activate_license':
                ...
                $fs = $this;
                if ($plugin_id != $this->get_id()) {
                    $fs = $this->is_addon_activated($plugin_id) ? self::get_instance_by_id($plugin_id) : null;
                }
                if (\is_object($fs)) {
            ...
            case 'check_updates':
                ...
                return;
            case 'change_owner':
                $state = \WP2FA_Vendor\fs_request_get('state', 'init');
                switch ($state) {
                    case 'init':
                        $candidate_email = \WP2FA_Vendor\fs_request_get('candidate_email', '');
                    ...
                    case 'candidate_confirmed':
                        if ($this->complete_change_owner()) {
                            $this->_admin_notices->add_sticky(\sprintf($this->get_text_inline('%s is the new owner of the account.', 'change-owner-request_candidate-confirmed'), '<b>' . $this->_user->email . '</b>'), 'ownership_changed', $this->get_text_x_inline('Congrats', 'as congratulations', 'congrats') . '!');
                        } else {
                ...
                return;
            case 'update_email':
                \WP2FA_Vendor\check_admin_referer('update_email');
                $new_email = \WP2FA_Vendor\fs_request_get('fs_email_' . $this->get_unique_affix(), '');
            ...
            case 'update_user_name':
                \WP2FA_Vendor\check_admin_referer('update_user_name');
                $result = $this->update_user_name();
                if (isset($result->error)) {
                    $this->_admin_notices->add($this->get_text_inline('Please provide your full name.', 'name-update-failed-message'), $oops_text, 'error');
                } else {
        ...
        if (\WP2FA_Vendor\WP_FS__IS_POST_REQUEST) {
            ...
            foreach ($properties as $p) {
                if ('update_' . $p === $action) {
                    ...
                    $site_property_value = \WP2FA_Vendor\fs_request_get('fs_' . $p . '_' . $this->get_unique_affix(), '');
    ...
    {
        $this->_logger->entrance();
        $this->_logger->info(\var_export($_REQUEST, \true));
        \WP2FA_Vendor\fs_enqueue_local_style('fs_account', '/admin/account.css');
        if ($this->has_addons()) {
            \WP2FA_Vendor\wp_enqueue_script('plugin-install');
            \WP2FA_Vendor\add_thickbox();
        ...
        if ($this->has_paid_plan() && !$this->has_any_license() && !$this->is_sync_executed() && $this->is_tracking_allowed()) {
            /**
        ...
        if (\is_object($this->_license) && $this->_license->user_id == $this->_user->id && !$this->is_whitelabeled(\true)) {
            $this->_admin_notices->add(\sprintf($this->get_text_inline("Is this your client's site? %s if you wish to hide sensitive info like your email, license key, prices, billing address & invoices from the WP Admin.", 'license_not_whitelabeled'), \sprintf('<a href="#" class="fs-toggle-whitelabel-mode">%s</a>', $this->get_text_inline('Click here', 'click-here'))), '', 'success', \false, 'license_not_whitelabeled');
        }
    ...
    function _affiliation_page_render()
    {
        ...
        \WP2FA_Vendor\fs_enqueue_local_style('fs_affiliation', '/admin/affiliation.css');
        $vars = array('id' => $this->_module_id);
        echo $this->apply_filters("/forms/affiliation.php", \WP2FA_Vendor\fs_get_template('/forms/affiliation.php', $vars));
    }
    /**
     * Render account page.
     *
    ...
    function _account_page_render()
    {
         ...
         * @since  1.2.1.6
         */
        echo $this->apply_filters("templates/{$template}", \WP2FA_Vendor\fs_get_template($template, $vars));
    }
    /**
    ...
    function _connect_page_render()
    {
         ...
         * @since  1.2.1.6
         */
        echo $this->apply_filters('templates/connect.php', \WP2FA_Vendor\fs_get_template('connect.php', $vars));
    }
    /**
    ...
    {
        $this->_logger->entrance();
        \WP2FA_Vendor\fs_enqueue_local_style('fs_addons', '/admin/add-ons.css');
        \WP2FA_Vendor\wp_enqueue_script('plugin-install');
        \WP2FA_Vendor\add_thickbox();
        function fs_addons_body_class($classes)
        {
    ...
    function _addons_page_render()
    {
         ...
         * @since  1.2.1.6
         */
        echo $this->apply_filters('templates/add-ons.php', \WP2FA_Vendor\fs_get_template('add-ons.php', $vars));
    }
    /* Pricing & Upgrade
    		------------------------------------------------------------------------------------------------------------------*/
    /**
    ...
    function _pricing_page_render()
    {
        $this->_logger->entrance();
        $vars = array('id' => $this->_module_id);
        if ('true' === \WP2FA_Vendor\fs_request_get('checkout', \false)) {
            echo $this->apply_filters('templates/checkout.php', \WP2FA_Vendor\fs_get_template('checkout.php', $vars));
        } else {
            echo $this->apply_filters('templates/pricing.php', \WP2FA_Vendor\fs_get_template('pricing.php', $vars));
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    function _fs_pricing_ajax_action_handler()
    {
        ...
        $result = null;
        $pricing_action = \WP2FA_Vendor\fs_request_get('pricing_action');
        switch ($pricing_action) {
            case 'fetch_pricing_data':
                $params = array('is_enriched' => \true, 'trial' => \WP2FA_Vendor\fs_request_get_bool('trial'), 'sandbox' => \WP2FA_Vendor\fs_request_get('sandbox'), 's_ctx_type' => \WP2FA_Vendor\fs_request_get('s_ctx_type'), 's_ctx_id' => \WP2FA_Vendor\fs_request_get('s_ctx_id'), 's_ctx_ts' => \WP2FA_Vendor\fs_request_get('s_ctx_ts'), 's_ctx_secure' => \WP2FA_Vendor\fs_request_get('s_ctx_secure'));
                $bundle_id = $this->get_bundle_id();
                $bundle_public_key = $this->get_bundle_public_key();
                $has_bundle_context = \WP2FA_Vendor\FS_Plugin::is_valid_id($bundle_id) && !empty($bundle_public_key);
                if (!$has_bundle_context) {
                    $api = $this->get_api_plugin_scope();
                } else {
                    $api = \WP2FA_Vendor\FS_Api::instance($bundle_id, 'plugin', $bundle_id, $bundle_public_key, !$this->is_live(), \false, $this->get_sdk_version());
                    $params['plugin_id'] = $this->get_id();
                ...
                break;
            case 'start_trial':
                $result = $this->opt_in(\false, \false, \false, \false, \false, \WP2FA_Vendor\fs_request_get('plan_id'));
        }
        if (\is_object($result) && $this->is_api_error($result)) {
            $this->_logger->api_error($result);
            self::shoot_ajax_failure(isset($result->error) ? \is_string($result->error) ? $result->error : $result->error->message : \var_export($result, \true));
        }
    ...
    function _contact_page_render()
    {
         ...
         * @since  2.1.3
         */
        echo $this->apply_filters('templates/contact.php', \WP2FA_Vendor\fs_get_template('contact.php', $vars));
    }
    #endregion ------------------------------------------------------------------------
    /**
    ...
    {
        \WP2FA_Vendor\remove_all_actions('admin_notices');
        \WP2FA_Vendor\remove_all_actions('network_admin_notices');
        \WP2FA_Vendor\remove_all_actions('all_admin_notices');
        \WP2FA_Vendor\remove_all_actions('user_admin_notices');
    }
     ...
     * @return \FS_Api
     */
    private function get_api_user_scope_by_user(\WP2FA_Vendor\FS_User $user)
    {
        return \WP2FA_Vendor\FS_Api::instance($this->_module_id, 'user', $user->id, $user->public_key, !$this->is_live(), $user->secret_key, $this->get_sdk_version());
    }
    /**
     *
     * @author Leo Fajardo (@leorw)
    ...
    {
        if (!$this->_is_network_active || isset($this->_user) && $this->_user instanceof \WP2FA_Vendor\FS_User) {
            return $this->get_api_user_scope($flush);
        }
        $user = $this->get_current_or_network_user();
        $this->_user_api = \WP2FA_Vendor\FS_Api::instance($this->_module_id, 'user', $user->id, $user->public_key, !$this->is_live(), $user->secret_key, $this->get_sdk_version());
        return $this->_user_api;
    }
    private $_site_api;
    /**
    ...
    private function get_api_site_scope($flush = \false)
    {
        if (!isset($this->_site_api) || $flush) {
            $this->_site_api = \WP2FA_Vendor\FS_Api::instance($this->_module_id, 'install', $this->_site->id, $this->_site->public_key, !$this->is_live(), $this->_site->secret_key, $this->get_sdk_version());
        }
    ...
    function get_api_plugin_scope()
    {
        if (!isset($this->_plugin_api)) {
            $this->_plugin_api = \WP2FA_Vendor\FS_Api::instance($this->_module_id, 'plugin', $this->_plugin->id, $this->_plugin->public_key, !$this->is_live(), \false, $this->get_sdk_version());
        }
    ...
    {
        return \WP2FA_Vendor\FS_Api::instance($this->get_bundle_id(), 'plugin', $this->get_bundle_id(), $this->get_bundle_public_key(), !$this->is_live(), \false, $this->get_sdk_version());
    }
    /**
    ...
    {
        return $this->is_registered() ? $this->get_api_site_scope() : $this->get_api_plugin_scope();
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.2.3.1
    ...
    {
        if ('cUrlMissing' !== $result->error->type && ('CurlException' !== $result->error->type || \CURLE_COULDNT_CONNECT != $result->error->code) && ('HttpRequestFailed' !== $result->error->type || \false === \strpos($result->error->message, 'cURL error ' . \CURLE_COULDNT_CONNECT))) {
            return;
        }
        $result->error->message = $this->esc_html_inline('We use PHP cURL library for the API calls, which is a very common library and usually installed and activated out of the box. Unfortunately, cURL is not activated (or disabled) on your server.', 'curl-missing-message') . ' ' . $this->esc_html_inline(\sprintf('Please contact your hosting provider and ask them to whitelist %s for external connection.', \implode(', ', $this->apply_filters('api_domains', array('api.freemius.com', 'wp.freemius.com')))), 'connectivity-whitelist') . ' ' . \sprintf($this->esc_html_inline('Once you are done, deactivate the %s and activate it again.', 'connectivity-reactivate-module'), $this->get_module_type());
    }
    /**
    ...
    function _check_for_trial_plans($plans)
    {
        ...
        $this->_storage->has_trial_plan = \WP2FA_Vendor\FS_Plan_Manager::instance()->has_trial_plan($plans);
    }
    /**
    ...
    function _fix_start_trial_menu_item_url()
    {
        ...
        \WP2FA_Vendor\fs_require_template('add-trial-to-pricing.php', $template_args);
    }
    /**
    ...
    function _add_trial_notice()
    {
        ...
        if ($this->is_premium() && !\WP2FA_Vendor\WP_FS__DEV_MODE) {
        ...
        if (!$this->apply_filters('show_trial', \true)) {
        ...
        if (!$was_promotion_shown_before && $this->_storage->install_timestamp > \time() - $this->apply_filters('show_first_trial_after_n_sec', \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC)) {
            return \false;
        }
        // OR if promotion was shown before, try showing it every 30 days.
        if ($was_promotion_shown_before && $this->apply_filters('reshow_trial_after_every_n_sec', 30 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC) > \time() - $last_time_trial_promotion_shown) {
            return \false;
        }
        ...
        if ($this->is_registered()) {
            ...
            $trial_plans = \WP2FA_Vendor\FS_Plan_Manager::instance()->get_trial_plans($this->_plans);
            ...
            $total_paid_plans = \count($this->_plans) - (\WP2FA_Vendor\FS_Plan_Manager::instance()->has_free_plan($this->_plans) ? 1 : 0);
            if ($total_paid_plans !== $trial_plans_count) {
                ...
                for ($i = 0; $i < $trial_plans_count; $i++) {
                    $plans_string .= \sprintf(' <a href="%s">%s</a>', $trial_url, $trial_plans[$i]->title);
        ...
        $message = \sprintf($this->get_text_x_inline('Hey', 'exclamation', 'hey') . '! ' . $this->get_text_inline('How do you like %s so far? Test all our %s premium features with a %d-day free trial.', 'trial-x-promotion-message'), \sprintf('<b>%s</b>', $this->get_plugin_name()), $plans_string, $trial_period);
        // "No Credit-Card Required" or "No Commitment for N Days".
        $cc_string = $require_payment ? \sprintf($this->get_text_inline('No commitment for %s days - cancel anytime!', 'no-commitment-for-x-days'), $trial_period) : $this->get_text_inline('No credit card required', 'no-cc-required') . '!';
        // Start trial button.
        $button = ' ' . \sprintf('<a style="margin-left: 10px; vertical-align: super;" href="%s"><button class="button button-primary">%s &nbsp;&#10140;</button></a>', $trial_url, $this->get_text_x_inline('Start free trial', 'call to action', 'start-free-trial'));
        $this->_admin_notices->add_sticky($this->apply_filters('trial_promotion_message', "{$message} {$cc_string} {$button}"), 'trial_promotion', '', 'promotion');
        $this->_storage->trial_promotion_shown = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
        return \true;
    }
    /**
    ...
    function _add_affiliate_program_notice()
    {
        ...
        if (!$this->has_affiliate_program() || !empty($this->_storage->affiliate_application_data)) {
            return \false;
        }
        if (!$this->apply_filters('show_affiliate_program_notice', \true)) {
         ...
         * activation with FS.
         */
        if ($was_notice_shown_before || $this->_storage->install_timestamp > \time() - \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC * 30) {
            return \false;
        }
        if (!$this->is_paying() && \WP2FA_Vendor\FS_Plugin::AFFILIATE_MODERATION_CUSTOMERS == $this->_plugin->affiliate_moderation) {
            // If the user is not a customer and the affiliate program is only for customers, don't show the notice.
            return \false;
        }
        $message = \sprintf($this->get_text_inline('Hey there, did you know that %s has an affiliate program? If you like the %s you can become our ambassador and earn some cash!', 'become-an-ambassador-admin-notice'), \sprintf('<strong>%s</strong>', $this->get_plugin_name()), $this->get_module_label(\true));
        // HTML code for the "Learn more..." button.
        $button = ' ' . \sprintf('<a style="display: block; margin-top: 10px;" href="%s"><button class="button button-primary">%s &nbsp;&#10140;</button></a>', $this->_get_admin_page_url('affiliation'), $this->get_text_inline('Learn more', 'learn-more') . '...');
        $this->_admin_notices->add_sticky($this->apply_filters('affiliate_program_notice', "{$message} {$button}"), 'affiliate_program', '', 'promotion');
        $this->_storage->affiliate_program_notice_shown = \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
        return \true;
    }
    /**
     * @author Vova Feldman (@svovaf)
    ...
    function _add_fs_theme_activation_dialog()
    {
        ...
        $vars = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_once_template('connect.php', $vars);
    }
    /* Action Links
    		------------------------------------------------------------------------------------------------------------------*/
    ...
    {
        $this->_logger->entrance();
        $this->_action_links_hooked = \true;
        $this->_logger->log('Adding action links hooks.');
        // Add action link to settings page.
        \add_filter('plugin_action_links_' . $this->_plugin_basename, array(&$this, '_modify_plugin_action_links_hook'), \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, 2);
        \add_filter('network_admin_plugin_action_links_' . $this->_plugin_basename, array(&$this, '_modify_plugin_action_links_hook'), \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, 2);
    }
    /**
     ...
     * @param bool $key
     */
    function add_plugin_action_link($label, $url, $external = \false, $priority = \WP2FA_Vendor\WP_FS__DEFAULT_PRIORITY, $key = \false)
    {
        $this->_logger->entrance();
        ...
        $this->_action_links[$priority][] = array('label' => $label, 'href' => $url, 'key' => $key, 'external' => $external);
    }
    /**
    ...
    function _add_upgrade_action_link()
    {
         ...
         * @since 2.3.0
         */
        $add_upgrade_link = ($add_action_links || $is_activation_mode && $this->is_only_premium()) && !\WP2FA_Vendor\WP_FS__DEMO_MODE && !$this->is_whitelabeled();
        $add_addons_link = $add_action_links && $this->has_addons();
        if (!$add_upgrade_link && !$add_addons_link) {
            return;
        }
        if ($add_upgrade_link && $this->is_pricing_page_visible() && $this->is_submenu_item_visible('pricing')) {
            $this->add_plugin_action_link($this->get_text_inline('Upgrade', 'upgrade'), $this->get_upgrade_url(), \false, 7, 'upgrade');
        }
        if ($add_addons_link && $this->has_addons() && $this->is_submenu_item_visible('addons')) {
            $this->add_plugin_action_link($this->get_text_inline('Add-Ons', 'add-ons'), $this->_get_admin_page_url('addons'), \false, 9, 'addons');
        }
    }
    /**
    ...
    function _add_license_action_link()
    {
        ...
        $link_text = $this->is_free_plan() ? $this->get_text_inline('Activate License', 'activate-license') : $this->get_text_inline('Change License', 'change-license');
        $this->add_plugin_action_link($link_text, '#', \false, 11, 'activate-license ' . $this->get_unique_affix());
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.0.2
    ...
    {
        if (!\WP2FA_Vendor\current_user_can('manage_options')) {
            return;
        }
        $this->_logger->entrance();
        /**
        ...
        if ($this->is_addon() && !$this->is_only_premium() && $this->_parent->is_anonymous()) {
            return;
        }
        if (\WP2FA_Vendor\fs_is_network_admin()) {
            if (!$this->_is_network_active) {
        ...
        if (\WP2FA_Vendor\fs_request_is_action_secure($this->get_unique_affix() . '_reconnect')) {
            if (!$this->is_registered() && $this->is_anonymous()) {
                $this->connect_again();
                return;
            }
        ...
        if ($this->is_activation_mode() && $this->is_premium() && !$this->is_registered()) {
        ...
        if ($this->is_registered()) {
            ...
            if ($this->is_anonymous() || $this->is_activation_mode()) {
                /**
                 * Show opt-in link only if skipped or in activation mode.
                 */
                $link_text_id = $this->get_text_inline('Opt In', 'opt-in');
                $params = !$this->is_anonymous() ? array() : array('nonce' => \wp_create_nonce($this->get_unique_affix() . '_reconnect'), 'fs_action' => $this->get_unique_affix() . '_reconnect');
                $url = $this->get_activation_url($params);
            }
        }
        \add_action('admin_footer', array(&$this, '_add_optout_dialog'));
        if (!empty($link_text_id) && $this->is_plugin() && self::is_plugins_page()) {
            $this->add_plugin_action_link($link_text_id, $url, \false, 13, "opt-in-or-opt-out {$this->_slug}");
        }
    }
    /**
                ...
                if (!empty($first_time_path)) {
                    ...
                    if (!empty($this->_dynamically_added_top_level_page_hook_name)) {
                        if ($this->is_network_registered()) {
                            ...
                            if ($this->is_pending_activation() || $this->is_network_anonymous()) {
                                $this->maybe_set_slug_and_network_menu_exists_flag();
                            }
                        }
                    }
    ...
    {
        if ($this->apply_filters('redirect_on_activation', \true)) {
            $url = $this->get_after_plugin_activation_redirect_url();
            if (\is_string($url)) {
                \WP2FA_Vendor\fs_redirect($url);
            }
        }
    }
    /**
    ...
    private function is_premium_version_installed()
    {
        ...
        $premium_plugin = \WP2FA_Vendor\get_plugins('/' . \dirname($premium_plugin_basename));
    ...
    private function get_complete_upgrade_instructions($plan_title = '')
    {
        ...
        if ($this->is_premium_version_installed()) {
            ...
            return \sprintf(
                ...
                $plan_title,
                \sprintf('<a style="margin-left: 10px;" href="%s"><button class="button button-primary">%s</button></a>', \WP2FA_Vendor\wp_nonce_url('plugins.php?action=activate&amp;plugin=' . $premium_plugin_basename, 'activate-plugin_' . $premium_plugin_basename), \esc_html(\sprintf(
            ...
            $deactivation_step = \version_compare($this->version, '1.2.1.5', '<') ? '<li>' . $this->esc_html_inline('Deactivate the free version', 'deactivate-free-version') . '.</li>' : '';
            return \sprintf(' %s: <ol><li>%s.</li>%s<li>%s (<a href="%s" target="_blank" rel="noopener">%s</a>).</li></ol>', $this->get_text_inline('Please follow these steps to complete the upgrade', 'follow-steps-to-complete-upgrade'), (empty($activate_license_string) ? '' : $activate_license_string . '</li><li>') . $this->get_latest_download_link(\sprintf(
                /* translators: %s: Plan title */
                $this->get_text_inline('Download the latest %s version', 'download-latest-x-version'),
                $plan_title
            )), $deactivation_step, $this->get_text_inline('Upload and activate the downloaded version', 'upload-and-activate'), $this->apply_filters('upload_and_install_video_url', '//bit.ly/upload-wp-' . $this->_module_type . 's'), $this->get_text_inline('How to upload and activate?', 'howto-upload-activate'));
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    {
        if (\WP2FA_Vendor\WP_FS__DEBUG_SDK || isset($_COOKIE['XDEBUG_SESSION'])) {
            $url = \add_query_arg('XDEBUG_SESSION_START', \rand(0, 9999999), $url);
            $url = \add_query_arg('XDEBUG_SESSION', 'PHPSTORM', $url);
            $request['cookies'] = array(new \WP_Http_Cookie(array('name' => 'XDEBUG_SESSION', 'value' => 'PHPSTORM')));
        }
    }
    /**
     * @author Leo Fajardo (@leorw)
    ...
    static function safe_remote_post(&$url, $request, $success_cache_expiration = 0, $failure_cache_expiration = 0, $maybe_enrich_request_for_debug = \true)
    {
        ...
        $cache_key = $should_cache ? \md5(\WP2FA_Vendor\fs_strip_url_protocol($url) . \json_encode($request)) : \false;
        $response = !\WP2FA_Vendor\WP_FS__DEBUG_SDK && \false !== $cache_key ? \get_transient($cache_key) : \false;
        if (\false === $response) {
            ...
            if ($response instanceof \WP_Error) {
                if ('https://' === \substr($url, 0, 8) && isset($response->errors) && isset($response->errors['http_request_failed'])) {
                    $http_error = \strtolower($response->errors['http_request_failed'][0]);
                    if (\false !== \strpos($http_error, 'ssl') || \false !== \strpos($http_error, 'curl error 35')) {
            ...
            if (\false !== $cache_key) {
                \set_transient($cache_key, $response, $response instanceof \WP_Error ? $failure_cache_expiration : $success_cache_expiration);
            }
        }
        return $response;
    }
    ...
    private function get_license_network_activation_notice()
    {
        ...
        if (\WP2FA_Vendor\get_blog_count() == 1) {
        ...
        return \sprintf($this->get_text_inline('%sClick here%s to choose the sites where you\'d like to activate the license on.', 'network-choose-sites-for-license'), '<a href="' . $this->get_account_url(\false, array('activate_license' => 'true')) . '">', '</a>');
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.2.1.7
    ...
    {
        return \WP2FA_Vendor\fs_text($key, $this->_slug);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.2.3
    ...
    {
        return \WP2FA_Vendor\_fs_text_inline($text, $key, $this->_slug);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.2.3
    ...
    {
        return \WP2FA_Vendor\_fs_text_x_inline($text, $context, $key, $this->_slug);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.2.3
    ...
    {
        return \esc_html(\WP2FA_Vendor\_fs_text_inline($text, $key, $this->_slug));
    }
    #----------------------------------------------------------------------------------
    #region Versioning
    #----------------------------------------------------------------------------------
    ...
    {
        return isset($this->_storage->sdk_upgrade_mode) ? $this->_storage->sdk_upgrade_mode : \false;
    }
    /**
    ...
    {
        return isset($this->_storage->plugin_upgrade_mode) ? $this->_storage->plugin_upgrade_mode : \false;
    }
    /**
    ...
    {
        $this->_storage->plugin_upgrade_mode = \false;
        $license_migration = !empty($this->_storage->license_migration) ? $this->_storage->license_migration : array();
    ...
    {
        return isset($this->_permissions[$permission]) && \true === $this->_permissions[$permission];
    }
    #endregion
    #----------------------------------------------------------------------------------
    ...
    function _install_premium_version_ajax_action()
    {
        ...
        if (!$this->is_registered()) {
            // Not registered.
            self::shoot_ajax_failure(array('message' => $this->get_text_inline('Auto installation only works for opted-in users.', 'auto-install-error-not-opted-in'), 'code' => 'premium_installed'));
        }
        $plugin_id = \WP2FA_Vendor\fs_request_get('target_module_id', $this->get_id());
        if (!\WP2FA_Vendor\FS_Plugin::is_valid_id($plugin_id)) {
            // Invalid ID.
            self::shoot_ajax_failure(array('message' => $this->get_text_inline('Invalid module ID.', 'auto-install-error-invalid-id'), 'code' => 'invalid_module_id'));
        }
        if ($plugin_id == $this->get_id()) {
            if ($this->is_premium()) {
                // Already using the premium code version.
                self::shoot_ajax_failure(array('message' => $this->get_text_inline('Premium version already active.', 'auto-install-error-premium-activated'), 'code' => 'premium_installed'));
            }
            if (!$this->can_use_premium_code()) {
                // Don't have access to the premium code.
                self::shoot_ajax_failure(array('message' => $this->get_text_inline('You do not have a valid license to access the premium version.', 'auto-install-error-invalid-license'), 'code' => 'invalid_license'));
            }
            if (!$this->has_release_on_freemius()) {
                // Plugin is a serviceware, no premium code version.
                self::shoot_ajax_failure(array('message' => $this->get_text_inline('Plugin is a "Serviceware" which means it does not have a premium code version.', 'auto-install-error-serviceware'), 'code' => 'premium_version_missing'));
            }
        } else {
            ...
            if ($this->is_addon_activated($plugin_id, \true)) {
                // Premium add-on version is already activated.
                self::shoot_ajax_failure(array('message' => $this->get_text_inline('Premium add-on version already installed.', 'auto-install-error-premium-addon-activated'), 'code' => 'premium_installed'));
            }
        }
        $this->_isAutoInstall = \true;
        // Try to install and activate.
        $updater = \WP2FA_Vendor\FS_Plugin_Updater::instance($this);
        $result = $updater->install_and_activate_plugin($plugin_id);
        if (\is_array($result) && !empty($result['message'])) {
            self::shoot_ajax_failure(array('message' => $result['message'], 'code' => $result['code']));
        }
        self::shoot_ajax_success($result);
    }
    /**
    ...
    function _add_auto_installation_dialog_box()
    {
        ...
        $vars = array('id' => $this->_module_id, 'target_module_id' => $plugin_id, 'slug' => $this->_slug);
        \WP2FA_Vendor\fs_require_template('auto-installation.php', $vars);
    }
    #endregion
    #--------------------------------------------------------------------------------
    ...
    {
        $this->_logger->entrance();
        if (!$this->is_product_settings_page() || !$this->is_matching_url($this->main_menu_url())) {
            return;
        }
        $params = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_once_template('tabs-capture-js.php', $params);
    }
    /**
    ...
    function _store_tabs_styles()
    {
        ...
        foreach ($wp_styles->queue as $handler) {
            if (\WP2FA_Vendor\fs_starts_with($handler, 'fs_')) {
            ...
            if (\WP2FA_Vendor\fs_starts_with($stylesheet->src, $theme_styles_url)) {
                $stylesheets[] = $stylesheet->src;
            }
        }
        if (!empty($stylesheets)) {
            $this->_cache->set('tabs_stylesheets', $stylesheets, 7 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC);
        }
    }
    /**
    ...
    private function should_page_include_tabs()
    {
        ...
        if ($this->is_admin_page('pricing') && \WP2FA_Vendor\fs_request_get_bool('checkout')) {
    ...
    function _add_tabs_before_content()
    {
        ...
        if (\is_array($stylesheets)) {
            for ($i = 0, $len = \count($stylesheets); $i < $len; $i++) {
                \WP2FA_Vendor\wp_enqueue_style("fs_{$this->_module_id}_tabs_{$i}", $stylesheets[$i]);
            }
        }
        // Cut closing </div> tag.
        echo \substr(\trim($this->get_tabs_html()), 0, -6);
        return \true;
    }
    /**
    ...
    function _add_freemius_tabs()
    {
        ...
        $params = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_once_template('tabs.php', $params);
    }
    #endregion
    #--------------------------------------------------------------------------------
    ...
    function _customizer_register($customizer)
    {
        ...
        if ($this->is_pricing_page_visible()) {
            require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/customizer/class-fs-customizer-upsell-control.php';
            $customizer->add_section('freemius_upsell', array('title' => '&#9733; ' . $this->get_text_inline('View paid features', 'view-paid-features'), 'priority' => 1));
            $customizer->add_setting('freemius_upsell', array('sanitize_callback' => 'esc_html'));
            $customizer->add_control(new \WP2FA_Vendor\FS_Customizer_Upsell_Control($customizer, 'freemius_upsell', array('fs' => $this, 'section' => 'freemius_upsell', 'priority' => 100)));
        }
        if ($this->is_page_visible('contact') || $this->is_page_visible('support')) {
            require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/customizer/class-fs-customizer-support-section.php';
            // Main Documentation Link In Customizer Root.
            $customizer->add_section(new \WP2FA_Vendor\FS_Customizer_Support_Section($customizer, 'freemius_support', array('fs' => $this, 'priority' => 1000)));
        }
    }
    #endregion
    /**
    ...
    function _style_premium_theme()
    {
        ...
        $params = null;
        \WP2FA_Vendor\fs_require_once_template('/js/jquery.content-change.php', $params);
        $params = array('slug' => $this->_slug, 'id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_template('/js/style-premium-theme.php', $params);
    }
    /**
    ...
    function get_local_icon_url()
    {
        ...
        if (\is_string($local_path)) {
            ...
            $img_dir = \WP2FA_Vendor\WP_FS__DIR_IMG;
            // Locate the main assets folder.
            if (1 < \count($fs_active_plugins->plugins)) {
                $plugin_or_theme_img_dir = $this->is_plugin() ? \WP2FA_Vendor\WP_PLUGIN_DIR : \WP2FA_Vendor\get_theme_root(\get_stylesheet());
                foreach ($fs_active_plugins->plugins as $sdk_path => &$data) {
                    if ($data->plugin_path == $this->get_plugin_basename()) {
                        $img_dir = $plugin_or_theme_img_dir . '/' . \str_replace('../' . \basename($plugin_or_theme_img_dir) . '/', '', $sdk_path) . '/assets/img';
                        break;
                    }
                }
            }
            // Try to locate the icon in the assets folder.
            $icons = \glob(\WP2FA_Vendor\fs_normalize_path($img_dir . "/{$this->_slug}.*"));
            if (!\is_array($icons) || 0 === \count($icons)) {
                if (!\WP2FA_Vendor\WP_FS__IS_LOCALHOST && $this->is_theme()) {
                    $icons = array(\WP2FA_Vendor\fs_normalize_path($img_dir . '/theme-icon.png'));
                } else {
                    $icon_found = \false;
                    $local_path = \WP2FA_Vendor\fs_normalize_path("{$img_dir}/{$this->_slug}.png");
                    if (!\function_exists('WP2FA_Vendor\\get_filesystem_method')) {
                        require_once \ABSPATH . 'wp-admin/includes/file.php';
                    }
                    $have_write_permissions = 'direct' === \WP2FA_Vendor\get_filesystem_method(array(), \WP2FA_Vendor\fs_normalize_path($img_dir));
                    /**
                     ...
                     * the repository.
                     */
                    if (\WP2FA_Vendor\WP_FS__IS_LOCALHOST && $have_write_permissions) {
                        ...
                        if (empty($icon) && $this->is_plugin() && $this->is_org_repo_compliant()) {
                            if (!\function_exists('WP2FA_Vendor\\plugins_api')) {
                                require_once \ABSPATH . 'wp-admin/includes/plugin-install.php';
                            }
                            $plugin_information = \WP2FA_Vendor\plugins_api('plugin_information', array('slug' => $this->_slug, 'fields' => array('sections' => \false, 'tags' => \false, 'icons' => \true)));
                            if (!\is_wp_error($plugin_information) && isset($plugin_information->icons) && !empty($plugin_information->icons)) {
                                /**
                        ...
                        if (!empty($icon)) {
                            ...
                            $local_path = \WP2FA_Vendor\fs_normalize_path("{$img_dir}/{$this->_slug}.{$ext}");
                            // Try to download the icon.
                            $icon_found = \WP2FA_Vendor\fs_download_image($icon, $local_path);
                        }
                    }
                    if (!$icon_found) {
                        ...
                        if ($have_write_permissions) {
                            ...
                            \copy(\WP2FA_Vendor\fs_normalize_path($img_dir . "/{$this->_module_type}-icon.png"), $local_path);
                        } else {
                            // If doesn't have write permissions, use default icon path.
                            $local_path = \WP2FA_Vendor\fs_normalize_path($img_dir . "/{$this->_module_type}-icon.png");
                        }
                    }
                    $icons = array($local_path);
                }
        ...
        return \WP2FA_Vendor\fs_img_url(\substr($icons[0], \strlen($icon_dir)), $icon_dir);
    }
    /**
    ...
    {
        return $this->get_api_plugin_scope()->get('info.json', \false, \WP2FA_Vendor\WP_FS__TIME_WEEK_IN_SEC);
    }
    /**
    ...
    function fetch_remote_icon_url()
    {
        ...
        return $this->is_api_result_object($info, 'icon') && \is_string($info->icon) ? $info->icon : '';
    }
    #--------------------------------------------------------------------------------
    #region GDPR
    #--------------------------------------------------------------------------------
    ...
    private function get_gdpr_admin_notice_string($user_plugins)
    {
        ...
        foreach ($user_plugins as $user_plugin) {
            ...
            if (\WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN === $user_plugin->type && !$has_addons) {
                if ($this->_module_id == $user_plugin->id) {
                    $addons = $this->get_addons();
                    $has_addons = !empty($addons);
                } else {
                    $plugin_api = \WP2FA_Vendor\FS_Api::instance($user_plugin->id, 'plugin', $user_plugin->id, $user_plugin->public_key, !$user_plugin->is_live, \false, $this->get_sdk_version());
                    $addons_result = $plugin_api->get('/addons.json?enriched=true', \true);
                    if ($this->is_api_result_object($addons_result, 'plugins') && \is_array($addons_result->plugins) && !empty($addons_result->plugins)) {
                        $has_addons = \true;
                    }
                }
            }
        ...
        if ($is_single_parent_product) {
            ...
            $thank_you = \sprintf("<span data-plugin-id='%d'>%s</span>", $single_parent_product->id, \sprintf($single_parent_product->has_addons ? $this->get_text_inline('Thank you so much for using %s and its add-ons!', 'thank-you-for-using-product-and-its-addons') : $this->get_text_inline('Thank you so much for using %s!', 'thank-you-for-using-product'), \sprintf('<b><i>%s</i></b>', $single_parent_product->title)));
            $already_opted_in = \sprintf($this->get_text_inline("You've already opted-in to our usage-tracking, which helps us keep improving the %s.", 'already-opted-in-to-product-usage-tracking'), \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME === $single_parent_product->type ? \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME : \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN);
        } else {
            ...
            foreach ($user_plugins as $user_plugin) {
                ...
                if (!$user_plugin->has_addons) {
                    $products_and_add_ons .= \sprintf("<span data-plugin-id='%d'>%s</span>", $user_plugin->id, $user_plugin->title);
                } else {
                    $products_and_add_ons .= \sprintf("<span data-plugin-id='%d'>%s</span>", $user_plugin->id, \sprintf($this->get_text_inline('%s and its add-ons', 'product-and-its-addons'), $user_plugin->title));
                }
            }
            $multiple_products_text = \sprintf("<small class='products'><strong>%s:</strong> %s</small>", $this->get_text_inline('Products', 'products'), $products_and_add_ons);
        }
        $actions = \sprintf('<ul><li>%s<span class="action-description"> - %s</span></li><li>%s<span class="action-description"> - %s</span></li></ul>', \sprintf('<button class="button button-primary allow-marketing">%s</button>', $this->get_text_inline('Yes', 'yes')), $this->get_text_inline('send me security & feature updates, educational content and offers.', 'send-updates'), \sprintf('<button class="button button-secondary">%s</button>', $this->get_text_inline('No', 'no')), \sprintf($this->get_text_inline('do %sNOT%s send me security & feature updates, educational content and offers.', 'do-not-send-updates'), '<span class="underlined">', '</span>'));
        return \sprintf('%s %s %s', $thank_you, $already_opted_in, \sprintf($this->get_text_inline('Due to the new %sEU General Data Protection Regulation (GDPR)%s compliance requirements it is required that you provide your explicit consent, again, confirming that you are onboard :-)', 'due-to-gdpr-compliance-requirements'), '<a href="https://eugdpr.org/" target="_blank" rel="noopener noreferrer">', '</a>') . '<br><br>' . '<b>' . $this->get_text_inline("Please let us know if you'd like us to contact you for security & feature updates, educational content, and occasional offers:", 'contact-for-updates') . '</b>' . $actions . ($is_single_parent_product ? '' : $multiple_products_text));
    }
    /**
    ...
    {
        $request = array('method' => 'POST', 'body' => array(), 'timeout' => \WP2FA_Vendor\WP_FS__DEBUG_SDK ? 60 : 30);
        ...
        $result = array();
        $url = \WP2FA_Vendor\WP_FS__ADDRESS . '/action/service/user_plugin/';
        ...
        for ($i = 1; $i <= $total_plugin_ids; $i += $plugin_ids_count_per_request) {
            ...
            $response = self::safe_remote_post($url, $request, \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, \WP2FA_Vendor\WP_FS__TIME_12_HOURS_IN_SEC);
            if (!\is_wp_error($response)) {
                $decoded = \is_string($response['body']) ? \json_decode($response['body']) : null;
                if (!\is_object($decoded) || !isset($decoded->success) || \true !== $decoded->success || !isset($decoded->data) || !\is_array($decoded->data)) {
                    return \false;
                }
    ...
    function _maybe_show_gdpr_admin_notice()
    {
        ...
        require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/class-fs-user-lock.php';
        $lock = \WP2FA_Vendor\FS_User_Lock::instance();
        /**
         ...
         */
        $current_fs_user = \WP2FA_Vendor\Freemius::_get_user_by_email($current_wp_user->user_email);
        $ten_years_in_sec = 10 * 365 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC;
        if (!\is_object($current_fs_user)) {
            // 10-year lock.
            $lock->lock($ten_years_in_sec);
            return;
        }
        $gdpr = \WP2FA_Vendor\FS_GDPR_Manager::instance();
        if ($gdpr->is_opt_in_notice_shown()) {
            // 30-day lock.
            $lock->lock(30 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC);
            return;
        }
        ...
        if ($was_notice_shown_before && 30 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC > \time() - $last_time_notice_shown) {
        ...
        if (empty($user_plugins)) {
            $lock->lock(\is_array($user_plugins) ? $ten_years_in_sec : \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC);
            return;
        }
        $has_unset_marketing_optin = \false;
        foreach ($user_plugins as $user_plugin) {
            if (\true == $user_plugin->is_marketing_allowed) {
                unset($plugin_ids_map[$user_plugin->plugin_id]);
            }
            if (!$has_unset_marketing_optin && \is_null($user_plugin->is_marketing_allowed)) {
                $has_unset_marketing_optin = \true;
            }
        }
        ...
        $modules = \array_merge(\array_values(self::maybe_get_entities_account_option('plugins', array())), \array_values(self::maybe_get_entities_account_option('themes', array())));
        foreach ($modules as $module) {
            if (!\WP2FA_Vendor\FS_Plugin::is_valid_id($module->parent_plugin_id) && isset($plugin_ids_map[$module->id])) {
        ...
        $gdpr->add_opt_in_sticky_notice($this->get_gdpr_admin_notice_string($plugin_ids_map), $plugin_title);
    ...
    {
        \WP2FA_Vendor\FS_GDPR_Manager::instance()->disable_opt_in_notice();
        require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/class-fs-user-lock.php';
        // 10-year lock.
        \WP2FA_Vendor\FS_User_Lock::instance()->lock(10 * 365 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.1.0
    ...
    {
        $vars = array('id' => $this->_module_id);
        \WP2FA_Vendor\fs_require_once_template('gdpr-optin-js.php', $vars);
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.1.0
    ...
    {
        \WP2FA_Vendor\fs_enqueue_local_style('fs_gdpr_optin_notice', '/admin/gdpr-optin-notice.css');
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since  2.1.0
    ...
    function _maybe_add_gdpr_optin_ajax_handler()
    {
        ...
        if (\WP2FA_Vendor\FS_GDPR_Manager::instance()->is_opt_in_notice_shown()) {
            $this->add_gdpr_optin_ajax_handler_and_style();
        }
    }
    /**
    ...
    function _fetch_is_marketing_required_flag_value_ajax_action()
    {
        ...
        $license_key = \WP2FA_Vendor\fs_request_get('license_key');
        if (empty($license_key)) {
            self::shoot_ajax_failure($this->get_text_inline('License key is empty.', 'empty-license-key'));
        }
        $user_plugins = $this->fetch_user_marketing_flag_status_by_plugins(null, $license_key, array($this->_module_id));
        if (!\is_array($user_plugins) || empty($user_plugins) || !isset($user_plugins[0]->plugin_id) || $user_plugins[0]->plugin_id != $this->_module_id) {
            /**
             * If faced an error or if the module ID do not match to the current module, ask for GDPR opt-in.
             *
             * @author Vova Feldman (@svovaf)
             */
            self::shoot_ajax_success(array('is_marketing_allowed' => null, 'license_owner_id' => null));
        }
        self::shoot_ajax_success(array('is_marketing_allowed' => $user_plugins[0]->is_marketing_allowed, 'license_owner_id' => isset($user_plugins[0]->license_owner_id) ? $user_plugins[0]->license_owner_id : null));
    }
    /**
     * @author Leo Fajardo (@leorw)
     * @since 2.3.2
    ...
    function _gdpr_optin_ajax_action()
    {
        ...
        if (!\WP2FA_Vendor\fs_request_has('is_marketing_allowed') || !\WP2FA_Vendor\fs_request_has('plugin_ids')) {
            self::shoot_ajax_failure();
        }
        $current_wp_user = self::_get_current_wp_user();
        $plugin_ids = \WP2FA_Vendor\fs_request_get('plugin_ids', array());
        ...
        $user_api = $this->get_api_user_scope_by_user(\WP2FA_Vendor\Freemius::_get_user_by_email($current_wp_user->user_email));
        foreach ($modules as $module) {
            $user_api->call("?plugin_id={$module->id}", 'put', array('is_marketing_allowed' => \true == \WP2FA_Vendor\fs_request_get_bool('is_marketing_allowed')));
        }
        \WP2FA_Vendor\FS_GDPR_Manager::instance()->remove_opt_in_notice();
        require_once \WP2FA_Vendor\WP_FS__DIR_INCLUDES . '/class-fs-user-lock.php';
        // 10-year lock.
        \WP2FA_Vendor\FS_User_Lock::instance()->lock(10 * 365 * \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC);
        self::shoot_ajax_success();
