...
class FS_Storage
{
    ...
    static function instance($module_type, $slug)
    {
        ...
        if (!isset(self::$_instances[$key])) {
            self::$_instances[$key] = new \WP2FA_Vendor\FS_Storage($module_type, $slug);
        }
    ...
    private function __construct($module_type, $slug)
    {
        ...
        if ($this->_is_multisite) {
            $this->_blog_id = \WP2FA_Vendor\get_current_blog_id();
            $this->_network_storage = \WP2FA_Vendor\FS_Key_Value_Storage::instance($module_type . '_data', $slug, \true);
        }
        $this->_storage = \WP2FA_Vendor\FS_Key_Value_Storage::instance($module_type . '_data', $slug, $this->_blog_id);
    }
    /**
     ...
     * @param bool          $flush
     */
    function store($key, $value, $network_level_or_blog_id = null, $flush = \true)
    {
     ...
     */
    function clear_all($store = \true, $exceptions = array(), $network_level_or_blog_id = null)
    {
        if (!$this->_is_multisite || \false === $network_level_or_blog_id || \is_null($network_level_or_blog_id) || \is_numeric($network_level_or_blog_id)) {
            $storage = $this->get_site_storage($network_level_or_blog_id);
            $storage->clear_all($store, $exceptions);
        }
        if ($this->_is_multisite && (\true === $network_level_or_blog_id || \is_null($network_level_or_blog_id))) {
     ...
     * @param null|bool|int $network_level_or_blog_id When an integer, use the given blog storage. When `true` use the multisite storage (if there's a network). When `false`, use the current context blog storage. When `null`, the decision which storage to use (MS vs. Current S) will be handled internally and determined based on the $option (based on self::$_BINARY_MAP).
     */
    function remove($key, $store = \true, $network_level_or_blog_id = null)
    {
        if ($this->should_use_network_storage($key, $network_level_or_blog_id)) {
    ...
    {
        if ($this->_is_network_active && (\true === $network_level_or_blog_id || \is_null($network_level_or_blog_id))) {
            $this->_network_storage->save();
        }
        if (!$this->_is_network_active || \true !== $network_level_or_blog_id) {
            $storage = $this->get_site_storage($network_level_or_blog_id);
            $storage->save();
        }
    }
    ...
    {
        if (!\is_numeric($blog_id) || $blog_id == $this->_blog_id || 0 == $blog_id) {
            return $this->_storage;
        }
        return \WP2FA_Vendor\FS_Key_Value_Storage::instance($this->_module_type . '_data', $this->_storage->get_secondary_id(), $blog_id);
    }
    #endregion
    #--------------------------------------------------------------------------------
    ...
    function __isset($k)
    {
        return $this->should_use_network_storage($k) ? isset($this->_network_storage->{$k}) : isset($this->_storage->{$k});
    }
    function __unset($k)
    {
        if ($this->should_use_network_storage($k)) {
    ...
    function __get($k)
    {
        return $this->should_use_network_storage($k) ? $this->_network_storage->{$k} : $this->_storage->{$k};
    }
    #endregion
}
