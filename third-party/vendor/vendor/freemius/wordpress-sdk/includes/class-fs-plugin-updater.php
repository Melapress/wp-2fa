...
class FS_Plugin_Updater
{
     ...
     */
    static function instance(\WP2FA_Vendor\Freemius $freemius)
    {
        $key = $freemius->get_id();
    ...
    #endregion
    private function __construct(\WP2FA_Vendor\Freemius $freemius)
    {
        $this->_fs = $freemius;
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_' . $freemius->get_slug() . '_updater', \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
        $this->filters();
    }
    /**
    ...
    private function filters()
    {
         ...
         */
        // WP 2.9+
        \add_action("after_plugin_row_{$this->_fs->get_plugin_basename()}", array(&$this, 'catch_plugin_update_row'), 9);
        \add_action("after_plugin_row_{$this->_fs->get_plugin_basename()}", array(&$this, 'edit_and_echo_plugin_update_row'), 11, 2);
        if (!$this->_fs->has_any_active_valid_license()) {
            \add_action('admin_head', array(&$this, 'catch_plugin_information_dialog_contents'));
        }
        if (!\WP2FA_Vendor\WP_FS__IS_PRODUCTION_MODE) {
            \add_filter('http_request_host_is_external', array($this, 'http_request_host_is_external_filter'), 10, 3);
        }
    ...
    {
        if ('plugin-information' !== \WP2FA_Vendor\fs_request_get('tab', \false) || $this->_fs->get_slug() !== \WP2FA_Vendor\fs_request_get('plugin', \false)) {
            return;
        }
    ...
    function edit_and_echo_plugin_information_dialog_contents($hook_suffix)
    {
        ...
        $subscription = \is_object($license) && !$license->is_lifetime() ? $this->_fs->_get_subscription($license->id) : null;
        $contents = \ob_get_clean();
        $update_button_id_attribute_pos = \strpos($contents, 'id="plugin_update_from_iframe"');
        if (\false !== $update_button_id_attribute_pos) {
            $update_button_start_pos = \strrpos(\substr($contents, 0, $update_button_id_attribute_pos), '<a');
             ...
             */
            $update_button = \preg_replace('/(\\<a.+)(id="plugin_update_from_iframe")(.+href=")([^\\s]+)(".*\\>)(.+)(\\<\\/a>)/is', \is_object($license) ? \sprintf('$1$3%s$5%s$7', $this->_fs->checkout_url(\is_object($subscription) ? 1 == $subscription->billing_cycle ? \WP2FA_Vendor\WP_FS__PERIOD_MONTHLY : \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY : \WP2FA_Vendor\WP_FS__PERIOD_LIFETIME, \false, array('licenses' => $license->quota)), \WP2FA_Vendor\fs_text_inline('Renew license', 'renew-license', $this->_fs->get_slug())) : \sprintf('$1$3%s$5%s$7', $this->_fs->pricing_url(), \WP2FA_Vendor\fs_text_inline('Buy license', 'buy-license', $this->_fs->get_slug())), $update_button);
            /**
    ...
    {
        \WP2FA_Vendor\remove_filter('pre_set_site_transient_update_plugins', array(&$this, 'pre_set_site_transient_update_plugins_filter'));
        \WP2FA_Vendor\remove_filter('pre_set_site_transient_update_themes', array(&$this, 'pre_set_site_transient_update_plugins_filter'));
    }
    /**
    ...
    function edit_and_echo_plugin_update_row($file, $plugin_data)
    {
        ...
        if ($this->_fs->has_any_active_valid_license()) {
            if ($has_beta_update) {
                 ...
                 * @since 2.3.0
                 */
                $plugin_update_row = \preg_replace('/(\\<div.+>)(.+)(\\<a.+href="([^\\s]+)"([^\\<]+)\\>.+\\<a.+)(\\<\\/div\\>)/is', '$1' . \sprintf(\WP2FA_Vendor\fs_text_inline('There is a %s of %s available.', 'new-version-available', $this->_fs->get_slug()), $has_beta_update ? \WP2FA_Vendor\fs_text_inline('new Beta version', 'new-beta-version', $this->_fs->get_slug()) : \WP2FA_Vendor\fs_text_inline('new version', 'new-version', $this->_fs->get_slug()), $this->_fs->get_plugin_title()) . ' ' . '$3' . '$6', $plugin_update_row);
            }
        } else {
            /**
             ...
             * @author Leo Fajardo (@leorw)
             */
            $plugin_update_row = \preg_replace('/(\\<div.+>)(.+)(\\<a.+href="([^\\s]+)"([^\\<]+)\\>.+\\<a.+)(\\<\\/div\\>)/is', '$1' . \sprintf(\WP2FA_Vendor\fs_text_inline('There is a %s of %s available.', 'new-version-available', $this->_fs->get_slug()), \sprintf('<a href="$4"%s>%s</a>', '$5', $has_beta_update ? \WP2FA_Vendor\fs_text_inline('new Beta version', 'new-beta-version', $this->_fs->get_slug()) : \WP2FA_Vendor\fs_text_inline('new version', 'new-version', $this->_fs->get_slug())), $this->_fs->get_plugin_title()) . ' ' . $this->_fs->version_upgrade_checkout_link($r->new_version) . '$6', $plugin_update_row);
        }
        if ($this->_fs->is_plugin() && isset($r->upgrade_notice) && \strlen(\trim($r->upgrade_notice)) > 0) {
            $slug = $this->_fs->get_slug();
            $upgrade_notice_html = \sprintf('<p class="notice fs-upgrade-notice fs-slug-%1$s fs-type-%2$s" data-slug="%1$s" data-type="%2$s"><strong>%3$s</strong> %4$s</p>', $slug, $this->_fs->get_module_type(), \WP2FA_Vendor\fs_text_inline('Important Upgrade Notice:', 'upgrade_notice', $slug), \esc_html($r->upgrade_notice));
    ...
    function change_theme_update_info_html($prepared_themes)
    {
        ...
        if (!isset($themes_update->response[$theme_basename]) || empty($themes_update->response[$theme_basename]['package'])) {
            return $prepared_themes;
        }
        $prepared_themes[$theme_basename]['update'] = \preg_replace('/(\\<p.+>)(.+)(\\<a.+\\<a.+)\\.(.+\\<\\/p\\>)/is', '$1 $2 ' . $this->_fs->version_upgrade_checkout_link($themes_update->response[$theme_basename]['new_version']) . '$4', $prepared_themes[$theme_basename]['update']);
    ...
    {
        return \false !== \strpos($host, 'freemius') ? \true : $allow;
    }
    /**
    ...
    function pre_set_site_transient_update_plugins_filter($transient_data)
    {
         ...
         * @since  1.2.2
         */
        if ("pre_set_site_transient_update_{$module_type}" !== \WP2FA_Vendor\current_filter()) {
            return $transient_data;
        }
        if (empty($transient_data) || \defined('WP2FA_Vendor\\WP_FS__UNINSTALL_MODE')) {
            return $transient_data;
        }
        global $wp_current_filter;
        $current_plugin_version = $this->_fs->get_plugin_version();
        if (!empty($wp_current_filter) && 'upgrader_process_complete' === $wp_current_filter[0]) {
            if (\is_null($this->_update_details) || \is_object($this->_update_details) && $this->_update_details->new_version !== $current_plugin_version) {
                /**
                ...
                $current_plugin_version = $this->_fs->get_plugin_version(\true);
            }
        }
        if (!isset($this->_update_details)) {
            // Get plugin's newest update.
            $new_version = $this->_fs->get_update(\false, \WP2FA_Vendor\fs_request_get_bool('force-check'), \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC / 24, $current_plugin_version);
        ...
        if (\is_object($this->_update_details)) {
            ...
            $transient_data->response[$basename] = $this->_fs->is_plugin() ? $this->_update_details : (array) $this->_update_details;
        } else {
            if (isset($transient_data->response)) {
                /**
        ...
        if ($this->_fs->is_org_repo_compliant() && $this->_fs->is_freemium()) {
            ...
            if (!empty($this->_translation_updates)) {
                $all_translation_updates = isset($transient_data->translations) && \is_array($transient_data->translations) ? $transient_data->translations : array();
                ...
                foreach ($this->_translation_updates as $translation_update) {
                    ...
                    if (!isset($current_plugin_translation_updates_map[$lang]) || \version_compare($translation_update['version'], $current_plugin_translation_updates_map[$lang]['version'], '>')) {
     ...
     * @return object
     */
    function get_update_details(\WP2FA_Vendor\FS_Plugin_Tag $new_version)
    {
        $update = new \stdClass();
        $update->slug = $this->_fs->get_slug();
        $update->new_version = $new_version->version;
        $update->url = \WP2FA_Vendor\WP_FS__ADDRESS;
        ...
        if ($this->_fs->is_premium()) {
            ...
            if (isset($latest_tag->readme) && isset($latest_tag->readme->upgrade_notice) && !empty($latest_tag->readme->upgrade_notice)) {
     ...
     * @return bool
     */
    private function is_new_version_premium(\WP2FA_Vendor\FS_Plugin_Tag $new_version)
    {
     ...
     * @param \FS_Plugin_Tag $new_version
     */
    function set_update_data(\WP2FA_Vendor\FS_Plugin_Tag $new_version)
    {
        $this->_logger->entrance();
        ...
        $transient_data = \is_object($transient_data) ? $transient_data : new \stdClass();
        // Alias.
        ...
        if (!isset($transient_data->response) || !\is_array($transient_data->response)) {
            ...
            if (!empty($transient_data->response[$basename])) {
                $version = $is_plugin ? !empty($transient_data->response[$basename]->new_version) ? $transient_data->response[$basename]->new_version : null : (!empty($transient_data->response[$basename]['new_version']) ? $transient_data->response[$basename]['new_version'] : null);
        ...
        $transient_data->response[$basename] = $is_plugin ? $this->_update_details : (array) $this->_update_details;
        if (!isset($transient_data->checked) || !\is_array($transient_data->checked)) {
    ...
    function delete_update_data()
    {
        ...
        // Alias
        $basename = $this->_fs->get_plugin_basename();
        if (!\is_object($transient_data) || !isset($transient_data->response) || !\is_array($transient_data->response) || empty($transient_data->response[$basename])) {
            return;
        }
        unset($transient_data->response[$basename]);
        // Remove the added filters.
    ...
    static function _fetch_plugin_info_from_repository($action, $args)
    {
        ...
        if ($ssl = \WP2FA_Vendor\wp_http_supports(array('ssl'))) {
            $url = \WP2FA_Vendor\set_url_scheme($url, 'https');
        }
        ...
        $res = \maybe_unserialize(\WP2FA_Vendor\wp_remote_retrieve_body($request));
    ...
    private function fetch_wp_org_module_translation_updates($module_type, $slug)
    {
        ...
        $locales = \array_values(\WP2FA_Vendor\get_available_languages());
        ...
        global $wp_version;
        $request_args = array('timeout' => 15, 'body' => array("{$module_type}" => \json_encode(array("{$module_type}" => array($plugin_basename => array('Name' => \trim(\str_replace($this->_fs->get_plugin()->premium_suffix, '', $plugin_data['Name'])), 'Author' => $plugin_data['Author'])))), 'translations' => \json_encode($this->get_installed_translations($module_type, $slug)), 'locale' => \json_encode($locales)), 'user-agent' => 'WordPress/' . $wp_version . '; ' . \home_url('/'));
        $url = "http://api.wordpress.org/{$module_type}/update-check/1.1/";
        if ($ssl = \WP2FA_Vendor\wp_http_supports(array('ssl'))) {
            $url = \WP2FA_Vendor\set_url_scheme($url, 'https');
        }
        $raw_response = \WP2FA_Vendor\Freemius::safe_remote_post($url, $request_args, \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, \WP2FA_Vendor\WP_FS__TIME_12_HOURS_IN_SEC, \false);
        if (\is_wp_error($raw_response)) {
            return null;
        }
        $response = \json_decode(\WP2FA_Vendor\wp_remote_retrieve_body($raw_response), \true);
    ...
    {
        if (\function_exists('WP2FA_Vendor\\wp_get_installed_translations')) {
            return \WP2FA_Vendor\wp_get_installed_translations($module_type);
        }
        $dir = "/{$module_type}";
        if (!\is_dir(\WP2FA_Vendor\WP_LANG_DIR . $dir)) {
            return array();
        }
        $files = \scandir(\WP2FA_Vendor\WP_LANG_DIR . $dir);
        if (!$files) {
            return array();
        }
        $language_data = array();
        foreach ($files as $file) {
            ...
            if ('.' === $file[0] || \is_dir(\WP2FA_Vendor\WP_LANG_DIR . "{$dir}/{$file}")) {
                continue;
            }
            if (\substr($file, -3) !== '.po') {
                continue;
            ...
            $language_data[$textdomain][$language] = \WP2FA_Vendor\wp_get_pomo_file_data(\WP2FA_Vendor\WP_LANG_DIR . "{$dir}/{$file}");
        }
    ...
    function plugins_api_filter($data, $action = '', $args = null)
    {
        ...
        if ($this->_fs->get_slug() !== $args->slug) {
            ...
            if ($this->_fs->is_addon_activated($addon->id)) {
                ...
                if ($this->_fs->is_addon_installed($addon->id)) {
                    $addon_plugin_data = \WP2FA_Vendor\get_plugin_data(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $this->_fs->get_addon_basename($addon->id), \false, \false);
        ...
        $plugin_version = $is_addon ? $addon_version : $this->_fs->get_plugin_version();
        // Get plugin's newest update.
        ...
        if (!\is_object($new_version) || empty($new_version->version)) {
            ...
            if ($is_addon) {
                ...
                $data->url = \WP2FA_Vendor\WP_FS__ADDRESS;
            ...
            if (isset($new_version->readme) && \is_object($new_version->readme)) {
                ...
                $wp_org_sections = array('author', 'author_profile', 'rating', 'ratings', 'num_ratings', 'support_threads', 'support_threads_resolved', 'active_installs', 'added', 'homepage');
    ...
    {
        return $this->_fs->_fetch_latest_version($addon_id, \true, \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, $newer_than, $fetch_readme);
    }
    /**
    ...
    {
        return $this->_fs->get_target_folder_name() == \trim(\dirname($this->_fs->get_plugin_basename()), '/\\');
    }
    /**
    ...
    {
        $basename = $this->_fs->get_plugin_basename();
        if (\true !== $response || empty($hook_extra) || empty($hook_extra['plugin']) || $basename !== $hook_extra['plugin']) {
            return $response;
        }
        $active_plugins_basenames = \WP2FA_Vendor\get_option('active_plugins');
        foreach ($active_plugins_basenames as $key => $active_plugin_basename) {
            if ($basename === $active_plugin_basename) {
                ...
                $new_basename = \WP2FA_Vendor\plugin_basename(\trailingslashit($this->_fs->is_premium() ? $this->_fs->get_premium_slug() : $this->_fs->get_slug()) . $filename);
                // Verify that the expected correct path exists.
                if (\file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $new_basename))) {
                    // Override active plugin name.
                    $active_plugins_basenames[$key] = $new_basename;
                    \WP2FA_Vendor\update_option('active_plugins', $active_plugins_basenames);
                }
                break;
            }
        }
    ...
    {
        if (!empty($plugin_id) && !\WP2FA_Vendor\FS_Plugin::is_valid_id($plugin_id)) {
            // Invalid plugin ID.
            return array('message' => $this->_fs->get_text_inline('Invalid module ID.', 'auto-install-error-invalid-id'), 'code' => 'invalid_module_id');
        }
        $is_addon = \false;
        if (\WP2FA_Vendor\FS_Plugin::is_valid_id($plugin_id) && $plugin_id != $this->_fs->get_id()) {
            ...
            $title = $this->_fs->get_plugin_title() . ($this->_fs->is_addon() ? ' ' . $this->_fs->get_text_inline('Add-On', 'addon') : '');
        }
        if ($this->is_premium_plugin_active($plugin_id)) {
            // Premium version already activated.
            return array('message' => $is_addon ? $this->_fs->get_text_inline('Premium add-on version already installed.', 'auto-install-error-premium-addon-activated') : $this->_fs->get_text_inline('Premium version already active.', 'auto-install-error-premium-activated'), 'code' => 'premium_installed');
        }
        ...
        $api = null;
        $install_url = \add_query_arg(array('action' => 'install-plugin', 'plugin' => \urlencode($slug)), 'update.php');
        if (!\class_exists('WP2FA_Vendor\\Plugin_Upgrader', \false)) {
            // Include required resources for the installation.
            require_once \ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
        }
        $skin_args = array('type' => 'web', 'title' => \sprintf($this->_fs->get_text_inline('Installing plugin: %s', 'installing-plugin-x'), $title), 'url' => \esc_url_raw($install_url), 'nonce' => 'install-plugin_' . $slug, 'plugin' => '', 'api' => $api, 'extra' => $extra);
        //			$skin = new Automatic_Upgrader_Skin( $skin_args );
        //			$skin = new Plugin_Installer_Skin( $skin_args );
        $skin = new \WP2FA_Vendor\WP_Ajax_Upgrader_Skin($skin_args);
        // Create a new instance of Plugin_Upgrader.
        $upgrader = new \WP2FA_Vendor\Plugin_Upgrader($skin);
        // Perform the action and install the plugin from the $source urldecode().
        \add_filter('upgrader_source_selection', array('FS_Plugin_Updater', '_maybe_adjust_source_dir'), 1, 3);
        $install_result = $upgrader->install($source);
        \WP2FA_Vendor\remove_filter('upgrader_source_selection', array('FS_Plugin_Updater', '_maybe_adjust_source_dir'), 1);
        if (\is_wp_error($install_result)) {
            return array('message' => $install_result->get_error_message(), 'code' => $install_result->get_error_code());
        } elseif (\is_wp_error($skin->result)) {
            return array('message' => $skin->result->get_error_message(), 'code' => $skin->result->get_error_code());
        } elseif ($skin->get_errors()->get_error_code()) {
            return array('message' => $skin->get_error_messages(), 'code' => 'unknown');
            ...
            if ($wp_filesystem instanceof \WP2FA_Vendor\WP_Filesystem_Base && \is_wp_error($wp_filesystem->errors) && $wp_filesystem->errors->get_error_code()) {
                $error_message = $wp_filesystem->errors->get_error_message();
            }
            return array('message' => $error_message, 'code' => $error_code);
        }
        ...
        if (\is_wp_error($activation_result)) {
            return array('message' => $activation_result->get_error_message(), 'code' => $activation_result->get_error_code());
        }
    ...
    {
        $activate = \WP2FA_Vendor\activate_plugin($file_path, '', $this->_fs->is_network_active());
        return \is_wp_error($activate) ? $activate : \true;
    }
    /**
    ...
    private function is_premium_plugin_active($plugin_id = \false)
    {
        if ($plugin_id != $this->_fs->get_id()) {
            return $this->_fs->is_addon_activated($plugin_id, \true);
        }
        return \WP2FA_Vendor\is_plugin_active($this->_fs->premium_plugin_basename());
    }
    /**
    ...
    static function _maybe_adjust_source_dir($source, $remote_source, $upgrader)
    {
        ...
        if (isset($upgrader->skin->options['extra'])) {
            ...
            if (!empty($basename)) {
                 ...
                 * @since 2.2.1
                 */
                $is_theme = !\WP2FA_Vendor\fs_ends_with($basename, '.php');
                $desired_slug = !$is_theme ? \dirname($basename) : $basename;
            } else {
        ...
        if (\is_multisite()) {
            ...
            if (!empty($basename)) {
                $fs = \WP2FA_Vendor\Freemius::get_instance_by_file($basename, $is_theme ? \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME : \WP2FA_Vendor\WP_FS__MODULE_TYPE_PLUGIN);
                if (!\is_object($fs)) {
                    /**
        ...
        $subdir_name = \untrailingslashit(\str_replace(\trailingslashit($remote_source), '', $source));
        if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
            $from_path = \untrailingslashit($source);
            $to_path = \trailingslashit($remote_source) . $desired_slug;
            if (\true === $GLOBALS['wp_filesystem']->move($from_path, $to_path)) {
                return \trailingslashit($to_path);
            }
            return new \WP_Error('rename_failed', \WP2FA_Vendor\fs_text_inline('The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'module-package-rename-failure'), array('found' => $subdir_name, 'expected' => $desired_slug));
        }
        return $source;
    }
    #endregion
