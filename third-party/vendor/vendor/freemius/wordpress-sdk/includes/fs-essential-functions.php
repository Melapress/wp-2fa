 ...
 * @since       1.1.5
 */
if (!\function_exists('WP2FA_Vendor\\fs_normalize_path')) {
    if (\function_exists('WP2FA_Vendor\\wp_normalize_path')) {
        /**
        ...
        {
            return \WP2FA_Vendor\wp_normalize_path($path);
        }
    } else {
        function fs_normalize_path($path)
        {
...
if (!\function_exists('WP2FA_Vendor\\fs_redirect')) {
    /**
    ...
    function fs_redirect($location, $exit = \true, $status = 302)
    {
        ...
        if (\headers_sent($file, $line)) {
            if (\WP2FA_Vendor\WP_FS__DEBUG_SDK && \class_exists('WP2FA_Vendor\\FS_Admin_Notices')) {
                $notices = \WP2FA_Vendor\FS_Admin_Notices::instance('global');
        ...
        if (\defined('WP2FA_Vendor\\DOING_AJAX')) {
        ...
        $location = \WP2FA_Vendor\fs_sanitize_redirect($location);
        if ($is_IIS) {
            \header("Refresh: 0;url={$location}");
        } else {
            if (\php_sapi_name() != 'cgi-fcgi') {
                \WP2FA_Vendor\status_header($status);
            }
    ...
    if (!\function_exists('WP2FA_Vendor\\fs_sanitize_redirect')) {
        /**
        ...
        function fs_sanitize_redirect($location)
        {
            ...
            $location = \WP2FA_Vendor\fs_kses_no_null($location);
    ...
    if (!\function_exists('WP2FA_Vendor\\fs_kses_no_null')) {
        /**
...
if (!\function_exists('WP2FA_Vendor\\__fs')) {
    ...
    {
        \WP2FA_Vendor\_deprecated_function(__FUNCTION__, '2.0.0', 'fs_text()');
    ...
    {
        \WP2FA_Vendor\fs_echo($key, $slug);
    }
}
if (!\function_exists('WP2FA_Vendor\\fs_get_ip')) {
    /**
     * Get client IP.
     *
     * @author Vova Feldman (@svovaf)
    ...
    function fs_get_ip()
    {
        $fields = array('HTTP_CF_CONNECTING_IP', 'HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR');
...
function fs_find_caller_plugin_file()
{
    ...
    for ($i = 1, $bt = \debug_backtrace(), $len = \count($bt); $i < $len; $i++) {
        ...
        if (\in_array(\WP2FA_Vendor\fs_normalize_path($bt[$i]['file']), $all_plugins_paths)) {
    ...
    if (\is_null($plugin_file)) {
        ...
        \wp_die('Freemius SDK couldn\'t find the plugin\'s main file. Please contact sdk@freemius.com with the current error.', 'Error', array('back_link' => \true));
    }
    return $plugin_file;
}
...
function fs_update_sdk_newest_version($sdk_relative_path, $plugin_file = \false)
{
    ...
    if (!\is_string($plugin_file)) {
        $plugin_file = \WP2FA_Vendor\plugin_basename(\WP2FA_Vendor\fs_find_caller_plugin_file());
    }
    if (!isset($newest_sdk->type) || 'theme' !== $newest_sdk->type) {
        if (!\function_exists('WP2FA_Vendor\\is_plugin_active')) {
            require_once \ABSPATH . 'wp-admin/includes/plugin.php';
        }
        $in_activation = !\WP2FA_Vendor\is_plugin_active($plugin_file);
    } else {
        $theme = \WP2FA_Vendor\wp_get_theme();
        $in_activation = $newest_sdk->plugin_path == $theme->stylesheet;
    }
    $fs_active_plugins->newest = (object) array('plugin_path' => $plugin_file, 'sdk_path' => $sdk_relative_path, 'version' => $newest_sdk->version, 'in_activation' => $in_activation, 'timestamp' => \time());
...
function fs_newest_sdk_plugin_first()
{
    ...
    $active_plugins = \WP2FA_Vendor\get_option('active_plugins', array());
    ...
    if ($plugin_found) {
        \WP2FA_Vendor\update_option('active_plugins', $updated_active_plugins);
        return \true;
    }
    if (\is_multisite()) {
        // Plugin is network active.
        $network_active_plugins = \WP2FA_Vendor\get_site_option('active_sitewide_plugins', array());
...
function fs_fallback_to_newest_active_sdk()
{
    ...
    foreach ($fs_active_plugins->plugins as $sdk_relative_path => $data) {
        if (\is_null($newest_sdk_data) || \version_compare($data->version, $newest_sdk_data->version, '>')) {
            ...
            if ('plugin' === $data->type) {
                $is_module_active = \WP2FA_Vendor\is_plugin_active($data->plugin_path);
            } else {
                $active_theme = \WP2FA_Vendor\wp_get_theme();
                $is_module_active = $data->plugin_path === $active_theme->get_template();
            }
            $is_sdk_exists = \file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $sdk_relative_path . '/start.php'));
    ...
    if (\is_null($newest_sdk_data)) {
        ...
        \WP2FA_Vendor\fs_update_sdk_newest_version($newest_sdk_path, $newest_sdk_data->plugin_path);
    }
}
