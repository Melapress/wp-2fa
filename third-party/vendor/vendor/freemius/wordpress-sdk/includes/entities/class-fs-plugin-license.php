 ...
 */
class FS_Plugin_License extends \WP2FA_Vendor\FS_Entity
{
    #region Properties
    /**
     * @var number
    ...
    function is_expired()
    {
        return !$this->is_lifetime() && \strtotime($this->expiration) < \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME;
    }
    /**
    ...
    function is_utilized($is_localhost = null)
    {
        if (\is_null($is_localhost)) {
            $is_localhost = \WP2FA_Vendor\WP_FS__IS_LOCALHOST_FOR_SERVER;
        }
        if ($this->is_unlimited()) {
            return \false;
        }
    ...
    {
        return \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC >= \strtotime($this->expiration) - \strtotime($this->created);
    }
    /**
     * @return int
     */
    ...
    {
        return \htmlspecialchars(\substr($secret_key, 0, 6)) . \str_pad('', (\strlen($secret_key) - 9) * 6, '&bull;') . \htmlspecialchars(\substr($secret_key, -3));
    }
}
