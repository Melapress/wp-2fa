...
class FS_Payment extends \WP2FA_Vendor\FS_Entity
{
    #region Properties
    /**
     * @var number
    ...
    {
        return ($this->gross < 0 ? '-' : '') . $this->get_symbol() . \number_format(\abs($this->gross), 2, '.', ',') . ' ' . \strtoupper($this->currency);
    }
    /**
    ...
    private function get_symbol()
    {
        if (!isset(self::$CURRENCY_2_SYMBOL)) {
            // Lazy load.
            self::$CURRENCY_2_SYMBOL = array(self::CURRENCY_USD => '$', self::CURRENCY_GBP => '&pound;', self::CURRENCY_EUR => '&euro;');
        }
