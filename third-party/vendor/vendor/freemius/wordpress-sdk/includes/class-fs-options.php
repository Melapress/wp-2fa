...
class FS_Options
{
    ...
    static function instance($id, $load = \false)
    {
        if (!isset(self::$_instances[$id])) {
            self::$_instances[$id] = new \WP2FA_Vendor\FS_Options($id, $load);
        }
    ...
    private function __construct($id, $load = \false)
    {
        ...
        if ($this->_is_multisite) {
            ...
            $this->_network_options = \WP2FA_Vendor\FS_Option_Manager::get_manager($id, $load, \true);
        }
        $this->_options = \WP2FA_Vendor\FS_Option_Manager::get_manager($id, $load, $this->_blog_id);
    }
    /**
    ...
    function set_site_blog_context($blog_id)
    {
        ...
        $this->_options = \WP2FA_Vendor\FS_Option_Manager::get_manager($this->_id, \false, $this->_blog_id);
    }
    /**
     * @author Leo Fajardo (@leorw)
     *
    ...
    {
        if (!$this->_is_multisite || \false === $network_level_or_blog_id || 0 == $network_level_or_blog_id || \is_null($network_level_or_blog_id)) {
            $site_options = $this->get_site_options($network_level_or_blog_id);
            $site_options->store();
        }
        if ($this->_is_multisite && (\is_null($network_level_or_blog_id) || \true === $network_level_or_blog_id)) {
    ...
    function migrate_to_network($blog_id = 0)
    {
        ...
        foreach ($keys as $option) {
            if ($this->is_site_option($option) || \in_array($option, array(
                ...
                if (\is_array($network_option) && \is_array($site_option)) {
                    ...
                    foreach ($site_option as $key => $value) {
                        if (!isset($network_option[$key])) {
                            ...
                            if (\is_array($network_option[$key]) && \is_array($value)) {
                                if (empty($network_option[$key])) {
                                    ...
                                    if (empty($value)) {
                                        ...
                                        \reset($value);
                                        $first_key = \key($value);
                                        if ($value[$first_key] instanceof \WP2FA_Vendor\FS_Entity) {
                                            // Merge entities by IDs.
                                            $network_entities_ids = array();
                                            foreach ($network_option[$key] as $entity) {
                                                $network_entities_ids[$entity->id] = \true;
                                            }
    ...
    {
        self::$_SITE_OPTIONS_MAP = array('sites' => \true, 'theme_sites' => \true, 'unique_id' => \true, 'active_plugins' => \true);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  2.0.0
    ...
    {
        if (\WP2FA_Vendor\WP_FS__ACCOUNTS_OPTION_NAME != $this->_id) {
            return \false;
        }
    ...
    private function get_site_options($blog_id = 0)
    {
        ...
        return \WP2FA_Vendor\FS_Option_Manager::get_manager($this->_id, \true, $blog_id);
    }
    /**
