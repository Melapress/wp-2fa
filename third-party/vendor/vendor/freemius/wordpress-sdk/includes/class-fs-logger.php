...
class FS_Logger
{
    ...
    public static function get_logger($id, $on = \false, $echo = \false)
    {
        ...
        if (!isset(self::$LOGGERS[$id])) {
            self::$LOGGERS[$id] = new \WP2FA_Vendor\FS_Logger($id, $on, $echo);
        }
    ...
    {
        self::$_ownerName = \function_exists('get_current_user') ? \get_current_user() : 'unknown';
        self::$_isStorageLoggingOn = 1 == \WP2FA_Vendor\get_option('fs_storage_logger', 0);
    ...
    {
        $this->_on = \true;
        if (!\function_exists('WP2FA_Vendor\\dbDelta')) {
    ...
    private function _log(&$message, $type, $wrapper = \false)
    {
        ...
        $log = \array_merge($caller, array('cnt' => self::$CNT++, 'logger' => $this, 'timestamp' => \microtime(\true), 'log_type' => $type, 'msg' => $message));
        if (self::$_isStorageLoggingOn) {
            $this->db_log($type, $message, self::$CNT, $caller);
        }
        self::$LOG[] = $log;
        if ($this->is_echo_on() && !\WP2FA_Vendor\Freemius::is_ajax()) {
    ...
    {
        $message = '';
        if (\is_object($api_result) && !empty($api_result->error) && !empty($api_result->error->message)) {
            $message = $api_result->error->message;
        } else {
            if (\is_object($api_result)) {
                $message = \var_export($api_result, \true);
            } else {
    ...
    public static function _set_storage_logging($is_on = \true)
    {
        ...
        if (\false !== $result) {
            \WP2FA_Vendor\update_option('fs_storage_logger', $is_on ? 1 : 0);
        }
        return \false !== $result;
    }
    /**
    ...
    private function db_log(&$type, &$message, &$log_order, &$caller)
    {
        global $wpdb;
        $request_type = 'call';
        if (\defined('WP2FA_Vendor\\DOING_CRON') && \WP2FA_Vendor\DOING_CRON) {
            $request_type = 'cron';
        } else {
            if (\defined('WP2FA_Vendor\\DOING_AJAX') && \WP2FA_Vendor\DOING_AJAX) {
                $request_type = 'ajax';
            }
        }
        $request_url = \WP2FA_Vendor\WP_FS__IS_HTTP_REQUEST ? $_SERVER['REQUEST_URI'] : '';
        return $wpdb->insert("{$wpdb->prefix}fs_logger", array('process_id' => self::$_processID, 'user_name' => self::$_ownerName, 'logger' => $this->_id, 'log_order' => $log_order, 'type' => $type, 'request_type' => $request_type, 'request_url' => $request_url, 'message' => $message, 'file' => isset($caller['file']) ? \substr($caller['file'], self::$_abspathLength) : '', 'line' => $caller['line'], 'function' => (!empty($caller['class']) ? $caller['class'] . $caller['type'] : '') . $caller['function'], 'created' => \microtime(\true)));
    }
    /**
    ...
    private static function build_db_logs_query($filters = \false, $limit = 200, $offset = 0, $order = \false, $escape_eol = \false)
    {
        ...
        if (\is_array($filters)) {
            ...
            if (!empty($filters['file'])) {
                $criteria[] = array('col' => 'file', 'op' => 'LIKE', 'val' => '%' . \WP2FA_Vendor\esc_sql($filters['file']));
            }
            if (!empty($filters['function'])) {
                $criteria[] = array('col' => 'function', 'op' => 'LIKE', 'val' => '%' . \WP2FA_Vendor\esc_sql($filters['function']));
            }
            ...
            if (!empty($filters['logger'])) {
                $criteria[] = array('col' => 'logger', 'op' => 'LIKE', 'val' => '%' . \WP2FA_Vendor\esc_sql($filters['logger']) . '%');
            }
            if (!empty($filters['message'])) {
                $criteria[] = array('col' => 'message', 'op' => 'LIKE', 'val' => '%' . \WP2FA_Vendor\esc_sql($filters['message']) . '%');
            }
            if (0 < \count($criteria)) {
                $query .= "\nWHERE\n";
                $first = \true;
                foreach ($criteria as $c) {
                    ...
                    if (\is_array($c['val'])) {
                        ...
                        for ($i = 0, $len = \count($c['val']); $i < $len; $i++) {
                            $c['val'][$i] = "'" . \WP2FA_Vendor\esc_sql($c['val'][$i]) . "'";
                        }
                        $val = '(' . \implode(',', $c['val']) . ')';
                    } else {
                        $operator = !empty($c['op']) ? $c['op'] : '=';
                        $val = "'" . \WP2FA_Vendor\esc_sql($c['val']) . "'";
                    }
    ...
    {
        global $wpdb;
        $query = self::build_db_logs_query($filters, $limit, $offset, $order);
    ...
    {
        global $wpdb;
        $query = self::build_db_logs_query($filters, $limit, $offset, $order, \true);
        $upload_dir = \WP2FA_Vendor\wp_upload_dir();
        if (empty($filename)) {
            $filename = 'fs-logs-' . \date('Y-m-d_H-i-s', \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME) . '.csv';
        }
