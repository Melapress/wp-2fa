...
class FS_Api
{
    ...
    static function instance($slug, $scope, $id, $public_key, $is_sandbox, $secret_key = \false, $sdk_version = null)
    {
        ...
        if (!isset(self::$_instances[$identifier])) {
            self::_init();
            self::$_instances[$identifier] = new \WP2FA_Vendor\FS_Api($slug, $scope, $id, $public_key, $secret_key, $is_sandbox, $sdk_version);
        }
    ...
    private static function _init()
    {
        ...
        if (!\class_exists('WP2FA_Vendor\\Freemius_Api_WordPress')) {
            require_once \WP2FA_Vendor\WP_FS__DIR_SDK . '/FreemiusWordPress.php';
        }
        self::$_options = \WP2FA_Vendor\FS_Option_Manager::get_manager(\WP2FA_Vendor\WP_FS__OPTIONS_OPTION_NAME, \true, \true);
        self::$_cache = \WP2FA_Vendor\FS_Cache_Manager::get_manager(\WP2FA_Vendor\WP_FS__API_CACHE_OPTION_NAME);
        self::$_clock_diff = self::$_options->get_option('api_clock_diff', 0);
        \WP2FA_Vendor\Freemius_Api_WordPress::SetClockDiff(self::$_clock_diff);
        if (self::$_options->get_option('api_force_http', \false)) {
            \WP2FA_Vendor\Freemius_Api_WordPress::SetHttp();
        }
    }
    /**
     * @param string      $slug
    ...
    {
        $this->_api = new \WP2FA_Vendor\Freemius_Api_WordPress($scope, $id, $public_key, $secret_key, $is_sandbox);
        $this->_slug = $slug;
        $this->_sdk_version = $sdk_version;
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_' . $slug . '_api', \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
    }
    /**
    ...
    private function _sync_clock_diff($diff = \false)
    {
        ...
        $new_clock_diff = \false === $diff ? \WP2FA_Vendor\Freemius_Api_WordPress::FindClockDiff() : $diff;
        ...
        self::$_options->set_option('api_clock_diff', self::$_clock_diff, \true);
    ...
    private function _call($path, $method = 'GET', $params = array(), $retry = \false)
    {
        ...
        if (self::is_temporary_down()) {
            ...
            if (!empty($this->_sdk_version)) {
                if (\false === \strpos($path, 'sdk_version=') && !isset($params['sdk_version'])) {
            ...
            if (null !== $result && isset($result->error) && isset($result->error->code) && 'request_expired' === $result->error->code) {
                if (!$retry) {
                    $diff = isset($result->error->timestamp) ? \time() - \strtotime($result->error->timestamp) : \false;
                    // Try to sync clock diff.
                    if (\false !== $this->_sync_clock_diff($diff)) {
                        // Retry call with new synced clock.
                        return $this->_call($path, $method, $params, \true);
                    }
                }
            }
        }
     ...
     */
    function get($path = '/', $flush = \false, $expiration = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC)
    {
        $this->_logger->entrance($path);
        $cache_key = $this->get_cache_key($path);
        // Always flush during development.
        if (\WP2FA_Vendor\WP_FS__DEV_MODE || $this->_api->IsSandbox()) {
            $flush = \true;
        }
        ...
        if ($flush || !self::$_cache->has_valid($cache_key, $expiration)) {
            ...
            if (!\is_object($result) || isset($result->error)) {
                ...
                if (\is_object($cached_result) && !isset($cached_result->error)) {
                    // If there was an error during a newer data fetch,
                    // fallback to older data version.
                    $result = $cached_result;
                    if ($this->_logger->is_on()) {
                        $this->_logger->warn('Fallback to cached API result: ' . \var_export($cached_result, \true));
                    }
                } else {
     ...
     * @param array  $params
     */
    function update_cache_expiration($path, $expiration = \WP2FA_Vendor\WP_FS__TIME_24_HOURS_IN_SEC, $method = 'GET', $params = array())
    {
    ...
    static function test()
    {
        ...
        if (\is_null($test)) {
            $test = \WP2FA_Vendor\Freemius_Api_WordPress::Test();
            if (\false === $test && \WP2FA_Vendor\Freemius_Api_WordPress::IsHttps()) {
                // Fallback to HTTP, since HTTPS fails.
                \WP2FA_Vendor\Freemius_Api_WordPress::SetHttp();
                self::$_options->set_option('api_force_http', \true, \true);
                $test = \WP2FA_Vendor\Freemius_Api_WordPress::Test();
                if (\false === $test) {
                    /**
                     ...
                     * @since 1.1.6
                     */
                    self::$_options->set_option('api_force_http', \false, \true);
                }
            }
            self::$_cache->set($cache_key, $test, \WP2FA_Vendor\WP_FS__TIME_5_MIN_IN_SEC);
        }
        return $test;
    }
    /**
    ...
    {
        return (object) array('error' => (object) array('type' => 'TemporaryUnavailable', 'message' => 'API is temporary unavailable, please retry in ' . (self::$_cache->get_record_expiration('ping_test') - \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME) . ' sec.', 'code' => 'temporary_unavailable', 'http' => 503));
    }
    /**
    ...
    function ping($unique_anonymous_id = null, $params = array())
    {
        ...
        $pong = \is_null($unique_anonymous_id) ? \WP2FA_Vendor\Freemius_Api_WordPress::Ping() : $this->_call('ping.json?' . \http_build_query(\array_merge(array('uid' => $unique_anonymous_id), $params)));
    ...
    {
        if (!\WP2FA_Vendor\Freemius_Api_WordPress::IsHttps()) {
            return \false;
        }
        return !\is_object($result) || !isset($result->error) || !isset($result->error->code) || !\in_array($result->error->code, array('curl_missing', 'cloudflare_ddos_protection', 'maintenance_mode', 'squid_cache_block', 'too_many_requests'));
    }
    /**
    ...
    {
        return \WP2FA_Vendor\Freemius_Api_WordPress::Test($pong);
    }
    function get_url($path = '')
    {
        return \WP2FA_Vendor\Freemius_Api_WordPress::GetUrl($path, $this->_api->IsSandbox());
    }
    /**
     * Clear API cache.
     *
    ...
    {
        return self::is_api_result_object($result, 'id') && \WP2FA_Vendor\FS_Entity::is_valid_id($result->id);
    }
    /**
