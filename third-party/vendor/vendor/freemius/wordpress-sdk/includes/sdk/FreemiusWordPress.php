...
if (!\defined('WP2FA_Vendor\\FS_SDK__USER_AGENT')) {
    \define('WP2FA_Vendor\\FS_SDK__USER_AGENT', 'fs-php-' . \WP2FA_Vendor\Freemius_Api_Base::VERSION);
}
if (!\defined('WP2FA_Vendor\\FS_SDK__SIMULATE_NO_CURL')) {
    \define('WP2FA_Vendor\\FS_SDK__SIMULATE_NO_CURL', \false);
}
if (!\defined('WP2FA_Vendor\\FS_SDK__SIMULATE_NO_API_CONNECTIVITY_CLOUDFLARE')) {
    \define('WP2FA_Vendor\\FS_SDK__SIMULATE_NO_API_CONNECTIVITY_CLOUDFLARE', \false);
}
if (!\defined('WP2FA_Vendor\\FS_SDK__SIMULATE_NO_API_CONNECTIVITY_SQUID_ACL')) {
    \define('WP2FA_Vendor\\FS_SDK__SIMULATE_NO_API_CONNECTIVITY_SQUID_ACL', \false);
}
if (!\defined('WP2FA_Vendor\\FS_SDK__HAS_CURL')) {
    if (\WP2FA_Vendor\FS_SDK__SIMULATE_NO_CURL) {
        \define('WP2FA_Vendor\\FS_SDK__HAS_CURL', \false);
    } else {
        $curl_required_methods = array('curl_version', 'curl_exec', 'curl_init', 'curl_close', 'curl_setopt', 'curl_setopt_array', 'curl_error');
        $has_curl = \true;
        ...
        \define('WP2FA_Vendor\\FS_SDK__HAS_CURL', $has_curl);
    }
}
if (!\defined('WP2FA_Vendor\\FS_SDK__SSLVERIFY')) {
    \define('WP2FA_Vendor\\FS_SDK__SSLVERIFY', \false);
}
$curl_version = \WP2FA_Vendor\FS_SDK__HAS_CURL ? \curl_version() : array('version' => '7.37');
if (!\defined('WP2FA_Vendor\\FS_API__PROTOCOL')) {
    \define('WP2FA_Vendor\\FS_API__PROTOCOL', \version_compare($curl_version['version'], '7.37', '>=') ? 'https' : 'http');
}
if (!\defined('WP2FA_Vendor\\FS_API__LOGGER_ON')) {
    \define('WP2FA_Vendor\\FS_API__LOGGER_ON', \false);
}
...
class Freemius_Api_WordPress extends \WP2FA_Vendor\Freemius_Api_Base
{
    ...
    {
        $address = $pIsSandbox ? \WP2FA_Vendor\FS_API__SANDBOX_ADDRESS : \WP2FA_Vendor\FS_API__ADDRESS;
        if (':' === $address[0]) {
     ...
     */
    private static $_protocol = \WP2FA_Vendor\FS_API__PROTOCOL;
    /**
    ...
    function GenerateAuthorizationParams($pResourceUrl, $pMethod = 'GET', $pPostParams = '')
    {
        ...
        $string_to_sign = \implode($eol, array($pMethod, $content_md5, $content_type, $date, $pResourceUrl));
        // If secret and public keys are identical, it means that
        // the signature uses public key hash encoding.
        $auth_type = $this->_secret !== $this->_public ? 'FS' : 'FSP';
        $auth = array('date' => $date, 'authorization' => $auth_type . ' ' . $this->_id . ':' . $this->_public . ':' . self::Base64UrlEncode(\hash_hmac('sha256', $string_to_sign, $this->_secret)));
    ...
    function GetSignedUrl($pPath)
    {
        ...
        return \WP2FA_Vendor\Freemius_Api_WordPress::GetUrl($pResourceUrl . '?' . (1 < \count($resource) && !empty($resource[1]) ? $resource[1] . '&' : '') . 'authorization=' . \urlencode($auth['authorization']) . '&auth_date=' . \urlencode($auth['date']), $this->_isSandbox);
    }
    /**
     * @author Vova Feldman
     *
    ...
    {
        $start = \microtime(\true);
        $response = \WP2FA_Vendor\wp_remote_request($pUrl, $pWPRemoteArgs);
        if (\WP2FA_Vendor\FS_API__LOGGER_ON) {
            ...
            self::$_logger[] = array('id' => \count(self::$_logger), 'start' => $start, 'end' => $end, 'total' => $end - $start, 'method' => $pWPRemoteArgs['method'], 'path' => $pUrl, 'body' => $has_body ? $pWPRemoteArgs['body'] : null, 'result' => !$is_http_error ? $response['body'] : \json_encode($response->get_error_messages()), 'code' => !$is_http_error ? $response['response']['code'] : null, 'backtrace' => \debug_backtrace());
        }
        return $response;
    }
    /**
     ...
     * @throws \Freemius_Exception
     */
    private static function MakeStaticRequest($pCanonizedPath, $pMethod = 'GET', $pParams = array(), $pWPRemoteArgs = null, $pIsSandbox = \false, $pBeforeExecutionFunction = null)
    {
        // Connectivity errors simulation.
        if (\WP2FA_Vendor\FS_SDK__SIMULATE_NO_API_CONNECTIVITY_CLOUDFLARE) {
            self::ThrowCloudFlareDDoSException();
        } else {
            if (\WP2FA_Vendor\FS_SDK__SIMULATE_NO_API_CONNECTIVITY_SQUID_ACL) {
                self::ThrowSquidAclException();
            }
        }
        if (empty($pWPRemoteArgs)) {
            $user_agent = 'Freemius/WordPress-SDK/' . \WP2FA_Vendor\Freemius_Api_Base::VERSION . '; ' . \home_url();
            $pWPRemoteArgs = array('method' => \strtoupper($pMethod), 'connect_timeout' => 10, 'timeout' => 60, 'follow_redirects' => \true, 'redirection' => 5, 'user-agent' => $user_agent, 'blocking' => \true);
        }
        ...
        if (\WP2FA_Vendor\FS_SDK__HAS_CURL) {
            // Disable the 'Expect: 100-continue' behaviour. This causes cURL to wait
            // for 2 seconds if the server does not support this header.
            $pWPRemoteArgs['headers']['Expect'] = '';
        }
        if ('https' === \substr(\strtolower($request_url), 0, 5)) {
            $pWPRemoteArgs['sslverify'] = \WP2FA_Vendor\FS_SDK__SSLVERIFY;
        }
        ...
        if (\is_null($decoded)) {
            if (\preg_match('/Please turn JavaScript on/i', $response_body) && \preg_match('/text\\/javascript/', $response_body)) {
                ...
                if (\preg_match('/Access control configuration prevents your request from being allowed at this time. Please contact your service provider if you feel this is incorrect./', $response_body) && \preg_match('/squid/', $response_body)) {
                    self::ThrowSquidAclException($response_body);
                } else {
                    $decoded = (object) array('error' => (object) array('type' => 'Unknown', 'message' => $response_body, 'code' => 'unknown', 'http' => 402));
                }
            }
        }
        return $decoded;
    ...
    public function MakeRequest($pCanonizedPath, $pMethod = 'GET', $pParams = array(), $pWPRemoteArgs = null)
    {
        ...
        return self::MakeStaticRequest($pCanonizedPath, $pMethod, $pParams, $pWPRemoteArgs, $this->_isSandbox, $sign_request ? array(&$this, 'SignRequest') : null);
    }
    /**
    ...
    {
        try {
            $result = self::MakeStaticRequest('/v' . \WP2FA_Vendor\FS_API__VERSION . '/ping.json');
        } catch (\WP2FA_Vendor\Freemius_Exception $e) {
            // Map to error object.
            $result = (object) $e->getResult();
        } catch (\Exception $e) {
            // Map to error object.
            $result = (object) array('error' => (object) array('type' => 'Unknown', 'message' => $e->getMessage() . ' (' . $e->getFile() . ': ' . $e->getLine() . ')', 'code' => 'unknown', 'http' => 402));
        }
        return $result;
    }
    #endregion
    ...
    private static function ThrowWPRemoteException(\WP_Error $pError)
    {
        if (self::IsCurlError($pError)) {
            ...
            if (!empty($missing_methods)) {
                throw new \WP2FA_Vendor\Freemius_Exception(array('error' => (object) array('type' => 'cUrlMissing', 'message' => $message, 'code' => 'curl_missing', 'http' => 402), 'missing_methods' => $missing_methods));
            }
            #endregion
            ...
            $e = new \WP2FA_Vendor\Freemius_Exception(array('error' => (object) array('code' => $code, 'message' => $message, 'type' => 'CurlException')));
        } else {
            $e = new \WP2FA_Vendor\Freemius_Exception(array('error' => (object) array('code' => $pError->get_error_code(), 'message' => $pError->get_error_message(), 'type' => 'WPRemoteException')));
        }
        throw $e;
    }
    /**
    ...
    {
        throw new \WP2FA_Vendor\Freemius_Exception(array('error' => (object) array('type' => 'CloudFlareDDoSProtection', 'message' => $pResult, 'code' => 'cloudflare_ddos_protection', 'http' => 402)));
    }
    /**
     * @param string $pResult
     *
    ...
    {
        throw new \WP2FA_Vendor\Freemius_Exception(array('error' => (object) array('type' => 'SquidCacheBlock', 'message' => $pResult, 'code' => 'squid_cache_block', 'http' => 402)));
    }
    #endregion
}
