 ...
 * under the License.
 */
if (!\defined('WP2FA_Vendor\\FS_API__VERSION')) {
    \define('WP2FA_Vendor\\FS_API__VERSION', '1');
}
if (!\defined('WP2FA_Vendor\\FS_SDK__PATH')) {
    \define('WP2FA_Vendor\\FS_SDK__PATH', \dirname(__FILE__));
}
if (!\defined('WP2FA_Vendor\\FS_SDK__EXCEPTIONS_PATH')) {
    \define('WP2FA_Vendor\\FS_SDK__EXCEPTIONS_PATH', \WP2FA_Vendor\FS_SDK__PATH . '/Exceptions/');
}
if (!\function_exists('json_decode')) {
    throw new \Exception('Freemius needs the JSON PHP extension.');
}
// Include all exception files.
$exceptions = array('Exception', 'InvalidArgumentException', 'ArgumentNotExistException', 'EmptyArgumentException', 'OAuthException');
foreach ($exceptions as $e) {
    require_once \WP2FA_Vendor\FS_SDK__EXCEPTIONS_PATH . $e . '.php';
}
if (\class_exists('WP2FA_Vendor\\Freemius_Api_Base')) {
    return;
}
    ...
    function CanonizePath($pPath)
    {
        ...
        switch ($this->_scope) {
            ...
            case 'install':
                ...
                break;
            default:
                throw new \WP2FA_Vendor\Freemius_Exception('Scope not implemented.');
        }
        return '/v' . \WP2FA_Vendor\FS_API__VERSION . $base . (!empty($pPath) ? '/' : '') . $pPath . (\false === \strpos($pPath, '.') ? '.' . self::FORMAT : '') . $query;
    }
