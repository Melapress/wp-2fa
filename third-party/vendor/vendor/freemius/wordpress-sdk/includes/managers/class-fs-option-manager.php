...
class FS_Option_Manager
{
     ...
     */
    private function __construct($id, $load = \false, $network_level_or_blog_id = \false, $autoload = null)
    {
        $id = \strtolower($id);
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_opt_mngr_' . $id, \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
        $this->_logger->entrance();
        $this->_logger->log('id = ' . $id);
        $this->_id = $id;
        $this->_autoload = $autoload;
        if (\is_multisite()) {
            $this->_is_network_storage = \true === $network_level_or_blog_id;
     ...
     */
    static function get_manager($id, $load = \false, $network_level_or_blog_id = \false, $autoload = null)
    {
        $key = \strtolower($id);
        if (\is_multisite()) {
        ...
        if (!isset(self::$_MANAGERS[$key])) {
            self::$_MANAGERS[$key] = new \WP2FA_Vendor\FS_Option_Manager($id, $load, $network_level_or_blog_id, $autoload);
        } else {
            if ($load && !self::$_MANAGERS[$key]->is_loaded()) {
    ...
    function load($flush = \false)
    {
        ...
        if ($flush || !isset($this->_options)) {
            ...
            if (\WP2FA_Vendor\WP_FS__DEBUG_SDK) {
                ...
                $this->_options = \wp_cache_get($option_name, $cache_group);
                $load_options = \false === $this->_options;
            }
            $cached = \true;
            if ($load_options) {
                if ($this->_is_network_storage) {
                    $this->_options = \WP2FA_Vendor\get_site_option($option_name);
                } else {
                    if ($this->_blog_id > 0) {
                        $this->_options = \WP2FA_Vendor\get_blog_option($this->_blog_id, $option_name);
                    } else {
                        $this->_options = \WP2FA_Vendor\get_option($option_name);
                    }
                }
            ...
            if (!\WP2FA_Vendor\WP_FS__DEBUG_SDK && !$cached) {
    ...
    function delete()
    {
        ...
        if ($this->_is_network_storage) {
            ...
            if ($this->_blog_id > 0) {
                \WP2FA_Vendor\delete_blog_option($this->_blog_id, $option_name);
            } else {
                \WP2FA_Vendor\delete_option($option_name);
            }
        }
    }
    /**
    ...
    function get_option($option, $default = null)
    {
        ...
        if (\is_array($this->_options)) {
            $value = isset($this->_options[$option]) ? $this->_options[$option] : $default;
        } else {
    ...
    function store()
    {
        ...
        if ($this->_logger->is_on()) {
            $this->_logger->info($option_name . ' = ' . \var_export($this->_options, \true));
        }
        // Update DB.
        if ($this->_is_network_storage) {
            ...
            if ($this->_blog_id > 0) {
                \WP2FA_Vendor\update_blog_option($this->_blog_id, $option_name, $this->_options);
            } else {
                \WP2FA_Vendor\update_option($option_name, $this->_options, $this->_autoload);
            }
        }
        if (!\WP2FA_Vendor\WP_FS__DEBUG_SDK) {
    ...
    {
        $site_options = \WP2FA_Vendor\FS_Option_Manager::get_manager($this->_id, \true, \false);
        $options = \is_object($site_options->_options) ? \get_object_vars($site_options->_options) : $site_options->_options;
        if (!empty($options)) {
    ...
    {
        $group = \WP2FA_Vendor\WP_FS__SLUG;
        if ($this->_is_network_storage) {
            $group .= '_ms';
        } else {
            if ($this->_blog_id > 0) {
