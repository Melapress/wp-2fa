...
class FS_Admin_Notice_Manager
{
        ...
        if (\is_multisite()) {
            if (\true === $network_level_or_blog_id) {
                ...
                if (\is_numeric($network_level_or_blog_id) && $network_level_or_blog_id > 0) {
                    ...
                    $network_level_or_blog_id = \WP2FA_Vendor\get_current_blog_id();
                    $key .= ":{$network_level_or_blog_id}";
                }
            }
        }
        if (!isset(self::$_instances[$key])) {
            self::$_instances[$key] = new \WP2FA_Vendor\FS_Admin_Notice_Manager($id, $title, $module_unique_affix, $is_network_and_blog_admins, $network_level_or_blog_id);
        }
    ...
    {
        $this->_id = $id;
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_' . $this->_id . '_data', \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
        $this->_title = !empty($title) ? $title : '';
        $this->_module_unique_affix = $module_unique_affix;
        $this->_sticky_storage = \WP2FA_Vendor\FS_Key_Value_Storage::instance('admin_notices', $this->_id, $network_level_or_blog_id);
        if (\is_multisite()) {
            $this->_is_network_notices = \true === $network_level_or_blog_id;
        ...
        $is_blog_admin = \WP2FA_Vendor\fs_is_blog_admin();
        if ($this->_is_network_notices && $is_network_admin || !$this->_is_network_notices && $is_blog_admin || $is_network_and_blog_admins && ($is_network_admin || $is_blog_admin)) {
            if (0 < \count($this->_sticky_storage)) {
                ...
                \add_action("wp_ajax_fs_dismiss_notice_action_{$ajax_action_suffix}", array(&$this, 'dismiss_notice_ajax_callback'));
                foreach ($this->_sticky_storage as $msg) {
                    // Add admin notice.
                    $this->add($msg['message'], $msg['title'], $msg['type'], \true, $msg['id'], \false, isset($msg['wp_user_id']) ? $msg['wp_user_id'] : null, !empty($msg['plugin']) ? $msg['plugin'] : null, $is_network_and_blog_admins);
                }
            }
        }
    }
    ...
    {
        $params = array();
        \WP2FA_Vendor\fs_require_once_template('sticky-admin-notice-js.php', $params);
    }
    ...
    {
        if (\function_exists('WP2FA_Vendor\\current_user_can') && !\WP2FA_Vendor\current_user_can('manage_options')) {
        ...
        foreach ($this->_notices as $id => $msg) {
            if (isset($msg['wp_user_id']) && \is_numeric($msg['wp_user_id'])) {
                if (\WP2FA_Vendor\get_current_user_id() != $msg['wp_user_id']) {
                    continue;
                }
            }
            /**
             ...
             * @since 2.2.0
             */
            $show_notice = \call_user_func_array('fs_apply_filter', array($this->_module_unique_affix, 'show_admin_notice', $show_admin_notices, $msg));
            if (\true !== $show_notice) {
                continue;
            }
            \WP2FA_Vendor\fs_require_template('admin-notice.php', $msg);
            if ($msg['sticky']) {
    ...
    {
        if (\function_exists('WP2FA_Vendor\\is_gutenberg_page') && \WP2FA_Vendor\is_gutenberg_page()) {
            // The Gutenberg plugin is on.
            return \true;
        }
        $current_screen = \WP2FA_Vendor\get_current_screen();
     ...
     * @uses   add_action()
     */
    function add($message, $title = '', $type = 'success', $is_sticky = \false, $id = '', $store_if_sticky = \true, $wp_user_id = null, $plugin_title = null, $is_network_and_blog_admins = \false)
    {
        ...
        $message_object = array('message' => $message, 'title' => $title, 'type' => $type, 'sticky' => $is_sticky, 'id' => $id, 'manager_id' => $this->_id, 'plugin' => !\is_null($plugin_title) ? $plugin_title : $this->_title, 'wp_user_id' => $wp_user_id);
    ...
    function add_sticky($message, $id, $title = '', $type = 'success', $wp_user_id = null, $plugin_title = null, $is_network_and_blog_admins = \false)
    {
        if (!empty($this->_module_unique_affix)) {
            $message = \WP2FA_Vendor\fs_apply_filter($this->_module_unique_affix, "sticky_message_{$id}", $message);
            $title = \WP2FA_Vendor\fs_apply_filter($this->_module_unique_affix, "sticky_title_{$id}", $title);
        }
        $this->add($message, $title, $type, \true, $id, \true, $wp_user_id, $plugin_title, $is_network_and_blog_admins);
    }
    /**
    ...
    {
        return $this->_is_network_notices ? 'network_admin_notices' : 'admin_notices';
    }
    #endregion
}
