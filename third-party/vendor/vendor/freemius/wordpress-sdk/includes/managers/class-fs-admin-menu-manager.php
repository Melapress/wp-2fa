...
class FS_Admin_Menu_Manager
{
    ...
    static function instance($module_id, $module_type, $module_unique_affix)
    {
        ...
        if (!isset(self::$_instances[$key])) {
            self::$_instances[$key] = new \WP2FA_Vendor\FS_Admin_Menu_Manager($module_id, $module_type, $module_unique_affix);
        }
        return self::$_instances[$key];
    }
    protected function __construct($module_id, $module_type, $module_unique_affix)
    {
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_' . $module_id . '_admin_menu', \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
    ...
    function init($menu, $is_addon = \false)
    {
        ...
        $this->_network_menu_exists = !empty($menu['network']) && \true === $menu['network'];
        $this->_menu_slug = $this->_menu_exists ? $menu['slug'] : $this->_module_unique_affix;
        $this->_default_submenu_items = array();
        // @deprecated
        $this->_type = 'page';
        $this->_is_top_level = \true;
        ...
        if (isset($menu)) {
            if (!$is_addon) {
                $this->_default_submenu_items = array('contact' => $this->get_bool_option($menu, 'contact', \true), 'support' => $this->get_bool_option($menu, 'support', \true), 'affiliation' => $this->get_bool_option($menu, 'affiliation', \true), 'account' => $this->get_bool_option($menu, 'account', \true), 'pricing' => $this->get_bool_option($menu, 'pricing', \true), 'addons' => $this->get_bool_option($menu, 'addons', \true));
                // @deprecated
    ...
    function get_first_time_path($is_network = \false)
    {
        ...
        if ($is_network) {
            return \WP2FA_Vendor\network_admin_url($this->_first_time_path);
        } else {
    ...
    function is_submenu_item_visible($id, $default = \true, $ignore_menu_existence = \false)
    {
        ...
        return \WP2FA_Vendor\fs_apply_filter($this->_module_unique_affix, 'is_submenu_visible', $this->get_bool_option($this->_default_submenu_items, $id, $default), $id);
    }
    /**
    ...
    {
        return (\false === \strpos($this->_menu_slug, '.php?') ? $this->_menu_slug : $this->_module_unique_affix) . (empty($page) ? '' : '-' . $page);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.3
    ...
    function is_cpt()
    {
        return 0 === \strpos($this->_menu_slug, 'edit.php?post_type=') || 'cpt' === $this->_type;
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.3
    ...
    {
        if ('cpt' === $this->_type) {
            return \add_query_arg(array('post_type' => $this->_menu_slug), 'edit.php');
        }
    ...
    {
        return $this->has_custom_parent() ? $this->get_parent_slug() : $this->get_raw_slug();
    }
    /**
    ...
    function is_activation_page($show_opt_in_on_themes_page = \false)
    {
        if ($show_opt_in_on_themes_page) {
             ...
             * @since 1.2.2
             */
            return \WP2FA_Vendor\WP_FS__MODULE_TYPE_THEME === $this->_module_type && \WP2FA_Vendor\Freemius::is_themes_page() && \WP2FA_Vendor\fs_request_get_bool($this->_module_unique_affix . '_show_optin');
        }
        if ($this->_menu_exists && (\WP2FA_Vendor\fs_is_plugin_page($this->_menu_slug) || \WP2FA_Vendor\fs_is_plugin_page($this->_module_unique_affix))) {
            /**
    ...
    {
        global $submenu;
        $menu_slug = \WP2FA_Vendor\plugin_basename($menu_slug);
        $parent_slug = \WP2FA_Vendor\plugin_basename($parent_slug);
        ...
        $hookname = \WP2FA_Vendor\get_plugin_page_hookname($menu_slug, $parent_slug);
        \WP2FA_Vendor\remove_all_actions($hookname);
        // Attach new action.
    ...
    private function find_top_level_menu()
    {
        ...
        $hook_name = \WP2FA_Vendor\get_plugin_page_hookname($menu_slug, '');
    ...
    private function find_main_submenu()
    {
        ...
        $position = -1;
        $found_submenu = \false;
        $hook_name = \WP2FA_Vendor\get_plugin_page_hookname($submenu_slug, '');
        ...
        return array('menu' => $found_submenu, 'parent_slug' => $top_level_menu_slug, 'position' => $position, 'hook_name' => $hook_name);
    }
    /**
    ...
    function remove_menu_item($remove_top_level_menu = \false)
    {
        ...
        \WP2FA_Vendor\remove_all_actions($top_level_menu['hook_name']);
    ...
    function main_menu_url()
    {
        ...
        $parent_slug = isset($menu['parent_slug']) ? $menu['parent_slug'] : 'admin.php';
        return \admin_url($parent_slug . (\false === \strpos($parent_slug, '?') ? '?' : '&') . 'page=' . $menu['menu'][2]);
    }
    /**
     * @author Vova Feldman (@svovaf)
     * @since  1.1.4
    ...
    function override_menu_item($function)
    {
        ...
        if (!$this->is_top_level() || !$this->is_cpt()) {
            $menu_slug = \WP2FA_Vendor\plugin_basename($this->get_slug());
            $hookname = \WP2FA_Vendor\get_plugin_page_hookname($menu_slug, '');
            ...
            $hookname = self::add_page($found_menu['menu'][3], $found_menu['menu'][0], 'manage_options', $this->get_slug(), $function, $found_menu['menu'][6], $found_menu['position']);
        }
        return $hookname;
    }
    /**
    ...
    function add_counter_to_menu_item($counter = 1, $class = '')
    {
        ...
        if ($this->_is_top_level) {
            ...
            if (\false !== $found_menu) {
                // Override menu label.
                $menu_ref[$found_menu['position']][0] = \sprintf($mask, $found_menu['menu'][0], $class, $counter);
            }
        } else {
            $found_submenu = $this->find_main_submenu();
            if (\false !== $found_submenu) {
                // Override menu label.
                $submenu_ref[$found_submenu['parent_slug']][$found_submenu['position']][0] = \sprintf($mask, $found_submenu['menu'][0], $class, $counter);
            }
        }
    }
     ...
     */
    static function add_page($page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '', $position = null)
    {
        $fn = 'add_menu' . '_page';
        return $fn($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $position);
    }
    /**
     ...
     * @return string
     */
    function add_page_and_update($page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '', $position = null)
    {
        $this->_menu_slug = $menu_slug;
        $this->_is_top_level = \true;
        $this->_menu_exists = \true;
        $this->_network_menu_exists = \true;
        return self::add_page($page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $position);
    }
    /**
     * Add a submenu page.
     *
    ...
    function add_subpage_and_update($parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '')
    {
        ...
        $this->_menu_exists = \true;
        $this->_network_menu_exists = \true;
        return self::add_subpage($parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function);
    }
}
