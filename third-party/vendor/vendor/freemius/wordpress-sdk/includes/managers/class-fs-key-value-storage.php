...
class FS_Key_Value_Storage implements \ArrayAccess, \Iterator, \Countable
{
    ...
    static function instance($id, $secondary_id, $network_level_or_blog_id = \false)
    {
        ...
        if (!isset(self::$_instances[$key])) {
            self::$_instances[$key] = new \WP2FA_Vendor\FS_Key_Value_Storage($id, $secondary_id, $network_level_or_blog_id);
        }
        return self::$_instances[$key];
    }
    protected function __construct($id, $secondary_id, $network_level_or_blog_id = \false)
    {
        $this->_logger = \WP2FA_Vendor\FS_Logger::get_logger(\WP2FA_Vendor\WP_FS__SLUG . '_' . $secondary_id . '_' . $id, \WP2FA_Vendor\WP_FS__DEBUG_SDK, \WP2FA_Vendor\WP_FS__ECHO_DEBUG_SDK);
        $this->_id = $id;
        $this->_secondary_id = $secondary_id;
        if (\is_multisite()) {
            $this->_is_multisite_storage = \true === $network_level_or_blog_id;
    ...
    {
        return \WP2FA_Vendor\FS_Option_Manager::get_manager(\WP2FA_Vendor\WP_FS__ACCOUNTS_OPTION_NAME, \true, $this->_is_multisite_storage ? \true : ($this->_blog_id > 0 ? $this->_blog_id : \false));
    }
    ...
    function load()
    {
        ...
        $this->_data = isset($all_plugins_data[$this->_secondary_id]) ? $all_plugins_data[$this->_secondary_id] : array();
    }
    /**
     * @author   Vova Feldman (@svovaf)
     * @since    1.0.7
    ...
    function store($key, $value, $flush = \true)
    {
        if ($this->_logger->is_on()) {
            $this->_logger->entrance($key . ' = ' . \var_export($value, \true));
        }
    ...
    function clear_all($store = \true, $exceptions = array())
    {
        ...
        if ($store) {
            ...
            $all_data[$this->_secondary_id] = $this->_data;
            $options_manager = $this->get_option_manager();
            $options_manager->set_option($this->_id, $all_data, \true);
        }
    }
    /**
