 ...
 */
class FS_Customizer_Upsell_Control extends \WP2FA_Vendor\WP_Customize_Control
{
    /**
     * Control type
     *
     ...
     */
    public function __construct(\WP2FA_Vendor\WP_Customize_Manager $manager, $id, array $args)
    {
    ...
    {
        \WP2FA_Vendor\fs_enqueue_local_style('fs_customizer', 'customizer.css');
    }
    /**
     * Json conversion
     */
    ...
    {
        $pricing_cta = \esc_html($this->fs->get_pricing_cta_label()) . '&nbsp;&nbsp;' . (\WP2FA_Vendor\is_rtl() ? '&#x2190;' : '&#x27a4;');
        parent::to_json();
        $this->json['button_text'] = $pricing_cta;
        $this->json['button_url'] = $this->fs->is_in_trial_promotion() ? $this->fs->get_trial_url() : $this->fs->get_upgrade_url();
        $api = \WP2FA_Vendor\FS_Plugin::is_valid_id($this->fs->get_bundle_id()) ? $this->fs->get_api_bundle_scope() : $this->fs->get_api_plugin_scope();
        // Load features.
        ...
        if ($this->fs->is_api_result_object($pricing, 'plans')) {
            ...
            if (\is_array($pricing->plans) && 0 < \count($pricing->plans)) {
                $support_features = array('kb' => 'Help Center', 'forum' => 'Support Forum', 'email' => 'Priority Email Support', 'phone' => 'Phone Support', 'skype' => 'Skype Support', 'is_success_manager' => 'Personal Success Manager');
                for ($i = 0, $len = \count($pricing->plans); $i < $len; $i++) {
                    if ('free' == $pricing->plans[$i]->name) {
                        continue;
                    }
                    if (!isset($pricing->plans[$i]->features) || !\is_array($pricing->plans[$i]->features)) {
                        $pricing->plans[$i]->features = array();
                    }
                    foreach ($support_features as $key => $label) {
                        $key = 'is_success_manager' !== $key ? "support_{$key}" : $key;
        ...
        $this->json['strings'] = array('plan' => $this->fs->get_text_x_inline('Plan', 'as product pricing plan', 'plan'));
    }
    /**
     * Control content
     */
