 ...
 * @access public
 */
class FS_Customizer_Support_Section extends \WP2FA_Vendor\WP_Customize_Section
{
    ...
    public function json()
    {
        ...
        if ($is_contact_visible) {
            $json['contact'] = array('label' => $this->fs->get_text_inline('Contact Us', 'contact-us'), 'url' => $this->fs->contact_url());
        }
        if ($is_support_visible) {
            $json['support'] = array('label' => $this->fs->get_text_inline('Support Forum', 'support-forum'), 'url' => $this->fs->get_support_forum_url());
        }
        return $json;
    }
    /**
