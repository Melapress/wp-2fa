...
foreach ($plans as $plan) {
    ...
    if (!empty($plan->support_email) || !empty($plan->support_skype) || !empty($plan->support_phone) || \true === $plan->is_success_manager) {
        if (!isset($features_plan_map['support'])) {
            $support_feature = new \stdClass();
            $support_feature->id = 'support';
            $support_feature->title = \WP2FA_Vendor\fs_text_inline('Support', $plugin->slug);
...
$updates_feature->title = \WP2FA_Vendor\fs_text_inline('Unlimited Updates', 'unlimited-updates', $plugin->slug);
...
foreach ($plans as $plan) {
    ...
    if (empty($plan->pricing)) {
        \WP2FA_Vendor\fs_esc_html_echo_inline('Free', 'free', $plugin->slug);
    } else {
        foreach ($plan->pricing as $pricing) {
            ...
            if (1 == $pricing->licenses) {
                if ($pricing->has_annual()) {
                    echo "\${$pricing->annual_price} / " . \WP2FA_Vendor\fs_esc_html_x_inline('year', 'as annual period', 'year', $plugin->slug);
                } else {
                    if ($pricing->has_monthly()) {
                        echo "\${$pricing->monthly_price} / " . \WP2FA_Vendor\fs_esc_html_x_inline('mo', 'as monthly period', 'mo', $plugin->slug);
                    } else {
...
foreach ($features_plan_map as $feature_id => $data) {
    ...
    ?>">
					<td><?php 
    echo \esc_html(\ucfirst($data['feature']->title));
    ?></td>
					<?php 
    foreach ($plans as $plan) {
        ?>
						<td>
							<?php 
        if (isset($data['plans'][$plan->id])) {
            ?>
								<?php 
            if (!empty($data['plans'][$plan->id]->value)) {
                ?>
									<b><?php 
                echo \esc_html($data['plans'][$plan->id]->value);
                ?></b>
								<?php 
            } else {
                ?>
