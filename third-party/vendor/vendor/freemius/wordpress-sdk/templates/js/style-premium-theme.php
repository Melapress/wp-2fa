...
?>-premium-name').parents('.theme');
			    ...
			    if (0 === $theme.find('.fs-premium-theme-badge-container').length) {
				    ...
				    $themeBadgeContainer.append( '<div class="fs-badge fs-premium-theme-badge">' + <?php 
echo \json_encode($fs->get_text_inline('Premium', 'premium'));
?> + '</div>' );

				    <?php 
if ($fs->is_beta()) {
    ?>
                    $themeBadgeContainer.append( '<div class="fs-badge fs-beta-theme-badge">' + <?php 
    echo \json_encode($fs->get_text_inline('Beta', 'beta'));
    ?> + '</div>' );
                    <?php 
}
?>
