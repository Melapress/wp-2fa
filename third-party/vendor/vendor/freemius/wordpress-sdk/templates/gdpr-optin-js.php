        ...
        $( document ).ready(function() {
            ...
            $gdprOptinNotice.on( 'click', '.button', function() {
                ...
                $.ajax({
                    url       : ajaxurl + '?' + $.param({
                        action   : '<?php 
echo $fs->get_ajax_action('gdpr_optin_action');
?>',
                        security : '<?php 
echo $fs->get_ajax_security('gdpr_optin_action');
?>',
                        module_id: '<?php 
echo $fs->get_id();
?>'
                    ...
                    beforeSend: function() {
                        $this.text( <?php 
\WP2FA_Vendor\fs_json_encode_echo_inline('Thanks, please wait', 'thanks-please-wait', $fs->get_slug());
?> + '...' );
