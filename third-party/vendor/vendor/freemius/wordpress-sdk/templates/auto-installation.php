...
if ($plugin_id != $fs->get_id()) {
    ...
    if (\is_object($addon)) {
        $title = $addon->title . ' ' . \WP2FA_Vendor\fs_text_inline('Add-On', 'addon', $slug);
    }
}
$plugin_title = \sprintf('<strong>%s</strong>', \esc_html($title));
$sec_countdown = 30;
$countdown_html = \sprintf(\WP2FA_Vendor\esc_js(
    /* translators: %s: Number of seconds */
    \WP2FA_Vendor\fs_text_inline('%s sec', 'x-sec', $slug)
...
$params = array();
$loader_html = \WP2FA_Vendor\fs_get_template('ajax-loader.php', $params);
// Pass unique auto installation URL if WP_Filesystem is needed.
$install_url = $fs->_get_sync_license_url($plugin_id, \true, array('auto_install' => 'true'));
\ob_start();
$method = '';
// Leave blank so WP_Filesystem can populate it as necessary.
$credentials = \WP2FA_Vendor\request_filesystem_credentials(\esc_url_raw($install_url), $method, \false, \WP2FA_Vendor\WP_PLUGIN_DIR, array());
...
<div class="fs-modal fs-modal-auto-install">
	<div class="fs-modal-dialog">
		<div class="fs-modal-header">
			<h4><?php 
echo \WP2FA_Vendor\esc_js(\WP2FA_Vendor\fs_text_inline('Automatic Installation', 'auto-installation', $slug));
?></h4>
		</div>
...
if ($require_credentials) {
    ...
    echo \sprintf(\WP2FA_Vendor\fs_esc_html_inline('An automated download and installation of %s (paid version) from %s will start in %s. If you would like to do it manually - click the cancellation button now.', 'installing-in-n', $slug), $plugin_title, \sprintf('<a href="%s" target="_blank" rel="noopener">%s</a>', 'https://freemius.com', 'freemius.com'), $countdown_html);
    ?></p>
			<?php 
}
?>
...
echo \sprintf(\WP2FA_Vendor\fs_esc_html_inline('The installation process has started and may take a few minutes to complete. Please wait until it is done - do not refresh this page.', 'installing-module-x', $slug), $plugin_title);
?></p>
		</div>
...
\WP2FA_Vendor\fs_esc_html_echo_inline('Cancel Installation', 'cancel-installation', $slug);
if (!$require_credentials) {
    ?> (<?php 
    echo $countdown_html;
    ?>)<?php 
	...
	(function ($) {
		$(document).ready(function () {
			var $modal             = $('.fs-modal-auto-install'),
			    ...
			    requireCredentials = <?php 
echo \json_encode($require_credentials);
?>,
			...
			var startAutoInstall = function () {
				...
				var data = {
					action          : '<?php 
echo $fs->get_ajax_action('install_premium_version');
?>',
					security        : '<?php 
echo $fs->get_ajax_security('install_premium_version');
?>',
					slug            : '<?php 
echo $slug;
?>',
...
\WP2FA_Vendor\fs_json_encode_echo_inline('Cancel Installation', 'cancel-installation', $slug);
?>);
