...
if ($fs->is_registered()) {
    $context_params = \array_merge($context_params, \WP2FA_Vendor\FS_Security::instance()->get_context_params($fs->get_site(), $timestamp, 'upgrade'));
} else {
...
$query_params = \array_merge($context_params, $_GET, array(
    ...
    // Billing cycle.
    'billing_cycle' => \WP2FA_Vendor\fs_request_get('billing_cycle', \WP2FA_Vendor\WP_FS__PERIOD_ANNUALLY),
    'is_network_admin' => \WP2FA_Vendor\fs_is_network_admin() ? 'true' : 'false',
    'currency' => $fs->apply_filters('default_currency', 'usd'),
));
$use_external_pricing = $fs->should_use_external_pricing();
if (!$use_external_pricing) {
    $pricing_js_url = \WP2FA_Vendor\fs_asset_url($fs->get_pricing_js_path());
    \WP2FA_Vendor\wp_enqueue_script('freemius-pricing', $pricing_js_url);
} else {
    if (!$fs->is_registered()) {
        $template_data = array('id' => $fs->get_id());
        \WP2FA_Vendor\fs_require_template('forms/trial-start.php', $template_data);
    }
    $view_params = array('id' => $VARS['id'], 'page' => \strtolower($fs->get_text_x_inline('Pricing', 'noun', 'pricing')));
...
if (!$use_external_pricing) {
    ?>
        <div id="fs_pricing_wrapper" data-public-url="<?php 
    echo \trailingslashit(\dirname($pricing_js_url));
    ?>"></div>
        <?php 
    $pricing_config = \array_merge(array('contact_url' => $fs->contact_url(), 'is_network_admin' => \WP2FA_Vendor\fs_is_network_admin(), 'is_production' => \defined('WP2FA_Vendor\\WP_FS__IS_PRODUCTION_MODE') ? \WP2FA_Vendor\WP_FS__IS_PRODUCTION_MODE : null, 'menu_slug' => $fs->get_menu_slug(), 'mode' => 'dashboard', 'fs_wp_endpoint_url' => \WP2FA_Vendor\WP_FS__ADDRESS, 'request_handler_url' => \admin_url('admin-ajax.php?' . \http_build_query(array('module_id' => $fs->get_id(), 'action' => $fs->get_ajax_action('pricing_ajax_action'), 'security' => $fs->get_ajax_security('pricing_ajax_action')))), 'selector' => '#fs_pricing_wrapper', 'unique_affix' => $fs->get_unique_affix()), $query_params);
    \WP2FA_Vendor\wp_add_inline_script('freemius-pricing', 'Freemius.pricing.new( ' . \json_encode($pricing_config) . ' )');
    ?>
        <?php 
} else {
    ?>
...
$params = array('page' => 'pricing', 'module_id' => $fs->get_id(), 'module_type' => $fs->get_module_type(), 'module_slug' => $slug, 'module_version' => $fs->get_plugin_version());
