...
$ms_text = \WP2FA_Vendor\fs_text_x_inline('ms', 'milliseconds');
?>
<h1><?php 
\WP2FA_Vendor\fs_echo_inline('API');
?></h1>

<h2><span>Total Time:</span><?php 
echo \WP2FA_Vendor\Freemius_Debug_Bar_Panel::total_time();
?></h2>

<h2><span>Total Requests:</span><?php 
echo \WP2FA_Vendor\Freemius_Debug_Bar_Panel::requests_count();
?></h2>
<?php 
...
<table class="widefat">
	...
	<tr>
		<th>#</th>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_inline('Method');
?></th>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_inline('Code');
?></th>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_inline('Length');
?></th>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_x_inline('Path', 'as file/folder path');
?></th>
		<?php 
if ($show_body) {
    ?>
			<th><?php 
    \WP2FA_Vendor\fs_esc_html_echo_inline('Body');
    ?></th>
		<?php 
}
?>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_inline('Result');
?></th>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_inline('Start');
?></th>
		<th><?php 
\WP2FA_Vendor\fs_esc_html_echo_inline('End');
?></th>
	</tr>
	</thead>
	<tbody>
...
foreach ($logger as $log) {
    ...
    ?></td>
			<td><?php 
    echo \number_format(100 * $log['total'], 2) . ' ' . $ms_text;
    ?></td>
			<td>
				<?php 
    ...
    ?>
				<table class="widefat" style="display: none">
					<tbody>
					<?php 
    for ($i = 0, $bt = $log['backtrace'], $len = \count($bt); $i < $len; $i++) {
        ?>
						<tr>
							<td><?php 
        echo $len - $i;
    ...
    if ($show_body) {
        ...
        if ('GET' !== $log['method']) {
            ?>
						<?php 
            $body = $log['body'];
            \printf('<a href="#" onclick="jQuery(this).parent().find(\'pre\').toggle(); return false;">%s</a>', \substr($body, 0, 32) . (32 < \strlen($body) ? '...' : ''));
            if ($pretty_print) {
                $body = $encode(\json_decode($log['body']), \JSON_PRETTY_PRINT);
            }
            ?>
						<pre style="display: none"><code><?php 
            echo \esc_html($body);
            ?></code></pre>
					<?php 
        }
    ...
    if ($is_not_empty_result) {
        \printf('<a href="#" onclick="jQuery(this).parent().find(\'pre\').toggle(); return false;">%s</a>', \substr($result, 0, 32) . (32 < \strlen($result) ? '...' : ''));
    }
    ...
    ?></code></pre>
			...
			<td><?php 
    echo \number_format(100 * ($log['start'] - \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME), 2) . ' ' . $ms_text;
    ?></td>
			<td><?php 
    echo \number_format(100 * ($log['end'] - \WP2FA_Vendor\WP_FS__SCRIPT_START_TIME), 2) . ' ' . $ms_text;
    ?></td>
		</tr>
	<?php 
}
