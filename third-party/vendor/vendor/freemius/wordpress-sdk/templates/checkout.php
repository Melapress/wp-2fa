...
\WP2FA_Vendor\fs_enqueue_local_style('fs_checkout', '/admin/checkout.css');
/**
 * @var array    $VARS
 * @var Freemius $fs
 */
...
$slug = $fs->get_slug();
$timestamp = \time();
$context_params = array('plugin_id' => $fs->get_id(), 'public_key' => $fs->get_public_key(), 'plugin_version' => $fs->get_plugin_version(), 'mode' => 'dashboard', 'trial' => \WP2FA_Vendor\fs_request_get_bool('trial'));
$plan_id = \WP2FA_Vendor\fs_request_get('plan_id');
if (\WP2FA_Vendor\FS_Plugin_Plan::is_valid_id($plan_id)) {
    $context_params['plan_id'] = $plan_id;
}
$licenses = \WP2FA_Vendor\fs_request_get('licenses');
if ($licenses === \strval(\intval($licenses)) && $licenses > 0) {
    $context_params['licenses'] = $licenses;
}
$plugin_id = \WP2FA_Vendor\fs_request_get('plugin_id');
...
if ($plugin_id == $fs->get_id()) {
    ...
    if ($fs->is_addon_activated($plugin_id)) {
        $fs_addon = \WP2FA_Vendor\Freemius::get_instance_by_id($plugin_id);
...
if ($fs->is_registered()) {
    ...
    $context_params = \array_merge($context_params, \WP2FA_Vendor\FS_Security::instance()->get_context_params($site, $timestamp, 'checkout'));
} else {
    ...
    $context_params = \array_merge($context_params, array('user_firstname' => $current_user->user_firstname, 'user_lastname' => $current_user->user_lastname, 'user_email' => $current_user->user_email, 'home_url' => \home_url()));
    $fs_user = \WP2FA_Vendor\Freemius::_get_user_by_email($current_user->user_email);
    if (\is_object($fs_user) && $fs_user->is_verified()) {
        $context_params = \array_merge($context_params, \WP2FA_Vendor\FS_Security::instance()->get_context_params($fs_user, $timestamp, 'checkout'));
    }
}
if ($fs->is_payments_sandbox()) {
    // Append plugin secure token for sandbox mode authentication.
    $context_params['sandbox'] = \WP2FA_Vendor\FS_Security::instance()->get_secure_token($fs->get_plugin(), $timestamp, 'checkout');
    /**
...
$can_user_install = $fs->is_plugin() && \WP2FA_Vendor\current_user_can('install_plugins') || $fs->is_theme() && \WP2FA_Vendor\current_user_can('install_themes');
$query_params = \array_merge($context_params, $_GET, array(
    // Current plugin version.
    'plugin_version' => $fs->get_plugin_version(),
    'sdk_version' => \WP2FA_Vendor\WP_FS__SDK_VERSION,
    'is_premium' => $is_premium ? 'true' : 'false',
    'can_install' => $can_user_install ? 'true' : 'false',
    'return_url' => $return_url,
));
$xdebug_session = \WP2FA_Vendor\fs_request_get('XDEBUG_SESSION');
if (\false !== $xdebug_session) {
    $query_params['XDEBUG_SESSION'] = $xdebug_session;
}
$view_params = array('id' => $VARS['id'], 'page' => \strtolower($fs->get_text_inline('Checkout', 'checkout')) . ' ' . $fs->get_text_inline('PCI compliant', 'pci-compliant'));
\WP2FA_Vendor\fs_require_once_template('secure-https-header.php', $view_params);
?>
			...
			(function ($) {
				$(function () {
					...
					var
						...
						frame_height = 800,
						base_url     = '<?php 
echo \WP2FA_Vendor\FS_CHECKOUT__ADDRESS;
?>',
...
?>#' + encodeURIComponent(document.location.href),
					...
					FS.PostMessage.receiveOnce('install', function (data) {
						...
						$.form('<?php 
echo \WP2FA_Vendor\fs_nonce_url($fs->_get_admin_page_url('account', array('fs_action' => $fs->get_unique_affix() . '_activate_new', 'plugin_id' => $plugin_id)), $fs->get_unique_affix() . '_activate_new');
?>', requestData).submit();
					});
					...
					FS.PostMessage.receiveOnce('pending_activation', function (data) {
						...
						$.form('<?php 
echo \WP2FA_Vendor\fs_nonce_url($fs->_get_admin_page_url('account', array('fs_action' => $fs->get_unique_affix() . '_activate_new', 'plugin_id' => $plugin_id, 'pending_activation' => \true)), $fs->get_unique_affix() . '_activate_new');
?>', requestData).submit();
					});
					...
					FS.PostMessage.receiveOnce('get_context', function () {
						...
						<?php 
$install_data = \array_merge($fs->get_opt_in_params(), array('activation_url' => \WP2FA_Vendor\fs_nonce_url($fs->_get_admin_page_url('', array('fs_action' => $fs->get_unique_affix() . '_activate_new', 'plugin_id' => $plugin_id)), $fs->get_unique_affix() . '_activate_new')));
?>
						FS.PostMessage.post('context', <?php 
echo \json_encode($install_data);
?>, frame[0]);
					});

					FS.PostMessage.receiveOnce('purchaseCompleted', <?php 
echo $fs->apply_filters('checkout/purchaseCompleted', 'function (data) {
