...
$cant_find_license_key_text = \WP2FA_Vendor\fs_text_inline("Can't find your license key?", 'cant-find-license-key', $slug);
$message_above_input_field = \WP2FA_Vendor\fs_text_inline('Please enter the license key that you received in the email right after the purchase:', 'activate-license-message', $slug);
$message_below_input_field = '';
$header_title = $fs->is_free_plan() ? \WP2FA_Vendor\fs_text_inline('Activate License', 'activate-license', $slug) : \WP2FA_Vendor\fs_text_inline('Update License', 'update-license', $slug);
if ($fs->is_registered()) {
    $activate_button_text = $header_title;
} else {
    $freemius_site_url = $fs->has_paid_plan() ? 'https://freemius.com/' : $fs->get_usage_tracking_terms_url();
    $freemius_link = '<a href="' . $freemius_site_url . '" target="_blank" rel="noopener" tabindex="0">freemius.com</a>';
    $message_below_input_field = \sprintf(\WP2FA_Vendor\fs_text_inline('The %1$s will be periodically sending data to %2$s to check for security and feature updates, and verify the validity of your license.', 'license-sync-disclaimer', $slug), $fs->get_module_label(\true), $freemius_link);
    $activate_button_text = \WP2FA_Vendor\fs_text_inline('Agree & Activate License', 'agree-activate-license', $slug);
}
$license_key_text = \WP2FA_Vendor\fs_text_inline('License key', 'license-key', $slug);
$is_network_activation = $fs->is_network_active() && \WP2FA_Vendor\fs_is_network_admin() && !$fs->is_delegated_connection();
...
if ($is_network_activation) {
    ...
    if ($is_network_activation) {
        $vars = array('id' => $fs->get_id(), 'sites' => $sites_details, 'require_license_key' => \true);
        $network_activation_html = \WP2FA_Vendor\fs_get_template('partials/network-activation.php', $vars);
    }
}
...
HTML;
    if ($total_available_licenses > 1) {
        ...
        foreach ($available_licenses as $license) {
            ...
            $label = \sprintf("%s-Site %s License - %s", 1 == $license->quota ? 'Single' : ($license->is_unlimited() ? 'Unlimited' : $license->quota), \is_object($plan) ? $plan->title : '', $license->get_html_escaped_masked_secret_key());
        ...
        $value = \sprintf("%s-Site %s License - %s", 1 == $available_license->quota ? 'Single' : ($available_license->is_unlimited() ? 'Unlimited' : $available_license->quota), $fs->_get_plan_by_id($available_license->plan_id)->title, $available_license->get_html_escaped_masked_secret_key());
        $license_input_html .= <<<HTML
                <input
...
$ownership_change_option_text = \WP2FA_Vendor\fs_text_inline("Associate with the license owner's account.", 'associate-account-with-license-owner', $slug);
...
?>,
			modalHtml =
				'<div class="fs-modal fs-modal-license-activation fs-modal-license-activation-<?php 
echo $unique_affix;
?>">'
				+ '	<div class="fs-modal-dialog">'
				+ '		<div class="fs-modal-header">'
				+ '		    <h4><?php 
echo \WP2FA_Vendor\esc_js($header_title);
?></h4>'
				+ '         <a href="!#" class="fs-close"><i class="dashicons dashicons-no" title="<?php 
echo \WP2FA_Vendor\esc_js(\WP2FA_Vendor\fs_text_x_inline('Dismiss', 'as close a window', 'dismiss', $slug));
?>"></i></a>'
				+ '		</div>'
...
\WP2FA_Vendor\fs_esc_js_echo_inline('Cancel', 'cancel', $slug);
?></button>'
				+ '			<button class="button button-primary button-activate-license"  tabindex="3"><?php 
echo \WP2FA_Vendor\esc_js($activate_button_text);
?></button>'
				+ '		</div>'
				+ '	</div>'
				+ '</div>',
            ...
            isNetworkActivation             = <?php 
echo $is_network_activation ? 'true' : 'false';
?>,
            isUserChangeSupported           = <?php 
echo $is_user_change_supported ? 'true' : 'false';
?>,
        ...
        var
            ...
            $otherLicenseKey     = $modal.find( '#other_license_key_<?php 
echo $unique_affix;
?>' ),
...
?>,
        ...
        var
            ...
            resetLoadingMode = function () {
                // Reset loading mode.
                $activateLicenseButton.text( <?php 
echo \json_encode($activate_button_text);
?> );
            ...
            afterLicenseUserDataLoaded = function () {
                if (
                    ...
                    otherLicenseOwnerID != <?php 
echo $fs->is_registered() ? $fs->get_user()->id : 'null';
?>
                ) {
            ...
            fetchLicenseUserData = function () {
                ...
                $activateLicenseButton.html( '<?php 
...
?>'
		...
		function registerEventHandlers() {
            ...
            $body.on( 'click', 'span.activate-license.<?php 
echo $unique_affix;
?> a, .activate-license-trigger.<?php 
echo $unique_affix;
?>', function (evt) {
				evt.preventDefault();
...
?>'
				...
				$.ajax({
					...
					beforeSend: function () {
						$activateLicenseButton.text( '<?php 
\WP2FA_Vendor\fs_esc_js_echo_inline('Activating license', 'activating-license', $slug);
?>...' );
					},
					success: function( result ) {
        ...
        function toggleActivationOnAllSites() {
            ...
            var
                activateLicenseCheckboxLabel = '<?php 
\WP2FA_Vendor\fs_esc_js_echo_inline('Choose up to %s site(s) to activate the license on.', 'choose-up-to-n-sites-to-activate-the-license-on', $slug);
?>';
        ...
        function resetActivateLicenseCheckboxLabel() {
            var activateLicenseCheckboxLabel = '<?php 
\WP2FA_Vendor\fs_esc_js_echo_inline('Activate license on all sites in the network.', 'activate-license-on-all-sites-in-the-network', $slug);
?>';
		...
		function showModal( evt ) {
            ...
            <?php 
if ($fs->apply_filters('enable_per_site_activation', \true)) {
    ?>
