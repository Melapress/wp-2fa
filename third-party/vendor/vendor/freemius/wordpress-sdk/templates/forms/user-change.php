...
$change_user_message = \WP2FA_Vendor\fs_text_inline('By changing the user, you agree to transfer the account ownership to:', 'change-user--message', $slug);
$header_title = \WP2FA_Vendor\fs_text_inline('Change User', 'change-user', $slug);
$user_change_button_text = \WP2FA_Vendor\fs_text_inline('I Agree - Change User', 'agree-change-user', $slug);
$other_text = \WP2FA_Vendor\fs_text_inline('Other', 'other', $slug);
$enter_email_address_placeholder_text = \WP2FA_Vendor\fs_text_inline('Enter email address', 'enter-email-address', $slug);
...
?>,
            modalHtml                   =
                '<div class="fs-modal fs-modal-change-user fs-modal-change-user-<?php 
...
echo \WP2FA_Vendor\esc_js(\WP2FA_Vendor\fs_text_x_inline('Dismiss', 'close window', 'dismiss', $slug));
?>"></i></a>'
                + '		</div>'
...
?></button>'
                + '			<button class="button button-primary fs-user-change-button" tabindex="3"><?php 
echo \WP2FA_Vendor\esc_js($user_change_button_text);
?></button>'
                + '		</div>'
                + '	</div>'
                + '</div>',
        ...
        function registerEventHandlers() {
            ...
            $modal.on( 'click', '.fs-user-change-button', function ( evt ) {
                ...
                $.ajax( {
                    ...
                    data      : {
                        action       : '<?php 
echo $fs->get_ajax_action('change_user');
?>',
                        security     : '<?php 
echo $fs->get_ajax_security('change_user');
?>',
...
?>'
                    ...
                    beforeSend: function () {
                        $userChangeButton
                            .text( '<?php 
\WP2FA_Vendor\fs_esc_js_echo_inline('Changing user, please wait', 'changing-user-please-wait', $slug);
?>...' )
                    ...
                    error     : function () {
                        ...
                        showError( '<?php 
\WP2FA_Vendor\fs_esc_js_echo_inline('Unexpected error, try again in 5 minutes. If the error persists, please contact support.', 'unexpected-error', $slug);
?>' );
        ...
        function resetUserChangeButton() {
            enableUserChangeButton();
            $userChangeButton.text( <?php 
echo \json_encode($user_change_button_text);
?> );
        }
