...
$message_header = \sprintf(
    ...
    \WP2FA_Vendor\fs_text_inline('You are 1-click away from starting your %1$s-day free trial of the %2$s plan.', 'start-trial-prompt-header', $slug),
    '<span class="var-trial_period"></span>',
    '<span class="var-plan_title"></span>'
);
$message_content = \sprintf(
    /* translators: %s: Link to freemius.com */
    \WP2FA_Vendor\fs_text_inline('For compliance with the WordPress.org guidelines, before we start the trial we ask that you opt in with your user and non-sensitive site information, allowing the %s to periodically send data to %s to check for version updates and to validate your trial.', 'start-trial-prompt-message', $slug),
    $fs->get_module_type(),
    \sprintf('<a href="%s" target="_blank" rel="noopener">%s</a>', 'https://freemius.com', 'freemius.com')
);
...
?>,
			    modalHtml        =
				    ...
				    + '		    <h4><?php 
echo \WP2FA_Vendor\esc_js(\WP2FA_Vendor\fs_text_x_inline('Start free trial', 'call to action', 'start-free-trial', $slug));
?></h4>'
				    + '		</div>'
				    + '		<div class="fs-modal-body">' + modalContentHtml + '</div>'
				    + '		<div class="fs-modal-footer">'
				    + '			<button class="button button-secondary button-close">' + <?php 
\WP2FA_Vendor\fs_json_encode_echo_inline('Cancel', 'cancel', $slug);
?> +'</button>'
				    + '			<button class="button button-primary button-connect">' + <?php 
\WP2FA_Vendor\fs_json_encode_echo_inline('Approve & Start Trial', 'approve-start-trial', $slug);
?> +'</button>'
				    + '		</div>'
				    + '	</div>'
				    + '</div>',
			...
			function registerEventHandlers() {
				...
				$modal.on('click', '.button-connect', function (evt) {
					...
					$.ajax({
						...
						data      : {
							action   : '<?php 
echo $fs->get_ajax_action('start_trial');
?>',
							security : '<?php 
echo $fs->get_ajax_security('start_trial');
?>',
							module_id: '<?php 
echo $fs->get_id();
?>',
						...
						beforeSend: function () {
							...
							$button.text(<?php 
\WP2FA_Vendor\fs_json_encode_echo_inline('Starting trial', 'starting-trial', $slug);
?> + '...');
						...
						success   : function (resultObj) {
							if (resultObj.success) {
								$button.text(<?php 
\WP2FA_Vendor\fs_json_encode_echo_inline('Please wait', 'please-wait', $slug);
?> + '...');
								...
								$button.text(<?php 
\WP2FA_Vendor\fs_json_encode_echo('approve-start-trial', $slug);
?>);
