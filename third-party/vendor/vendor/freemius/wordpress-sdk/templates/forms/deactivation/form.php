...
HTML;
    ...
    if ($is_anonymous) {
        $anonymous_feedback_checkbox_html = \sprintf('<label class="anonymous-feedback-label"><input type="checkbox" class="anonymous-feedback-checkbox"> %s</label>', \WP2FA_Vendor\fs_esc_html_inline('Anonymous feedback', 'anonymous-feedback', $slug));
    }
}
// Aliases.
$deactivate_text = \WP2FA_Vendor\fs_text_inline('Deactivate', 'deactivate', $slug);
$theme_text = \WP2FA_Vendor\fs_text_inline('Theme', 'theme', $slug);
$activate_x_text = \WP2FA_Vendor\fs_text_inline('Activate %s', 'activate-x', $slug);
...
?>,
	    modalHtml                      =
		    '<div class="fs-modal fs-modal-deactivation-feedback<?php 
echo empty($confirmation_message) ? ' no-confirmation-message' : '';
?>">'
		    + '	<div class="fs-modal-dialog">'
		    + '		<div class="fs-modal-header">'
		    + '		    <h4><?php 
\WP2FA_Vendor\fs_esc_attr_echo_inline('Quick Feedback', 'quick-feedback', $slug);
?></h4>'
		    + '		</div>'
		    + '		<div class="fs-modal-body">'
		    + '			<div class="fs-modal-panel" data-panel-id="confirm"><p><?php 
echo $confirmation_message;
?></p></div>'
		    + '			<div class="fs-modal-panel active" data-panel-id="reasons"><h3><strong><?php 
echo \WP2FA_Vendor\esc_js(\sprintf(\WP2FA_Vendor\fs_text_inline('If you have a moment, please let us know why you are %s', 'deactivation-share-reason', $slug), $fs->is_plugin() ? \WP2FA_Vendor\fs_text_inline('deactivating', 'deactivating', $slug) : \WP2FA_Vendor\fs_text_inline('switching', 'switching', $slug)));
?>:</strong></h3><ul id="reasons-list">' + reasonsHtml + '</ul></div>'
		    + '		</div>'
		    + '		<div class="fs-modal-footer">'
			+ '         <?php 
echo $anonymous_feedback_checkbox_html;
?>'
...
?></a>'
		...
		isAnonymous                    = <?php 
echo $is_anonymous ? 'true' : 'false';
?>,
		otherReasonID                  = <?php 
echo \WP2FA_Vendor\Freemius::REASON_OTHER;
?>,
		dontShareDataReasonID          = <?php 
echo \WP2FA_Vendor\Freemius::REASON_DONT_LIKE_TO_SHARE_MY_INFORMATION;
?>,
        deleteThemeUpdateData          = <?php 
echo $fs->is_theme() && $fs->is_premium() && !$fs->has_any_active_valid_license() ? 'true' : 'false';
?>,
        $subscriptionCancellationModal = $( '.fs-modal-subscription-cancellation-<?php 
echo $fs->get_id();
?>' ),
        showDeactivationFeedbackForm   = <?php 
echo $show_deactivation_feedback_form ? 'true' : 'false';
?>,
        $body                          = $( 'body' );
...
?>', function( evt, cancelSubscription ) {
            ...
            if ( false === cancelSubscription ) {
                ...
                <?php 
$subscription_cancellation_context = $fs->is_paid_trial() ? \WP2FA_Vendor\fs_text_inline('trial', 'trial', $slug) : \WP2FA_Vendor\fs_text_inline('subscription', 'subscription', $slug);
?>
                ...
                $.ajax({
                    ...
                    data      : {
                        action   : '<?php 
echo $fs->get_ajax_action('cancel_subscription_or_trial');
?>',
                        security : '<?php 
echo $fs->get_ajax_security('cancel_subscription_or_trial');
?>',
                        module_id: '<?php 
echo $fs->get_id();
?>'
...
echo \WP2FA_Vendor\esc_js(\sprintf(\WP2FA_Vendor\fs_text_inline('Cancelling %s...', 'cancelling-x', $slug), $subscription_cancellation_context));
?>' );
                    },
                    success: function( result ) {
                        if ( result.success ) {
                            ...
                            $subscriptionCancellationModal.find( '.fs-modal-footer .button-primary' ).html( <?php 
...
?> );
	...
	function registerEventHandlers() {
		$body.on( 'click', '#the-list .deactivate > a', function ( evt ) {
		    if ( 0 === $( this ).next( '[data-module-id=<?php 
echo $fs->get_id();
?>].fs-module-id' ).length ) {
		        return true;
            }
...
if (!$fs->is_plugin()) {
     ...
     */
    ?>
		$('body').on('click', '.theme-browser .theme:not([data-slug=<?php 
    echo $fs->get_premium_slug();
    ?>]) .theme-actions .button.activate', function (evt) {
			evt.preventDefault();
				...
				if (0 === $radio.length) {
				    if ( ! deleteThemeUpdateData ) {
                        ...
                        $.ajax({
                            ...
                            data      : {
                                action   : '<?php 
echo $fs->get_ajax_action('delete_theme_update_data');
?>',
                                security : '<?php 
echo $fs->get_ajax_security('delete_theme_update_data');
?>',
                                module_id: '<?php 
echo $fs->get_id();
?>'
				...
				$.ajax({
					...
					data      : {
						action      : '<?php 
echo $fs->get_ajax_action('submit_uninstall_reason');
?>',
						security    : '<?php 
echo $fs->get_ajax_security('submit_uninstall_reason');
?>',
						module_id   : '<?php 
echo $fs->get_id();
?>',
		...
		$modal.on('click', 'input[type="radio"]', function () {
			...
			$modal.find('.button-deactivate').html('<?php 
echo \WP2FA_Vendor\esc_js(\sprintf(\WP2FA_Vendor\fs_text_inline('Submit & %s', 'deactivation-modal-button-submit', $slug), $fs->is_plugin() ? $deactivate_text : \sprintf($activate_x_text, $theme_text)));
?>');
			...
			if (_parent.hasClass('has-input')) {
				...
				if (isOtherReasonSelected()) {
					showMessage('<?php 
echo \WP2FA_Vendor\esc_js(\WP2FA_Vendor\fs_text_inline('Kindly tell us the reason so we can improve.', 'ask-for-reason-message', $slug));
?>');
					disableDeactivateButton();
				}
			}
...
echo \json_encode(\sprintf(\WP2FA_Vendor\fs_text_inline('Yes - %s', 'deactivation-modal-button-confirm', $slug), $fs->is_plugin() ? $deactivate_text : \sprintf($activate_x_text, $theme_text)));
?> );
		} else {
            $deactivateButton.html( <?php 
echo \json_encode(\sprintf(\WP2FA_Vendor\fs_text_inline('Skip & %s', 'skip-and-x', $slug), $fs->is_plugin() ? $deactivate_text : \sprintf($activate_x_text, $theme_text)));
?> );
		}
	}
