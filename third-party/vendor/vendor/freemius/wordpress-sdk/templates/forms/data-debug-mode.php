...
$has_last_license_user_id = \WP2FA_Vendor\FS_User::is_valid_id($last_license_user_id);
$message_above_input_field = !$has_last_license_user_id ? \WP2FA_Vendor\fs_text_inline('Please enter the license key to enable the debug mode:', 'submit-developer-license-key-message', $slug) : \sprintf(\WP2FA_Vendor\fs_text_inline('To enter the debug mode, please enter the secret key of the license owner (UserID = %d), which you can find in your "My Profile" section of your User Dashboard:', 'submit-addon-developer-key-message', $slug), $last_license_user_id);
$processing_text = \WP2FA_Vendor\fs_esc_js_inline('Processing', 'processing', $slug) . '...';
$submit_button_text = \WP2FA_Vendor\fs_text_inline('Submit', 'submit', $slug);
$debug_license_link_text = \WP2FA_Vendor\fs_esc_html_inline('Start Debug', 'start-debug-license', $slug);
$license_or_user_key_text = !$has_last_license_user_id ? \WP2FA_Vendor\fs_text_inline('License key', 'license-key', $slug) : \WP2FA_Vendor\fs_text_inline('User key', 'user-key', $slug);
...
?>,
			modalHtml                 =
				'<div class="fs-modal fs-modal-developer-license-debug-mode fs-modal-developer-license-debug-mode-<?php 
echo $unique_affix;
?>">'
...
?></button>'
				+ '			<button class="button button-primary button-submit-license-or-user-key"  tabindex="3"><?php 
echo \WP2FA_Vendor\esc_js($submit_button_text);
?></button>'
				+ '		</div>'
				+ '	</div>'
				+ '</div>',
            ...
            isDebugMode                        = <?php 
echo $fs->is_data_debug_mode() ? 'true' : 'false';
?>;
		...
		function setDeveloperLicenseDebugMode( licenseOrUserKey ) {
            var data = {
                action             : '<?php 
echo $fs->get_ajax_action('set_data_debug_mode');
?>',
                security           : '<?php 
echo $fs->get_ajax_security('set_data_debug_mode');
?>',
...
?>'
            ...
            $.ajax( {
                ...
                data      : data,
                beforeSend: function () {
                    $debugLicenseLink.find('span').text( '<?php 
echo $processing_text;
?>' );
                    $submitKeyButton.text( '<?php 
echo $processing_text;
?>' );
                },
                ...
                error     : function () {
                    showError( <?php 
echo \json_encode(\WP2FA_Vendor\fs_text_inline('An unknown error has occurred.', 'unknown-error', $slug));
?> );
                    resetButtons();
                }
            });
		...
		function resetButtons() {
			enableSubmitButton();
			$submitKeyButton.text( <?php 
echo \json_encode($submit_button_text);
?> );
			$debugLicenseLink.find('span').text( <?php 
echo \json_encode($debug_license_link_text);
?> );
		}
