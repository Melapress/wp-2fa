...
$slug = $fs->get_slug();
$send_button_text = \WP2FA_Vendor\fs_text_inline('Send License Key', 'send-license-key', $slug);
$cancel_button_text = \WP2FA_Vendor\fs_text_inline('Cancel', 'cancel', $slug);
$email_address_placeholder = \WP2FA_Vendor\fs_esc_attr_inline('Email address', 'email-address', $slug);
...
$message_above_input_field = \WP2FA_Vendor\fs_esc_html_inline("Enter the email address you've used for the upgrade below and we will resend you the license key.", 'ask-for-upgrade-email-address', $slug);
...
?>,
			    modalHtml        =
				    '<div class="fs-modal fs-modal-license-key-resend <?php 
echo $is_freemium ? 'fs-freemium' : 'fs-premium';
?>">'
				    + ' <div class="fs-modal-dialog">'
				    + '     <div class="fs-modal-header">'
				    + '         <h4><?php 
echo \WP2FA_Vendor\esc_js($send_button_text);
?></h4>'
...
?>',
			...
			function registerEventHandlers() {
				$('a.show-license-resend-modal-<?php 
...
?>').click(function (evt) {
				...
				$modal.on('click', '.button', function (evt) {
					...
					$.ajax({
						...
						data      : {
							action     : '<?php 
echo $fs->get_ajax_action('resend_license_key');
?>',
							security   : '<?php 
echo $fs->get_ajax_security('resend_license_key');
?>',
							module_id  : moduleID,
							email      : email
						},
						beforeSend: function () {
							$sendButton.text('<?php 
\WP2FA_Vendor\fs_esc_js_echo_inline('Sending license key', 'sending-license-key', $slug);
?>...');
						},
			...
			function resetButton() {
				updateButtonState();
				$sendButton.text(<?php 
echo \json_encode($send_button_text);
?>);
			}
