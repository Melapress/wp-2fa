...
$slug = $fs->get_slug();
$open_addon_slug = \WP2FA_Vendor\fs_request_get('slug');
$open_addon = \false;
...
$view_details_text = \WP2FA_Vendor\fs_text_inline('View details', 'view-details', $slug);
...
if (!$has_tabs) {
    ?>
		<h2><?php 
    echo \esc_html(\sprintf(\WP2FA_Vendor\fs_text_inline('Add Ons for %s', 'add-ons-for-x', $slug), $fs->get_plugin_name()));
    ?></h2>
		<?php 
}
?>
		...
		<?php 
$fs->do_action('addons/after_title');
?>
...
if (!$has_addons) {
    ?>
				<h3><?php 
    echo \esc_html(\sprintf('%s... %s', \WP2FA_Vendor\fs_text_x_inline('Oops', 'exclamation', 'oops', $slug), \WP2FA_Vendor\fs_text_inline('We couldn\'t load the add-ons list. It\'s probably an issue on our side, please try to come back in few minutes.', 'add-ons-missing', $slug)));
    ?></h3>
			<?php 
}
?>
...
if ($has_addons) {
    ...
    foreach ($addons as $addon) {
        ...
        $is_addon_installed = \file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $basename));
        if (!$is_addon_installed && $hide_all_addons_data) {
            continue;
        }
        $is_addon_activated = $is_addon_installed ? $fs->is_addon_activated($addon->id) : \false;
        $is_plugin_active = $is_addon_activated || isset($active_plugins_directories_map[\dirname($basename)]);
            ...
            if (\is_array($plans) && 0 < \count($plans)) {
                foreach ($plans as $plan) {
                    ...
                    foreach ($plan->pricing as $pricing) {
                        $pricing = new \WP2FA_Vendor\FS_Pricing($pricing);
                        if (!$pricing->is_usd()) {
                            /**
        ...
        ?>
						<li class="fs-card fs-addon" data-slug="<?php 
        echo $addon->slug;
        ?>">
							<?php 
        $view_details_link = \sprintf('<a href="%s" aria-label="%s" data-title="%s"', \esc_url(\WP2FA_Vendor\network_admin_url('plugin-install.php?fs_allow_updater_and_dialog=true' . (!empty($fs_blog_id) ? '&fs_blog_id=' . $fs_blog_id : '') . '&tab=plugin-information&parent_plugin_id=' . $fs->get_id() . '&plugin=' . $addon->slug . '&TB_iframe=true&width=600&height=550')), \esc_attr(\sprintf(\WP2FA_Vendor\fs_text_inline('More information about %s', 'more-information-about-x', $slug), $addon->title)), \esc_attr($addon->title)) . ' class="thickbox%s">%s</a>';
        echo \sprintf(
            $view_details_link,
            /**
             * Additional class.
        ...
        if ($is_plugin_active || $is_addon_installed) {
            echo \sprintf('<span class="fs-badge fs-installed-addon-badge">%s</span>', \esc_html($is_plugin_active ? \WP2FA_Vendor\fs_text_x_inline('Active', 'active add-on', 'active-addon', $slug) : \WP2FA_Vendor\fs_text_x_inline('Installed', 'installed add-on', 'installed-addon', $slug)));
        }
        ?></li>
									<!-- <li class="fs-tag"></li> -->
        ...
        if ($is_whitelabeled) {
            ...
            $descriptors = array();
            if ($has_free_plan) {
                $descriptors[] = \WP2FA_Vendor\fs_text_inline('Free', 'free', $slug);
            }
            if ($has_paid_plan && $price > 0) {
                $descriptors[] = '$' . \number_format($price, 2);
            }
            if ($has_trial) {
                $descriptors[] = \WP2FA_Vendor\fs_text_x_inline('Trial', 'trial period', 'trial', $slug);
            }
        ...
        $is_allowed_to_install = $fs->is_allowed_to_install() || $is_free_only_wp_org_compliant;
        $show_premium_activation_or_installation_action = \true;
        if (!\in_array($addon->id, $account_addon_ids)) {
            ...
            if ($is_addon_installed) {
                 ...
                 * @since 2.4.5
                 */
                $fs_addon = $is_addon_activated ? $fs->get_addon_instance($addon->id) : null;
                $premium_plugin_basename = \is_object($fs_addon) ? $fs_addon->premium_plugin_basename() : "{$addon->premium_slug}/{$addon->slug}.php";
                if ($is_addon_activated && $fs_addon->is_premium() || \file_exists(\WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $premium_plugin_basename))) {
                    $basename = $premium_plugin_basename;
                }
                $show_premium_activation_or_installation_action = (!$is_addon_activated || !$fs_addon->is_premium()) && !$is_plugin_active;
            }
        }
        ?>
                                    <?php 
        if (!$show_premium_activation_or_installation_action) {
            ?>
									<li class="fs-cta"><a class="button"><?php 
            echo \esc_html($view_details_text);
            ?></a></li>
                                    <?php 
        } else {
            ?>
                                        <?php 
            $latest_download_local_url = $is_free_only_wp_org_compliant ? null : $fs->_get_latest_download_local_url($addon->id);
            ?>

                                        <li class="fs-cta fs-dropdown">
                                        <div class="button-group">
                                            <?php 
            if ($is_allowed_to_install) {
                ?>
                                            <?php 
                if (!$is_addon_installed) {
                    echo \sprintf('<a class="button button-primary" href="%s">%s</a>', \WP2FA_Vendor\wp_nonce_url(\WP2FA_Vendor\self_admin_url('update.php?' . ($has_paid_plan || !$addon->is_wp_org_compliant ? 'fs_allow_updater_and_dialog=true&' : '') . 'action=install-plugin&plugin=' . $addon->slug), 'install-plugin_' . $addon->slug), \WP2FA_Vendor\fs_esc_html_inline('Install Now', 'install-now', $slug));
                } else {
                    echo \sprintf('<a class="button button-primary edit" href="%s" title="%s" target="_parent">%s</a>', \WP2FA_Vendor\wp_nonce_url('plugins.php?action=activate&amp;plugin=' . $basename, 'activate-plugin_' . $basename), \WP2FA_Vendor\fs_esc_attr_inline('Activate this add-on', 'activate-this-addon', $addon->slug), \WP2FA_Vendor\fs_text_inline('Activate', 'activate', $addon->slug));
                }
                ?>
                                            <?php 
            } else {
                ?>
                                            <a target="_blank" rel="noopener" class="button button-primary" href="<?php 
                echo $latest_download_local_url;
                ?>"><?php 
            ...
            ?>
                                            <div class="button button-primary fs-dropdown-arrow-button"><span class="fs-dropdown-arrow"></span><ul class="fs-dropdown-list" style="display: none">
		                                            <?php 
            if ($is_allowed_to_install && !empty($latest_download_local_url)) {
                ?>
			                                            <li><a target="_blank" rel="noopener" href="<?php 
                echo $latest_download_local_url;
                ?>"><?php 
                echo \esc_html($download_latest_text);
                ?></a></li>
...
?>
		...
		<?php 
$fs->do_action('addons/after_addons');
?>
	</div>
...
if ($open_addon) {
     ...
     */
    ?>
				$('.fs-card[data-slug=<?php 
    echo $open_addon_slug;
    ?>] a').click();
...
$params = array('page' => 'addons', 'module_id' => $fs->get_id(), 'module_type' => $fs->get_module_type(), 'module_slug' => $slug, 'module_version' => $fs->get_plugin_version());
