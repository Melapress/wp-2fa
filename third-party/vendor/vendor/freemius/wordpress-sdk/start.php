...
if (!\function_exists('WP2FA_Vendor\\fs_find_caller_plugin_file')) {
 ...
 * @since  1.2.2.6
 */
$file_path = \WP2FA_Vendor\fs_normalize_path(__FILE__);
 ...
 * @since 2.2.3
 */
$themes_directory = \WP2FA_Vendor\get_theme_root(\get_stylesheet());
$themes_directory_name = \basename($themes_directory);
$theme_candidate_basename = \basename(\dirname($fs_root_path)) . '/' . \basename($fs_root_path);
if ($file_path == \WP2FA_Vendor\fs_normalize_path(\realpath(\trailingslashit($themes_directory) . $theme_candidate_basename . '/' . \basename($file_path)))) {
    $this_sdk_relative_path = '../' . $themes_directory_name . '/' . $theme_candidate_basename;
    $is_theme = \true;
} else {
    $this_sdk_relative_path = \WP2FA_Vendor\plugin_basename($fs_root_path);
    $is_theme = \false;
}
if (!isset($fs_active_plugins)) {
    // Load all Freemius powered active plugins.
    $fs_active_plugins = \WP2FA_Vendor\get_option('fs_active_plugins', new \stdClass());
...
if (empty($fs_active_plugins->abspath)) {
    ...
    if (\ABSPATH !== $fs_active_plugins->abspath) {
        ...
        foreach ($fs_active_plugins->plugins as $sdk_path => $data) {
            if (!\file_exists((isset($data->type) && 'theme' === $data->type ? $themes_directory : \WP2FA_Vendor\WP_PLUGIN_DIR) . '/' . $sdk_path)) {
        ...
        if ($has_changes) {
            ...
            \WP2FA_Vendor\update_option('fs_active_plugins', $fs_active_plugins);
        }
    }
}
if (!\function_exists('WP2FA_Vendor\\fs_find_direct_caller_plugin_file')) {
...
if (!isset($fs_active_plugins->plugins[$this_sdk_relative_path]) || $this_sdk_version != $fs_active_plugins->plugins[$this_sdk_relative_path]->version) {
    if ($is_theme) {
        ...
        $plugin_path = \WP2FA_Vendor\plugin_basename(\WP2FA_Vendor\fs_find_direct_caller_plugin_file($file_path));
    }
    $fs_active_plugins->plugins[$this_sdk_relative_path] = (object) array('version' => $this_sdk_version, 'type' => $is_theme ? 'theme' : 'plugin', 'timestamp' => \time(), 'plugin_path' => $plugin_path);
}
...
if (!isset($fs_active_plugins->newest)) {
     ...
     */
    \WP2FA_Vendor\fs_update_sdk_newest_version($this_sdk_relative_path, $fs_active_plugins->plugins[$this_sdk_relative_path]->plugin_path);
    $is_current_sdk_newest = \true;
} else {
    if (\version_compare($fs_active_plugins->newest->version, $this_sdk_version, '<')) {
         ...
         */
        \WP2FA_Vendor\fs_update_sdk_newest_version($this_sdk_relative_path, $fs_active_plugins->plugins[$this_sdk_relative_path]->plugin_path);
        if (\class_exists('WP2FA_Vendor\\Freemius')) {
            // Older SDK version was already loaded.
            if (!$fs_active_plugins->newest->in_activation) {
                // Re-order plugins to load this plugin first.
                \WP2FA_Vendor\fs_newest_sdk_plugin_first();
            }
            // Refresh page.
            \WP2FA_Vendor\fs_redirect($_SERVER['REQUEST_URI']);
        }
    } else {
        ...
        if (!$is_newest_sdk_type_theme) {
            $is_newest_sdk_plugin_active = \WP2FA_Vendor\is_plugin_active($fs_newest_sdk->plugin_path);
        } else {
             ...
             */
            if (!$is_newest_sdk_plugin_active && $current_theme_parent instanceof \WP2FA_Vendor\WP_Theme) {
        ...
        if ($is_current_sdk_newest && !$is_newest_sdk_plugin_active && !$fs_active_plugins->newest->in_activation) {
            ...
            $fs_active_plugins->newest->in_activation = \true;
            \WP2FA_Vendor\update_option('fs_active_plugins', $fs_active_plugins);
        }
        if (!$is_theme) {
            $sdk_starter_path = \WP2FA_Vendor\fs_normalize_path(\WP2FA_Vendor\WP_PLUGIN_DIR . '/' . $this_sdk_relative_path . '/start.php');
        } else {
            $sdk_starter_path = \WP2FA_Vendor\fs_normalize_path($themes_directory . '/' . \str_replace("../{$themes_directory_name}/", '', $this_sdk_relative_path) . '/start.php');
        }
        ...
        if (!($is_newest_sdk_plugin_active || $fs_active_plugins->newest->in_activation) || !$is_newest_sdk_path_valid || $this_sdk_relative_path == $fs_active_plugins->newest->sdk_path && \version_compare($fs_active_plugins->newest->version, $this_sdk_version, '>')) {
            ...
            if ($is_newest_sdk_plugin_active && $this_sdk_relative_path == $fs_active_plugins->newest->sdk_path && ($fs_active_plugins->newest->in_activation || \class_exists('WP2FA_Vendor\\Freemius') && (!\defined('WP2FA_Vendor\\WP_FS__SDK_VERSION') || \version_compare(\WP2FA_Vendor\WP_FS__SDK_VERSION, $this_sdk_version, '<')))) {
                ...
                if (\WP2FA_Vendor\fs_newest_sdk_plugin_first()) {
...
if (\version_compare($this_sdk_version, $fs_active_plugins->newest->version, '<')) {
    ...
    $plugins_or_theme_dir_path = !isset($newest_sdk->type) || 'theme' !== $newest_sdk->type ? \WP2FA_Vendor\WP_PLUGIN_DIR : $themes_directory;
    $newest_sdk_starter = \WP2FA_Vendor\fs_normalize_path($plugins_or_theme_dir_path . '/' . \str_replace("../{$themes_directory_name}/", '', $fs_active_plugins->newest->sdk_path) . '/start.php');
...
if (!\class_exists('WP2FA_Vendor\\Freemius')) {
    if (!\defined('WP2FA_Vendor\\WP_FS__SDK_VERSION')) {
        \define('WP2FA_Vendor\\WP_FS__SDK_VERSION', $this_sdk_version);
    }
    $plugins_or_theme_dir_path = \WP2FA_Vendor\fs_normalize_path(\trailingslashit($is_theme ? $themes_directory : \WP2FA_Vendor\WP_PLUGIN_DIR));
    if (0 === \strpos($file_path, $plugins_or_theme_dir_path)) {
        ...
        if (empty($sdk_symlink)) {
            ...
            while ('/' !== $partial_path_left && (\false === $realpath || $file_path !== \WP2FA_Vendor\fs_normalize_path($realpath))) {
                $partial_path_right = \trailingslashit(\basename($partial_path_left)) . $partial_path_right;
            ...
            if (!empty($partial_path_left) && '/' !== $partial_path_left) {
                $sdk_symlink = \WP2FA_Vendor\fs_normalize_path(\dirname($partial_path_right));
                // Cache value.
        ...
        if (!empty($sdk_symlink)) {
            ...
            \define('WP2FA_Vendor\\WP_FS__DIR', $plugins_or_theme_dir_path . $sdk_symlink);
        }
    }
    // Load SDK files.
    ...
    {
        return \WP2FA_Vendor\Freemius::instance($module_id);
    }
    /**
     * @param string $slug
    ...
    {
        $fs = \WP2FA_Vendor\Freemius::instance($plugin_id, $slug, \true);
    ...
    {
        $fs = \WP2FA_Vendor\Freemius::instance($module['id'], $module['slug'], \true);
        $fs->dynamic_init($module);
        return $fs;
    }
    function fs_dump_log()
    {
        \WP2FA_Vendor\FS_Logger::dump();
    }
}
