...
return array('prefix' => 'WP2FA_Vendor', 'finders' => array(
    // General dependencies.
    \WP2FA_Vendor\Isolated\Symfony\Component\Finder\Finder::create()->files()->ignoreVCS(\true)->notName('/LICENSE|.*\\.md|.*\\.dist|Makefile|composer\\.(json|lock)/')->exclude(array('doc', 'test', 'test_old', 'tests', 'Tests', 'vendor-bin'))->path('#^deliciousbrains/#')->in('vendor'),
), 'files-whitelist' => array(), 'patchers' => array(function ($file_path, $prefix, $contents) {
    /*
    ...
    return $contents;
}), 'whitelist' => array('add_action'), 'whitelist-global-constants' => \false, 'whitelist-global-classes' => \false, 'whitelist-global-functions' => \true);
